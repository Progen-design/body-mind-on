...
export function hasLocalMealRecipeDetail(model) {
  ...
  // OBECNÝ NÁVOD NENÍ „RECEPT MÁME LOKÁLNĚ“.
  //
  // Plán si nese snímek receptu z doby svého vzniku. Starší plány postup
  // nemají, takže model dosadí obecné „Připrav suroviny podle seznamu“ — a
  // dokud se to počítalo za plnohodnotný detail, nikdy se nesáhlo do katalogu,
  // kde konkrétní postup leží.
  //
  // Kontrola stojí PŘED výjimkou pro fallback schválně: recept označený za
  // fallback bývá právě ten, kterému postup chybí, takže dřívější `return true`
  // ho z katalogu spolehlivě odřízl.
  const idVKatalogu = model.catalogId ?? catalogLookupIdFromMeal(model.normalizedMeal);
  if (model.instructionsGenerated && idVKatalogu != null) return false;

  // Knihovní a fallbackové jídlo katalog nemá — tam je vlastní obsah vším,
  // co existuje.
