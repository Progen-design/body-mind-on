 ...
 *    (½ allowed for bread slices / pečivo only — never eggs, banana, apple, cans)
...
/** Eggs / fruit / cans must stay whole — half eggs look absurd on a plan. */
const WHOLE_ONLY_NAME_RE = /(vejce|egg|ban[aá]n|jablk|apple|konzerv)/i;
/** Bread slices may be halves; common veg halves (okurka ½) stay cookable. */
const HALF_OK_NAME_RE = /(pečiv|peciv|chléb|chleb|toast|rohlík|rohlik|bageta|plátek|platek|okurk|cibul|paprik|avok[aá]d)/i;
...
/**
 * Bread slices / pečivo may be halves (0.5). Eggs, banana, apple, cans = whole only.
 * @param {string|null|undefined} unit
 * @param {string|null|undefined} [name]
 */
export function allowsHalfStep(unit, name = '') {
  const raw = normalizeUnitRaw(unit);
  const ascii = normalizeUnit(unit);
  const n = String(name || '').toLowerCase();
  if (WHOLE_ONLY_NAME_RE.test(n)) return false;
  if (/konzerv|plechov/.test(ascii)) return false;
  if (/plat/.test(ascii) || raw.startsWith('plát')) return true;
  if (HALF_OK_NAME_RE.test(n)) return true;
  return false;
}
