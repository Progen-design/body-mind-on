...
export function normalizeMeasurementPoints({
  ...
  // JEDEN DEN = JEDEN BOD, ALE ROZHODUJE ZDROJ, NE ČAS ZÁPISU.
  //
  // Dřív se v rámci dne prostě přepisovalo pozdějším `measured_at`. To ale
  // znamenalo, že registrační váha přebila ruční měření zapsané týž den:
  // registrace proběhla večer (22:46), měření bylo za ráno (10:00), takže
  // uživatel zadal váhu, uložila se — a v profilu se dál svítila ta
  // z registrace. Změřeno 15. 8. 2026, vypadalo to jako že zápis nefunguje.
  //
  // Ruční zápis a měření ze zařízení jsou skutečná měření. `body_metrics` je
  // úprava profilu a `registration` jednorázový snímek při registraci —
  // ty musí ustoupit. Uvnitř téže priority rozhoduje pozdější čas měření.
  const PRIORITA_ZDROJE = { manual: 3, withings: 3, body_metrics: 2, registration: 1 };
  const prioritaBodu = (p) => PRIORITA_ZDROJE[p.source] ?? 2;

  const weightByDate = new Map();
  for (const p of points.sort((a, b) => String(a.measured_at).localeCompare(String(b.measured_at)))) {
    if (p.weight_kg == null) continue;
    const stavajici = weightByDate.get(p.date);
    if (stavajici && prioritaBodu(stavajici) > prioritaBodu(p)) continue;
...
export function computeActivitySummary({
  ...
  return {
    ...
    /**
     * Konkrétní dny, ne jen počet. Souhrn se v profilu slučuje se statistikou
     * ze zařízení a bez seznamu dnů by se aktivní dny sčítaly dvakrát —
     * plavání zapsané ručně ve stejný den, kdy hodinky nahlásily pohyb, je
     * pořád jeden aktivní den.
     */
    activeDayKeys: [...activeDaySet],
    periodDays,
    habitCompletions,
