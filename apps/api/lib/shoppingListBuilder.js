...
import { aggregateShoppingIngredientLines, ingredientRecordToDisplayLine } from './shoppingListAggregate.js';
...
export function buildShoppingItemsForMeal({
  ...
  const sourceMeal = structuredMeal || meal || {};
  const recipeIngredients = Array.isArray(sourceMeal?.recipe?.ingredients)
    ? sourceMeal.recipe.ingredients
    : [];
  const fromRecipeObjects = recipeIngredients
    .map((ing) => ingredientRecordToDisplayLine(ing))
    .filter(Boolean);

  const structuredCandidates = fromRecipeObjects.length > 0
    ? fromRecipeObjects
    : dedupeShoppingItems([
