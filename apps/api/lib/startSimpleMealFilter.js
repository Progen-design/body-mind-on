 ...
 */
export function resolveSimpleStartLocalSlot(slotMeal, slotTarget, slotSalt = 0, bodyMetrics = null) {
  ...
  if (libraryMeal && !mealContainsExcludedFood(libraryMeal, exclusions)) {
...
export function buildStartSafeFallbackMeal(slotMeal, slotTarget, seed = 0) {
  ...
  if (hasSimpleStartRecipeTitle(resolvedTitle, slotMealType)) {
    ...
    if (libraryMeal && dietOk(libraryMeal)) {
      const meal = JSON.parse(JSON.stringify(libraryMeal));
    ...
    if (libraryMeal) {
      console.warn('[start-fallback] knihovni jidlo neproslo dietou', {
        title: resolvedTitle,
        diet_type: rules.dietType,
        reason: checkCandidateAgainstDiet(libraryMeal, rules).code,
      });
    }
  }
  const mealType = slotMeal?.type || 'lunch';
  ...
  if (agentTpl?.name_cs && agentTpl?.kcal
    ...
    // řešit (žádný plán je lepší než plán s lepkem pro celiaka).
    const allowed = templates.filter(
      (item) => !mealContainsExcludedFood(item, exclusions) && dietOk(item)
    );
    if (!allowed.length) {
      console.error('[start-fallback] pro dietu nezbyla zadna nouzova sablona', {
        meal_type: mealType,
        diet_type: rules.dietType,
        agent_name: slotMeal?.name_cs ?? null,
      });
