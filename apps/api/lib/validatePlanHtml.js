...
function buildDayHeadingPattern() {
  ...
  // planRenderer používá <h4>Pondělí (4. 5.)</h4> – datum v závorce nesmí rozbít validaci
  return new RegExp(`<h[34][^>]*>\\s*(${escaped})(?:\\s*\\([^)]*\\))?\\s*</h[34]>`, 'gi');
}
...
/** Počet rozpoznatelných dnů v plan_html (h3/h4 s českým názvem dne). */
export function countPlanDaysInHtml(html) {
  return findDayBlocks(html).length;
}

/**
 * Detekce kompletního e-mailového dokumentu místo čistého plan_html fragmentu
 * (historicky se odeslaný e-mail ukládal zpět do plan_html). Čistý fragment
 * z renderPlanHtmlFromStructured nikdy neobsahuje doctype ani <html>.
 */
export function isWrappedEmailDocument(html) {
  const s = String(html || '');
...
function structuredPlanValidationBypass(html, dayCount) {
  ...
  return {
    ...
    matchedSections: [],
    missingCoreSections: [],
    reason: len > 0 ? null : 'html_missing_or_not_string',
    structure: {
      dayCount: dayCount > 0 ? dayCount : 7,
      daysMissingMeals: [],
      daysMissingTrainingBlock: [],
    },
  };
...
/**
 * @param {string} html
 * @param {{ generationSource?: string, generation_source?: string, structuredPlanJson?: object, structured_plan_json?: object }} [opts]
 * @returns {{
 *   ok: boolean,
 *   length: number,
 ...
 * }}
 */
export function validatePublishedPlanHtml(html, opts = {}) {
  const emptyStructure = { dayCount: 0, daysMissingMeals: [], daysMissingTrainingBlock: [] };
  const generationSource = opts?.generationSource ?? opts?.generation_source ?? null;
  const structuredPlanJson = opts?.structuredPlanJson ?? opts?.structured_plan_json ?? null;
  const structuredDays = Array.isArray(structuredPlanJson?.days) ? structuredPlanJson.days : [];
  if (structuredDays.length >= 7) {
    return structuredPlanValidationBypass(html, structuredDays.length);
  }
  if (generationSource === 'agent_v6' || generationSource === 'structured') {
    return structuredPlanValidationBypass(html, structuredDays.length);
  }
  if (!html || typeof html !== 'string') {
    return {
