...
function formatMacroValue(value) {
  if (value == null) {
    return '<span style="color:#6B6480;font-weight:400;">—</span>';
  }
  return `${value}<span style="font-size:13px;color:#6B6480;font-weight:400;"> g</span>`;
}

function formatInlineMacro(value, color) {
  if (value == null) {
    return `<span style="color:#6B6480;font-weight:700;">—</span>`;
  }
  return `<span style="color:${color};font-weight:700;">${value} g</span>`;
}
...
function renderMacroCell(label, value, color, borderRight) {
  ...
  return `<td width="25%" style="${pad}">
      ...
      <div class="macro-num" style="font-family:'JetBrains Mono',monospace;font-size:26px;color:#FFFFFF;font-weight:700;margin-top:6px;line-height:1;">${formatMacroValue(value)}</div>
    </td>`;
}
...
function renderMealCard(meal, day, planJson, appBaseUrl) {
  ...
  const recipeBtn = recipeUrl
    ? `<a href="${escapeHtml(recipeUrl)}" target="_blank" rel="noopener noreferrer" class="btn-recipe" style="display:inline-block;padding:12px 18px;background:#1F1A2E;border:1px solid ${theme.color};font-family:'JetBrains Mono',monospace;font-size:11px;color:${theme.color};letter-spacing:1.5px;font-weight:700;text-decoration:none;">RECEPT →</a>`
    : '';
...
function renderDayCard(day, index, totalDays, appBaseUrl, planJson) {
  ...
  const mealMacroList = meals.map((meal) => mealMacros(meal));
  const sumMacro = (key) => {
    if (mealMacroList.length === 0) return null;
    let total = 0;
    for (const m of mealMacroList) {
                ...
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    ...
                    <td align="right" style="font-family:'JetBrains Mono',monospace;font-size:24px;color:#FFFFFF;font-weight:700;">
                      ${dailyKcal != null ? dailyKcal : '<span style="color:#6B6480">—</span>'} <span style="font-size:12px;color:#6B6480;letter-spacing:2px;">KCAL</span>
                    </td>
                  </tr>
                </table>
                <div style="margin-top:14px;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#B8B0CC;line-height:1.6;">
                  Bílkoviny ${formatInlineMacro(dailyProtein, '#22D3EE')} &nbsp;·&nbsp; Sacharidy ${formatInlineMacro(dailyCarbs, '#FBBF24')} &nbsp;·&nbsp; Tuky ${formatInlineMacro(dailyFat, '#EC4899')} &nbsp;·&nbsp; Vláknina ${formatInlineMacro(dailyFiber, '#10B981')}
                </div>
              </td>
            </tr>
          </table>
...
export function buildWeeklyPlanEmailV2Document(options = {}) {
  ...
  // Výchozí cíl je přihlášení s návratem do profilu, NE kořen aplikace.
  // Kořen znamená hlavní stránku — tlačítko „Otevřít můj profil“, které vede
  // na rozcestník, je přesně ta stížnost ze 14. 8. 2026. V7 a V8 to měly
  // správně, V2/V4/V5/V6 ne. Dnes sem `ctaUrl` vždycky přijde z mail.js, takže
  // to byla latentní vada — ale fallback nemá mířit jinam než hlavní cesta.
