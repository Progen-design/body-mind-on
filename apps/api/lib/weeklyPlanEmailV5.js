...
import { getPublicAppUrl, getPlanEmailCtaUrl } from './siteUrls.js';
...
/**
 * Minify HTML string while preserving content inside <pre>/<style>/<script> blocks and conditional comments.
 * - Collapses inter-tag whitespace
 * - Removes redundant whitespace inside style="" attributes
 * - Does NOT touch text nodes between tags (Czech vowels/diacritics safe)
 */
function minifyHtml(html) {
  let out = html;
  // Preserve Outlook conditional comments (<!--[if mso]>...<![endif]-->) – they are needed by Outlook
  const COND_PLACEHOLDER = (i) => `__COND_${i}__`;
  const conditionals = [];
  out = out.replace(/<!--\[if[\s\S]*?<!\[endif\]-->/g, (m) => {
    conditionals.push(m);
    return COND_PLACEHOLDER(conditionals.length - 1);
  });
  // Remove regular HTML comments (BMON markers were already replaced before)
  out = out.replace(/<!--[\s\S]*?-->/g, '');
  // Collapse runs of whitespace between tags
  out = out.replace(/>\s+</g, '><');
  // Trim style="..." attribute internal whitespace (preserve quoted font names)
...
function renderMacrosBlock(targets, commentary, palette) {
  const rows = [
    { label: 'BÍLKOVINY', value: targets?.protein_g, comment: commentary.protein || '', color: palette.primary, rgb: palette.primary_rgb, barWidth: 85 },
    { label: 'SACHARIDY', value: targets?.carbs_g, comment: commentary.carbs || '', color: palette.secondary, rgb: palette.secondary_rgb, barWidth: 70 },
    { label: 'TUKY', value: targets?.fat_g, comment: commentary.fat || '', color: palette.accent, rgb: palette.accent_rgb, barWidth: 55 },
  ];
  return rows
    .map((row) => {
      const valueHtml = formatTargetMacroBig(row.value);
      const commentHtml = row.comment
        ? `<td align="right" valign="middle" style="font-family:${FONT_BODY};font-size:16px;color:#D4C7F0;font-weight:400;line-height:1.5;max-width:280px;">${escapeHtml(row.comment)}</td>`
        : '';
      return `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;border-left:3px solid ${row.color};"><tr><td bgcolor="#0F0820" style="background-color:#0F0820;background-image:linear-gradient(90deg,rgba(${row.rgb},0.08) 0%,rgba(${row.rgb},0.02) 60%,transparent 100%);padding:28px 32px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:${row.color};letter-spacing:2.5px;font-weight:600;text-transform:uppercase;margin-bottom:8px;">${escapeHtml(row.label)}</div><div class="macro-big" style="font-family:${FONT_BODY};font-size:48px;color:#F8F4FF;font-weight:700;line-height:0.95;letter-spacing:-1.5px;">${valueHtml}</div></td>${commentHtml}</tr></table><table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:18px;"><tr><td bgcolor="${row.color}" style="background-color:${row.color};height:4px;line-height:4px;font-size:0;width:${row.barWidth}%;">&nbsp;</td><td bgcolor="#1A0F2E" style="background-color:#1A0F2E;height:4px;line-height:4px;font-size:0;width:${100 - row.barWidth}%;">&nbsp;</td></tr></table></td></tr></table>`;
    })
    .join('');
}
...
function renderHabits(habits, palette) {
  ...
  const accents = [
    { color: palette.primary, colorDark: palette.primary_dark, rgb: palette.primary_rgb },
    { color: palette.secondary, colorDark: palette.secondary_dark, rgb: palette.secondary_rgb },
    { color: palette.accent, colorDark: palette.accent_dark, rgb: palette.accent_rgb },
  ];
  return habits
    .map((habit, idx) => {
      ...
      const descriptionHtml = description
        ? `<div style="font-family:${FONT_BODY};font-size:15px;color:#C4B5E0;font-weight:400;line-height:1.6;">${escapeHtml(description)}</div>`
        : '';
      return `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="${mb}border:1px solid rgba(${accent.rgb},0.18);"><tr><td valign="top" width="120" bgcolor="${accent.colorDark}" style="background-color:${accent.colorDark};background-image:linear-gradient(135deg,${accent.color} 0%,${accent.colorDark} 100%);padding:36px 32px;"><div style="font-family:${FONT_BODY};font-size:48px;color:#F8F4FF;font-weight:800;line-height:0.95;letter-spacing:-1.5px;">${num}</div></td><td valign="top" bgcolor="#0F0820" style="background-color:#0F0820;background-image:linear-gradient(135deg,rgba(${accent.rgb},0.06) 0%,rgba(${accent.rgb},0.01) 100%);padding:32px 36px;"><div style="font-family:${FONT_BODY};font-size:24px;color:#F8F4FF;font-weight:700;line-height:1.2;letter-spacing:-0.5px;margin-bottom:12px;">${escapeHtml(title)}</div>${descriptionHtml}</td></tr></table>`;
    })
    .filter(Boolean)
    .join('');
}
...
function renderMealCard(meal, day, planJson, appBaseUrl, palette) {
  ...
  const recipeButton = recipeUrl
    ? `<a href="${escapeHtml(recipeUrl)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:10px 18px;background-color:rgba(${mealAccent.rgb},0.18);border:1px solid rgba(${mealAccent.rgb},0.4);font-family:${FONT_MONO};font-size:11px;color:${mealAccent.color};letter-spacing:1.5px;font-weight:600;text-decoration:none;text-transform:uppercase;">RECEPT →</a>`
    : '';

  const topPadding = type === 'breakfast' ? '16px' : '12px';

  return `<tr><td class="px-mobile" bgcolor="#06050A" style="background-color:#06050A;padding:${topPadding} 64px 0 64px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(${mealAccent.rgb},0.2);"><tr><td bgcolor="#0F0820" style="background-color:#0F0820;background-image:linear-gradient(135deg,rgba(${mealAccent.rgb},0.10) 0%,rgba(6,5,10,0.95) 70%);padding:36px 40px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:20px;"><tr><td><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:${mealAccent.color};letter-spacing:2.5px;font-weight:600;text-transform:uppercase;">${timeMeta.icon} &nbsp; ${escapeHtml(timeMeta.time_word)} · ${escapeHtml(timeMeta.time)}</div></td><td align="right">${recipeButton}</td></tr></table><div class="meal-title" style="font-family:${FONT_BODY};font-size:30px;color:#F8F4FF;font-weight:700;line-height:1.1;letter-spacing:-1px;margin-bottom:32px;">${escapeHtml(title || label)}</div><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td width="25%" style="border-top:1px solid rgba(${palette.primary_rgb},0.25);padding:20px 16px 0 0;"><div class="mono" style="font-family:${FONT_MONO};font-size:10px;color:${palette.primary};letter-spacing:1.5px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">BÍLKOVINY</div><div style="font-family:${FONT_BODY};font-size:32px;color:#F8F4FF;font-weight:700;line-height:1;letter-spacing:-1px;">${formatMacroCell(macros.protein_g)}</div></td><td width="25%" style="border-top:1px solid rgba(${palette.secondary_rgb},0.25);padding:20px 16px 0 16px;"><div class="mono" style="font-family:${FONT_MONO};font-size:10px;color:${palette.secondary};letter-spacing:1.5px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">SACHARIDY</div><div style="font-family:${FONT_BODY};font-size:32px;color:#F8F4FF;font-weight:700;line-height:1;letter-spacing:-1px;">${formatMacroCell(macros.carbs_g)}</div></td><td width="25%" style="border-top:1px solid rgba(${palette.accent_rgb},0.25);padding:20px 16px 0 16px;"><div class="mono" style="font-family:${FONT_MONO};font-size:10px;color:${palette.accent};letter-spacing:1.5px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">TUKY</div><div style="font-family:${FONT_BODY};font-size:32px;color:#F8F4FF;font-weight:700;line-height:1;letter-spacing:-1px;">${formatMacroCell(macros.fat_g)}</div></td><td width="25%" style="border-top:1px solid rgba(16,185,129,0.25);padding:20px 0 0 16px;"><div class="mono" style="font-family:${FONT_MONO};font-size:10px;color:#10B981;letter-spacing:1.5px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">VLÁKNINA</div><div style="font-family:${FONT_BODY};font-size:32px;color:#F8F4FF;font-weight:700;line-height:1;letter-spacing:-1px;">${formatMacroCell(macros.fiber_g)}</div></td></tr></table></td></tr></table></td></tr>`;
}
...
function renderDailyTotal(day, palette) {
  ...
  const kcalDisplay = dailyKcal != null ? String(dailyKcal) : '<span style="color:#9F8FC0">—</span>';

  return `<tr><td class="px-mobile" bgcolor="#06050A" style="background-color:#06050A;padding:20px 64px 0 64px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(${palette.primary_rgb},0.25);"><tr><td bgcolor="#1A0F2E" style="background-color:#1A0F2E;background-image:linear-gradient(90deg,rgba(${palette.primary_rgb},0.15) 0%,rgba(${palette.secondary_rgb},0.12) 50%,rgba(${palette.accent_rgb},0.15) 100%);padding:32px 40px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:${palette.primary};letter-spacing:3px;font-weight:600;text-transform:uppercase;">CELKEM ZA DEN</div></td><td align="right" style="font-family:${FONT_BODY};font-size:48px;color:#F8F4FF;font-weight:800;letter-spacing:-2px;line-height:1;">${kcalDisplay} <span class="mono" style="font-family:${FONT_MONO};font-size:14px;color:#C4B5E0;letter-spacing:2px;font-weight:500;">KCAL</span></td></tr></table><div style="margin-top:16px;font-family:${FONT_BODY};font-size:14px;color:#D4C7F0;line-height:1.6;">Bílkoviny ${formatInlineMacro(dailyProtein, palette.primary)} &nbsp;·&nbsp; Sacharidy ${formatInlineMacro(dailyCarbs, palette.secondary)} &nbsp;·&nbsp; Tuky ${formatInlineMacro(dailyFat, palette.accent)} &nbsp;·&nbsp; Vláknina ${formatInlineMacro(dailyFiber, '#10B981')}</div></td></tr></table></td></tr>`;
}
...
function renderWorkout(day, coachVoice, palette, appBaseUrl) {
  ...
  if (!exercises.length) {
    return `<tr><td class="px-mobile" bgcolor="#06050A" style="background-color:#06050A;padding:24px 64px 0 64px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${palette.primary_dark}" style="background-color:${palette.primary_dark};background-image:linear-gradient(135deg,#1A0B33 0%,${palette.primary_dark} 60%,${palette.primary} 100%);padding:48px 40px;"><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:#F0EBFF;letter-spacing:3px;font-weight:600;margin-bottom:20px;text-transform:uppercase;">▲ &nbsp; POHYB &nbsp; ▲</div><div style="font-family:${FONT_BODY};font-size:36px;color:#F8F4FF;font-weight:800;letter-spacing:-1.5px;line-height:1.1;margin-bottom:14px;">${escapeHtml(restCopy.intro)}</div><div style="font-family:${FONT_BODY};font-size:16px;color:#F0EBFF;font-weight:400;line-height:1.6;max-width:520px;">${escapeHtml(restCopy.description)}</div></td></tr></table></td></tr>`;
  }
  ...
  if (intensity === 'easy') {
    ...
    blockBgColor = palette.primary_dark;
  ...
  const exerciseRows = exercises
    .map((ex, idx) => {
      ...
      const borderBottom = isLast ? '' : 'border-bottom:1px solid rgba(248,244,255,0.12);';
      return `<tr><td style="padding:24px 28px;${borderBottom}"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td width="40" style="font-family:${FONT_MONO};font-size:14px;color:#D4C7F0;font-weight:600;letter-spacing:1px;">${num}</td><td style="font-family:${FONT_BODY};font-size:20px;color:#F8F4FF;font-weight:600;letter-spacing:-0.3px;">${escapeHtml(name)}</td><td align="right" class="mono" style="font-family:${FONT_MONO};font-size:18px;color:${palette.accent};font-weight:600;letter-spacing:1px;">${escapeHtml(repsUnit)}</td></tr></table></td></tr>`;
    })
    .join('');

  return `<tr><td class="px-mobile" bgcolor="#06050A" style="background-color:#06050A;padding:24px 64px 0 64px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${blockBgColor}" style="background-color:${blockBgColor};background-image:${blockGradient};padding:48px 40px;"><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:#F0EBFF;letter-spacing:3px;font-weight:600;margin-bottom:20px;text-transform:uppercase;">▲ &nbsp; POHYB &nbsp; ▲</div><div style="font-family:${FONT_BODY};font-size:36px;color:#F8F4FF;font-weight:800;letter-spacing:-1.5px;line-height:1.1;margin-bottom:14px;">${escapeHtml(copy.intro)}</div><div style="font-family:${FONT_BODY};font-size:16px;color:#F0EBFF;font-weight:400;line-height:1.6;margin-bottom:36px;max-width:520px;">${escapeHtml(copy.description)}</div><table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(248,244,255,0.15);"><tr><td bgcolor="${blockBgColor}" style="background-color:${blockBgColor};padding:0;"><table width="100%" cellpadding="0" cellspacing="0" border="0">${exerciseRows}</table></td></tr></table><div style="margin-top:24px;font-family:${FONT_BODY};font-size:14px;color:#F0EBFF;line-height:1.6;">Videa s technikou v <a href="${escapeHtml(appBaseUrl)}" target="_blank" rel="noopener noreferrer" style="color:#F8F4FF;border-bottom:1px solid rgba(248,244,255,0.5);text-decoration:none;font-weight:600;">aplikaci</a>.</div></td></tr></table></td></tr>`;
}
...
function renderDayCard(day, index, totalDays, planJson, appBaseUrl, palette, coachVoice) {
  ...
  return `<tr><td class="px-mobile" bgcolor="#06050A" style="background-color:#06050A;padding:32px 64px 0 64px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${palette.primary_dark}" style="background-color:${palette.primary_dark};background-image:${palette.day_gradient};padding:72px 56px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td><div class="mono" style="font-family:${FONT_MONO};font-size:11px;color:#F0EBFF;letter-spacing:3px;font-weight:600;margin-bottom:24px;text-transform:uppercase;">DEN ${dayIndexPad} / ${totalPad} &nbsp;·&nbsp; ${escapeHtml(ordinal)}</div><div class="day-mega" style="font-family:${FONT_BODY};font-size:80px;color:#F8F4FF;font-weight:800;letter-spacing:-3px;line-height:0.95;text-shadow:0 0 30px rgba(0,0,0,0.4);">${escapeHtml(dayName)}</div><div style="font-family:${FONT_BODY};font-size:22px;color:#F0EBFF;font-weight:400;margin-top:14px;letter-spacing:-0.3px;">${escapeHtml(dateDisplay)}</div></td><td align="right" valign="bottom" class="hide-mobile"><div style="font-family:${FONT_MONO};font-size:96px;color:#F0EBFF;opacity:0.25;font-weight:800;letter-spacing:-4px;line-height:1;">${escapeHtml(dayShort)}</div></td></tr></table></td></tr></table></td></tr>${mealsHtml}${dailyTotalHtml}${workoutHtml}`;
}
...
export function buildWeeklyPlanEmailV5Document(options = {}) {
  ...
  // Viz weeklyPlanEmailV2.js — výchozí cíl je login s návratem do profilu,
  // ne kořen aplikace.
  ...
  const planChangeNote = options.planChangeContext
    ? `<tr><td bgcolor="#06050A" style="background-color:#06050A;">
          ...
          <tr><td align="center">
            <table class="container" width="1100" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:1100px;">
              <tr><td class="px-mobile" style="padding:32px 64px 0 64px;">
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr><td bgcolor="#0F0820" style="background-color:#0F0820;padding:20px 24px;border-left:3px solid ${palette.accent};font-family:'Geist','Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;font-size:15px;color:#D4C7F0;line-height:1.6;">
  ...
  html = html.replace('<!--BMON_LOGIN_BLOCK-->', planChangeNote);
  ...
  const filledHtml = applyVars(html, {
    ...
    palette_primary_rgb: palette.primary_rgb,
    palette_secondary_rgb: palette.secondary_rgb,
    palette_accent_rgb: palette.accent_rgb,
