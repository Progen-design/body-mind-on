...
import { getPlanCalorieHonestyStatus } from './nutrition/calorieHonesty.js';
...
export function buildWeeklyPlanEmailV8Document(options = {}) {
  ...
  const honesty = getPlanCalorieHonestyStatus(planJson);
  const honestyBannerHtml = honesty?.banner_cs
    ? `<tr><td class="mobile-pad" bgcolor="${COL.PAGE_BG}" style="background-color:${COL.PAGE_BG};padding:0 16px 12px 16px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${COL.CARD_BG}" style="background-color:${COL.CARD_BG};border:1px solid ${honesty.invalid_inflated ? 'rgba(245,158,11,0.45)' : 'rgba(14,165,233,0.35)'};border-radius:12px;border-collapse:separate !important;"><tr><td style="padding:14px 18px;font-family:${FONT_BODY};font-size:13px;color:${COL.TEXT_PRIMARY};line-height:1.5;">${escapeHtml(honesty.banner_cs)}${honesty.avg_achieved_kcal != null && honesty.target_kcal != null ? `<div style="margin-top:8px;font-size:12px;color:${COL.TEXT_MUTED};">Reálný průměr: ${Math.round(honesty.avg_achieved_kcal)} kcal/den · cíl: ${Math.round(honesty.target_kcal)} kcal/den</div>` : ''}</td></tr></table></td></tr>`
    : '';
  ...
  html = html.replace('<!--BMON_PROFILE_EXTRA-->', `${honestyBannerHtml}${profileExtraHtml}`);
