...
### 1.3 Success URL – přesměrování na profil po platbě (důležité)
Po zaplacení má uživatel skončit na svém profilu, ne na stránce Stripe „Děkujeme za předplacení“. Nastav to ve Stripe:

**Payment Link (odkaz typu buy.stripe.com):**
1. Ve Stripe jdi na **Payment Links** (v levém menu).
2. Klikni na svůj payment link (např. „START for Kč499.00 CZK / month“) – otevře se detail.
3. Klikni na **Edit** (Úpravy) nebo na název linku, aby se otevřela editace.
4. V průvodci úpravami najdi krok **„After the payment“** (Po platbě). Pokud ho nevidíš, zkus záložku **Customize** nebo **Configuration**.
5. V sekci **Confirmation page** (Potvrzovací stránka) zvol **„Redirect customers to a URL“** / **„Přesměrovat zákazníky na adresu“** (ne „Show confirmation page“).
6. Do pole **Redirect URL** zadej:  
   `https://app.bodyandmindon.cz/profil?payment=success`  
7. Ulož změny (**Save**).

**Pricing Table (embed na webu):**  
**Product catalog → Pricing tables** → otevři tabulku → **Edit payment page** (nebo nastavení u produktu) → stejně **After the payment** → Redirect → stejná URL.

Na profilu se po přesměrování zobrazí toast: „Platba proběhla. Tvůj přístup bude aktivní během chvíle.“
