...
// PROMPT_UKLID.md (2026-09-17) — dřív 12 mrtvých `_legacy-next` cest a 8 živých
// `lib/` souborů v jednom poli. `_legacy-next` mizí, takže tu zůstávají jen
// živé soubory — přesně ty, které ještě generickým kontrolám níž (žádné jméno
// z makety, žádné vzorové kalorie atd.) ještě záleží. `lib/profile/profilZalozky.js`
// nemá dnes žádného živého konzumenta v src/ (jen vlastní test), ale pořád
// existuje a čte se — kontrola zůstává neškodná, jen bez dopadu na UI.
const SOUBORY = [
...
test('odznak regenerace nemá napsané číslo natvrdo', () => {
  ...
  // by to vypadalo jako naměřené skóre. Živý ekvivalent lišty záložek je
  // `src/components/NavigationTabs.tsx` (PROMPT_UKLID.md 2026-09-17) — má
  // vlastní komentář o téže opravě a `regenerace` položku bez `badge` vůbec.
  const kod = kodBezKomentaruTsx(obsah('lib/profile/profilZalozky.js'))
    + kodBezKomentaruTsx(obsah('src/components/NavigationTabs.tsx'));
...
// PROMPT_UKLID.md (2026-09-17) — architektura se změnila, ne jen soubor.
// Návrh měl v ExportMealPlanModal vlastní skládání HTML JAKO DRUHOU cestu
// vedle `lib/planPdf.js`'s `buildPlanPdfHtml` — dvě konkurenční implementace
// nad týmiž daty byl ten problém. Živý `src/components/ExportMealPlanModal.tsx`
// (jediný konzument `window.print()` v src/, ověřeno greppem) `buildPlanPdfHtml`
// vůbec nepoužívá a `lib/planPdf.js` dnes nemá v src/ žádného konzumenta —
// tiskne se přímo živý React strom modalu do dialogu prohlížeče, s komentářem
// v kódu, že je to záměrně JEDNA cesta ("Žádný jsPDF, žádná atrapa, jedna
// cesta."). Pravidlo "jen jeden mechanismus" pořád platí, jen jinou cestou:
// kontroluje se, že se do modalu nevrátila druhá knihovna/generátor vedle
// window.print().
test('export plánu má jeden tiskový mechanismus, ne dva konkurenční', () => {
  const kod = kodBezKomentaruTsx(obsah('src/components/ExportMealPlanModal.tsx'));
  assert.match(kod, /window\.print\(\)/, 'export musí jít přes tiskový dialog prohlížeče');
  for (const zakazane of ['jsPDF', 'html2canvas', 'html2pdf', 'buildPlanPdfHtml']) {
    assert.ok(!kod.includes(zakazane), `modal nesmí mít druhý PDF mechanismus (${zakazane})`);
  }
});
...
// PROMPT_UKLID.md (2026-09-17) — `_legacy-next/components/HabitTracker.js`
// byla jediná komponenta v repu, co zlozvyky (badHabits) barevně odlišovala.
// Živý ekvivalent: `App.tsx` čte `badHabits` (naZlozvyky), ale žádná
// komponenta je nikdy nedostala k vykreslení. PROMPT_UX_DNES.md (18. 9. 2026)
// zrušilo `OverviewBentoGrid.tsx` (dřív jediné místo, kam se `badHabits`
// posílalo jako nevyužitý prop) — gap tím není o nic blíž vyřešení, jen se
// `badHabits` teď nepředává vůbec nikam, ani jako mrtvý prop. Gap se
// nevymýšlí, jen se pinuje a hlásí v reportu k PROMPT_UKLID.md.
test('GAP: zlozvyky (badHabits) se nikde v Dnes nevykreslují', () => {
  const app = kodBezKomentaruTsx(obsah('src/App.tsx'));
  assert.match(app, /setBadHabits\(naZlozvyky/, 'App.tsx už zlozvyky vůbec nepočítá — gap zmizel jinak, přepiš test');
  assert.ok(
    !/badHabits=\{badHabits\}/.test(app),
    'App.tsx zase posílá badHabits do nějaké komponenty jako prop — ' +
    'pokud tenhle test spadl, vykreslování se dopsalo a tenhle test je zastaralý, přepiš ho na kontrolu červené barvy'
  );
});
...
const SOUBORY_SPA = [
  ...
  // ProfileSection.tsx a OverviewBentoGrid.tsx se 18. 9. 2026
  // (PROMPT_UX_DNES.md) sloučily do DnesniPrehled.tsx a rozstěhovaly do
  // NutritionSection.tsx / BodyStatsGrid.tsx. DnesniPrehled.tsx nahradila
  // 21. 9. 2026 časová osa dne (PROMPT_DNES_WOW.md).
  'src/components/CasovaOsaDne.tsx',
  'src/components/DnesObrazovka.tsx',
  'src/components/TvojeCesta.tsx',
  'src/components/NutritionSection.tsx',
  'src/components/BodyStatsGrid.tsx',
  // src/lib/syncEngine.ts je od 23. 9. 2026 smazaný — viz test níž.
...
test('SPA negeneruje biometrii ani tréninkové rady z náhodných čísel', () => {
  // SOUBOR UŽ NEEXISTUJE, A TO JE NEJSILNĚJŠÍ VÝSLEDEK. Po odstranění
  // generátoru v něm zbyla jediná funkce `applyWeightRecord()`, která lepila
  // nové vážení zvlášť do každé časové řady a u „1R" mu přepisovala datum na
  // „09.2026". S opravdovými okny (`src/lib/vahaFiltry.ts`, 23. 9. 2026) to
  // přestalo dávat smysl, poslední volající zmizel a soubor s ním.
  //
  // Test zůstává: kdyby se `syncEngine.ts` vrátil, musí projít stejnou
  // kontrolou jako dřív.
  if (!existsSync(new URL('../../src/lib/syncEngine.ts', import.meta.url))) return;
...
test('odhlášení je v aplikaci jen jednou', () => {
  ...
  // ProfileSection.tsx zmizelo 18. 9. 2026 (PROMPT_UX_DNES.md) — kontroluje
  // se jeho nástupce a Účet a předplatné, kam se přestěhovalo odhlašování
  // po smazání účtu.
  const vDnesku = kodBezKomentaruTsx(obsah('src/components/CasovaOsaDne.tsx'));
  assert.ok(!vDnesku.includes('Odhlásit se'), 'druhé odhlášení je zpátky v Dnešku');
  const vUctu = kodBezKomentaruTsx(obsah('src/components/UcetASpravaSection.tsx'));
  assert.ok(!vUctu.includes('Odhlásit se'), 'druhé odhlášení je zpátky v Účtu a předplatném');
