...
const SOUBORY = [
  ...
  'components/profile/VlastniJidlaPanel.jsx',
  'components/profile/VlastniPolozkyNakupu.jsx',
  'lib/profile/vlastniJidlo.js',
...
/* ── FÁZE 2 (jídlo) ──────────────────────────────────────────────────────────
 *
 * NutritionCard z návrhu měl vzorová čísla přímo v signatuře komponenty
 * (`currentCalories = 2146`), takže při chybějících datech tiše vykreslil
 * maketu. NutritionSection k tomu přidával štítek „Fáze: Čistá hypertrofie“ —
 * tréninkovou fázi v produktu nesledujeme.
 */

test('žádné výchozí hodnoty props se vzorovými čísly', () => {
  // `const X = ({ a = 2146 })` — fallback na maketu místo prázdného stavu.
  for (const f of SOUBORY) {
    const kod = kodBezKomentaru(obsah(f));
...
test('PDF export jde přes lib/planPdf.js, ne přes druhý mechanismus', () => {
  ...
  // Dva generátory PDF vedle sebe by znamenaly dvě různé podoby jídelníčku.
  const kod = obsah('components/PlanViewer.js');
  assert.ok(kod.includes("import { buildPlanPdfHtml }"), 'PlanViewer musí používat lib/planPdf.js');
  assert.ok(!/window\.print\(\)/.test(kodBezKomentaru(kod)), 'druhý mechanismus tisku');
});

test('vlastní jídlo se nikde nedopočítává odhadem', () => {
  // Kdyby endpoint nebo panel dosazoval kalorie, součet by míchal měřené
  // a hádané hodnoty — přesně to, co má hlídat metrika `complete` u receptů.
  const kod = kodBezKomentaru(obsah('lib/profile/vlastniJidlo.js'));
  assert.ok(!/kcal_rucne\s*\?\?\s*\d/.test(kod), 'kalorie se nesmí dosazovat');
  assert.ok(!/Number\(.*kcal.*\)\s*\|\|\s*\d/.test(kod), 'kalorie se nesmí dosazovat přes ||');
});
