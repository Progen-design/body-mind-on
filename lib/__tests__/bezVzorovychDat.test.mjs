...
// Živý ekvivalent: `App.tsx` čte `badHabits` (naZlozvyky) a posílá je do
// `OverviewBentoGrid.tsx` jako prop — ale `OverviewBentoGrid.tsx` je nikde
// neiteruje ani nevykresluje (ověřeno greppem: `badHabits.` se v souboru
// nevyskytuje ani jednou, na rozdíl od `habits.` u dobrých návyků). Data
// tečou, UI je zahazuje beze zbytku — není to špatná barva, je to úplná
// absence. Gap se nevymýšlí, jen se pinuje a hlásí v reportu k PROMPT_UKLID.md.
test('GAP: zlozvyky (badHabits) se v OverviewBentoGrid vůbec nevykreslují', () => {
  const app = kodBezKomentaruTsx(obsah('src/App.tsx'));
  assert.match(app, /badHabits=\{badHabits\}/, 'App.tsx pořád posílá badHabits do OverviewBentoGrid');

  const grid = kodBezKomentaruTsx(obsah('src/components/OverviewBentoGrid.tsx'));
  assert.ok(
    !/badHabits\.\w/.test(grid),
    'OverviewBentoGrid teď badHabits vůbec nepoužívá — pokud tenhle test spadl, ' +
    'vykreslování se dopsalo a tenhle test je zastaralý, přepiš ho na kontrolu červené barvy'
  );
});
