...
test('limit platí pro PŘEPOČTENOU hodnotu, ne jen pro registrační', () => {
  // Drobná starší žena v redukci. I s TDEE (9.1/B1) místo váhového vzorce
  // je 0,8× TDEE u dost nízkého BMR pořád pod absolutním minimem 1200.
  const bm = { gender: 'female', height_cm: 150, age: 60, goal: 'redukce', activity: 'nizka' };

  const cile = cilProVahu(bm, 35);
  const limit = minimalniKalorickyCil({ weightKg: 35, heightCm: 150, age: 60, gender: 'female' });
...
test('normální případ limitem neprochází a nehlásí ho', () => {
  ...
  // TDEE, ne váha (docs/DALSI_KROK.md 9.1/B1): BMR 1780 (viz test výš) ×
  // FAKTOR_AKTIVITY_STREDNE (1,55, VLASTNÍ tabulka pro TDEE, ne
  // activityMultiplier()) = 2759, „udržování" = TDEE beze změny.
  assert.equal(cile.calories_target, 2759);
