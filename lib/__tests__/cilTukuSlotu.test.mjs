...
// ─────────────────────────────────────────────────────────────────────────
// docs/DALSI_KROK.md 8.11 — VÁHY (bod 4) se NEMĚNÍ, dokud nejsou vyzkoušené
// body 1–3 (nouzová větev + tvrdý strop na výběr). Penalta výš je pořád
// spojitá a jde ji "přeplatit" kalorickou trefou — proto přibyl TVRDÝ STROP
// na to, kdo se do užšího výběru vůbec dostane: `prednostniPoolPodleTuku`.

test('prednostniPoolPodleTuku bez cíle vrací sortedRows beze změny', () => {
  const rows = [{ id: 'a', kcal: 700, fat_g: 100 }, { id: 'b', kcal: 700, fat_g: 5 }];
  const { pool, zPrednostniho } = prednostniPoolPodleTuku(rows, null, 5);
  assert.deepEqual(pool, rows);
  assert.equal(zPrednostniho, false);
});

test('prednostniPoolPodleTuku filtruje na STROP_TUKU_VYBERU a zachová pořadí', () => {
  const slot = 700;
  const rows = [
    { id: 'libovy', kcal: slot, fat_g: (0.28 * slot) / 9 },
    { id: 'hranice', kcal: slot, fat_g: (STROP_TUKU_VYBERU * slot) / 9 },
    { id: 'tucny', kcal: slot, fat_g: (0.50 * slot) / 9 },
  ];
  const { pool, zPrednostniho } = prednostniPoolPodleTuku(rows, 0.28, 2);
  assert.deepEqual(pool.map((r) => r.id), ['libovy', 'hranice'], 'přesně na stropu (≤) ještě patří do poolu');
  assert.equal(zPrednostniho, true);
});

test('KDYŽ JE PŘEDNOSTNÍCH MÍŇ NEŽ minPocet, doplní se zbytkem — libové PRVNÍ, tučné až za nimi', () => {
  const slot = 700;
  // Tučný je v `sortedRows` SCHVÁLNĚ na první pozici (nejlepší catalogPickRank,
  // třeba díky přesné kalorické trefě) a libový až za ním — přesně ten
  // případ, kdy „vrať sortedRows beze změny" by přednost úplně zahodilo:
  // tučný by zůstal první, i když libový byl hned vedle.
  const rows = [
    { id: 'tucny1', kcal: slot, fat_g: (0.50 * slot) / 9 },
    { id: 'libovy', kcal: slot, fat_g: (0.28 * slot) / 9 },
    { id: 'tucny2', kcal: slot, fat_g: (0.55 * slot) / 9 },
  ];
  const { pool, zPrednostniho } = prednostniPoolPodleTuku(rows, 0.28, 2);
  assert.deepEqual(
    pool.map((r) => r.id), ['libovy', 'tucny1', 'tucny2'],
    'libové musí být před tučnými, i když v původním pořadí (catalogPickRank) bylo tučné první; pořadí uvnitř skupin (tucny1 před tucny2) se nesmí rozhodit',
  );
  assert.equal(zPrednostniho, false);
});

test('prednostniPoolPodleTuku bere recept bez maker jako neutrální (smí do poolu)', () => {
  const rows = [{ id: 'bezMaker', kcal: 700, fat_g: null }];
  const { pool, zPrednostniho } = prednostniPoolPodleTuku(rows, 0.28, 1);
  assert.deepEqual(pool.map((r) => r.id), ['bezMaker']);
  assert.equal(zPrednostniho, true);
});

test('prednostniPoolPodleTuku na prázdném/chybějícím vstupu nespadne', () => {
  assert.deepEqual(prednostniPoolPodleTuku([], 0.28, 5), { pool: [], zPrednostniho: false });
  assert.deepEqual(prednostniPoolPodleTuku(null, 0.28, 5), { pool: [], zPrednostniho: false });
  assert.deepEqual(prednostniPoolPodleTuku(undefined, null, 5), { pool: [], zPrednostniho: false });
});
