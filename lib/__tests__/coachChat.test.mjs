...
test('prompt rozlisuje namerenou hodnotu (pravidlo A) od odvozeneho doporuceni (pravidlo B)', () => {
  // Pravidlo A: namerena hodnota se nevymysli, ani jako priklad — beze zmeny.
  assert.match(SYSTEM_PROMPT_CHATU, /PRAVIDLO A/);
  assert.match(SYSTEM_PROMPT_CHATU, /naměřenou hodnotu si nikdy nevymýšlíš/i);
  assert.match(SYSTEM_PROMPT_CHATU, /Nedopočítávej ho, neodhaduj, neber ho z obecných znalostí/i);
  assert.match(SYSTEM_PROMPT_CHATU, /Ani jako příklad/i);

  // Pravidlo B: odvozene doporuceni (pitny rezim apod.) spocitat SMI a MA.
  assert.match(SYSTEM_PROMPT_CHATU, /PRAVIDLO B/);
  assert.match(SYSTEM_PROMPT_CHATU, /odvozené doporučení spočítat smíš a máš/i);
  // Musi rict, z ceho vypocet vysel — jinak vypada jako namerene cislo.
  assert.match(SYSTEM_PROMPT_CHATU, /vždy řekni, z čeho vyšla/i);
});

test('sekce HRANICE se pri rozdeleni pravidla nezmenila ani o pismeno', () => {
  const HRANICE_DNES = `HRANICE
...
- Když si nejsi jistý, řekni to. Nevymyšlené „nevím" je lepší než jistota bez dat.`;
  ...
  assert.ok(
    SYSTEM_PROMPT_CHATU.includes(HRANICE_DNES),
    'sekce HRANICE se nesmi menit — odpoved na dotaz o steroidech byla spravna'
  );
});
