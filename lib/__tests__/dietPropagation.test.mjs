...
test('calories_target patch píše jen sloupce, které v body_metrics jsou', () => {
  // `protein_g`/`carbs_g`/`fat_g` (bez `_target_g`) v tabulce nejsou — to
  // dřív Postgres odmítl a neuložilo se ani `calories_target`. Chyba se
  // spolkla do console.warn a v logu ležela potřetí.
  ...
  assert.deepEqual(
    ...
    ['calories_target', 'carbs_target_g', 'fat_target_g', 'protein_target_g'],
...
test('calories_target patch přepočítá makra spolu s kaloriemi, ne jen kalorie', () => {
  // docs/DALSI_KROK.md 6.7(a): po přepočtu cíle (výška, váha, preference)
  // zůstávala makra ze starého cíle — součet gramů seděl na dřívější
  // calories_target, ne na nový. Makra musí být spočítaná ze stejného
  // volání calculateNutritionTargets jako calories_target, ne převzatá
  // odjinud jako mrtvá kopie.
  const bodyMetrics = {
    weight_kg: 80,
    goal: 'redukce',
    activity: 'stredni',
  ...
  const patch = buildCalorieTargetBodyMetricsPatch(bodyMetrics, { forceRecalculate: true });

  const soucetKcal = patch.protein_target_g * 4 + patch.carbs_target_g * 4 + patch.fat_target_g * 9;
  assert.ok(
    Math.abs(soucetKcal - patch.calories_target) <= 8,
    `makra (${patch.protein_target_g}/${patch.carbs_target_g}/${patch.fat_target_g} g = ${soucetKcal} kcal) neodpovídají calories_target (${patch.calories_target})`
  );
  assert.notEqual(
    patch.protein_target_g,
    bodyMetrics.protein_target_g,
    'protein_target_g zůstal beze změny ze starého cíle'
  );
});
