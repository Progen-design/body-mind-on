...
test('každý klíč s natvrdo zapsaným GIFem má očekávaný řádek v registry', () => {
  // TRUSTED_EXERCISE_GIF_BY_KEY i TRUSTED_EXTENDED_GIF_BY_KEY jsou od 14. 9. 2026
  // natrvalo prázdné (docs/DALSI_KROK.md 9.12 — byla to díra, přes kterou se
  // uživateli i po smazání z DB pořád ukazovaly Gym Visual GIFy). Test proto
  // teď projde vždy — necháváme ho tu jako hlídku, kdyby se do slovníku někdy
  // znovu něco natvrdo zapsalo.
  const ocekavane = ocekavaneKliceZMigrace();
  const vsechnyGify = {
    ...TRUSTED_EXERCISE_GIF_BY_KEY,
    ...TRUSTED_EXTENDED_GIF_BY_KEY,
