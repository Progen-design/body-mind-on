...
test('oslovení používá sdílený vokativ (lib/vokativ.js), nejisté jméno vynechá', () => {
  ...
  // souhláskové jména, která dřív vypadla — teď je skloňuje sdílený vokativ
  assert.equal(osloveni('Jan Příkopa'), 'Ahoj Jane,');
  assert.equal(osloveni('Tomas'), 'Ahoj Tomáši,');
  assert.equal(osloveni('Tomáš'), 'Ahoj Tomáši,');
  assert.equal(osloveni('Petr'), 'Ahoj Petře,');
  assert.equal(osloveni('Marek'), 'Ahoj Marku,');
  // neznámé nebo nejisté jméno → jen „Ahoj,“
  for (const j of ['Denis', 'X', null, '   ', 'Jan2']) {
