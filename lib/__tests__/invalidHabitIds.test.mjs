...
test('endpointy návyky od 21. 9. 2026 nepřijímají — validátor zůstává jen jako sdílená utilita', () => {
  const prefs = fs.readFileSync(new URL('../../api/profile-preferences.js', import.meta.url), 'utf8');
  const registrace = fs.readFileSync(
    new URL('../registration/bodyMetricsRegistration.js', import.meta.url),
    'utf8'
  );
  assert.ok(!/invalidHabitIds\(b\.selected_habits\)/.test(prefs), 'profile-preferences už návyky nepřijímá');
  assert.ok(!/invalidHabitIds\(b\.selected_habits\)/.test(registrace), 'registrace už návyky nepřijímá');
});
