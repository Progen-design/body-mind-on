...
test('karta bere komentáře z last_replies, nechodí si pro ně zvlášť', () => {
  ...
  // Od feedu ve stylu Instagramu (PR 3) karta nevolá API vůbec — „Napiš
  // komentář…" otevře vlákno s chatovací lištou. Kdyby si každá karta
  // tahala vlákno sama, znamenal by jeden scroll feedu dvacet dotazů.
  for (const [soubor, kod] of [['karta', karta], ['komentáře', komentare]]) {
    assert.doesNotMatch(kod, /apiFetch/, `${soubor} volá API`);
  }
  // Odesílá se jen z vlákna, přes stejný endpoint.
  assert.match(cti('src/components/komunita/CommunityPostDetail.tsx'), /'\/api\/community\/reply'/);
});
