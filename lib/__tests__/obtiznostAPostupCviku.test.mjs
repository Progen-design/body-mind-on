...
test('easier_key/harder_key ukazují jen na klíče, které v seedu existují (žádný mrtvý odkaz)', () => {
  const klice = new Set(seedKlice());
  for (const [klic, radek] of Object.entries(SEED_OBTIZNOST_A_POSTUP_CVIKU)) {
    if (radek.easier_key) assert.ok(klice.has(radek.easier_key), `${klic}.easier_key -> ${radek.easier_key} neexistuje v seedu`);
    if (radek.harder_key) assert.ok(klice.has(radek.harder_key), `${klic}.harder_key -> ${radek.harder_key} neexistuje v seedu`);
  }
});
...
test('pushup.easier_key je záměrně NULL ("na kolenou" variantu nemáme) — regrese na zadání', () => {
  assert.equal(SEED_OBTIZNOST_A_POSTUP_CVIKU.pushup.easier_key, null);
});
