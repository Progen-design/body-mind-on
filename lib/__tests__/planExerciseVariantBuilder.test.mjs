...
test('sestavVariantuCviku: nese svalovou partii a nářadí, aby karta po záměně neztratila „zabírá <partie>"', () => {
  const out = sestavVariantuCviku({
    targetRow: {
    ...
    nazevPodleKlice: new Map(),
    previousTitle: 'Rumunský mrtvý tah',
    current: { sets: 3, reps: '8-10', duration_sec: null },
  });
  assert.equal(out.primary_muscle, 'hamstrings');
  assert.equal(out.equipment_class, 'dumbbell');
  assert.deepEqual(out.instructions_cs, ['Krok jedna.', 'Krok dva.']);
  assert.equal(out.obtiznost, 'lehké');
  assert.equal(out.sets, 3);
  assert.equal(out.reps, '8-10');
});

test('sestavVariantuCviku: chybějící partie se vrací jako null, nikdy jako vymyšlená hodnota', () => {
  const out = sestavVariantuCviku({
    targetRow: { canonical_key: 'plank', display_name_cs: 'Prkno', level: 'beginner' },
    nazevPodleKlice: new Map(),
    previousTitle: 'Boční prkno',
  });
  assert.equal(out.primary_muscle, null);
  assert.equal(out.equipment_class, null);
});
