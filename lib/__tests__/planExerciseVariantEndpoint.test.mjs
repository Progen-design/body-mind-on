...
test('UPDATE je taky omezený na vlastníka — obrana do hloubky, ne jen SELECT', () => {
  // Hledá se regexem přes libovolné bílé znaky, ne přesným řetězcem s "\n":
  // git na Windows dělá při checkoutu z LF CRLF, takže doslovné hledání
  // odsazení podle konce řádku po `git pull` přestane sedět.
  const m = /\.update\(\{\s*structured_plan_json/.exec(zdroj);
  assert.ok(m, 'update blok nenalezen');
  const usek = zdroj.slice(m.index, m.index + 300);
  assert.match(usek, /\.eq\('id', planId\)/);
  assert.match(usek, /\.eq\('user_id', user\.id\)/);
});
