...
test('buildStartSafeFallbackMeal: vyloučené jídlo se nevrátí, i když je kalorický nejbližší', () => {
  // Jméno musí být bez knihovního receptu, ať se testuje snapshotová větev
  // (ne knihovní, viz `hasSimpleStartRecipeTitle` na začátku funkce). Dřív
  // tu bylo 'Tvaroh s vločkami a banánem' — PROMPT_PRO_CODE.md PR 1
  // (2026-09-17) pro něj recept doplnil, takže by teď šlo přes knihovní
  // větev a testovalo by se tím jiné chování, než chce tenhle test.
  const slotMeal = {
    type: 'breakfast',
    name_cs: 'Nesmyslné jídlo bez receptu xyz789',
    fallback_meal_template: { name_cs: 'Nesmyslné jídlo bez receptu xyz789', kcal: 420 },
...
test('pickSimpleStartMealAlternative: requireLibraryRecipe=true vybere jen šablonu s knihovním receptem', () => {
  ...
  assert.ok(vybrano, 'mělo se něco vybrat — standard/breakfast má šablony s receptem');
  // Dřív se tu kontrolovalo i `vybrano.title !== 'tvaroh s vločkami a banánem'`
  // — v době vzniku testu to byla JEDINÁ šablona bez receptu blízko cíle, takže
  // šlo o pojistku proti konkrétnímu produkčnímu nálezu. PROMPT_PRO_CODE.md
  // PR 1 (2026-09-17) pro ni recept doplnil, takže je teď platný kandidát;
  // skutečný invariant hlídá řádek níž (`vybrano.lib`), ne konkrétní název.
