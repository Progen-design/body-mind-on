 ...
 *
 * STROP, KTERÝ TENHLE BOD PŘÍMO NEZVEDÁ. U vegan diety má
 * `pickSimpleStartMealAlternative` s `requireLibraryRecipe: true` (jak ho
 * volá `replaceMealInStructuredPlan`) i po opravě lookupu 0 kandidátů pro
 * snack a dinner (obě šablony jsou tam BEZ receptu vůbec, ne jen s jiným
 * typem — žádná ze 7 oprav se jich netýká). U breakfastu a lunche oprava
 * lookupu 1 kandidáta odemkne (`Chleba s arašídovým máslem a banánem` pro
 * breakfast; `Fazole s rýží` pro lunch měla recept už dřív). Zbytek
 * (14 šablon zcela bez receptu) je bod D, tenhle PR ho neřeší.
 */
...
// 4 kandidáti s receptem v toleranci — Vejce (450), Ovesná kaše (480),
// Cottage s pečivem (340, nový díky opravě E1: recept je snack, šablona
// breakfast), Šunka (400). Ověřeno přímo přes pickSimpleStartMealAlternative.
...
test('snídaně: opakovaná záměna TÉHOŽ slotu projde všechny 4 kandidáty v pravidelné rotaci', async () => {
  ...
  assert.equal(
    distinct.size, 4,
    `za 8 záměn se měly ukázat všechny 4 kandidáty, ukázalo se ${distinct.size}: ${[...distinct].join(', ')}`
  );
...
// 6 kandidátů v toleranci (bylo 3 se starou ±20 % tolerancí a starým,
...
test('svačina: 6 záměn po sobě na témže slotu dá 6 různých jídel (bylo jen 2, pak 3 po #237)', async () => {
  ...
  assert.equal(
    distinct.size, 6,
    `za 6 záměn se mělo ukázat všech 6 kandidátů, ukázalo se ${distinct.size}: ${[...distinct].join(', ')}`
  );
  for (const title of titles) {
...
// ─────────────────────────────────────────────────────────────────────────
// Vegan strop — bod D/E ho nezvedá úplně (viz hlavička souboru).
// ─────────────────────────────────────────────────────────────────────────

test('vegan snack: záměna zůstává NO_ALTERNATIVE i po opravě lookupu — obě šablony jsou bez receptu vůbec', async () => {
  const veganDen = [
  ...
  await assert.rejects(
    () => replaceMealInStructuredPlan(plan, 0, 2, veganBodyMetrics),
    /NO_ALTERNATIVE/,
    'vegan/snack má obě šablony zcela bez receptu (bod D) — oprava lookupu na to nemá vliv'
  );
});
