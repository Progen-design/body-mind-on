...
/**
 * Je `title` v kalorické toleranci `max(20 %, 120 kcal)` cíle? Používá stejný
 * zdroj bazální kcal jako `pickSimpleStartMealAlternative` (knihovní recept),
 * takže test nemusí hardcodovat konkrétní sadu jmen — knihovna PROMPT_PRO_CODE.md
 * bodem C dál roste a hardcoded seznam by po každém přidání šablony zastaral.
 */
function jeVTolerance(title, mealType, targetKcal) {
  const lib = findSimpleStartRecipeByTitle(title, mealType);
  if (!lib) return false;
  return Math.abs(lib.calories - targetKcal) <= Math.max(targetKcal * 0.2, 120);
}
...
// PROMPT_PRO_CODE.md bod C (2026-09-17) dál rozšiřuje knihovnu, takže počet
// a jména kandidátů v toleranci nejsou stabilní — kontroluje se MECHANISMUS
// (round-robin navštíví víc než dva kandidáty, nikdy mimo toleranci), ne
// pevná množina jmen.
test('snídaně: opakovaná záměna TÉHOŽ slotu projde víc než dva kandidáty v pravidelné rotaci', async () => {
  ...
  assert.ok(
    distinct.size > 2,
    `za 8 záměn se měl ukázat víc než dvoucyklus (#237), ukázalo se ${distinct.size}: ${[...distinct].join(', ')}`
  );
  for (const title of titles) {
    assert.ok(jeVTolerance(title, 'breakfast', 440), `"${title}" není mezi kandidáty v kalorické toleranci`);
  }
});

// 5 jídel/den @ 2200 kcal, druhý snack -> slotTargetKcal(snack) = 308 kcal.
// Stejný důvod jako u snídaně výš — knihovna roste (bod C), kontroluje se
// mechanismus, ne pevná množina jmen.
...
test('svačina: opakovaná záměna TÉHOŽ slotu dá víc než dvě různá jídla (bylo jen 2, pak 3 po #237)', async () => {
  ...
  assert.ok(
    distinct.size > 3,
    `za 6 záměn se mělo ukázat víc než 3 jídla, ukázalo se ${distinct.size}: ${[...distinct].join(', ')}`
  );
  for (const title of titles) {
    assert.ok(jeVTolerance(title, 'snack', 308), `"${title}" není mezi kandidáty v kalorické toleranci`);
  }
});
...
test('E1: Jogurt s ovocem (recept otagovaný snack) je použitelný kandidát i pro breakfast', () => {
  ...
  const OSTATNI_SNIDANE = [
    ...
    // PROMPT_PRO_CODE.md PR 2 přidal do standard/breakfast dvě další
    // šablony s knihovním receptem (vysokokalorické varianty) — musí se
    // vyřadit taky, jinak bez targetKcal vyhraje jedna z nich (jsou v poli
    // první) místo "Jogurt s ovocem", o kterém je tenhle test.
    'Ovesná kaše s proteinem, velká porce',
    'Vejce se šunkou a pečivem, velká porce',
  ];
  const vybrano = pickSimpleStartMealAlternative({
    mealType: 'breakfast',
...
test('PR 2: vysokokalorické varianty pokrývají cíle, které byly dřív mimo toleranci', () => {
  // PROMPT_PRO_CODE.md PR 2 (2026-09-17). Ověřeno přes git stash, že na
  // kódu PŘED touhle šablonovou dávkou byly obě množiny prázdné (E2 test
  // výš to hlídal jmenovitě). Práh je max(20 %, 120 kcal).
  const lunch784 = pickSimpleStartMealAlternative({
    mealType: 'lunch',
    currentTitle: 'nic-takoveho-neexistuje',
    bodyMetrics: { diet_type: 'standard' },
    targetKcal: 784,
  ...
  assert.ok(lunch784, 'lunch@784 musí mít kandidáta po PR 2');
  assert.ok(
    Math.abs(lunch784.baseKcal - 784) <= Math.max(784 * 0.2, 120),
    `lunch@784: "${lunch784.title}" (${lunch784.baseKcal} kcal) měl vyjít V toleranci`
  );

  const breakfast660 = pickSimpleStartMealAlternative({
    mealType: 'breakfast',
    currentTitle: 'nic-takoveho-neexistuje',
    bodyMetrics: { diet_type: 'standard' },
    targetKcal: 660,
  ...
  assert.ok(breakfast660, 'breakfast@660 musí mít kandidáta po PR 2');
  assert.ok(
    Math.abs(breakfast660.baseKcal - 660) <= Math.max(660 * 0.2, 120),
    `breakfast@660: "${breakfast660.title}" (${breakfast660.baseKcal} kcal) měl vyjít V toleranci`
  );
});

test('E2: mechanismus — prázdná množina v toleranci spadne na legacyFallback, ne na chybu nebo tichý průchod', () => {
  // Dřív tenhle test hlídal konkrétní cíle (2800/5 lunch @784, 3300/5
  // breakfast/lunch @660/@924), u kterých byla množina v toleranci prázdná.
  // PROMPT_PRO_CODE.md PR 2 (2026-09-17) doplnil vysokokalorické varianty
  // právě do tohohle rozsahu — @784 a @660 už dnes kandidáta MAJÍ (to je
  // úspěch PR 2, ne regrese). Přesně tohle si měl tenhle test podle
  // hlavičky souboru pamatovat: „po PR 2 už množina prázdná nebude, takže
  // ten test napiš tak, aby popisoval mechanismus, ne konkrétní cíl."
  //
  // Cíl níž (5000 kcal) je záměrně o řád nad čímkoli v knihovně (nejvyšší
  // šablona po PR 2 je 1009 kcal lunch) — mechanismus (prázdná množina ->
  // legacyFallback na nejbližší MIMO toleranci) se testuje bez vazby na
  // aktuální strop, který se bude PR 2/3 a budoucím obohacením dál hýbat.
  const target = 5000;
  for (const mealType of ['lunch', 'breakfast', 'dinner']) {
    const vybrano = pickSimpleStartMealAlternative({
      mealType,
      currentTitle: 'nic-takoveho-neexistuje',
      bodyMetrics: { diet_type: 'standard' },
    ...
    assert.ok(
      ...
      `${mealType}@${target}: "${vybrano.title}" (${vybrano.baseKcal} kcal) je nečekaně V toleranci`
    );
  }
});
