...
test('GAP: žádná komponenta v src/ nečte plan_renewal ani nezobrazuje notu o rozsahu trialu', () => {
  ...
  const cteDoPlanRenewal = soubory.filter((p) => /plan_renewal/.test(readFileSync(p, 'utf8')));

  assert.deepEqual(
    cteDoPlanRenewal,
    [],
    'src/ teď plan_renewal nečte vůbec — pokud tenhle test spadl, frontend stranu ' +
    'už někdo dopsal a tenhle test je zastaralý, přepiš ho na kontrolu skutečné komponenty'
  );
});
