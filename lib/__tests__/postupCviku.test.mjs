...
test('cron: recepty mají přednost, cviky přijdou na řadu až když fronta receptů mlčí', () => {
  const cron = readFileSync(join(KOREN, 'api', 'cron', 'translate-recipes.js'), 'utf8');
  assert.match(cron, /runExerciseInstructionTranslation/);
  const podminka = cron.indexOf("if (result.translated === 0)");
  const volani = cron.indexOf('await runExerciseInstructionTranslation()');
  assert.ok(podminka > 0 && volani > podminka,
    'cviky se překládají jen když receptová dávka nic nepřeložila — jeden běh = nejvýš jedno velké OpenAI volání');
});
