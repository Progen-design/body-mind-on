...
test('webhook: invoice.paid je napojený, stav členství se v něm nemění', () => {
  const kod = readFileSync(join(import.meta.dirname, '..', '..', 'api', 'webhooks', 'stripe.js'), 'utf8');
  const vetev = kod.slice(kod.indexOf("case 'invoice.paid':"), kod.indexOf('default:', kod.indexOf("case 'invoice.paid':")));
  assert.ok(vetev.length > 50, 'větev invoice.paid chybí');
  assert.match(vetev, /zpracujInvoicePaid\(event\.data\.object/);
  assert.match(vetev, /failStripeEvent\(event\.id, vysledek\.chyba\)/);
  assert.doesNotMatch(vetev, /upsertMembership/);
});
