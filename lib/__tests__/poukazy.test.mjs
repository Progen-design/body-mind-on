...
test('Stripe trial_end: zbytek poukazového trialu, jen s poukazem a ≥ 48 h', () => {
  const clenstvi = { trial_ends_at: '2026-10-24T12:00:00Z', stripe_subscription_id: null };
  const p = { code: 'ABCD-EFGH-IJKL' };
  assert.equal(stripeTrialEndZPoukazu(clenstvi, p, TED), Date.parse('2026-10-24T12:00:00Z') / 1000);
  assert.equal(stripeTrialEndZPoukazu(clenstvi, null, TED), undefined, 'bez poukazu nic');
  assert.equal(stripeTrialEndZPoukazu({ ...clenstvi, stripe_subscription_id: 'sub_1' }, p, TED), undefined);
  assert.equal(stripeTrialEndZPoukazu({ ...clenstvi, trial_ends_at: '2026-09-25T12:00:00Z' }, p, TED), undefined, 'Stripe chce ≥ 48 h');
});
...
test('Stripe checkout: s poukazem trial_end, jinak jako dřív', () => {
  const api = cti('api/stripe/create-checkout-session.js');
  assert.match(api, /stripeTrialEndZPoukazu\(membership, poukaz\)/);
  assert.match(api, /subscriptionData\.trial_end = trialEnd;/);
  assert.match(api, /\} else if \(trialDays\) \{\s*subscriptionData\.trial_period_days = trialDays;/);
});
