...
test('prahShodnySMigraci — CHECK v migraci ma tentyz strop jako kod', () => {
  ...
  //
  // docs/DALSI_KROK.md 8.12: puvodni CHECK (20260823120000) mel strop 0,55 =
  // MEZE_PODILU.MAX_PODIL — teoreticky strop VYPOCTU dluhu pro zbytek dne,
  // ne toho, co se smi ZAPSAT do fronty. Kod ale od 8.5 kazdy zapis orizne
  // funkci omezPodilProObjednavku() na MAX_PODIL_OBJEDNAVKY (0,25) — schema
  // bylo slabsi nez kod a stare radky ve fronte si nesly az 0,55 dal
  // (zmereno: vegetarianska svacina s 55 % kalorii z bilkovin). Migrace
  // 20260904150000 CHECK zprisnila na 0,25 — test proto cte TENHLE soubor,
  // ne puvodni z 23. 8., ktery uz zivy stav nepopisuje.
  const sql = fs.readFileSync('supabase/migrations/20260904150000_uklid_fronty_protein_hint_a_nadbytecna.sql', 'utf8');
  ...
  assert.equal(shody[0], MAX_PODIL_OBJEDNAVKY, 'migrace a kod (omezPodilProObjednavku) se rozesly');
});
...
test('podil nad stropem objednavky se orizne, ne odmitne', () => {
  // docs/DALSI_KROK.md 8.12: od migrace 20260904150000 tohle hlida i schema
  // (CHECK na protein_hint <= 0.25, viz test prahShodnySMigraci vyse) — do
  // te doby to hlidal jen tenhle kod a stare radky ve fronte si nesly az
  // 0,55 dal, protoze CHECK jim to nezakazoval.
