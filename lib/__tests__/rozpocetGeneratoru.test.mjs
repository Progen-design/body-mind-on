...
import { estimateOpenAICostUSD } from '../openai.js';
...
test('preskoceny beh se PRESTO zapise do ai_runs, at je duvod videt', async () => {
  ...
  // PROMPT_NAKLADY_AI_DODATEK.md (2026-09-18) — `recipe_generator_beh` je
  // agregační řádek (viz AGREGACNI_PURPOSY, lib/openai.js), cenu nenese
  // v dolarových sloupcích ani u skipnutého běhu, proto null, ne 0.
  assert.equal(zaznam.radky.cost_usd, null);
});
...
// PROMPT_NAKLADY_AI.md (2026-09-18) — binarni "obsahuje mini" heuristika
// pocitala VSECHNY *-mini modely na sazbu gpt-4o-mini (0,15/0,60). gpt-4.1-mini
// (lib/spoonacular/catalogTranslate.js, lib/prekladPostupuCviku.js) stoji
// 0,40/1,60 - 2,67x vic. Overeno na platform.openai.com/docs/pricing.
test('odhad ceny nepocita gpt-4.1-mini na sazbu gpt-4o-mini', () => {
  const cena = estimateOpenAICostUSD('gpt-4.1-mini', 1_000_000, 1_000_000);
  assert.equal(Math.round(cena * 100) / 100, 2.0);
});

test('odhad ceny zna gpt-4.1', () => {
  const cena = estimateOpenAICostUSD('gpt-4.1', 1_000_000, 1_000_000);
  assert.equal(Math.round(cena * 100) / 100, 10.0);
});

test('neznamy model dostane konzervativni (nejdrazsi znamou) sazbu, ne nulu', () => {
  const cena = estimateOpenAICostUSD('gpt-5-nejaky-novy', 1_000_000, 1_000_000);
  assert.equal(Math.round(cena * 100) / 100, 12.5);
});
