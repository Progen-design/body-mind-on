...
test('volá se při odemčení, NE při trialing → active o týden později', () => {
  assert.match(webhook, /report\.membershipStatus === 'active' \|\| report\.membershipStatus === 'trial'\)/,
    'odemčení platí pro trial i active');

  // Jen checkout — v subscription.updated (trialing → active) se volat NESMÍ,
  // jinak by se plán posunul podruhé, týden po odemčení.
  const volani = [...webhook.matchAll(/await prekotviPrvniPlanNaOdemceni\(/g)];
  assert.equal(volani.length, 1, 'překotvení má jediné volání');
  const pred = webhook.slice(Math.max(0, volani[0].index - 400), volani[0].index);
  assert.match(pred, /if \(event\.type === 'checkout\.session\.completed' && aktivace\) \{/,
    'volání je jen ve větvi checkout.session.completed');
});
