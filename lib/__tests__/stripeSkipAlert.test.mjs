...
test('(c) webhook posílá do skipu konkrétní price ID', () => {
  const zdroj = readFileSync(join(KOREN, 'api', 'webhooks', 'stripe.js'), 'utf8');

  const vyskyty = [...zdroj.matchAll(/finishSkipped\(\s*event,\s*'skipped_unknown_price'([\s\S]{0,240}?)\)/g)];
  // Od syncSubscription (lib/stripeSync.js) jdou checkout i subscription.* jednou
  // cestou — neznámou cenu hlásí jedna větev pro všechny události.
  assert.ok(vyskyty.length >= 1, 'neznámá cena se musí hlásit do skipu');
