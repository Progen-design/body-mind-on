...
// PROMPT_UX_DOLADENI.md bod C (produkční nález, dead bug) — „na stranu"/
// „na nohu" (workoutStartProgram.js `reps_note`) se lepilo doprostřed věty:
// „3 série po 14–16 na stranu opakováních" místo „…opakováních na stranu".
test('jednostranné cviky: "na stranu"/"na nohu" patří AŽ ZA slovo "opakováních"', () => {
  assert.equal(serieOpakovaniSlovy(3, '14-16 na stranu'), '3 série po 14–16 opakováních na stranu');
  assert.equal(serieOpakovaniSlovy(3, '10-12 na stranu'), '3 série po 10–12 opakováních na stranu');
  assert.equal(serieOpakovaniSlovy(3, '10 na nohu'), '3 série po 10 opakováních na nohu');
});

test('výdrž na čas se nepopisuje jako opakování', () => {
  assert.equal(serieOpakovaniSlovy(3, '30 s'), '3 série po 30 s');
  // Doplněk platí i u výdrže na čas, ne jen u počtu opakování.
  assert.equal(serieOpakovaniSlovy(3, '30 s na stranu'), '3 série po 30 s na stranu');
});
