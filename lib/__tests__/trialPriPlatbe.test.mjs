...
test('webhook: checkout i subscription.updated zapisují stripe_subscription_id a trial_ends_at (přes syncSubscription)', () => {
  const api = cti('api/webhooks/stripe.js');
  const sync = cti('lib/stripeSync.js');
  // Checkout i subscription.* jdou přes jeden sync — žádný ruční zápis ve webhooku.
  assert.match(api, /'checkout\.session\.completed',[\s\S]{0,120}'customer\.subscription\.updated',/);
  assert.match(api, /report = await sync\(stripe, subId, \{ userIdHint \}\)/);
  assert.match(sync, /if \(stav\.status === 'trial'\) novy\.trial_ends_at = stav\.trialEndsAt;/);
  assert.match(sync, /if \(stav\.status === 'active'\) novy\.trial_ends_at = null;/);
  assert.match(sync, /stripe_subscription_id: sub\.id,/);
});
