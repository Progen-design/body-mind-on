...
test('brána platí i tady — trial úlohu nedostane', async () => {
  // `trial_ends_at` SE POČÍTÁ ZE SKUTEČNÉHO ČASU, NE Z `NOW`.
  //
  // `produceWeeklyTaskForUser` bere `now` a používá ho na výpočet cílového
  // týdne, ale členská brána si stáří trialu porovnává vlastními hodinami.
  // Fixture proto stavěná na `NOW + 1 den` fungovala jen do 14. 8. 2026 18:00
  // UTC — pak trial „vypršel“ a test začal dostávat
  // `start_trial_expired_upgrade_required`. V produkci se oba časy shodují,
  // takže to není chyba chování; je to chyba fixture, která se rozejde s realitou.
  // Kdyby se `now` mělo protáhnout až do brány, mění se lib, ne tenhle test.
