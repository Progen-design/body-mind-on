...
test('připojenému uživateli karta neříká, ať se připojí (docs/DALSI_KROK.md 7.2e)', () => {
  // Do 31. 8. 2026 se odstavec "Propojte svou chytrou váhu…" kreslil bez
  // podmínky i pod odznakem "Online" a "Poslední úspěšná synchronizace".
  const stav = withingsCardStav({ hasConnection: true, lastSyncedText: 'před 1 h 22 min' });
  assert.doesNotMatch(stav.description, /propojte|připoj/i, 'připojenému účtu karta pořád radí připojit se');
});

test('nepřipojenému uživateli karta nabídne propojení', () => {
  const stav = withingsCardStav({ hasConnection: false, lastSyncedText: null });
  assert.match(stav.description, /propoj/i);
});
