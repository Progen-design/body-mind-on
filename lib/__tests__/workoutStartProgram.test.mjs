/**
 * START PROGRAM — OPAKOVANÝ FULL-BODY A/B/C/D S PROGRESÍ.
 *
 ...
 *
 * DOCS/DALSI_KROK.md 8.15 (5. 9. 2026): do 3 tréninků týdně se dřív rotovaly
 * jen A/B (`PRAH_ROZSIRENE_ROTACE`, zrušeno) — C a D byly hotové pro všechna
 * tři prostředí, ale nikdy se nepoužily. Produkce: 9 různých cviků na plán,
 * „Prkno“ 42× napříč 21 plány. Teď se rotuje přes A-D vždy — při 3× týdně
 * A-B-C, další týden D-A-B, další C-D-A. Testy níž, které dřív čekaly pevné
 * A-B-A, jsou přepsané na tenhle nový, ale pořád deterministický vzorec.
 */
...
test('týden má tři různé tréninky (A-B-C), ne jeden opakovaný dvakrát (docs/DALSI_KROK.md 8.15)', () => {
  // Do 8.15 tu bylo "A-B-A, ne tři různé dny" — právě tahle pevná dvojice
  // znamenala, že C a D se při 3× týdně nikdy nepoužily (9 různých cviků na
  // plán na produkci). Teď je zadání opačné: tři různé dny, ne opakování.
  for (const bm of [GYM, HOME_BW, HOME_EQ]) {
    const { days } = tyden(bm);
    const varianty = days.map((d) => d.start_program_variant);
    assert.deepEqual(varianty, ['A', 'B', 'C'], `${bm.training_environment}: čekáno A-B-C`);

    const podpisy = new Set(days.map((d) => d.exercises.map((e) => e.canonical_key).join('|')));
    assert.equal(podpisy.size, 3, `${bm.training_environment}: musí být 3 různé tréninky, je ${podpisy.size}`);
  }
});

test('další týden pokračuje D-A-B, ne že by se A-B-C opakovalo', () => {
  const t0 = tyden(GYM, 0).days.map((d) => d.start_program_variant);
  const t1 = tyden(GYM, 1).days.map((d) => d.start_program_variant);
  assert.deepEqual(t0, ['A', 'B', 'C']);
  assert.deepEqual(t1, ['D', 'A', 'B']);

  // Stejná varianta ale MUSÍ dát stejné cviky bez ohledu na to, ve kterém
  // týdnu se objeví — obsah šablony se neváže na týden, jen na písmeno.
  const cvikyVariantu = (weekIndex, variant) => {
    const den = tyden(GYM, weekIndex).days.find((d) => d.start_program_variant === variant);
    assert.ok(den, `týden ${weekIndex}: varianta ${variant} chybí`);
    return den.exercises.map((e) => e.canonical_key);
  };
  assert.deepEqual(cvikyVariantu(0, 'A'), cvikyVariantu(1, 'A'));
  assert.deepEqual(cvikyVariantu(0, 'B'), cvikyVariantu(1, 'B'));
});

test('cviky se v týdnu OPAKUJÍ i se třemi variantami — 12 unikátních z 15, ne 15 různých', () => {
  ...
  // A, B a C sdílejí leg_press, plank a overhead_press (viz GYM_A/B/C výš) —
  // 15 pozic, 12 unikátních cviků.
  const unikatni = new Set(vsechny);
  assert.equal(unikatni.size, 12, `A+B+C sdílí tři cviky, čekáno 12 unikátních, ne ${unikatni.size}`);

  // Původní stav před 8.9 (patnáct různých cviků v jednom týdnu): tenhle test
  // by na něm spadl.
  assert.ok(unikatni.size < vsechny.length, 'aspoň jeden cvik se musí v týdnu opakovat');
});
...
test('A, B a C se po filtru prostředí navzájem liší', () => {
  for (const bm of [GYM, HOME_BW, HOME_EQ]) {
    ...
    assert.notEqual(podpis(days[1]), podpis(days[2]), `${env}: B a C jsou po filtru totožné`);
    assert.notEqual(podpis(days[0]), podpis(days[2]), `${env}: A a C jsou po filtru totožné`);
  }
});
...
test('střídání A-D je deterministické napříč týdny (docs/DALSI_KROK.md 8.15)', () => {
  // 3× týdně: A-B-C | D-A-B | C-D-A | B-C-D … — všechny čtyři varianty
  // do tří týdnů, ne A-B-A napořád.
  const seq = [];
  for (let w = 0; w < 4; w += 1) {
    for (let s = 0; s < 3; s += 1) seq.push(startVariantForSession(w, s, 3));
  }
  assert.deepEqual(seq.join(''), 'ABCDABCDABCD');

  // 2× týdně: A-B | C-D — i při dvou trénincích týdně se dřív rotovalo jen
  // A/B napořád; teď se stejně jako u 3× a 1× projede celá čtveřice.
  const seq2 = [];
  for (let w = 0; w < 4; w += 1) {
    for (let s = 0; s < 2; s += 1) seq2.push(startVariantForSession(w, s, 2));
  }
  assert.deepEqual(seq2.join(''), 'ABCDABCDABCD'.slice(0, 8));

  // Stejný vstup musí dát stejný výstup — čistá funkce indexu, ne stav.
  assert.equal(startVariantForSession(3, 1, 3), startVariantForSession(3, 1, 3));
});
