...
const { membershipStateFromSubscription, vytvorWebhook } = await import('../../api/webhooks/stripe.js');
...
/** Na které subscription endpoint po úspěchu zavolal syncSubscription. */
const synced = [];
...
function sestav({ stripe, onClub = true, clenstvi = { status: 'active', tier: 'START', stripe_subscription_id: 'sub_1' } } = {}) {
  ...
  const handler = vytvorHandler({
    ...
    sync: async (_s, id) => { synced.push(id); return {}; },
...
test('upgrade aktivního STARTu: výměna ceny, doúčtování hned, potvrzení smlouvy e-mailem', async () => {
  ...
  assert.ok(synced.includes('sub_1'), 'po změně se DB srovná přes syncSubscription');
...
async function posliWebhook(sub, id) {
  ...
  // Stripe API = atrapa (sync si subscription načte čerstvě), podpis skutečný.
  const webhook = vytvorWebhook({ stripe: () => ({ subscriptions: { retrieve: async () => sub } }) });
...
test('cena mimo mapu START/ON_CLUB: GET vrátí důvod neznama_cena, POST 409 s důvodem', async () => {
  const stripe = atrapaStripe({ sub: predplatne({ cena: 'price_zahada' }) });
  const { handler } = sestav({ stripe });
  const g = await zavolej(handler, 'GET');
  assert.deepEqual(g.telo, { muze_menit: false, duvod: 'neznama_cena' });
  const p = await zavolej(handler, 'POST', { tier: 'ON_CLUB', souhlas: true });
  assert.equal(p.stav, 409);
  assert.equal(p.telo.duvod, 'neznama_cena');
  assert.equal(stripe.z.update.length, 0);
});
