...
function buildTriggerState(hasPlan, hasVeryRecentPlan, hint, eventType, hasEverHadPlan = false) {
  return {
    // `missing_plan` znamená „ještě nikdy žádný plán nedostal“, ne „zrovna teď
    // žádný nemá“. Rozdíl je podstatný od chvíle, kdy deactivate_expired_plans
    // sundává propadlé plány: bez téhle podmínky by každý uživatel po expiraci
    // spadl do missing_plan a decision engine by mu každou hodinu zakládal nový
    // initial_plan. Dedup v createAITasksFromDecisions chytá jen pending/processing,
    // takže dokončený task další běh nezastaví — je to přesně ta smyčka, kvůli
    // které bylo generateAITasks zamrzlé a nakonec odstraněné.
    //
    // Obnovu plánu po expiraci dělá producent týdenních úloh
    // (lib/weeklyPlanProducer.js) přes weekly_plan_update, s idempotencí
    // vynucenou v databázi. Decision engine do toho nemluví.
    missing_plan: !hasPlan && !hasEverHadPlan,
...
export async function evaluateUserState(userId, options = {}) {
  ...
  const [metricsRes, latestPlanRes, sharedFactsRes, progress, dbRules] = await Promise.all([
    ...
    supabaseServer
      ...
      .limit(1)
      .maybeSingle(),
    // Load multiple shared facts (not just 1) for richer decision context
    supabaseServer
      .from('user_ai_memory')
      .select('memory_type, content, created_at, source_agent_slug')
      .eq('user_id', userId)
      .ilike('memory_type', 'shared_%')
      .order('created_at', { ascending: false })
      .limit(5),
  ...
  // Normalized shared facts bundle – passed to task payloads for agent context
  const sharedFacts = (sharedFactsRes?.data ?? []).map((r) => ({
    type: r.memory_type,
    content: r.content,
  ...
  const basePayload = {
    goal,
    // Full shared facts array (trainers and coach tasks receive this in their payload)
    shared_facts: sharedFacts,
    ...eventPayload,
  };

  // Řádek v ai_generated_plans je jediný důkaz, že uživatel někdy plán dostal —
  // is_active o tom nevypovídá, ten se po expiraci sundává.
  const hasEverHadPlan = latestPlan != null;

  const state = {
    ...buildTriggerState(hasPlan, hasVeryRecentPlan, hint, eventType, hasEverHadPlan),
    hasVeryRecentPlan,
  };
