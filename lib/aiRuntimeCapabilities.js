...
export function getAIRuntimeCapabilities() {
  ...
  return {
    ...
    ai: {
      ...
      web_search_runtime: process.env.OPENAI_WEB_SEARCH_ENABLED === 'true',
      notes:
        'Agents run via OpenAI Responses API. Web search is available when OPENAI_WEB_SEARCH_ENABLED=true. Supporting documents are passed in context.',
    },
    enrichment: {
      spoonacular: {
