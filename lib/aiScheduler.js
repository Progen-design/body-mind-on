...
/** Max. úloh za jedno volání cronu / run-scheduler (default 1 — jeden task může použít celý maxDuration). Více přes AI_SCHEDULER_MAX_TASKS_PER_RUN (max 30). */
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      if (task.agent_slug === 'trainer' && task.task_type === 'initial_plan') {
        console.error('[aiScheduler] trainer/initial_plan failed', { task_id: task.id, user_id: task.user_id, error: errMsg });
      }
