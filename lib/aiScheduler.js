...
import { executeAITask } from './aiTaskExecutors';
    ...
    try {
      ...
      await supabaseServer
        ...
        .update({
          ...
          result: execution?.result ?? { error: 'Task executor returned no result' },
      ...
      await writeAILog({
        ...
        message: execution?.result?.side_effect || task.task_type,
      });

      if (execution?.ok) completed++;
      else failed++;
    } catch (err) {
