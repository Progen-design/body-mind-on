...
import { processPendingAIEvents } from './aiEvents.js';
import { getMaxTaskAttempts, getRetryBackoffMinutes, writeAILog } from './aiOps.js';
import { executeAITask } from './taskExecutors.js';
import { isPermanentSpoonacularOrTaskError } from './spoonacularHttp.js';
import { canRunPlanTask } from './planGenerationGate.js';
...
async function executeSchedulerTaskBatch(tasks, nowIso, opts = {}) {
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      // Guard platí pro OBA typy plánovacích úloh. Dřív pokrýval jen initial_plan,
      // takže weekly_plan_update, který doběhl bez vloženého plánu, se označil jako
      // completed — úloha „hotová“, plán nikde a nikdo se to nedozvěděl. U weekly
      // je to horší než u initial: idempotenční klíč zůstane v tabulce, takže
      // producent na ten týden znovu nesáhne a uživatel prostě plán nedostane.
      const isTrainerPlanTask = task.agent_slug === 'trainer'
        && (task.task_type === 'initial_plan' || task.task_type === 'weekly_plan_update');
      const result = execution?.result ?? {};
      const hasPlanId = result.outcome_type === 'plan_generated' && (result.plan_id != null && result.plan_id !== '');
      const mustFailCompleted = execution?.ok && isTrainerPlanTask && !hasPlanId;
      ...
      if (permanentFailure) {
        await supabaseServer
          ...
          .update({
            ...
            next_retry_at: null,
            last_error: sanitizeErrorMessage(finalResult?.error || execution?.error || 'permanent_failure'),
...
/**
 * Event-driven: zpracuje pending coach/onboarding_message pro konkrétního uživatele (registrace).
 * Idempotentní — pending claim; completed/failed task se přeskočí (žádná duplicitní zpráva).
 * @param {string} userId
 */
export async function runRegistrationCoachInline(userId) {
  if (!userId) return { skipped: true, reason: 'no_user' };
  if (process.env.AI_RUN_COACH_AFTER_INITIAL_PLAN === 'false') {
    return { skipped: true, reason: 'disabled' };
  }

  const { data: coachTask } = await supabaseServer
    .from('ai_tasks')
    .select('id, user_id, agent_slug, task_type, payload, status')
    .eq('user_id', userId)
    .eq('agent_slug', 'coach')
    .eq('task_type', 'onboarding_message')
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();

  if (!coachTask?.id) return { skipped: true, reason: 'no_task' };
  if (coachTask.status !== 'pending') {
    return { skipped: true, reason: 'already_processed', status: coachTask.status };
  }
  ...
  if (!t?.id) return { skipped: true, reason: 'claim_lost' };

  try {
    const execution = await executeAITask(t);
    const ok = execution?.ok === true;
    const finalResult = execution?.result ?? { error: 'Task executor returned no result' };
    await supabaseServer
      .from('ai_tasks')
      .update({
        ...
        result: finalResult,
        processed_at: new Date().toISOString(),
        last_error: ok ? null : sanitizeErrorMessage(finalResult?.error || execution?.error || 'coach_inline_failed'),
      })
      .eq('id', t.id);
    ...
    await writeAILog({
      ...
      user_id: userId,
      task_id: t.id,
      status: finalResult?.fallback === true ? 'fallback' : (ok ? 'completed' : 'failed'),
      cache_hit: false,
      duration_ms: 0,
      input_tokens: 0,
      output_tokens: 0,
      estimated_cost_usd: 0,
      message: finalResult?.summary ?? 'registration_coach_inline',
    });
    ...
    await supabaseServer
      ...
      .update({
        ...
        last_error: errMsg,
        result: { error: errMsg, registration_inline_retry: true },
      })
      .eq('id', t.id);
    console.warn('[runRegistrationCoachInline] failed — left pending for daily cron fallback', {
      user_id: userId,
      task_id: t.id,
      error: errMsg,
    });
    return { processed: true, ok: false, task_id: t.id, error: errMsg, left_pending: true };
  }
}

/**
 * Coach-only scheduler — nebere trainer/plan tasky, nevolá decision engine.
 * Denní Vercel cron fallback pro pending coach tasky (Hobby-safe).
