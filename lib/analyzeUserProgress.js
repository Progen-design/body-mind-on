...
import { fetchRecentAdherenceAverage } from './dailyAdherence.js';
...
export async function analyzeUserProgress(userId) {
  ...
  try {
    ...
    const derivedAdherence = await fetchRecentAdherenceAverage(userId, ADHERENCE_LOOKBACK_DAYS);
    const adherence_score = derivedAdherence != null
      ? derivedAdherence
