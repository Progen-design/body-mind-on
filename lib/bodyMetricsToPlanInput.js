...
export function bodyMetricsToPlanInput(bm) {
  ...
  const workoutsPerWeek = deriveWorkoutsPerWeek(bm.freq_choice, bm.weekly_sessions_user);
  ...
  return {
    ...
    diet_type: dietType,
    meals_per_day: Math.min(6, Math.max(2, asNum(bm.meals_per_day) ?? 3)),
    workouts_per_week: workoutsPerWeek,
    preferred_workout_days: preferredWorkoutDays.length > 0 ? preferredWorkoutDays : [1, 3, 5].slice(0, workoutsPerWeek),
    equipment: Array.isArray(bm.equipment) ? bm.equipment.join(', ') : (bm.equipment || 'bodyweight'),
    age: asNum(bm.age),
    ...
    workout_duration_min: Math.min(90, Math.max(20, asNum(bm.workout_duration_min) ?? 45)),
  };
}
