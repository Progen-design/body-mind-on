...
import { computeTargetsForPlan } from './services/deterministicFallback.js';
import {
...
export function bodyMetricsToPlanInput(bm) {
  ...
  // DIETU TU NESMÍME ZPLOŠTIT.
  //
  // Do 10. 8. 2026 tu stálo `['vegetarian','vegan'].includes(dietRaw) ? dietRaw
  // : 'standard'`. Výstup téhle funkce se v runUnifiedPlanPipeline rozprostírá
  // PŘES body_metrics (`{ ...bm, ...planNorm }`), takže tenhle řádek přepsal
  // uživateli `diet_type: 'gluten_free'` na `'standard'` — a všechno za tím už
  // pracovalo s nepravdou:
  //   • dietTagsFromProfile() dostal 'standard' → dietTags: [] v 16 dotazech
  //   • buildSimpleStartMealSkeleton() → šablony 'standard' → „Cottage
  //     s pečivem“, „Sendvič se šunkou“
  //   • enforceDietaryPublishGate() čte původní bm, takže jediný dietu znal —
  //     a plán odmítl. Tři kola hledání, protože brána byla poslední, kdo měl
  //     pravdu, a první, kdo si stěžoval.
  //
  // `diet_type` je teď skutečná hodnota. Kdo potřebuje úzký trojstav
  // (standard|vegetarian|vegan) — dnes jen prompt pro GPT — vezme si
  // `diet_type_ai`. Zúžení je jeho starost, ne starost zdroje pravdy.
  const dietType = (bm.diet_type || 'standard').toLowerCase().trim() || 'standard';
  ...
  return {
    ...
    diet_type: dietType,
    /** Úzký trojstav pro GPT prompt. NIKDY nepoužívat k filtrování jídel. */
    diet_type_ai: ['vegetarian', 'vegan'].includes(dietType) ? dietType : 'standard',
