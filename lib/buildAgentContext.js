...
import { analyzeUserProgress } from './analyzeUserProgress.js';
import { getAIRuntimeCapabilities } from './aiRuntimeCapabilities.js';
import { getSharedMemory, getAgentSpecificMemory } from './aiSharedMemory.js';
import { loadAgentDocumentsContext } from './loadAgentDocumentsContext.js';
...
export async function buildAgentContext(profileSlugOrAgentSlug, userId, extraInput = {}, agentSlugHint = null) {
  ...
  if (branch === 'trainer_coach') {
    base.supporting_documents = await loadAgentDocumentsContext(agentSlug);
    if (userId) {
      const [metricsRes, plansRes, progressResult, agentMemory, sharedMemory, habitsRes] = await Promise.all([
        supabaseServer
          .from('body_metrics')
          .select('*')
          .eq('user_id', userId)
        ...
        // Agent-specific memory (non-shared, filtered by this agent)
        getAgentSpecificMemory(userId, agentSlug || 'trainer', MAX_MEMORY_RECORDS),
        // Shared cross-agent memory (trainer + coach both see this)
        getSharedMemory(userId, 8),
        supabaseServer
          .from('user_habits')
      ...
      // TED (chat) HTML PLÁNU NEDOSTÁVÁ — dostává `request.dnesni_plan`.
      //
      // `plan_html` má v produkci ~46 000 znaků, sem se vejde 12 000, takže do
      // kontextu šla necelá čtvrtina týdne bez označení, který den je dnes.
      // Ověřeno naostro 9. 9. 2026: TED na otázku „proč mám dnes zrovna tenhle
      // trénink" odpověděl, že plán pro dnešek nevidí, přestože profil nad
      // chatem ukazoval „Trénink B · 60 min".
      //
      // `api/coach-chat.js` mu místo toho posílá dnešek ze
      // `structured_plan_json` (lib/coachChatPlanDnes.js) — stovky tokenů
      // strukturovaně místo tisíců tokenů uříznutého HTML. Posílat obojí by
      // znamenalo platit za data, která si přímo protiřečí, kdykoli se plán
      // mezitím změní.
      //
      // Ostatní agenti (trainer, coach) HTML dál potřebují: skládají z něj
      // celý týden, ne odpověď na jednu otázku.
      const bezHtmlPlanu = String(agentSlug || '').toLowerCase() === 'coach_chat';
      const plans = (plansRes?.data ?? []).map((p) => ({
        ...p,
        plan_html: bezHtmlPlanu ? null : truncatePlanHtml(p.plan_html),
      }));
      ...
      base.user_context = {
        ...
        // Agent-specific memory (this agent's own history)
        user_ai_memory: agentMemory.map((r) => ({
          type: r.type,
          content: r.content,
          at: r.created_at,
        })),
        // Shared cross-agent facts (trainer sees coach facts, coach sees trainer facts)
        // Examples: shared_recovery_priority, shared_low_adherence_pattern, shared_plan_simplicity_needed
        shared_memory: sharedMemory.map((r) => ({
          type: r.type,
          content: r.content,
          at: r.created_at,
