...
export function buildCoachFallbackMessage({ bodyMetrics, userHabits, latestPlan, taskType }) {
  ...
  // CO SI ČLOVĚK Z PLÁNU SÁM NEVYČTE.
  //
  // Plán ukazuje CO má dělat. Zpráva od kouče má přidat PROČ zrovna tohle
  // a na co si dnes dát pozor — jinak je to jen druhá kopie plánu jinými slovy.
  // Fallback to neumí odvodit z dat (běží právě proto, že model je nedostupný),
  // takže drží obecné, ale pravdivé vysvětlení podle typu dne.
  const planTyp = String(latestPlan?.plan_type || '').toLowerCase();
  const procDnes = planTyp === 'redukce'
    ? 'Porce jsou menší schválně — v redukci jde o to udržet bílkoviny nahoře a hlad dole, ne hladovět.'
    : planTyp === 'nabirani_svaly'
      ? 'Opakování jsou nižší schválně — sval roste z napětí, ne z počtu opakování.'
      : 'Plán je postavený tak, abys ho zvládl i ve všední den, ne jen v ideálním týdnu.';
  const pozorNa = 'Dej si dnes pozor na tempo — poslední série má být těžká, ne uspěchaná.';
  ...
  if (type === 'recovery_message') {
    message = `${greeting}, dnes je v pořádku jít na lehčí režim. Prioritou je spánek, voda a klid. ${habitTip} Lehčí den není ztracený den — regenerace je část tréninku, ne pauza od něj. Zítra navážeš silnější.`;
  } else if (type === 'apple_health_daily_review') {
    message = `${greeting}, podle dat z Apple Watch dává smysl dnes držet střední intenzitu. Prioritou je regenerace, spánek a pravidelný pitný režim. ${habitTip} Skóre regenerace je orientační — ne zdravotní diagnóza.`;
  } else if (type === 'motivation_message') {
    message = `${greeting}, nemusíš mít všechno hned perfektní. Udělej dnes jeden malý krok podle plánu. ${procDnes} ${habitTip} ${pozorNa} Rytmus drží víc než jeden dokonalý den.`;
  } else if (type === 'positive_reinforcement') {
    message = `${greeting}, dobrá práce — drž se plánu i nadále. ${procDnes} Dnes stačí plynule navázat dalším jídlem nebo tréninkem z aplikace. ${pozorNa} Tohle už není náhoda, tak si to nerozbij spěchem.`;
  } else {
    message = `${greeting}, ${planRef}. Dnes neřeš dokonalost — začni prvním jídlem z plánu a jedním krátkým tréninkem, pokud ho dnes máš. ${procDnes} ${habitTip} ${pozorNa} Pokud nestihneš všechno, nevadí — první týden rozhoduje rytmus, ne výsledek.`;
  }
