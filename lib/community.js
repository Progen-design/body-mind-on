...
export async function jmenoAutora(user) {
  ...
  const prezdivka = String(profil?.preferred_address || '').trim();
  const zProfilu = String(user.user_metadata?.name || '').trim();
  const zEmailu = String(user.email || '').split('@')[0].trim();

  return (prezdivka || zProfilu || zEmailu || 'Člen').slice(0, 100);
}
 ...
 */
function rozeberDataUrl(dataUrl) {
 ...
 */
async function zmensAOcisti(vstup) {
  const { data, info } = await sharp(vstup)
    .rotate()
    .resize({ width: MAX_HRANA_PX, height: MAX_HRANA_PX, fit: 'inside', withoutEnlargement: true })
    .jpeg({ quality: 80 })
...
export async function prekrocilDenniLimit(userId) {
  ...
  return (count || 0) >= MAX_PRISPEVKU_DENNE;
}
