 ...
 */
export async function smazFotkyUzivatele(userId) {
  if (!userId) return { smazano: 0 };

  const kosik = supabaseServer.storage.from(BUCKET_FOTEK);
  const kCesty = [];
  ...
  if (error) {
    console.error('[community] uklid fotek', userId, error.message);
    throw error;
  }
