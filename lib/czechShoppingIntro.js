/**
 * České tvary pro úvod e-mailu s nákupním seznamem („pro sobotu“, ne „na Sobota“).
 */
...
/** Z řetězce typu „Sobota (25. 4.)“ nebo „Sobota“ vrátí 2. pád pro předložku „pro“. */
export function dayLabelToProAccusative(dayLabel) {
...
export function buildShoppingListEmailIntro(dayLabel, datePart = '') {
  const prep = esc(dayLabelToProAccusative(dayLabel));
  const m = String(dayLabel || '').match(/\(([^)]+)\)/);
  const dateRaw = (datePart && String(datePart).trim()) || (m ? m[1].trim() : '');
  const date = dateRaw ? esc(dateRaw) : '';
  const when = date ? `<strong>pro ${prep}</strong> (${date})` : `<strong>pro ${prep}</strong>`;
