// /lib/dailyDigest.js – Denní e-mail; HTML dne podle plan output mode (trénink jen při nutrition_training).
import nodemailer from 'nodemailer';
import { stripNutritionOnlyTrainingFromPlanHtml, stripPlanMediaAttrsFromHtml } from './emailTemplates.js';
import { getPublicAppUrl } from './siteUrls.js';
import { getPlanOutputMode, shouldIncludeTrainingInDigest } from './planOutputMode.js';

const CZECH_DAYS = ['Neděle', 'Pondělí', 'Úterý', 'Středa', 'Čtvrtek', 'Pátek', 'Sobota'];

/** @typedef {'workout' | 'rest' | 'unknown'} DayTrainingKind */
...
/**
 * Z plan_html vybere surový blok pro daný den (h4 + obsah do dalšího h4). Server-safe regex.
 * @returns {string|null}
 */
function extractRawDayBlockFromPlanHtml(planHtml, dayNameCzech) {
...
/**
 * Odhad z HTML dne: je přiřazen reálný trénink, jen odpočinek, nebo sekce chybí.
 * @param {string|null|undefined} rawDayHtml
 * @returns {DayTrainingKind}
 */
export function getDayTrainingKindFromRawDayHtml(rawDayHtml) {
  if (!rawDayHtml || typeof rawDayHtml !== 'string') return 'unknown';
  const trainMatch = rawDayHtml.match(
    /<p[^>]*>\s*<(?:b|strong)>\s*Trénink\s+tento\s+den\s*:?\s*<\/(?:b|strong)>\s*<\/p>\s*<ul[^>]*>([\s\S]*?)<\/ul>/i
  );
  if (!trainMatch) return 'unknown';
  const ulInner = trainMatch[1] || '';
  const items = [];
  ...
  const liIsRestOrLightActivity = (text) => {
    const t = String(text).trim().toLowerCase();
    if (!t) return true;
    if (t.includes('odpočinek') || t.includes('odpocinek')) return true;
    if (/^rest\.?$/i.test(t)) return true;
    if (t.includes('lehká procházka') || t.includes('lehka prochazka')) return true;
    if (t.includes('trénink celkem') || t.includes('trenink celkem')) return true;
    return false;
  };

  return items.every(liIsRestOrLightActivity) ? 'rest' : 'workout';
}

/**
 * Jídelní část dne pro e-mail (trénink v HTML jen při nutrition_training).
 * @returns {{ html: string|null, trainingKind: DayTrainingKind }}
 */
function extractDayBlockFromPlanHtml(planHtml, dayNameCzech, includeTrainingInHtml) {
  const raw = extractRawDayBlockFromPlanHtml(planHtml, dayNameCzech);
  if (!raw) return { html: null, trainingKind: 'unknown' };

  const trainingKind = getDayTrainingKindFromRawDayHtml(raw);
  ...
  const afterTrain = includeTrainingInHtml ? inner : stripNutritionOnlyTrainingFromPlanHtml(inner);
  const styled = afterTrain
  ...
  return { html: stripPlanMediaAttrsFromHtml(styled), trainingKind };
}
...
export async function buildDigestPayload(supabase, userId, email, userName = null) {
  ...
  const outputMode = getPlanOutputMode(plan, null, {});
  const includeTrainingDigest = shouldIncludeTrainingInDigest(outputMode);

  const { html: todayMealsHtml, trainingKind: trainingDayKind } = plan?.plan_html
    ? extractDayBlockFromPlanHtml(plan.plan_html, dayNameCzech, includeTrainingDigest)
    : { html: null, trainingKind: 'unknown' };
...
export async function sendDailyDigestEmail(email, payload) {
  ...
  const daySubtitle = `${dayNameCzech} – co dnes jíst a výživové cíle`;

  const subject = `${dayNameCzech} – jídelní plán a výživa 💪`;
  ...
  const onClubUrl = `${appBase}/on-club`;
  const vipUrl = `${appBase}/chci-vip`;
  ...
  const trialBlock = trialExpiresTomorrow
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;background:linear-gradient(135deg,rgba(234,179,8,0.15),rgba(180,130,20,0.08));border-radius:12px;border:1px solid rgba(234,179,8,0.4);" bgcolor="rgba(234,179,8,0.08)">
      ...
      <tr><td style="padding:0 18px 16px;"><a href="${onClubUrl}" style="display:inline-block;padding:10px 18px;font-size:14px;font-weight:600;color:#fff;text-decoration:none;background:#7c3aed;border-radius:10px;margin-right:10px;">ON Club</a> <a href="${vipUrl}" style="display:inline-block;padding:10px 18px;font-size:14px;font-weight:600;color:#fff;text-decoration:none;background:#ca8a04;border-radius:10px;">VIP Coaching</a></td></tr>
    </table>`
    : '';
  ...
  const planExpiresBlock = planExpiresTomorrow && !trialExpiresTomorrow
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;background:rgba(234,179,8,0.1);border-radius:12px;border:1px solid rgba(234,179,8,0.35);" bgcolor="rgba(234,179,8,0.06)">
      <tr><td style="padding:12px 18px;color:#fde68a;font-weight:600;font-size:14px;">📅 Tvůj týdenní program končí zítra</td></tr>
      <tr><td style="padding:8px 18px 12px;color:#e2e8f0;font-size:14px;line-height:1.6;">Chceš pokračovat dál? Vygeneruj si v aplikaci nový týden, nebo přejdi na placený program s plným přístupem.</td></tr>
      <tr><td style="padding:0 18px 12px;color:#e2e8f0;font-size:14px;line-height:1.6;"><a href="${appBase}/profil" style="color:#a78bfa;font-weight:600;">Otevři profil</a> – tam můžeš navázat další týden. Registrace nového týdenního plánu je i na stránce <a href="${appBase}/start" style="color:#a78bfa;">START</a>.</td></tr>
      <tr><td style="padding:0 18px 16px;"><a href="${onClubUrl}" style="display:inline-block;padding:10px 18px;font-size:14px;font-weight:600;color:#fff;text-decoration:none;background:#7c3aed;border-radius:10px;margin-right:10px;">ON Club</a> <a href="${vipUrl}" style="display:inline-block;padding:10px 18px;font-size:14px;font-weight:600;color:#fff;text-decoration:none;background:#ca8a04;border-radius:10px;">VIP Coaching</a></td></tr>
    </table>`
    : '';
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#12121a;border-radius:16px;overflow:hidden;" bgcolor="#12121a">
        <tr>
          <td style="padding:24px 24px 16px;background:#1a0a2e;border-bottom:1px solid #2a2a3d;" bgcolor="#1a0a2e">
            ...
            <p style="margin:6px 0 0;font-size:14px;color:#94a3b8;">${escapeHtml(daySubtitle)}</p>
          </td>
        </tr>
        <tr><td style="padding:24px;">
          ...
          <p style="text-align:center;margin:20px 0 0;">
            <a href="${appBase}/profil" style="display:inline-block;padding:12px 24px;border-radius:12px;color:#fff;text-decoration:none;font-weight:600;font-size:15px;background:#7c3aed;">Otevřít profil →</a>
          </p>
