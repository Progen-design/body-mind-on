...
export async function buildDigestPayload(supabase, userId, email, userName = null) {
  ...
  if (workouts.length > 0) {
    recommendation += `V aplikaci máš zapsanou aktivitu (${workouts.length}×) – pokračuj podle svého režimu. `;
    ...
    recommendation += timeStr
      ? `V kalendáři máš dnes položku „${first.summary}“ (${timeStr}). `
      : `V kalendáři máš dnes položku „${first.summary}“. `;
  } else {
