// /lib/dailyDigest.js – Denní e-mail: co jíst dnes (z plánu), doporučení; blok tréninku z těla zprávy záměrně mažeme (kvůli přehledu), stav tréninku reflektujeme v textu.
import nodemailer from 'nodemailer';
import { stripInlineTrainingDayBlockFromHtml, stripPlanMediaAttrsFromHtml } from './emailTemplates';
...
/**
 * Jídelní část dne pro e-mail (bez bloku trénink).
...
function extractDayBlockFromPlanHtml(planHtml, dayNameCzech) {
  ...
  const styled = stripInlineTrainingDayBlockFromHtml(
    raw
...
export async function buildDigestPayload(supabase, userId, email, userName = null) {
  ...
  if (trainingDayKind === 'workout') {
    recommendation +=
      'Dnes máš v plánu trénink – přesný seznam cviků je v aplikaci v týdenním plánu pod dneškem. ';
  } else if (trainingDayKind === 'rest') {
    recommendation +=
      'Dnes je v plánu odpočinek nebo lehká aktivita – zaměř se na jídelníček a regeneraci. ';
  }
...
export async function sendDailyDigestEmail(email, payload) {
  ...
  const daySubtitle =
    trainingDayKind === 'workout'
      ? `${dayNameCzech} – co dnes jíst, trénink (cviky v aplikaci) a výživové cíle`
      : trainingDayKind === 'rest'
        ? `${dayNameCzech} – co dnes jíst a výživové cíle (dnes odpočinek v plánu)`
        : `${dayNameCzech} – co dnes jíst a výživové cíle`;

  const subject =
    trainingDayKind === 'workout'
      ? `${dayNameCzech} – jídla, trénink a výživa 💪`
      : `${dayNameCzech} – jídelní plán a výživa 💪`;
  ...
  const planExpiresBlock = planExpiresTomorrow && !trialExpiresTomorrow
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;background:rgba(234,179,8,0.1);border-radius:12px;border:1px solid rgba(234,179,8,0.35);" bgcolor="rgba(234,179,8,0.06)">
      ...
      <tr><td style="padding:8px 18px 16px;color:#e2e8f0;font-size:14px;line-height:1.6;">Vygeneruj si nový plán v aplikaci – <a href="${appBase}/profil" style="color:#a78bfa;">otevři profil</a> a přejdi na stránku <a href="${appBase}/start" style="color:#a78bfa;">START</a>.</td></tr>
    </table>`
    : '';
