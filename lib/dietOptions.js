...
export const DIET_OPTIONS = Object.freeze([
  ...
  // PROČ JE VEGAN POŘÁD VYPNUTÝ, KDYŽ SOUČTY ZA SLOT VYPADAJÍ DOBŘE.
  //
  // Změřeno na produkci 22. 9. 2026 (`recipes_catalog`, `active = true`,
  // tag `vegan`). Součet za slot je nad `MIN_RECEPTU_NA_SLOT` u všech čtyř
  // chodů — přesně to hlídá watchdog `dieta_pod_kritickym_poctem`, a proto
  // taky mlčí. Jenže slot se neskládá z celého katalogu, ale z kanonického
  // kalorického pásma chodu (`KANONICKA_PASMA` v lib/recipeGenerationBands.js),
  // a tam to nevychází:
  //
  //   chod      pásmo      celkem  pod  V PÁSMU  nad
  //   snídaně   300–520      14      5     9      0
  //   oběd      450–680      12      4     6 ✗    2
  //   svačina   170–370      10      1     7 =    2
  //   večeře    300–650      12      1    11      0
  //
  // Oběd má v pásmu 6 receptů, práh je 7 — chybí jeden. Svačina sedí přesně
  // na prahu, bez jediné rezervy. Zapnout dietu v tomhle stavu znamená
  // jídelníček, ve kterém se obědy opakují dřív než za týden.
  //
  // Vypnuté to tedy NENÍ zastaralé `enabled: false`: součty klamou, protože
  // watchdog pásma neřeší. Až oběd přeleze 7 v pásmu 450–680 (a svačina
  // dostane rezervu), je přepnutí na `true` bezpečné.
  Object.freeze({
    value: 'vegan',
    label: 'Vegan',
    enabled: false,
