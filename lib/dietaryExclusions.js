...
/**
 * OŘECHY A PODOBNÉ ALERGENY. Nadmnožina toho, co lidé myslí slovem „ořechy“.
 *
 * Nález 8. 9. 2026: účet s „Alergie na ořechy“ ve `dietary_restrictions`
 * dostal plán, kde bylo arašídové máslo v pěti jídlech, mandlové mléko ve
 * čtyřech a mandle v jednom — jedna z nich rovnou jako snídaně. Text se
 * ukládal i propisoval do kontextu plánu správně, ale jako výraz k hledání
 * byl bezcenný: hledala se surovina jménem „alergie na orechy“.
 *
 * Skupina se proto chová stejně jako `DAIRY_TERMS` — jakmile uživatel vyloučí
 * ořechy, blokuje se celá. Arašíd je botanicky luštěnina, ale alergenně patří
 * sem, stejně jako mandle (peckovice).
 *
 * `vlassk` v seznamu SCHVÁLNĚ NENÍ: chytilo by „vlašský salát“, kde žádný
 * ořech není. „Vlašský ořech“ pokrývá kmen `orech` sám.
 * `orisk` naopak chytí i „muškátový oříšek“, což je koření — u alergika je
 * zbytečně vyřazené koření menší chyba než propuštěný ořech.
 * Kokos tu není: pro běžnou ořechovou alergii není rizikový a jeho vyřazení
 * by bezlaktózovým účtům vzalo kokosové mléko jako náhradu.
 * @type {readonly string[]}
 */
const NUT_TERMS = Object.freeze([
  // `oris` místo `orisk`: kmen `orisk` mine „oříšek“ (orisek), což je zdaleka
  // nejčastější tvar. `oris` chytí oříšek i oříšky.
  'orech', 'oris',
  'mandl',
  'arasid',
  'kesu',
  ...
  'pekan',
  'makadam',
  'nutella', 'nugat', 'marcipan',
]);

/**
 * HOUBY. Lidé napíšou „houby“, v receptu jsou „žampiony“ nebo „hlíva“.
 *
 * Doslovný výčet uživatele jinak minul každý konkrétní druh — stejná mezera
 * jako u ořechů, jen bez zdravotního rizika.
 * @type {readonly string[]}
 */
const MUSHROOM_TERMS = Object.freeze([
  'houb', 'hrib', 'zampion', 'hliv', 'shiitake', 'shitake',
  'portobello', 'vaclavk', 'lisky', 'liska', 'kral', 'penizovk',
]);

/**
 * Uvozovací fráze, které lidé píšou před samotnou potravinu.
 *
 * Pole se jmenuje „Zdravotní omezení (alergie, intolerance, nemoci)“, takže
 * odpověď skoro nikdy není holý název suroviny — je to věta. „Alergie na
 * ořechy“ se bez očištění hledá jako surovina toho jména a nikdy se netrefí.
 * @type {readonly RegExp[]}
 */
const UVOZOVACI_FRAZE = Object.freeze([
  /^alergie na /, /^alergie /, /^alergicky na /, /^alergicka na /,
  /^intolerance na /, /^intolerance /, /^intolerantni na /,
  /^nesnasim /, /^nejim /, /^neji /, /^nemuzu /, /^nemohu /,
  /^vynechat /, /^vynechavam /, /^vyhybam se /,
  /^bez /, /^zadne /, /^zadny /, /^zadna /,
]);

/**
 * @param {string} term už složený výraz
 * @returns {string} výraz bez uvozovací fráze
 */
function odstranUvozovaciFrazi(term) {
  let vysledek = term;
  // Opakovaně: „bez alergie na ořechy“ i „nejím bez lepku“ se stanou samotnou
  // potravinou. Cyklus končí, jakmile se nic neodebralo.
  for (let i = 0; i < 3; i += 1) {
    const pred = vysledek;
    for (const fraze of UVOZOVACI_FRAZE) {
      vysledek = vysledek.replace(fraze, '');
    }
    if (vysledek === pred) break;
  }
...
function splitFoodTerms(raw) {
  ...
  return String(raw)
    .split(/[,;\n]+/)
    .map((part) => odstranUvozovaciFrazi(normalizeFoodText(part)))
    .filter(Boolean);
}
 ...
 * @returns {boolean}
 */
function rawTermsIncludeNuts(rawTerms) {
  return rawTerms.some((t) => NUT_TERMS.some((term) => matchesTermStart(t, term)));
}
 ...
 * @returns {boolean}
 */
function rawTermsIncludeMushrooms(rawTerms) {
  return rawTerms.some((t) => matchesTermStart(t, 'houb'));
}

/**
 * @param {object|null|undefined} bodyMetrics
 * @returns {{ rawTerms: string[], cheeseExcluded: boolean, dairyExcluded: boolean, nutsExcluded: boolean, mushroomsExcluded: boolean, blockedTerms: string[] }}
 */
export function parseDietaryExclusions(bodyMetrics) {
  ...
  const nutsExcluded = rawTermsIncludeNuts(rawTerms);
  const mushroomsExcluded = rawTermsIncludeMushrooms(rawTerms);

  const blockedTerms = [];
  if (nutsExcluded) blockedTerms.push(...NUT_TERMS);
  if (mushroomsExcluded) blockedTerms.push(...MUSHROOM_TERMS);
  if (dairyExcluded) {
...
export function textContainsExcludedFood(text, exclusions) {
  ...
  const jeMlecnyVyraz = (term) => DAIRY_TERMS.includes(term) && !NUT_TERMS.includes(term) && !MUSHROOM_TERMS.includes(term);
  ...
  // OŘECHY SE HLEDAJÍ V NEMASKOVANÉM TEXTU. Maska škrtá rostlinné varianty
  // („mandlové mléko“, „arašídové máslo“) proto, že pro bezlaktózovou dietu
  // jsou v pořádku — pro alergika na ořechy jsou to ale právě ty zakázané
  // suroviny. Kdyby se hledalo v maskovaném textu, mandlové mléko by prošlo.
  if (exclusions?.nutsExcluded) {
