...
/** Odstraní atributy náhledů / médií z HTML plánu (e-mail bez obrázků). */
function stripMealAndExerciseMediaAttrsForEmail(html) {
  if (!html || typeof html !== 'string') return html;
  return html
...
/** Odstraní sekce tréninku z uloženého HTML (zůstává jen výživa). */
function stripTrainingSectionsFromPlanHtml(html) {
  if (!html || typeof html !== 'string') return html;
  let s = html;
...
/**
 * Transformuje HTML plánu pro e-mail: karty, tmavý prémiový vzhled, makra jako tagy (bez obrázků a bez tréninku).
 */
export function formatPlanHtmlForEmail(html) {
  if (!html || typeof html !== 'string') return '';
  let out = stripTrainingSectionsFromPlanHtml(html);
  ...
  if (dayMatch) {
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        content = stripMealAndExerciseMediaAttrsForEmail(content);
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 18px;background:${EMAIL_CARD};border-radius:16px;border:1px solid rgba(124,58,237,0.25);overflow:hidden;font-family:Arial,sans-serif;" bgcolor="${EMAIL_CARD}">
  ...
  out = stripTrainingSectionsFromPlanHtml(stripMealAndExerciseMediaAttrsForEmail(out));
...
<head>
  ...
  <title>Tvůj osobní jídelní plán Body &amp; Mind ON</title>
  <style type="text/css">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="email-card" style="max-width:600px;width:100%;margin:0 auto;background-color:${EMAIL_CARD};border-radius:16px;overflow:hidden;border:1px solid rgba(124,58,237,0.35);" bgcolor="${EMAIL_CARD}">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center" style="padding:26px 22px 20px;">
                    ...
                    <p style="margin:10px 0 0;font-size:13px;color:#94a3b8;max-width:320px;font-family:Arial,sans-serif;">Osobní jídelní plán</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td class="email-body-pad" style="padding:24px 20px 8px;background-color:${EMAIL_CARD};color:${EMAIL_TEXT};" bgcolor="${EMAIL_CARD}">
              ...
              <p style="margin:0 0 20px;font-size:14px;color:#94a3b8;line-height:1.55;font-family:Arial,sans-serif;">Níže máš přehled jídel a výživových hodnot. Kompletní plán otevřeš v aplikaci v profilu.</p>
              ${loginBlock}
