...
/** Jedna buňka „pilulky“ makra — tabulka + bgcolor na &lt;td&gt; (Gmail spolehlivěji než jen rgba v CSS). */
function mealMacroPillCell(label, val, borderHex, fg, bgHex) {
  return `<td style="padding:0 8px 8px 0;vertical-align:middle;mso-padding-alt:0 8px 8px 0;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;border-spacing:0;"><tr><td bgcolor="${bgHex}" style="padding:8px 13px;border-radius:999px;background-color:${bgHex};border:2px solid ${borderHex};color:${fg};font-size:11px;font-weight:800;font-family:Arial,sans-serif;line-height:1.25;white-space:nowrap;">${label}&nbsp;${escapeHtml(
    val
  )}&nbsp;g</td></tr></table></td>`;
}
...
function transformMealNutritionLinesToMacroTags(html) {
  return html.replace(
    ...
    (_, inner) => {
      ...
      const cells = [];
      if (b) cells.push(mealMacroPillCell('Bílkoviny', b, '#3b82f6', '#dbeafe', '#152642'));
      if (s) cells.push(mealMacroPillCell('Sacharidy', s, '#ca8a04', '#fef9c3', '#2a2310'));
      if (t) cells.push(mealMacroPillCell('Tuky', t, '#ef4444', '#fee2e2', '#2a1518'));
      if (fiber) cells.push(mealMacroPillCell('Vláknina', fiber, '#22c55e', '#dcfce7', '#132618'));
      if (!cells.length) {
