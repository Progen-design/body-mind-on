...
/**
 * E-mail: když v HTML chybí data-image-url, ale je data-recipe-id — stejný CDN náhled jako v profilu.
 */
function injectSpoonacularMealImageUrlsForEmail(html) {
  if (!html || typeof html !== 'string') return html;
  return html.replace(/<p([^>]*)>/gi, (full, attrs) => {
    if (!/\bdata-recipe-id\s*=\s*"(\d+)"/i.test(attrs)) return full;
    const existingImg = attrs.match(/\bdata-image-url\s*=\s*"([^"]*)"/i)?.[1]?.trim();
    if (existingImg) return full;
    const rid = attrs.match(/\bdata-recipe-id\s*=\s*"(\d+)"/i)?.[1];
    if (!rid) return full;
    const url = escapeHtml(`https://img.spoonacular.com/recipes/${rid}-312x231.jpg`);
    return `<p${attrs} data-image-url="${url}">`;
  });
}
  ...
  if (dayMatch) {
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        content = injectSpoonacularMealImageUrlsForEmail(content);
