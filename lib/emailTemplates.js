...
function formatMacrosBlockHtml(rawUlBlock) {
  ...
  const macroHeroRow = (mainHtml, borderHex, fg, bgHex) =>
    `<tr><td style="padding:0 0 8px 0;vertical-align:top;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;"><tr><td bgcolor="${bgHex}" style="padding:10px 14px;border-radius:999px;border:2px solid ${borderHex};background-color:${bgHex};color:${fg};font-size:13px;font-weight:800;font-family:Arial,sans-serif;line-height:1.35;">${mainHtml}</td></tr></table></td></tr>`;

  const rows = [];
  if (calNum) {
    rows.push(macroHeroRow(`${calNum} kcal`, '#a78bfa', '#ede9fe', '#221830'));
  }
  if (protNum) {
    rows.push(macroHeroRow(`${protNum} g bílk.`, '#3b82f6', '#dbeafe', '#152642'));
  }
  if (carbsNum) {
    rows.push(macroHeroRow(`${carbsNum} g sach.`, '#ca8a04', '#fef9c3', '#2a2310'));
  }
  if (fatNum) {
    rows.push(macroHeroRow(`${fatNum} g tuků`, '#ef4444', '#fee2e2', '#2a1518'));
  }

  if (!rows.length) return styleListForEmail(rawUlBlock);

  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0;font-family:Arial,sans-serif;">${rows.join('')}</table>`;
}
...
export function formatPlanHtmlForEmail(html, emailOptions = {}) {
  ...
  <tr><td height="5" style="height:5px;font-size:5px;line-height:5px;background-color:#a78bfa;" bgcolor="#a78bfa">&nbsp;</td></tr>
  <tr><td style="padding:14px 18px;border-bottom:1px solid rgba(167,139,250,0.22);color:${EMAIL_ACCENT};font-weight:800;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;background-color:#1f1830;" bgcolor="#1f1830">Návyky na tento týden</td></tr>
  <tr><td style="padding:16px 14px 18px 18px;background-color:#161022;" bgcolor="#161022">${tipsTable}</td></tr>
</table>`;
  }
    ...
    <tr>
      <td align="center" class="email-outer-pad" style="padding:40px 20px;background-image:radial-gradient(ellipse 95% 75% at 50% 0%, rgba(167,139,250,0.16) 0%, transparent 58%);">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="email-card" style="max-width:600px;width:100%;margin:0 auto;">
          <tr>
            <td align="center" style="background:radial-gradient(ellipse 100% 145% at 50% -30%, rgba(167,139,250,0.55) 0%, transparent 55%),radial-gradient(ellipse 60% 50% at 18% 95%, rgba(99,102,241,0.32) 0%, transparent 60%),radial-gradient(ellipse 60% 50% at 88% 90%, rgba(192,132,252,0.28) 0%, transparent 60%),linear-gradient(160deg,#251248 0%,#160e2a 48%,#070418 100%);border-radius:22px 22px 0 0;padding:48px 36px 40px;text-align:center;border-bottom:1px solid rgba(167,139,250,0.32);box-shadow:inset 0 1px 0 rgba(255,255,255,0.09);">
              <div style="display:inline-block;background:linear-gradient(135deg,#c4b5fd 0%,#a78bfa 35%,#6366f1 70%,#4f46e5 100%);border-radius:999px;padding:11px 24px;margin-bottom:26px;box-shadow:0 8px 34px rgba(124,58,237,0.55), 0 0 28px rgba(167,139,250,0.35);">
                <span style="color:#ffffff;font-size:11px;font-weight:900;letter-spacing:3.2px;text-transform:uppercase;">BODY &amp; MIND ON</span>
              </div>
              <div style="margin:0 0 12px;font-size:11px;font-weight:800;letter-spacing:0.32em;text-transform:uppercase;color:#c4b5fd;font-family:Arial,sans-serif;">Premium týdenní plán</div>
              <h1 style="margin:0 0 14px;font-size:34px;font-weight:900;color:#ffffff;line-height:1.08;font-family:Arial,sans-serif;letter-spacing:-0.04em;">Tvůj týden,<br/><span style="background:linear-gradient(90deg,#c4b5fd,#f0abfc 60%,#fde68a);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:#f0abfc;">naplánovaný do detailu</span></h1>
              <p style="margin:0 auto;max-width:440px;font-size:15px;color:#e2e8f0;font-family:Arial,sans-serif;line-height:1.6;">${greetingLine}</p>
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:22px auto 0;border-collapse:separate;">
                <tr>
                  <td style="padding:0 6px;"><div style="background:rgba(167,139,250,0.18);border:1px solid rgba(196,181,253,0.4);color:#ede9fe;padding:7px 14px;border-radius:999px;font-family:Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.04em;">7 dní jídla</div></td>
                  <td style="padding:0 6px;"><div style="background:rgba(99,102,241,0.22);border:1px solid rgba(165,180,252,0.42);color:#e0e7ff;padding:7px 14px;border-radius:999px;font-family:Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.04em;">Makra u jídel</div></td>
                  <td style="padding:0 6px;"><div style="background:rgba(244,114,182,0.18);border:1px solid rgba(249,168,212,0.4);color:#fbcfe8;padding:7px 14px;border-radius:999px;font-family:Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.04em;">Pohyb na den</div></td>
                </tr>
              </table>
            </td>
          </tr>
          <tr>
            <td class="email-body-pad" style="padding:0;background-color:#100a1a;background:linear-gradient(185deg,#1a1028 0%,#120b1c 42%,#0a0612 100%);" bgcolor="#100a1a">
              ${loginBlock}
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0;">
                <tr>
                  <td class="email-plan-pad" style="padding:26px 28px 22px;color:${EMAIL_TEXT};font-size:14px;line-height:1.55;font-family:Arial,sans-serif;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(167,139,250,0.24);border-radius:18px;overflow:hidden;" bgcolor="#160f24">
                      <tr><td style="padding:22px 20px 20px;background-color:#160f24;" bgcolor="#160f24">
                    ${safePlanHtml}
                      </td></tr>
                    </table>
                  </td>
          ...
          <tr>
            <td style="background:#0f0f1a;padding:24px 32px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.05);">
              <!--[if mso]>
              <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" href="${escapeHtml(planCta)}" style="height:52px;v-text-anchor:middle;width:300px;" arcsize="12%" stroke="f" fillcolor="${EMAIL_ACCENT_DEEP}">
                <w:anchorlock/>
                <center style="color:#ffffff;font-family:Arial,sans-serif;font-size:16px;font-weight:bold;">Otevřít plán v aplikaci</center>
              </v:roundrect>
              <![endif]-->
              <a href="${escapeHtml(planCta)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background:linear-gradient(135deg,${EMAIL_ACCENT_DEEP},#4f46e5);color:#ffffff;font-size:16px;font-weight:700;text-decoration:none;padding:16px 36px;border-radius:999px;letter-spacing:0.35px;font-family:Arial,sans-serif;box-shadow:0 8px 28px rgba(79,70,229,0.35);mso-hide:all;line-height:1.2;">Otevřít plán v aplikaci</a>
          ...
          <tr>
            <td style="background:#06040f;border-radius:0 0 20px 20px;padding:24px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.04);">
              <p style="margin:0 0 14px;font-size:12px;color:#64748b;font-family:Arial,sans-serif;line-height:1.5;">Tip: odkaz na aplikaci si můžeš uložit do záložek v prohlížeči — k plánu se dostaneš jedním klepnutím.</p>
