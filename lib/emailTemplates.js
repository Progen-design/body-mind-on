  ...
  if (workoutListRaw) {
    ...
    while ((liMatch = liRe.exec(workoutListRaw)) !== null) {
      ...
      const apiLinks = wgerId
        ? `<div style="margin-top:8px;"><a href="https://wger.de/api/v2/exerciseinfo/${encodeURIComponent(wgerId)}/" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-right:6px;margin-bottom:6px;padding:5px 9px;border-radius:7px;border:1px solid rgba(124,58,237,0.45);color:#c4b5fd;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Detail cviku</a><a href="https://wger.de/api/v2/exerciseimage/?exercise=${encodeURIComponent(wgerId)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-right:6px;margin-bottom:6px;padding:5px 9px;border-radius:7px;border:1px solid rgba(59,130,246,0.45);color:#93c5fd;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Média cviku</a><a href="https://wger.de/api/v2/video/?exercise=${encodeURIComponent(wgerId)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-bottom:6px;padding:5px 9px;border-radius:7px;border:1px solid rgba(16,185,129,0.45);color:#86efac;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Video cviku</a></div>`
        : '';
      cards.push(
    ...
    if (cards.length > 0) {
      ...
      workoutMeta = `<p style="margin:0 0 10px;color:#94a3b8;font-size:12px;line-height:1.45;font-family:Arial,sans-serif;">Kliknutím otevřeš detail cviku ve wger API (info, média, video).</p>`;
      workoutLinks = `<div style="margin-top:4px;"><a href="https://wger.de/api/v2/exerciseinfo/" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-right:6px;margin-bottom:4px;padding:5px 9px;border-radius:7px;border:1px solid rgba(124,58,237,0.45);color:#c4b5fd;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Seznam cviků</a><a href="https://wger.de/api/v2/" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-bottom:4px;padding:5px 9px;border-radius:7px;border:1px solid rgba(148,163,184,0.4);color:#cbd5e1;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">wger API v2</a></div>`;
    }
  }
  ...
  if (dayMatch) {
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        const dayInner = `${content}${workoutCardHtml}`;
        mealHtml += `
...
export function buildPlanEmailDocument({
  ...
  const greetingLine = planChangeContext
    ? (namePart ? `Hej ${escapeHtml(namePart)}, upravili jsme jídelní i tréninkový plán podle tvých změn 💪` : 'Hej, upravili jsme jídelní i tréninkový plán podle tvých změn 💪')
    : (namePart ? `Hej ${escapeHtml(namePart)}, tvůj jídelní i tréninkový plán je připraven 💪` : 'Hej, tvůj jídelní i tréninkový plán je připraven 💪');
