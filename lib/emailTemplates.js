...
import { recipeFromCatalogApiUrl } from './recipeDetailUrl.js';
import { EMAIL_DESIGN_TOKENS } from './email/emailDesignTokens.js';
import {
  getPlanOutputMode,
...
const EMAIL_TEXT = EMAIL_DESIGN_TOKENS.colors.legacyText;
const EMAIL_MUTED = EMAIL_DESIGN_TOKENS.colors.legacyMuted;
const EMAIL_CARD = EMAIL_DESIGN_TOKENS.colors.legacyCard;
const EMAIL_BG = EMAIL_DESIGN_TOKENS.colors.legacyBg;
const EMAIL_ACCENT = EMAIL_DESIGN_TOKENS.colors.legacyAccent;
const EMAIL_ACCENT_DEEP = EMAIL_DESIGN_TOKENS.colors.legacyAccentDeep;
...
export function stripPlanMediaAttrsFromHtml(html) {
  ...
  let s = html
    ...
    .replace(/<source\b[^>]*>/gi, '')
    .replace(/<img\b(?![^>]*\bclass\s*=\s*["'][^"']*\brecipe-detail-image\b)[^>]*>/gi, '')
...
function appendInlineRecipeLinksForEmailMeals(html) {
  ...
  return html.replace(
    /<p([^>]*\bdata-recipe-id\s*=\s*["'](\d+)["'][^>]*)>([\s\S]*?)<\/p>(?!\s*<p[^>]*\bplan-meal-external-recipe\b)(?!\s*<p[^>]*>\s*<a[^>]*href\s*=\s*["'][^"']*(?:spoonacular-recipe|recipe-from-catalog))/gi,
    (full, attrs, id, inner) => {
      if (/meal-nutrition-line|plan-meal-external-recipe/i.test(attrs)) return full;
      if (/spoonacular-recipe|recipe-from-catalog|\/api\/(?:spoonacular-recipe|recipe-from-catalog)/i.test(inner || '')) return full;
      const href = recipeFromCatalogApiUrl(id, app, { format: 'html' });
...
/**
 * Plain-text fallback for transactional weekly plan emails.
 * Used by both Gmail and Resend providers.
 */
export function buildPlanEmailPlainText({
  firstName = '',
  ...
  const intro = planChangeContext
    ? (name ? `Ahoj ${name}, posilame ti upraveny plan.` : 'Ahoj, posilame ti upraveny plan.')
    : (name ? `Ahoj ${name}, tvuj tydenni plan je pripraven.` : 'Ahoj, tvuj tydenni plan je pripraven.');
  const planText = htmlToPlainText(safePlanHtml);
  const lines = [
    intro,
    '',
    'Body & Mind ON',
    '',
    planText || 'Detail planu otevri v aplikaci.',
  ];
  if (ctaUrl) {
    lines.push('', `Otevrit plan v aplikaci: ${String(ctaUrl).trim()}`);
  }
  if (loginUrl) {
    lines.push(`Prihlaseni: ${String(loginUrl).trim()}`);
  }
  lines.push('', 'Podpora: info@bodyandmindon.cz');
  return lines.join('\n');
}
 ...
 * @param {string} [ctaUrl] – primární CTA (výchozí přihlášení v aplikaci)
 */
export function buildPlanEmailDocument({
  ...
  const resolvedLoginUrl = (loginUrl || getDefaultLoginUrl()).replace(/\/$/, '');
  const planCta = (ctaUrl || getPlanEmailCtaUrl()).replace(/\/$/, '');
...
<head>
  ...
  <style type="text/css">
    @media only screen and (max-width: 640px) {
      ...
      .email-body-pad { padding: 24px 12px 16px !important; }
      .email-plan-pad { padding: 18px 12px !important; }
      .email-hero-pad { padding: 32px 16px 28px !important; }
      ...
      .full-btn { display: block !important; width: 100% !important; max-width: 100% !important; box-sizing: border-box !important; }
    }
  </style>
</head>
<body bgcolor="${emailOuterBg}" style="margin:0;padding:0;background-color:${emailOuterBg};color:${EMAIL_TEXT};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;font-size:16px;line-height:1.5;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="body-bg" bgcolor="${emailOuterBg}" style="background-color:${emailOuterBg};background-image:radial-gradient(ellipse 70% 50% at 50% 0%, rgba(167,139,250,0.32) 0%, transparent 60%),radial-gradient(ellipse 50% 40% at 0% 30%, rgba(99,102,241,0.2) 0%, transparent 55%),radial-gradient(ellipse 50% 40% at 100% 70%, rgba(192,132,252,0.22) 0%, transparent 55%),url('data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2240%22%20height%3D%2240%22%3E%3Ccircle%20cx%3D%221.5%22%20cy%3D%221.5%22%20r%3D%221%22%20fill%3D%22%23a78bfa%22%20fill-opacity%3D%220.05%22%2F%3E%3C%2Fsvg%3E'),linear-gradient(180deg,#21133f 0%,${emailOuterBg} 50%,#0c061c 100%);">
    <tr>
      <td align="center" class="email-outer-pad" style="padding:0;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="email-card" style="max-width:840px;width:100%;margin:0 auto;">
          <tr>
            <td bgcolor="${heroSolid}" align="center" class="email-hero-pad" style="background-color:${heroSolid};background-image:radial-gradient(ellipse 60% 50% at 18% 22%, rgba(167,139,250,0.45) 0%, transparent 55%),radial-gradient(ellipse 55% 45% at 82% 78%, rgba(236,72,153,0.3) 0%, transparent 55%),url('data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2236%22%20height%3D%2236%22%3E%3Ccircle%20cx%3D%221.5%22%20cy%3D%221.5%22%20r%3D%221%22%20fill%3D%22%23ffffff%22%20fill-opacity%3D%220.07%22%2F%3E%3C%2Fsvg%3E'),linear-gradient(160deg,#5b21b6 0%,#3b1d72 48%,#1e1148 100%);border-radius:22px 22px 0 0;padding:46px 36px 38px;text-align:center;">
              <!--[if mso]>
              <v:rect xmlns:v="urn:schemas-microsoft-com:vml" fill="true" stroke="false" style="position:absolute;top:0;left:0;width:840px;height:340px;z-index:-1;mso-hide:all;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 18px;">
                <tr>
                  <td align="center" style="padding:22px 24px 8px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;margin:0 auto;border:2px solid rgba(167,139,250,0.45);border-radius:24px;background-color:#1a1030;background-image:radial-gradient(ellipse 80% 60% at 50% 0%, rgba(124,58,246,0.28) 0%, transparent 70%);">
                      <tr>
                        <td align="center" style="padding:24px 22px 22px;">
                          <div style="margin:0 0 8px;font-size:11px;font-weight:800;letter-spacing:0.22em;text-transform:uppercase;color:#c4b5fd;font-family:Arial,sans-serif;">Tvůj plán je připraven</div>
                          <p style="margin:0 0 18px;font-size:15px;line-height:1.55;color:#e2e8f0;font-family:Arial,sans-serif;">Otevři aplikaci a projdi celý týden v profilu — recepty, makra i tréninky na jednom místě.</p>
                          <a href="${escapeHtml(planCta)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background-color:${EMAIL_ACCENT_DEEP};background-image:linear-gradient(135deg,${EMAIL_ACCENT_DEEP} 0%,#4f46e5 60%,#3730a3 100%);color:#ffffff;font-size:16px;font-weight:800;text-decoration:none;padding:16px 34px;border-radius:999px;letter-spacing:0.3px;font-family:Arial,sans-serif;line-height:1.2;">Otevřít v aplikaci →</a>
                        </td>
                      </tr>
                    </table>
                  </td>
          ...
          <tr>
            <td bgcolor="#1a0e34" align="center" style="background-color:#1a0e34;background-image:radial-gradient(ellipse 90% 80% at 50% 0%, rgba(124,58,246,0.32) 0%, transparent 60%),linear-gradient(180deg,#1d0e3d 0%,#100722 100%);padding:36px 32px 36px;text-align:center;border-top:1px solid rgba(167,139,250,0.22);">
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:24px auto 0;border-collapse:separate;">
                <tr>
                  <td style="padding:0 8px;"><a href="${escapeHtml(resolvedLoginUrl)}" target="_blank" rel="noopener noreferrer" style="color:${EMAIL_ACCENT};font-size:13px;font-weight:700;text-decoration:none;font-family:Arial,sans-serif;border-bottom:1px solid rgba(167,139,250,0.4);padding-bottom:1px;">Přihlášení</a></td>
                </tr>
              </table>
            </td>
          </tr>
