...
/** Tréninkový blok v e-mailu: bez médií, čitelný seznam. */
function styleTrainingUlForEmail(htmlFrag) {
  if (!htmlFrag || typeof htmlFrag !== 'string') return '';
  let s = stripPlanMediaAttrsFromHtml(htmlFrag.trim());
  s = s.replace(/<ul\b/gi, '<ul style="margin:0;padding:0;list-style:none;font-family:Arial,sans-serif;"');
  s = s.replace(/<li\b/gi, '<li style="margin:0 0 8px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);color:#e2e8f0;font-size:14px;line-height:1.45;font-family:Arial,sans-serif;"');
  return s;
}
...
export function formatPlanHtmlForEmail(html) {
  ...
  let globalTrainingCard = '';
  const gtm = out.match(/<h3[^>]*>\s*[^<]*Tréninkový plán[^<]*<\/h3>[\s\S]*?(?=<h3[^>]*>|$)/i);
  if (gtm) {
    out = out.replace(gtm[0], '');
    let innerGlobal = stripPlanMediaAttrsFromHtml(gtm[0]);
    innerGlobal = innerGlobal.replace(
      /<h3[^>]*>[\s\S]*?<\/h3>/i,
      `<p style="margin:0 0 14px;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:${EMAIL_ACCENT};font-family:Arial,sans-serif;">🏋️ Tréninkový plán</p>`
    );
    innerGlobal = innerGlobal
      .replace(/<p(?!\s[^>]*style=)/gi, `<p style="margin:0 0 8px;color:${EMAIL_TEXT};font-size:14px;line-height:1.55;font-family:Arial,sans-serif;"`)
      .replace(/<ul/gi, `<ul style="margin:8px 0 0;padding:0 0 0 18px;color:${EMAIL_TEXT};font-size:14px;line-height:1.55;font-family:Arial,sans-serif;"`)
      .replace(/<li/gi, `<li style="margin:4px 0;"`);
    globalTrainingCard = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 18px;background:${EMAIL_CARD};border-radius:14px;border:1px solid rgba(124,58,237,0.35);overflow:hidden;font-family:Arial,sans-serif;" bgcolor="${EMAIL_CARD}">
  <tr><td style="padding:16px 18px;color:${EMAIL_TEXT};font-size:14px;line-height:1.6;">${innerGlobal}</td></tr>
</table>`;
  }
  ...
  if (dayMatch) {
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        const trainingRe = /(<p[^>]*>\s*<(?:b|strong)>\s*Trénink tento den:\s*<\/(?:b|strong)>\s*<\/p>\s*)([\s\S]*)$/i;
        const tm = dayContent.match(trainingRe);
        let mealOnly = tm ? dayContent.slice(0, tm.index).trim() : dayContent;
        const trainingTail = tm ? String(tm[2] || '').trim() : '';
        ...
        const trainingStyled = trainingTail
          ? `<div style="margin-top:16px;padding-top:14px;border-top:1px solid rgba(124,58,237,0.28);font-family:Arial,sans-serif;"><p style="margin:0 0 10px;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:${EMAIL_ACCENT};">🏋️ Trénink tento den</p>${styleTrainingUlForEmail(trainingTail)}</div>`
          : '';

        const dayInner = content + trainingStyled;
        mealHtml += `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 18px;background:${EMAIL_CARD};border-radius:16px;border:1px solid rgba(124,58,237,0.25);overflow:hidden;font-family:Arial,sans-serif;" bgcolor="${EMAIL_CARD}">
  ...
  return numbersHeroBlock + macrosHeroBlock + mindsetHeroBlock + out + globalTrainingCard;
}
