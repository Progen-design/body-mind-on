...
function defaultWorkoutCardHtml() {
  const workoutList =
    '<ul style="margin:0;padding-left:18px;color:#e2e8f0;font-size:13px;line-height:1.55;font-family:Arial,sans-serif;"><li style="margin:6px 0;">Odpočinek (v plánu chybí blok tréninku – zobrazíme orientační výchozí stav).</li></ul>';
...
function extractWorkoutBlock(content) {
  ...
  if (workoutListRaw) {
    ...
    while ((liMatch = liRe.exec(workoutListRaw)) !== null) {
      ...
      const canonicalKey = attrs.match(/\bdata-exercise-key\s*=\s*["']([^"']+)["']/i)?.[1] || '';
      const detailRow = [];
      if (canonicalKey) {
        detailRow.push(`<span style="display:inline-block;margin-right:6px;padding:2px 7px;border-radius:999px;background:rgba(15,23,42,0.55);border:1px solid rgba(148,163,184,0.28);color:#cbd5e1;font-size:11px;line-height:1.3;">Klíč: ${escapeHtml(canonicalKey)}</span>`);
      }
      if (wgerId) {
        detailRow.push(`<span style="display:inline-block;padding:2px 7px;border-radius:999px;background:rgba(15,23,42,0.55);border:1px solid rgba(167,139,250,0.4);color:#c4b5fd;font-size:11px;line-height:1.3;">wger ID: ${escapeHtml(wgerId)}</span>`);
      }
      const apiLinks = wgerId
        ? `<div style="margin-top:8px;line-height:1.6;"><a href="https://wger.de/en/exercise/${encodeURIComponent(wgerId)}/view" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin:0;padding:6px 11px;border-radius:7px;border:1px solid rgba(124,58,237,0.45);color:#c4b5fd;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Detail cviku na wger.de</a></div>`
        : '';
      cards.push(
        `<div style="margin:0 0 10px;padding:10px 10px;background:rgba(15,23,42,0.35);border:1px solid rgba(148,163,184,0.2);border-radius:10px;"><div style="margin:0;color:#f8fafc;font-size:13px;line-height:1.5;font-weight:700;font-family:Arial,sans-serif;">${escapeHtml(labelHtml || 'Cvik')}</div>${detailRow.length ? `<div style="margin-top:6px;">${detailRow.join('')}</div>` : ''}${apiLinks}</div>`
      );
    }
    if (cards.length > 0) {
      workoutList = cards.join('');
      workoutMeta = `<p style="margin:0 0 10px;color:#94a3b8;font-size:12px;line-height:1.45;font-family:Arial,sans-serif;">U ověřených cviků vede odkaz na veřejný popis na wger.de (bez přihlášení).</p>`;
      workoutLinks = `<div style="margin-top:10px;padding-top:8px;border-top:1px solid rgba(124,58,237,0.22);line-height:1.6;"><a href="https://wger.de/en/exercise/overview/" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:6px 11px;border-radius:7px;border:1px solid rgba(124,58,237,0.45);color:#c4b5fd;text-decoration:none;font-size:11px;font-weight:700;font-family:Arial,sans-serif;">Další cviky — přehled na wger.de</a></div>`;
    }
  }
...
/**
 * Obálka surovin (<p class="meal-ingredient-portions-h"> + <ul>) do <details>.
 * Shrnutý řádek = stručný přehled; seznam jen po rozkliknutí (Gmail/Android typicky ano; některé Outlook klienty <details> nepodporují).
 */
export function wrapMealIngredientBlocksForEmail(html) {
  if (!html || typeof html !== 'string') return html;
  const re =
    /<p([^>]*class="[^"]*meal-ingredient-portions-h[^"]*"[^>]*)>([\s\S]*?)<\/p>\s*<ul([^>]*class="[^"]*meal-ingredient-portions[^"]*"[^>]*)>([\s\S]*?)<\/ul>/gi;
  return html.replace(re, (_, pAttrs, pInner, ulAttrs, liBlock) => {
    const itemCount = (liBlock.match(/<li\b/gi) || []).length;
    const summaryText =
      itemCount > 0 ? `Suroviny na 1 porci (${ingredientCountPhraseCs(itemCount)}) — rozbalit` : 'Suroviny na 1 porci — rozbalit';
    const det =
      'margin:10px 0 18px;border:1px solid rgba(147,122,246,0.38);border-radius:14px;background:linear-gradient(165deg,rgba(49,46,89,0.55) 0%,rgba(18,21,43,0.55) 100%);overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,0.25);';
    const sum =
      'padding:12px 16px;cursor:pointer;font-size:13px;font-weight:800;color:#f5f3ff;font-family:Arial,sans-serif;line-height:1.4;background:rgba(124,58,237,0.14);outline:none;text-align:left;display:block;width:100%;box-sizing:border-box;list-style:none;';
    const inner =
      'padding:14px 16px;border-top:1px solid rgba(139,92,246,0.28);background:rgba(10,14,31,0.35);';

    const pHtml = `<p${pAttrs}>${pInner}</p>`;
    const ulHtml = `<ul${ulAttrs}>${liBlock}</ul>`;

    return `<details style="${det}">
<summary style="${sum}">${escapeHtml(summaryText)}</summary>
<div style="${inner}">
${pHtml}
${ulHtml}
</div>
  ...
  if (dayMatch) {
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        content = wrapMealIngredientBlocksForEmail(content);
        ...
        const recipeEmailHint = `<p style="margin:14px 0 0;padding-top:12px;border-top:1px solid rgba(124,58,237,0.2);color:#94a3b8;font-size:12px;line-height:1.55;font-family:Arial,sans-serif;">Plný recept včetně postupu otevři v aplikaci (tlačítko Recept). Seznam surovin rozbal v e-mailu klepnutím na řádek výše — některé poštovní programy (hlavně Outlook) mohou seznam zobrazit vždy rozbalený. Kompletní plán vždy na profilu.</p>`;
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 22px;background:${EMAIL_CARD};border-radius:18px;border:1px solid rgba(124,58,237,0.32);overflow:hidden;box-shadow:0 8px 28px rgba(0,0,0,0.35);font-family:Arial,sans-serif;" bgcolor="${EMAIL_CARD}">
  <tr><td style="padding:16px 20px;background:linear-gradient(118deg,#2e1b52 0%,#1a2340 45%,#16132a 100%);color:#ffffff;font-weight:700;font-size:15px;letter-spacing:-0.02em;border-bottom:1px solid rgba(139,92,246,0.45);">
    <span style="display:inline-block;width:4px;height:18px;border-radius:2px;background:linear-gradient(180deg,#c4b5fd,#7c3aed);margin-right:12px;vertical-align:middle;line-height:18px;">&nbsp;</span>
    ${escapeHtml(dayName)}
  </td></tr>
  <tr><td style="padding:20px 20px;color:${EMAIL_TEXT};font-size:14px;line-height:1.55;">
...
export function buildPlanEmailDocument({
  ...
  const greetingLine = planChangeContext
    ? (namePart ? `Hej ${escapeHtml(namePart)}, upravili jsme jídelníček podle tvých změn 🍽️` : 'Hej, upravili jsme jídelníček podle tvých změn 🍽️')
    : (namePart ? `Hej ${escapeHtml(namePart)}, tvůj jídelníček na týden je připraven 🍽️` : 'Hej, tvůj jídelníček na týden je připraven 🍽️');
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="email-card" style="max-width:600px;width:100%;margin:0 auto;">
          <tr>
            <td align="center" style="background:radial-gradient(ellipse 80% 120% at 50% -20%, rgba(124,58,246,0.35) 0%, transparent 55%),linear-gradient(135deg,#140b24 0%,#0a0814 55%,#05060c 100%);border-radius:22px 22px 0 0;padding:40px 36px 34px;text-align:center;border-bottom:1px solid rgba(139,92,246,0.35);box-shadow:inset 0 1px 0 rgba(255,255,255,0.06);">
              ...
              <h1 style="margin:0 0 10px;font-size:30px;font-weight:800;color:#f8fafc;line-height:1.15;font-family:Arial,sans-serif;letter-spacing:-0.03em;">Tvůj týdenní jídelníček 🍽️</h1>
              <p style="margin:0 0 10px;font-size:15px;color:#94a3b8;font-family:Arial,sans-serif;">Personalizovaný jídelníček připravený pro tvůj týden</p>
              <p style="margin:0;font-size:15px;color:#cbd5e1;font-family:Arial,sans-serif;line-height:1.55;">${greetingLine}</p>
            </td>
          </tr>
          <tr>
