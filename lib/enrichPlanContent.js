 ...
 * ─────────────────────────────────────────────────────────────────────────────
 */
import { enrichExercise } from './exerciseEnrichment.js';
 ...
 * Returns array of { name, exercise_key, wger_exercise_id, embedded_image_url, embedded_gif_url }.
 * exercise_key z data-exercise-key; média z data-image-url / data-gif-url (planRenderer) → lze přeskočit živé wger.
...
function parseExerciseNamesFromHtml(html) {
  ...
  function processLiBlock(blockHtml) {
    ...
    while ((m = liRe.exec(blockHtml)) !== null) {
      ...
      const imgAttr = (m[1] || '').match(/data-image-url\s*=\s*["']([^"']*)["']/i);
      const gifAttr = (m[1] || '').match(/data-gif-url\s*=\s*["']([^"']*)["']/i);
      const videoAttr = (m[1] || '').match(/data-video-url\s*=\s*["']([^"']*)["']/i);
      const embedded_image_url = imgAttr && imgAttr[1] ? String(imgAttr[1]).trim() : null;
      const embedded_gif_url = gifAttr && gifAttr[1] ? String(gifAttr[1]).trim() : null;
      const embedded_video_url = videoAttr && videoAttr[1] ? String(videoAttr[1]).trim() : null;
      const dedupe = exercise_key || normalizedKey(name);
      if (seen.has(dedupe)) continue;
      seen.add(dedupe);
      out.push({ name, exercise_key, wger_exercise_id, embedded_image_url, embedded_gif_url, embedded_video_url });
    }
  }
...
export async function enrichPlanContent({ html = '', parsedPlan = null } = {}) {
  ...
  const exercises = [];
  for (const { name, exercise_key, wger_exercise_id, embedded_image_url, embedded_gif_url, embedded_video_url } of exerciseEntries) {
    const hasEmbeddedMedia = Boolean(
      (embedded_image_url && embedded_image_url.length > 4) ||
      (embedded_gif_url && embedded_gif_url.length > 4) ||
      (embedded_video_url && embedded_video_url.length > 4)
    );
    if (hasEmbeddedMedia) {
      const canonicalKey = resolveToCanonicalKey(name) ?? exercise_key ?? null;
      exercises.push({
        name: name.split(':')[0].trim() || name,
        canonical_key: canonicalKey,
        query_name: name,
        exercise_key: exercise_key ?? canonicalKey,
        image_url: embedded_image_url || null,
        gif_url: embedded_gif_url || null,
        video_url: embedded_video_url || null,
        body_part: null,
        target: null,
        equipment: null,
        source: 'wger',
        trust_level: 'exact',
        wger_exercise_id: wger_exercise_id ?? null,
      });
      continue;
    }
    try {
