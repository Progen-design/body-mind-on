 ...
 * Returns array of { name, meal_key }. meal_key is set from data-meal-key when present; otherwise null.
 * Dedupe in enrichPlanContent is by canonical key (meal_key || normalizedKey(name)).
 */
function parseMealNamesFromHtml(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    out.push({ name: text, meal_key, recipe_id });
  }
...
export async function enrichPlanContent({ html = '', parsedPlan = null } = {}) {
  ...
  for (const { name, meal_key, recipe_id } of mealEntries) {
    const key = meal_key || normalizedKey(name);
    if (seenMealKey.has(key)) continue;
    seenMealKey.add(key);
    try {
      const enriched = await enrichMeal(name, meal_key || undefined, recipe_id || undefined);
