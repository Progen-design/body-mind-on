...
function parseExerciseNamesFromHtml(html) {
  ...
  function processLiBlock(blockHtml) {
    ...
    while ((m = liRe.exec(blockHtml)) !== null) {
      ...
      seen.add(dedupe);
      out.push({ name, exercise_key, wger_exercise_id, embedded_image_url, embedded_gif_url });
    }
  }
...
export async function enrichPlanContent({ html = '', parsedPlan = null } = {}) {
  ...
  const exercises = [];
  for (const { name, exercise_key, wger_exercise_id, embedded_image_url, embedded_gif_url } of exerciseEntries) {
    const hasEmbeddedMedia = Boolean(
      (embedded_image_url && embedded_image_url.length > 4) || (embedded_gif_url && embedded_gif_url.length > 4)
    );
    if (hasEmbeddedMedia) {
