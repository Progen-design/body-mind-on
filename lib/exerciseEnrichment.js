 ...
 *   4. Pexels → trust_level: "fallback" → returned runtime ONLY, NOT stored to registry
 *   5. None → trust_level: "none"
 *
 * PERSISTENCE RULE:
 *   Only trust_level="exact" assets are written to exercise_asset_registry.
 *   Pexels fallback assets are returned for the current request but never persisted
 *   as canonical truth, preventing a bad fallback image from becoming permanent.
 * ─────────────────────────────────────────────────────────────────────────────
 */
 ...
 * Fallback entries (Pexels) must not be served as canonical truth.
 */
...
export async function enrichExercise(exerciseName) {
  ...
  // IMPORTANT: Pexels result is returned for this request but NEVER stored to the
  // canonical exercise_asset_registry. Storing a Pexels fallback would permanently
  // cement a potentially irrelevant image as the canonical visual for this exercise.
