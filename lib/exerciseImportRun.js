...
/**
 * Kroky provedení ze zdroje — docs/DALSI_KROK.md 9.9.
 *
 * Dataset má u každého cviku pole `instructions`; do 7. 9. 2026 se při
 * importu zahazovalo, protože nebylo kam ho uložit. Ukládá se syrová
 * angličtina do `instructions_en` (do UI nikdy nejde) a překlad do
 * `instructions_cs` dělá stejná překladová linka jako u receptů
 * (lib/prekladPostupuCviku.js). Postup se NIKDY negeneruje modelem —
 * cvičební pokyny jsou zdravotně citlivé; co ve zdroji není, zůstane NULL.
 *
 * @param {unknown} instructions
 * @returns {string[]|null} neprázdné kroky, nebo null — NULL je poctivější než []
 */
export function krokyZeZdroje(instructions) {
  if (!Array.isArray(instructions)) return null;
  const kroky = instructions.map((s) => String(s || '').trim()).filter(Boolean);
  return kroky.length ? kroky : null;
}
...
export function pripravRadek(cvik) {
  ...
  return {
    radek: {
      ...
      external_id: cvik.id,
      instructions_en: krokyZeZdroje(cvik?.instructions),
    },
  };
}
