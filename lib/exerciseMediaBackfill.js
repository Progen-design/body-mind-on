...
/**
 * Ověří, jestli daný snímek už v registru nepatří JINÉMU cviku.
 *
 * Kontroluje obě sloupce, kam se obrázek může dostat — wger_exercise_image_url
 * (kam píše persistWgerMedia) i image_url (odkud pochází snímky z
 * free-exercise-db) — aby wger nepřepsal duplicitou to, co už katalog má
 * z jiného zdroje.
 *
 * Selhání dotazu se vyhodnocuje jako „kolize", ne jako „bez kolize": když
 * se nedá ověřit, že je snímek unikátní, je bezpečnější médium nezapsat.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} client
 * @param {string} canonicalKey cvik, kterému se snímek chystáme zapsat
 * @param {string} imageUrl
 * @returns {Promise<boolean>} true = snímek už patří jinému cviku
 */
export async function mediumUzPatriJinemuCviku(client, canonicalKey, imageUrl) {
  try {
    const { data, error } = await client
      .from('exercise_asset_registry')
      .select('canonical_key')
      .or(`wger_exercise_image_url.eq.${imageUrl},image_url.eq.${imageUrl}`)
      .neq('canonical_key', canonicalKey)
      .limit(1);
    if (error) {
      console.error(`[exerciseMediaBackfill] kontrolu unikátnosti média se nepodařilo provést (${canonicalKey}):`, error.message || error);
      return true;
    }
    return Array.isArray(data) && data.length > 0;
  } catch (err) {
    console.error(`[exerciseMediaBackfill] kontrola unikátnosti média selhala (${canonicalKey}):`, err?.message || err);
    return true;
  }
}
...
export async function persistWgerMedia(client, canonicalKey, wgerResult) {
  ...
  // SDÍLENÝ OBRÁZEK SE NEZAPÍŠE. Vyhledávání ve wgeru umí na různé termíny
  // vrátit tentýž snímek — v produkci takhle 13 cviků skončilo se stejným
  // obrázkem gluteálního mostu a uživatel viděl u „Tlaky s jednoručkami"
  // úplně jiný cvik (opraveno migrací 20260908160000). Jeden snímek smí
  // patřit právě jednomu canonical_key; kolize se zaloguje a přeskočí,
  // protože žádný obrázek je pořád lepší než cizí.
  if (await mediumUzPatriJinemuCviku(client, canonicalKey, imageUrl)) {
    console.warn(`[exerciseMediaBackfill] wger vrátil obrázek, který už patří jinému cviku — přeskočeno (${canonicalKey}): ${imageUrl}`);
    return;
  }
