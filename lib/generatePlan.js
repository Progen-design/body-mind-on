...
async function generatePlan(params = {}) {
  ...
  const [agentConfig, supportingDocs] = await Promise.all([
  ...
  const targetStartDate = params.targetStartDate ?? null;
  const userPrompt = buildUserPrompt(bm, pinnedMeals, targetStartDate, taskContext);
