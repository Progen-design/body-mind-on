...
async function generatePlan(params = {}) {
  ...
  if (!firstValid.ok) {
    const strictRetryPrompt = `PŘEDCHOZÍ VÝSTUP NEBYL PLATNÝ (chybí: ${(firstValid.missingCoreSections || []).join(', ') || firstValid.reason}). NEPŘIJATELNÉ je vrátit jen Regenerace/Suplementace/Mindset. Vrať POUZE validní JSON: {ok:true, metrics:{...}, html:'...'}. Pole html MUSÍ obsahovat KOMPLETNÍ plán:
- <h3>Jídelníček</h3> – celý jídelníček na 7 dní
- <h3>Trénink</h3> – celý tréninkový blok
- 7 dní, u každého Snídaně, Oběd, Večeře (konkrétní jídla, ne prázdné)
- U každého dne „Trénink tento den“: <p><b>Trénink tento den:</b></p><ul><li>…</li></ul>
Bez těchto sekcí je výstup ZAMÍTNUT. Žádné vysvětlování mimo JSON.`;
