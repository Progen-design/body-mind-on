 ...
 */
function buildMealsOnlyPrompt(bm, trainingBlocksInOrder) {
...
Vrať platný JSON: {"ok":true,"metrics":{...},"html":"..."}. V html ponech i sekce Regenerace, Suplementace, Nákupní seznam, Mindset – můžeš je zkrátit nebo ponechat obecně. Hlavní je nový jídelníček a ZACHOVANÉ tréninkové bloky u každého dne.`;
}
  ...
  try {
    ...
    if (options.mealsOnly === true && bm.user_id) {
      ...
      if (trainingBlocks.length >= 7) {
        ...
        const mealsOnlyPrompt = buildMealsOnlyPrompt(bm, trainingBlocks);
        const out = await runAssistantWithPrompt(mealsOnlyPrompt);
    ...
    if (!rawContent) {
      const userPrompt = buildUserPrompt(bm);
      const out = await runAssistantWithPrompt(userPrompt);
      rawContent = out.rawContent;
      threadId = out.threadId;
    }
