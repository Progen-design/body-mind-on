...
KONTEXT: Stejná struktura a tón jako hlavní plán. Uživatel musí z obsahu hned vědět, co to je a co má dělat. Žádný zbytečný úvod – každá sekce = nadpis + konkrétní data.
...
HTML struktura (pouze inline styly, bez <html>, <body>, skriptů ani externího CSS):
...
async function generatePlan(params = {}) {
  ...
  if (dietTypeForCheck === 'vegetarian' || dietTypeForCheck === 'vegan') {
    if (planViolatesDiet(html, dietTypeForCheck)) {
      ...
      messages.push({
        role: 'user',
        content: `Kontrola: V předchozím výstupu byl v jídelníčku maso, ryby nebo drůbež${dietTypeForCheck === 'vegan' ? ', vejce, mléčné, med nebo želatina' : ''}. Klient má diet_type: ${dietTypeForCheck}. Přegeneruj CELÝ plán jako JSON bez zakázaných položek.`,
      });
      ...
      if (retryRaw) {
        ...
        if (!planViolatesDiet(retryHtml, dietTypeForCheck)) {
          html = retryHtml;
  ...
  try {
    ...
    if (dietTypeForCheck === 'vegetarian' || dietTypeForCheck === 'vegan') {
      if (planViolatesDiet(planHtml, dietTypeForCheck)) {
        console.warn('⚠️ Plán obsahuje zakázané položky pro diet_type. Přegenerovávám jednou.');
        messages.push({ role: 'assistant', content: rawContent });
        messages.push({
          role: 'user',
          content: `Kontrola: V předchozím výstupu byl v jídelníčku zakázaná položka pro diet_type: ${dietTypeForCheck}. Přegeneruj CELÝ plán jako JSON bez masa, ryb, drůbeže${dietTypeForCheck === 'vegan' ? ', vajec, mléčných, medu a želatiny' : ''}.`,
        });
        const retryCompletion = await openai.chat.completions.create({
          model: 'gpt-4o-mini',
          temperature: 0.5,
    ...
    const m = parsedForMetrics?.metrics;
