...
async function generatePlan(params = {}) {
  ...
  if (!firstValid.ok) {
    const strictRetryPrompt = `PŘEDCHOZÍ VÝSTUP NEBYL PLATNÝ (chybí: ${(firstValid.missingCoreSections || []).join(', ') || firstValid.reason}). Vrať pouze validní JSON s polem html. Html musí obsahovat kompletní jídelníček a kompletní tréninkový plán. Výstup bez Jídelníčku a Tréninku je zamítnut. POVINNĚ: <h3>Jídelníček</h3>, <h3>Trénink</h3>, 7 dní se Snídaně/Oběd/Večeře, u každého dne „Trénink tento den“. Žádné vysvětlování mimo JSON.`;
