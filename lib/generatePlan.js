...
/** Odstraní markdown bloky ```html ... ``` z výstupu AI, aby se v e-mailu nezobrazoval surový kód. */
function extractHtmlFromAiOutput(raw) {
  if (!raw || typeof raw !== 'string') return raw || '';
  let s = raw.trim();
  // Odstranit úvodní ```html nebo ``` (i s volitelnými mezerami/řádky)
  const htmlBlockStart = /^```\s*html\s*\n?/i;
  const codeBlockEnd = /\n?```\s*$/;
  if (htmlBlockStart.test(s)) {
    s = s.replace(htmlBlockStart, '').replace(codeBlockEnd, '').trim();
  } else if (s.startsWith('```')) {
    s = s.replace(/^```\s*\n?/, '').replace(codeBlockEnd, '').trim();
  }
  return s;
}
