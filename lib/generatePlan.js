...
async function generatePlan(params = {}) {
  ...
  if (parsed && parsed.ok && typeof parsed.html === 'string') {
    ...
    html = enrichTrainingSection(html);
    if (html.length < 1000) {
      const strictRetryPrompt = `Vrať pouze validní JSON ve tvaru {ok:true, metrics:{...}, html:'...'}. Pole html musí obsahovat kompletní 7denní jídelníček (Snídaně, Oběd, Večeře pro každý den) a tréninkové bloky „Trénink tento den“ u každého dne. Žádné vysvětlování mimo JSON. Předchozí výstup nebyl platný.`;
      ...
      if (parsed && parsed.ok && typeof parsed.html === 'string') {
        ...
        if (fallbackHtml && fallbackHtml.length >= 1000) html = enrichTrainingSection(fallbackHtml);
      }
    }
  }
  ...
  const sectionPatterns = [/jídelníček|jidelníček|jidelnicek/i, /trénink|trenink/i, /snídaně|snidane/i, /oběd|obed/i, /večeře|vecere/i, /regenerace/i, /suplementace/i, /mindset/i];
  const sectionCount = sectionPatterns.filter((p) => p.test(html || '')).length;
  const hasMinSections = sectionCount >= 3;

  if (!html || typeof html !== 'string' || html.length < 1000 || !hasMinSections) {
    console.error('[generatePlan] validation failed', { html_length: (html || '').length, section_count: sectionCount });
    throw new Error('Generated plan is incomplete');
  }
  ...
  try {
    ...
    if (!opts.skipEmail) {
      ...
      if (!hasMinLength || !hasPlanSections) {
        console.warn('⚠️ Plán není dostatečně kompletní pro e-mail (délka nebo chybějící sekce). Uloženo do DB.');
        return {
          ok: true,
