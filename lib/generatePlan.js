...
/**
 * Zavolá OpenAI Asistenta (Body and Mind ON) – instrukce jsou jen v dashboardu OpenAI.
 * @param {string} userMessage - text zprávy (VSTUP + požadavek na plán)
 * @param {string} [existingThreadId] - pokud jde o retry, předat threadId a přidáme další zprávu + run
 * @returns {Promise<{ rawContent: string, threadId: string }>}
 */
async function runAssistantWithPrompt(userMessage, existingThreadId = null) {
  if (!ASSISTANT_ID.trim()) {
    throw new Error('Chybí OPENAI_ASSISTANT_ID v env. Nastav ID asistenta z platform.openai.com (Assistants → Body and Mind ON → ID).');
  }
  ...
  const run = await openai.beta.threads.runs.create(threadId, {
    assistant_id: ASSISTANT_ID.trim(),
  });
  ...
  while (runStatus === 'queued' || runStatus === 'in_progress' || runStatus === 'cancelling') {
    if (Date.now() - start > RUN_TIMEOUT_MS) {
      throw new Error('Asistent nepřijal odpověď včas (timeout). Zkus to znovu.');
    }
    await new Promise((r) => setTimeout(r, RUN_POLL_MS));
    const runRetrieved = await openai.beta.threads.runs.retrieve(threadId, runId);
    runStatus = runRetrieved.status;
    if (runStatus === 'failed' || runStatus === 'cancelled' || runStatus === 'expired') {
      const errMsg = runRetrieved.last_error?.message || runStatus;
      throw new Error('Asistent selhal: ' + errMsg);
    }
  }
  if (runStatus !== 'completed') {
    throw new Error('Asistent nevrátil plán (status: ' + runStatus + ').');
  }
  const messages = await openai.beta.threads.messages.list(threadId, { order: 'desc', limit: 5 });
  const assistantMsg = messages.data.find((m) => m.role === 'assistant');
  if (!assistantMsg || !assistantMsg.content || !assistantMsg.content.length) {
    throw new Error('OpenAI asistent nevrátil žádný text.');
  }
  const parts = [];
...
/** Minimální délka textu v sekci Trénink (znaky bez HTML), pod kterou doplníme výchozí blok. */
const TRAINING_SECTION_MIN_LENGTH = 280;
...
function buildUserPrompt(bm) {
  ...
  // Sestavit pořadí 7 dnů začínající od dnešního dne
...
Vygeneruj kompletní plán jako JSON podle struktury. Respektuj diet_type a preferences. Jídelníček: 7 dní, 3 jídla denně – POVINNĚ začni od dne "${startDay}" a pokračuj v tomto pořadí: ${orderedDays.join(', ')}. Suplementace povinně dle diet_type. Nákupní seznam a Mindset na tento týden musí být v html.

DŮLEŽITÉ – Sekce Trénink (v html musí být <h3>Trénink</h3> nebo <h3>Tréninkový plán</h3>): NEPIŠ jen jednu větu. Tréninkový blok musí být rozvinutý jako od profesionálního trenéra: (1) Doporučené dny a typ tréninku podle weekly_sessions a cíle (redukce/nabírání/udržování). (2) Rozcvička: 5–10 min (kardio, dynamický strečink, aktivace). (3) Hlavní část: struktura jednotky (silový/kardio/kombinace), délka 45–60 min, příklady cviků nebo skupin svalů dle cíle. (4) Závěrečný strečink 5 min. (5) Krátká poznámka k progresi (postupně přidávat zátěž/série) a bezpečnost (správné dýchání, ne přes bolest). Použij odstavce <p> a případně <ul>/<li>.`;
}
...
async function generatePlan(params = {}) {
  ...
  const { rawContent: rawContentFirst, threadId } = await runAssistantWithPrompt(userPrompt);
  ...
  if (needsRetryDiet || needsRetryGluten) {
    ...
    try {
      const { rawContent: retryRaw } = await runAssistantWithPrompt(retryMessage, threadId);
      if (retryRaw) {
...
async function generatePlanForEmail(email, options = {}) {
  try {
    ...
    const { rawContent, threadId } = await runAssistantWithPrompt(userPrompt);
    if (!rawContent) throw new Error('OpenAI asistent nevrátil žádný plán.');
