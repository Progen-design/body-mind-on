 ...
 * Respektuje goal, workout_days, diet_type, dietary_restrictions, foods_to_avoid.
 */
function buildDeterministicFallbackPlanHtml(bodyMetrics) {
  ...
  const breakfast = diet === 'vegan' ? 'Ovesná kaše s ovocem a ořechy, čaj' : diet === 'vegetarian' ? 'Ovesná kaše s banánem a medem, čaj' : 'Ovesná kaše s banánem, vejce, čaj';
  const lunch = diet === 'vegan' ? 'Čočka na kyselo, rýže, zeleninový salát' : diet === 'vegetarian' ? 'Čočka s rýží, zeleninový salát' : 'Kuřecí prsa s rýží a zeleninou';
  const dinner = diet === 'vegan' ? 'Tofu s quinoou a dušená zelenina' : diet === 'vegetarian' ? 'Sýr s celozrnným chlebem, zelenina' : 'Ryba s bramborami a salát';

  let daysHtml = '';
  for (const dayName of orderedDays) {
    const trainBlock = isWorkoutDay(dayName)
      ? '<p><b>Trénink tento den:</b></p><ul><li>Trénink celkem: 45 min (přizpůsobeno cíli).</li><li>Rozcvička 5–10 min.</li><li>Dřepy, výpady, kliky, přítahy – 3 série po 10–12.</li><li>Strečink 5 min.</li></ul>'
      : '<p><b>Trénink tento den:</b></p><ul><li>Odpočinek.</li></ul>';
    daysHtml += `
<h3>${dayName}</h3>
<p><b>Snídaně:</b> ${breakfast}${dietNote}</p>
<p><b>Oběd:</b> ${lunch}</p>
<p><b>Večeře:</b> ${dinner}</p>
${trainBlock}
`;
  }
...
<p>Každý malý krok se počítá. Sleduj pokrok v aplikaci.</p>
