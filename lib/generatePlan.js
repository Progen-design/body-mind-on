...
async function generatePlan(params = {}) {
  ...
  if (parsed && parsed.ok && typeof parsed.html === 'string') {
    ...
    html = enrichTrainingSection(html);
    if (html.length < 1000) {
      const strictRetryPrompt = `Vrať pouze validní JSON ve tvaru {ok:true, metrics:{...}, html:'...'} a html musí obsahovat kompletní jídelníček na 7 dní + trénink. Žádný jiný formát. Předchozí výstup nebyl platný.`;
  ...
  const sectionPatterns = [/jídelníček|jidelníček|jidelnicek/i, /trénink|trenink/i, /snídaně|snidane/i, /oběd|obed/i, /večeře|vecere/i, /mindset/i];
