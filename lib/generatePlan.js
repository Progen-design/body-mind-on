...
async function generatePlan(params = {}) {
  ...
  if (!firstValid.ok) {
    const strictRetryPrompt = `PŘEDCHOZÍ VÝSTUP NEBYL PLATNÝ (chybí: ${(firstValid.missingCoreSections || []).join(', ') || firstValid.reason}). Vrať POUZE validní JSON: {ok:true, metrics:{...}, html:'...'}. Pole html MUSÍ obsahovat:
- <h3>Jídelníček</h3> – hlavní sekce jídelníčku
- <h3>Trénink</h3> – hlavní sekce tréninku
- 7 dní s Snídaně, Oběd, Večeře (konkrétní jídla)
- U každého dne blok „Trénink tento den“: <p><b>Trénink tento den:</b></p><ul><li>…</li></ul>
Žádné vysvětlování mimo JSON.`;
    ...
    if (parsed && parsed.ok && typeof parsed.html === 'string') {
      ...
      if (fallbackHtml) html = enrichTrainingSection(fallbackHtml);
    }
  }
  ...
  const planValid = validatePublishedPlanHtml(html || '');
  if (!planValid.ok) {
    console.error('[generatePlan] validation failed', { reason: planValid.reason, html_length: planValid.length, missing_core_sections: planValid.missingCoreSections });
