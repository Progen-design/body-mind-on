...
/** Položky v „Trénink tento den“, které nejsou cviky – nepřidáváme data-exercise-key. */
const TRAINING_SKIP_LI_PREFIXES = ['odpočinek', 'lehká procházka', 'trénink celkem', 'rozcvička', 'závěr'];

/**
 * Do HTML plánu doplní data-exercise-key u <li> v blocích „Trénink tento den“, pokud tam chybí.
 * AI často negeneruje tyto atributy – enrichment a PlanViewer pak potřebují canonical key pro zobrazení médií.
 */
    ...
    (block) => {
      return block.replace(/<ul([^>]*)>([\s\S]*?)<\/ul>/i, (_, ulAttrs, ulInner) => {
        const newUlInner = ulInner.replace(/<li([^>]*)>([\s\S]*?)<\/li>/gi, (liMatch, liAttrs, liContent) => {
          ...
          const key = resolveToCanonicalKey(name);
