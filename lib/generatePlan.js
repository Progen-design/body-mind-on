...
import { resolveToCanonicalKey, normalizeExerciseLookupKey } from './exerciseCanonicalMap.js';
import { findGlutenTerm } from './dietCriticalTerms.js';
...
function buildDeterministicFallbackPlanHtml(bodyMetrics, targetStartDate = null) {
  ...
  // Vlastní seznam lepkových slov tu stával do 10. 8. 2026 — byl kratší než
  // v bráně, takže šablona pustila to, co brána pak zablokovala. Zdroj pravdy
  // je lib/dietCriticalTerms.js.
  const hasGluten = (text) => findGlutenTerm(text) !== null;
  const pickMeal = (arr, arrAlt, dayIdx) => {
    ...
    if (hasGlutenRestriction && hasGluten(avoid)) {
      for (let k = 1; k < base.length; k++) {
        const alt = base[(dayIdx + k) % base.length];
        if (!hasGluten(alt.text || '')) return alt;
      }
    }
    return item;
  };

  /** Orientační rozpad B/S/T na jídla (součet ≈ denní cíle) — pro e-mailové makro tagy i když chybí Spoonacular. */
  const mealMacroSmall = (bFrac, sFrac, tFrac) => {
    const b = Math.max(0, Math.round(protein * bFrac));
    const s = Math.max(0, Math.round(carbs * sFrac));
    const t = Math.max(0, Math.round(fat * tFrac));
    return `<p class="meal-nutrition-line"><small>B ${b} g · S ${s} g · T ${t} g</small></p>`;
  };
  const dayTotalKcal = Math.round(calories);
  const fiberEst = Math.max(8, Math.min(35, Math.round(carbs / 18)));
...
function buildUserPrompt(bm, pinnedMeals = [], targetStartDate = null, taskContext = null) {
  ...
  // ZDROJ PRAVDY JE, CO UŽIVATEL ZAŠKRTL — `weekly_sessions_user`.
  //
  // Sloupec `weekly_sessions` NEPÍŠE aplikace, ale databázový trigger
  // `bm_fill_calculated_fields`. Ten mapuje `freq_choice` podle slovníku
  // '0-1' / '2-3' / '4plus', jenže registrace ukládá „2-3x týdně“ a
  // „4-5x týdně“ — nic z toho se netrefí a trigger spadne na náhradu
  // ODVOZENOU ZE STRESU (low→5, medium→4, high→3).
  //
  // Změřeno 18. 8. 2026: účet se `stress_level='medium'` a volbou „4-5x týdně“
  // má `weekly_sessions_user = 5`, ale `weekly_sessions = 4`. Ondra (25b7017a)
  // má 5 proti 4 dnům v `workout_days`. Plán se tím stavěl na frekvenci, kterou
  // uživatel nikdy nezvolil.
  //
  // `bodyMetricsToPlanInput` už `weekly_sessions_user` preferuje; tohle je
  // druhá cesta do generátoru, která na to zapomněla.
  const rawFreq = bm.weekly_sessions_user ?? bm.freq_choice ?? bm.frequency ?? bm.weekly_sessions;
...
async function generatePlan(params = {}) {
  ...
  const { runUnifiedPlanPipeline } = await import('./unifiedPlanPipeline.js');
...
async function generatePlanForEmail(email, options = {}) {
  try {
    ...
    const { generatePlanForEmailViaUnified } = await import('./unifiedPlanPipeline.js');
    return generatePlanForEmailViaUnified(email, {
      ...
      applyWithingsPlanAdjustment: options.applyWithingsPlanAdjustment === true,
