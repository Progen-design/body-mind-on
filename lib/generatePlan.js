...
DŮLEŽITÉ: Vrať pouze čistý HTML kód (tagy <h2>, <section>, <ul>, <li>, <p> atd.). Nepoužívej markdown ani bloky \`\`\`html – výstup má být přímo HTML bez obalování do kódu.
...
/** Odstraní markdown bloky a text "html" z výstupu AI, aby se v e-mailu nezobrazoval surový kód. */
function extractHtmlFromAiOutput(raw) {
  if (!raw || typeof raw !== 'string') return raw || '';
  let s = raw.trim();
  // Odstranit úvodní ```html nebo ``` (různé varianty včetně unicode)
  s = s.replace(/^```\s*html\s*\n?/i, '').replace(/\n?```\s*$/g, '').trim();
  s = s.replace(/^```\s*\n?/, '').replace(/\n?```\s*$/g, '').trim();
  // Odstranit samostatný řádek nebo slovo "html" / "HTML" na začátku (AI někdy přidá)
  s = s.replace(/^(html|HTML)(\s*\n|\s+)/i, '$2').trim();
  if (/^html\n/i.test(s)) s = s.replace(/^html\n/i, '');
  // Pokud pořád začíná na "html" (např. "html<h2>"), odstranit
...
// Oba exporty jsou potřeba: generatePlan pro /api/generate-plan, generatePlanForEmail pro body-metrics
