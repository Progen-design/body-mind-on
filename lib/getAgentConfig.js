...
/**
 * Fallback config when DB row exists but some fields are null.
 * enabled is NEVER true in fallback – agent must exist in ai_agents to run.
 */
function buildFallback(agentSlug = 'trainer') {
  ...
  return {
    ...
    temperature: 0.2,
    enabled: false, // Agent must be in DB – no fallback enabled in production
    version: 1,
    prompt_version: 1,
  };
}
...
export async function getAgentConfig(agentSlug) {
  ...
  try {
    const { data, error } = await supabaseServer
      .from('ai_agents')
      .select('slug, name, model, system_prompt, temperature, enabled, version, prompt_version')
