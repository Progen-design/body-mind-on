...
// Plný přístup k kalendáři (čtení + zápis). Po změně scope musí trenér kalendář znovu propojit.
...
function getRedirectUri() {
  const fromEnv = process.env.NEXT_PUBLIC_APP_URL && String(process.env.NEXT_PUBLIC_APP_URL).replace(/\/$/, '');
  const fromVercel = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}`.replace(/\/$/, '') : '';
  const base = fromEnv || fromVercel || getPublicAppUrl();
...
export async function listEvents(accessToken, calendarId = 'primary', timeMin, timeMax) {
  ...
  const items = (data.items || []).map((ev) => {
    ...
    const allAttendees = attendeeList.map((a) => (a.email || '').toLowerCase()).filter(Boolean);
    const organizer = (ev.organizer?.email || '').toLowerCase();
    const attendees = allAttendees.filter((e) => e !== organizer);
    const hasNeedsAction = attendeeList.some((a) => (a.responseStatus || '').toLowerCase() === 'needsaction');
    const confirmed = !!attendees.length && !hasNeedsAction;
    return {
      id: ev.id,
      ...
      attendees,
      organizer,
      unconfirmed: !!attendees.length && hasNeedsAction,
      confirmed,
    };
  });
  return items;
...
export async function createEvent(accessToken, calendarId = 'primary', opts) {
  ...
  const query = body.attendees?.length ? '?sendUpdates=all' : '';
  const url = `${CALENDAR_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events${query}`;
  const res = await fetch(url, {
    method: 'POST',
    headers: {
...
export function eventIsForUser(ev, userEmail) {
  ...
  // Událost bez účastníků a bez „Pro:“ – vidí jen organizátor (trenér), ne klienti
  return false;
}
