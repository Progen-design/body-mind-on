...
export const POSITIVE_HABITS = [
  ...
  { id: 'daily_movement', label: 'Denní pohyb', emoji: '🚶', description: 'pohyb navíc (procházka, kroky)' },
  ...
  { id: 'healthy_diet', label: 'Zdravá strava', emoji: '🥗', description: 'kvalitní jídlo, dodržení jídelníčku nebo vyvážené porce' },
 ...
 *
 * Do 31. 8. 2026 registrace i editace profilu neplatné klíče prostě
 * vyfiltrovaly (`.filter(isValidHabitId)`) a request prošel bez chyby.
 * Změřeno 31. 8. 2026 skriptem, který volal API přímo (ne přes UI):
 * `selected_habits: ['zdrava_strava', 'kvalitni_spanek']` — české klíče
 * místo anglických `healthy_diet`/`quality_sleep` — prošlo se stavem 200
 * u všech deseti účtů a `user_habits` zůstalo prázdné.
 * Registrační UI bere id z tohoto modulu (StartRegistrace.tsx importuje
 * POSITIVE_HABITS/NEGATIVE_HABITS), takže samo neplatný klíč neposílá —
 * chráníme se před jiným klientem a před rozejitím dvou seznamů.
 * Viz docs/DALSI_KROK.md 6.7 (nález nad rámec bodu).
 *
 * Sdílené místo pro obě volající cesty (api/body-metrics.js,
 * api/profile-preferences.js), ať se nerozejdou v tom, co považují
 * za platný klíč.
 *
 * @param {unknown} ids
 * @returns {unknown[]} neplatné položky (prázdné pole = vstup je v pořádku)
 */
export function invalidHabitIds(ids) {
  if (!Array.isArray(ids)) return [];
  return ids.filter((id) => {
    const trimmed = typeof id === 'string' ? id.trim() : id;
    return !(typeof trimmed === 'string' && trimmed && isValidHabitId(trimmed));
  });
}

/**
 * VÝCHOZÍ NÁVYKY PRO NOVÝ ÚČET.
 *
 * Od 9. 9. 2026 se návyky v registraci nevybírají — krok 5 zůstal jen na
 * souhlasu. Prázdné `user_habits` by ale znamenalo prázdnou záložku Návyky
 * a TEDa bez kontextu (`loadUserHabitsForCoach` v lib/taskExecutors.js
 * čte právě tuhle tabulku), proto se sada zakládá na serveru.
 */
export const VYCHOZI_NAVYKY = ['training', 'healthy_diet', 'hydration', 'quality_sleep'];

/**
 * Sada návyků k založení při registraci: napřed to, co vyplývá z cíle,
 * aktivity a stresu (`getSuggestedHabits`), pak výchozí sada jako doplněk.
 *
 * ZLOZVYKY SE NESEEDUJÍ. `naZlozvyky()` (src/data/adaptery.ts) jim dává
 * natvrdo `status: 'clean'` a nikde se neloguje prohřešek — dokud se
 * nesledují, nemá smysl někomu předvyplnit „Kouření" podle poznámky.
 *
 * @param {Object} metrics - registrační payload pro body_metrics
 * @returns {string[]} habit_id v pořadí, v jakém se mají uložit
 */
export function seedHabitIdsForRegistration(metrics) {
  const pozitivni = new Set(POSITIVE_HABITS.map((h) => h.id));
  const navrzene = getSuggestedHabits(metrics).filter((id) => pozitivni.has(id));
  return [...new Set([...navrzene, ...VYCHOZI_NAVYKY])].slice(0, 6);
}
