 ...
 * @returns {string}
 */
export function inferCanonicalQualifier(canonicalKey, parsedQualifier = '') {
...
export function convertToCanonicalUnit(amount, unit) {
  ...
  if (u === 'ks' || u === 'plátek') return { amount: n, unit: u, aggregatable: true };
...
export function normalizeParsedIngredient(parsed) {
  ...
  const qualifier = inferCanonicalQualifier(canonical.key, parsed.qualifier || '');
