...
/** Czech display forms by count branch (canonical unit → forms). */
const UNIT_DISPLAY_FORMS = Object.freeze({
  konzerva: { one: 'konzerva', few: 'konzervy', many: 'konzerv', decimal: 'konzervy' },
  'plátek': { one: 'plátek', few: 'plátky', many: 'plátků', decimal: 'plátku' },
  'stroužek': { one: 'stroužek', few: 'stroužky', many: 'stroužků', decimal: 'stroužku' },
  ks: { one: 'ks', few: 'ks', many: 'ks', decimal: 'ks' },
});

const UNINFLECTED_DISPLAY_UNITS = new Set(['g', 'ml', 'l', 'lžíce', 'lžička', 'ks']);

/**
 * @param {number} amount
 * @returns {'one'|'few'|'many'|'decimal'}
 */
function czechCountBranch(amount) {
  const n = Number(amount);
  if (!Number.isFinite(n)) return 'many';
  const abs = Math.abs(n);
  const isWhole = Math.abs(abs - Math.round(abs)) < 1e-9;
  if (!isWhole) return 'decimal';
  const whole = Math.round(abs);
  if (whole === 1) return 'one';
  if (whole >= 2 && whole <= 4) return 'few';
  return 'many';
}

/**
 * Czech-inflected unit for shopping list display (aggregation key stays singular).
 * @param {number} amount rounded display amount
 * @param {string|null|undefined} canonicalUnit normalized singular unit
 * @returns {string}
 */
export function formatDisplayUnit(amount, canonicalUnit) {
  const u = normalizeShoppingUnit(canonicalUnit);
  if (!u || UNINFLECTED_DISPLAY_UNITS.has(u)) return u;
  const forms = UNIT_DISPLAY_FORMS[u];
  if (!forms) return u;
  return forms[czechCountBranch(amount)] || u;
}
