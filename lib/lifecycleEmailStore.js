...
export async function listCandidateMemberships() {
  const { data, error } = await supabaseServer
    .from('memberships')
    .select('user_id, tier, status, started_at, trial_ends_at, stripe_subscription_id')
...
/**
 * Jméno k oslovení — z `body_metrics.name` (tam ho zapisuje registrace),
 * stejně jako připomínka neaktivity. Chyba = null, e-mail pak začne „Ahoj,".
 * @returns {Promise<string|null>}
 */
export async function getUserName(userId) {
  const { data, error } = await supabaseServer
    .from('body_metrics').select('name')
    .eq('user_id', userId).not('name', 'is', null).neq('name', '')
