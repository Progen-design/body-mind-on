...
export async function sendPlanEmail(email, planHtml, options = {}) {
  ...
  try {
    ...
    const requestedVersionRaw = String(options.templateVersion || process.env.EMAIL_TEMPLATE_VERSION || 'v8').trim().toLowerCase();
    const requestedVersion = ['v8', 'v7', 'v6', 'v5', 'v4', 'v2', 'legacy'].includes(requestedVersionRaw) ? requestedVersionRaw : 'v8';
    ...
    if (canUseStructuredPlan && requestedVersion === 'v8') {
      try {
