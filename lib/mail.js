...
import { countPlanDaysInHtml } from './validatePlanHtml.js';
...
/**
 * Detekce už zabaleného e-mailového dokumentu (plan_html přepsané dříve odeslaným e-mailem
 * přes persistSentPlanEmailHtml). Čistý plan_html fragment nikdy neobsahuje doctype/<html>.
 */
function isWrappedEmailDocument(html) {
  const s = String(html || '');
  return /<!DOCTYPE\s+html/i.test(s) || /<html[\s>]/i.test(s);
}

/**
 * Po úspěšném odeslání uloží finální HTML e-mailu do ai_generated_plans.plan_html (audit / ověření z DB).
 * @param {string|null|undefined} planId
 * @param {string|null|undefined} sentHtml
 */
export async function persistSentPlanEmailHtml(planId, sentHtml) {
  const id = planId != null ? String(planId).trim() : '';
  const html = typeof sentHtml === 'string' ? sentHtml.trim() : '';
  if (!id || !html) return { ok: false, reason: 'missing_id_or_html' };
  const { error } = await supabaseServer
    .from('ai_generated_plans')
    .update({ plan_html: html })
    .eq('id', id);
  if (error) {
    console.warn('[persistSentPlanEmailHtml] update failed', { plan_id: id, error: error.message });
    return { ok: false, reason: error.message };
  }
  console.info('[persistSentPlanEmailHtml] saved sent email html', { plan_id: id, html_bytes: html.length });
  return { ok: true };
}
...
export async function sendPlanEmail(email, planHtml, options = {}) {
  ...
  try {
    ...
    const bodyMetricsForEmail =
      options.bodyMetricsForEmail && typeof options.bodyMetricsForEmail === 'object'
        ...
        : options.bodyMetrics && typeof options.bodyMetrics === 'object'
          ? options.bodyMetrics
          : null;
    // Ochrana proti dvojitému zabalení: plan_html mohlo být přepsáno dříve odeslaným
    // e-mailovým dokumentem (persistSentPlanEmailHtml). Nikdy nebalit obálku do obálky.
    ...
    const requestedVersion = ['v8', 'v7', 'v6', 'v5', 'v4', 'v2', 'legacy', 'plan_html'].includes(requestedVersionRaw)
      ? requestedVersionRaw
      : 'v8';
    // Jeden zdroj pravdy: kompletní plan_html z renderPlanHtmlFromStructured (7 dní) má prioritu před V8 structured renderem.
    const usePlanHtmlTemplate =
      requestedVersion === 'legacy' ||
      requestedVersion === 'plan_html' ||
      hasFullWeekInPlanHtml ||
