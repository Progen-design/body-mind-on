...
export async function sendPlanEmail(email, planHtml, options = {}) {
  ...
  try {
    ...
    const requestedVersionRaw = String(options.templateVersion || process.env.EMAIL_TEMPLATE_VERSION || 'v5').trim().toLowerCase();
    const requestedVersion = ['v5', 'v4', 'v2', 'legacy'].includes(requestedVersionRaw) ? requestedVersionRaw : 'v5';
    ...
    if (canUseStructuredPlan && requestedVersion === 'v5') {
      try {
