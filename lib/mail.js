...
export async function sendPlanEmail(email, planHtml, options = {}) {
  ...
  try {
    ...
    const requestedVersionRaw = String(options.templateVersion || process.env.EMAIL_TEMPLATE_VERSION || 'v6').trim().toLowerCase();
    const requestedVersion = ['v6', 'v5', 'v4', 'v2', 'legacy'].includes(requestedVersionRaw) ? requestedVersionRaw : 'v6';
    ...
    if (canUseStructuredPlan && requestedVersion === 'v6') {
      try {
