...
/** Transformuje plán do vizuálně přehlednější podoby pro e-mail – karty pro dny, ikony u jídel. */
function formatPlanHtmlForEmail(html) {
  if (!html || typeof html !== 'string') return '';
  let out = html;

  // Extrahovat Mindset sekci a přesunout nahoře jako hero blok
  ...
  if (mindsetMatch) {
    const mindsetContent = mindsetMatch[1].trim()
      .replace(/<p[^>]*>/gi, '<p style="margin:0 0 12px;color:#d8b4fe;font-size:15px;line-height:1.7;padding:12px 16px;background:rgba(255,255,255,0.05);border-radius:10px;border-left:3px solid #7c3aed;">')
      .replace(/<b>/gi, '<b style="display:block;margin-bottom:4px;font-size:12px;letter-spacing:0.06em;text-transform:uppercase;color:#f3e8ff;">');
    out = out.replace(mindsetMatch[0], '');
    mindsetHeroBlock = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 28px;background:#1e0a3e;border-radius:16px;border:1px solid #7c3aed;overflow:hidden;" bgcolor="#1e0a3e">
  <tr><td style="padding:18px 24px;background:linear-gradient(135deg,#3d1a6e,#2a0a4e);border-bottom:1px solid rgba(124,58,237,0.4);" bgcolor="#3d1a6e">
    <p style="margin:0;font-size:12px;color:#c4b5fd;text-transform:uppercase;letter-spacing:0.1em;">🧠 Mindset na tento týden</p>
  </td></tr>
  ...
  // Extrahovat Trénink sekci a zobrazit jako výraznou kartu (jako Mindset)
  ...
  if (trainingMatch && trainingMatch[1].trim()) {
    ...
    trainingHeroBlock = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 28px;background:#1a1f2e;border-radius:16px;border:1px solid #475569;overflow:hidden;" bgcolor="#1a1f2e">
  <tr><td style="padding:18px 24px;background:linear-gradient(135deg,#334155,#1e293b);border-bottom:1px solid rgba(71,85,105,0.5);" bgcolor="#334155">
    <p style="margin:0;font-size:12px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;">🏋️ Tréninkový plán – jak cvičit</p>
  </td></tr>
  <tr><td style="padding:20px 24px;color:#e2e8f0;font-size:14px;line-height:1.65;">
    ${trainingContent}
  </td></tr>
</table>`;
  }
  ...
  if (dayMatch) {
    const beforeMeal = out.slice(0, dayMatch.index + dayMatch[0].indexOf(dayMatch[2]));
    const afterMeal = out.slice(dayMatch.index + dayMatch[0].length);
    const mealSection = dayMatch[2];

    // Rozdělit na dny: h4 + obsah do dalšího h4
    ...
    for (const block of dayBlocks) {
      ...
      if (h4Match) {
        ...
        const dayContent = (h4Match[2] || '').trim();
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 16px;background:#1a1a24;border-radius:12px;border:1px solid #2e2e42;overflow:hidden;" bgcolor="#1a1a24">
  ...
  <tr><td style="padding:16px 18px;color:#d4d4d8;font-size:14px;line-height:1.5;">${content}</td></tr>
    ...
    out = beforeMeal + mealHtml + afterMeal;
  }

  // Sekce h3 – přidat vizuální oddělení
  ...
  return mindsetHeroBlock + trainingHeroBlock + out;
}
