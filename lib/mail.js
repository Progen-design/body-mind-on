...
export async function sendShoppingListEmail(email, items, title = null, options = {}) {
  try {
    ...
    const normalizeItem = (item) => {
      ...
      if (!s) return '';
      if (/suroviny se nepodařilo přesně rozpoznat/i.test(s)) return s;
      if (/\b(ks|g|kg|ml|l|lžice|lžička|porce|hrst|plátek|balení|dle chuti|orientačně)\b/i.test(s)) return s;
