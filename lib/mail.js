...
export async function sendTrainingInvitationEmail(to, { title, start, end }) {
  ...
  try {
    ...
    const startFormatted = isNaN(startDate.getTime()) ? start : startDate.toLocaleString('cs-CZ', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' });
    const endFormatted = isNaN(endDate.getTime()) ? end : endDate.toLocaleString('cs-CZ', { hour: '2-digit', minute: '2-digit' });
