...
export async function sendPlanEmail(email, planHtml, options = {}) {
  ...
  try {
    ...
    const requestedVersion = String(options.templateVersion || process.env.EMAIL_TEMPLATE_VERSION || 'v4').trim().toLowerCase();
    const useV4 = canUseStructuredPlan && requestedVersion !== 'v2' && requestedVersion !== 'legacy';
    const useV2 = canUseStructuredPlan && !useV4 && requestedVersion === 'v2';
    ...
    const htmlTemplate = useV4
      ? buildWeeklyPlanEmailV4Document(structuredBuilderArgs)
      : useV2
        ? buildWeeklyPlanEmailV2Document(structuredBuilderArgs)
    ...
    const templateVersion = useV4 ? 'v4' : useV2 ? 'v2' : 'legacy';
