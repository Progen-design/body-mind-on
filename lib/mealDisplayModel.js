...
function librarySourceFromMeal(meal) {
  ...
  if (meal?.planner_source === 'simple_meal_planner_agent') {
    const title = asString(meal?.display_name_cs) || asString(meal?.display_name) || asString(meal?.name_cs);
    if (findSimpleStartRecipeByTitle(title, meal?.type)) return 'simple_start_library';
  }
  return null;
}
...
export function createMealDisplayModel(meal, appBaseUrl = '') {
  ...
  const recipeUrl = normalizedIsFallback || isLibrary
