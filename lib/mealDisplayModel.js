...
export function createMealDisplayModel(meal, appBaseUrl = '') {
  ...
  const ingredients = normalizeIngredients(meal, isFallback);
  const instructions = normalizeInstructions(meal, title, ingredients, isFallback);
  const nutrition = normalizeNutrition(meal, isFallback);
  const recipeSource = classifyRecipeSource(meal, isFallback);
  const consistencyStatus = buildConsistencyStatus(meal, title, ingredients, instructions, recipeSource, isFallback);
  ...
  const normalizedMeal = normalizedIsFallback
    ? {
      ...meal,
      name_cs: title,
  ...
  const recipeUrl = normalizedIsFallback
  ...
  return {
    ...
    recipeUrl,
    recipeSource: normalizedIsFallback ? 'fallback' : recipeSource,
