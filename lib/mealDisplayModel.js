import { vyberPostup } from './profile/postupReceptu.js';
...
import { ingredientRecordToDisplayLine } from './shoppingListAggregate.js';
...
function normalizeIngredients(meal, isFallback) {
  const recipeIngredients = Array.isArray(meal?.recipe?.ingredients)
    ? meal.recipe.ingredients.map((line) => ingredientRecordToDisplayLine(line)).filter(Boolean)
    : [];
  ...
  if (!isFallback && Array.isArray(meal?.ingredients)) {
    return meal.ingredients.map((line) => ingredientRecordToDisplayLine(line)).filter(Boolean);
  }
  return [];
}

/**
 * Postup převedený na řádky. Přijímá pole i jeden slepený řetězec.
 * @param {unknown} raw
 * @returns {string[]}
 */
function instrukceNaRadky(raw) {
  if (Array.isArray(raw)) return raw.map((line) => asString(line)).filter(Boolean);
  const text = asString(raw);
  // Slepeny retezec se ROZDELI po radcich. Do 15. 9. 2026 se vracel jako
  // jeden prvek, takze z sesti kroku byl v UI jeden odstavec — presne to,
  // co dokumentace teto funkce („prijima pole i jeden slepeny retezec")
  // slibovala, ze delat nebude. Stejna rodina vady jako prazdny postup
  // u zameneneho jidla, jen tise jinak.
  return text ? text.split('\n').map((r) => r.trim()).filter(Boolean) : [];
}

/**
 * SKUTEČNÝ POSTUP VYHRÁVÁ, I KDYŽ JE RECEPT OZNAČENÝ ZA FALLBACK.
 *
 * Do 20. 8. 2026 se reálný postup četl jen pod podmínkou `!isFallback`, takže
 * recept, který neprošel heuristikou jednoduchosti, dostal generické
 * „Připrav suroviny podle seznamu“ — přestože měl v databázi konkrétní návod.
 * Druhá polovina téže chyby: `asString` nad JSONB polem vrací prázdno, takže
 * i mimo fallback se postup uložený jako pole zahodil.
 *
 * Generický návod je až poslední možnost — pro recepty, které postup opravdu
 * nemají.
 */
function normalizeInstructions(meal, title, ingredients) {
  return normalizeInstructionsDetail(meal, title, ingredients).lines;
}

/**
 * Postup + informace, jestli je skutečný, nebo jen obecná náhrada.
 *
 * Rozdíl je podstatný: obecný návod se nesmí počítat jako „recept máme lokálně“.
 * Kvůli tomu se totiž nikdy nesáhlo do katalogu, kde konkrétní postup leží —
 * plán si nese snímek receptu z doby svého vzniku a ten postup mít nemusí.
 */
function normalizeInstructionsDetail(meal, title, ingredients) {
  const zdroje = [
    meal?.simple_instructions_cs,
    meal?.recipe?.instructions_cs,
    meal?.recipe?.instructions,
    meal?.instructions_cs,
    meal?.instructions,
  ];
  /* Uložený postup vyhrává — ale jen když není kostrbatý.
     `coach_seed_v1` má recepty o dvou krocích („Uvař vejce natvrdo. /
     Podávej s pečivem a zeleninou.“) a ty dosud blokovaly kurátorovanou
     šestikrokovou verzi z knihovny, protože „neprázdný“ stačilo. */
  let ulozene = [];
  for (const zdroj of zdroje) {
    const radky = instrukceNaRadky(zdroj);
    if (radky.length) { ulozene = radky; break; }
  }
  /* KNIHOVNA A VATA NEJSOU TOTÉŽ. `buildSimpleFallbackInstructions` vrací
     buď kurátorovaný recept ze `simple_start`, nebo obecné „Připrav suroviny
     podle seznamu“. Jako soupeře uloženého postupu smí vystupovat jen ta
     kurátorovaná část — jinak by obecná vata přebila skutečné dva kroky. */
  const knihovna = findSimpleStartRecipeByTitle(title, meal?.type)?.instructions || [];
  const vybrano = vyberPostup({
    ulozene,
    knihovna,
    fallback: buildSimpleFallbackInstructions(title, ingredients),
  });
  return { lines: vybrano.kroky, generated: vybrano.zdroj !== 'ulozeny' };
}
...
export function createMealDisplayModel(meal, appBaseUrl = '') {
  ...
  const title = (meal?.catalog_id != null && String(meal.catalog_id).trim() !== ''
    ? (asString(meal?.recipe?.title_cs) || asString(meal.display_name_cs) || asString(meal.display_name) || asString(meal.name_cs))
    : (asString(meal.display_name_cs) || asString(meal.display_name) || asString(meal.name_cs)))
    || 'Jídlo';
  ...
  const instrukce = normalizeInstructionsDetail(ingredientSourceMeal, title, ingredients);
  const instructions = instrukce.lines;
  ...
  // ŠABLONA SMÍ DOPLNIT, NIKDY PŘEPSAT.
  //
  // Do 20. 8. 2026 měla šablona přednost, takže recept, který neprošel
  // heuristikou jednoduchosti, dostal cizí suroviny — a nechal si vlastní
  // název. Uživateli se pod „Rychlá avokádová pomazánka“ (katalog 866:
  // avokádo, toast, citron) zobrazil tvaroh, vločky a banán z generické
  // snídaňové šablony. Nešlo o špatné čtení plánu; plán i katalog byly
  // v pořádku, lhalo až tohle dosazení.
  //
  // Skutečné suroviny proto vyhrávají vždycky. Šablona nastupuje jen tam, kde
  // recept nemá ingredience žádné — tam je obecný návrh lepší než prázdno.
  const fallbackIngredients = ingredients.length
    ? ingredients
    : (fallbackTemplate?.shopping_ingredient_lines || []).map((line) => asString(line)).filter(Boolean);
  // Totéž pravidlo jako u surovin: skutečný postup vyhrává nad generickým.
  // Vlastní recepty postup mají (viz migrace 20260820120000), takže dosazovat
  // „Připrav suroviny podle seznamu“ by zahodilo konkrétní návod, který
  // v databázi je.
  const fallbackInstructions = instructions.length
    ? instructions
    : buildSimpleFallbackInstructions(title, fallbackIngredients);
  ...
  return {
    ...
    /** true = postup je jen obecná náhrada, ne návod k tomuhle jídlu. */
    instructionsGenerated: instrukce.generated,
    /**
     * ID receptu v katalogu ze VSTUPNÍHO jídla.
     * `normalizedMeal` ho u fallbacku ztrácí, takže by se podle něj nedalo
     * poznat, že je kam sáhnout pro chybějící postup.
     */
    catalogId: meal?.catalog_id != null && String(meal.catalog_id).trim() !== ''
      ? Number(meal.catalog_id) || null
      : null,
    recipeUrl,
