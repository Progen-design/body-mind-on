...
function fallbackSourceFromMeal(meal) {
  ...
  if (
    (meal?.catalog_source === 'meal_cache' || meal?.catalog_source === 'catalog' || meal?.catalog_id != null)
    && !findSimpleStartRecipeByTitle(title, meal?.type)
  ) {
    return 'simple_start_fallback';
  }
  return null;
...
function librarySourceFromMeal(meal) {
  ...
  const title = asString(meal?.display_name_cs) || asString(meal?.display_name) || asString(meal?.name_cs);
  const shouldTryLibrary = meal?.planner_source === 'simple_meal_planner_agent'
    || meal?.catalog_source === 'meal_cache'
    || meal?.catalog_source === 'catalog'
    || meal?.catalog_id != null
    || meal?.recipe_id != null;
  if (shouldTryLibrary && findSimpleStartRecipeByTitle(title, meal?.type)) {
    return 'simple_start_library';
  }
  return null;
}
...
export function createMealDisplayModel(meal, appBaseUrl = '') {
  ...
  const nutrition = normalizeNutrition(ingredientSourceMeal, isFallback || isLibrary);
