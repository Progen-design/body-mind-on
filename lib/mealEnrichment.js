 ...
 *   image_url: vždy z prvního výsledku prvního neprázdného complexSearch (results[0]), jinak z nejlepšího
 *     skórovaného receptu — bez ohledu na confidence (exact vs illustrative dle threshold).
...
export async function searchMealMetadata(mealName, mealKey = null, opts = {}) {
  ...
  // `name` = interní label pro shodu (ne UI); display_name_cs z AI. recipe_id vždy pro dedup obrázků v plánu.
  const result = {
    ...
    recipe_id: recipeId != null && Number.isFinite(recipeId) ? recipeId : null,
