 ...
 * Uses buildMealSearchCandidates for fallback query chain — includes Czech and English
 * translation candidates, giving Spoonacular a much better chance of a confident match.
 */
export async function searchMealMetadata(mealName) {
  if (!mealName || typeof mealName !== 'string') return EMPTY_MEAL(mealName);

  const hints = buildMealSearchCandidates(mealName);
