 ...
 */
async function callSpoonacular(query, timeoutMs = API_TIMEOUT_MS) {
  if (!SPOONACULAR_KEY) return [];

  const n = SPOONACULAR_SHORTLIST_SIZE;
  const url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${encodeURIComponent(SPOONACULAR_KEY)}&query=${encodeURIComponent(query)}&number=${n}&addRecipeInformation=true&addRecipeNutrition=true`;
 ...
 * @param {{ maxCandidates?: number, timeoutMs?: number }} [opts] - maxCandidates: limit pro rychlejší registraci; timeoutMs: kratší timeout pro fastMode
 */
export async function searchMealMetadata(mealName, mealKey = null, opts = {}) {
  ...
  let candidates = keyNorm.length >= 3
  ...
  if (maxC != null && maxC > 0 && candidates.length > maxC) candidates = candidates.slice(0, maxC);
  ...
  for (const candidate of candidates) {
    ...
    const recipes = await callSpoonacular(candidate, timeoutMs);
  ...
  return {
    ...
    // Internal fields: used by enrichMeal / resolveMealForPlan, stripped before cache/public output.
    _bestCandidate: best.candidate,
    _recipe: isExact && best.recipe ? best.recipe : null,
    ...nutrition,
  };
}
 ...
 *   1. When recipe_id provided: try getCachedMealByRecipeId first
 *   2. Check meal_metadata_cache (key = meal_key or normalized mealName)
 *   3. Spoonacular → score → if confident: exact, else: metadata only
 *
 ...
 * @param {number|null} recipeId Optional recipe_id from data-recipe-id (skip search when cached).
 * @returns {Promise<{...}>}
 */
export async function enrichMeal(mealName, mealKey = null, recipeId = null) {
  // 1. When we have recipe_id from HTML (structured plan), try cache by recipe_id first
  if (recipeId != null && Number.isFinite(recipeId)) {
    const byRecipe = await getCachedMealByRecipeId(recipeId);
    if (byRecipe && byRecipe.name) {
      return {
        ...EMPTY_MEAL(mealName),
        ...byRecipe,
      };
  ...
  // 2. Cache hit (trust-level-aware TTL)
  ...
  // 3. Spoonacular with dual-score confidence evaluation (meal_key as first candidate when provided)
