 ...
 *
 * EARLY EXIT (query candidates):
 *   After each search candidate's shortlist is scored, we stop trying further
 *   candidates only when best score ≥ max(MEAL_CONFIDENCE_THRESHOLD, 0.48), so weak
 *   first queries still try paraphrases; strong matches skip extra calls.
 *
...
/** Předčasný konec procházení dalších textových kandidátů — až když už máme rozumnou shodu (šetří volání bez přeskakování slabých prvních dotazů). */
const CANDIDATE_LOOP_EXIT_MIN_SCORE = Math.max(CONFIDENCE_THRESHOLD, 0.48);
...
/** Default shortlist size (overriden per opts); odpovídá complexSearch `number`. */
...
async function callSpoonacular(query, timeoutMs = API_TIMEOUT_MS, ctx = null) {
  ...
  if (!spoonacularLiveOutboundEnabled(ctx?.allowLiveSpoonacular)) {
    return [];
  }
  ...
  const fetchOnce = async (paramBase) => {
    const qs = buildComplexSearchQueryString(paramBase, SPOONACULAR_KEY);
    ...
    return normalizeComplexSearchResults(data);
  };

  try {
    let results = await fetchOnce(base);
    if (ctx?.compressSpoonacular) {
      if (candidateCache instanceof Map) candidateCache.set(ckey, results);
      return Array.isArray(results) ? results : [];
    }
    const hadMinCarbs = base.minCarbs != null && String(base.minCarbs).trim() !== '';
    if ((!Array.isArray(results) || results.length === 0) && hadMinCarbs) {
      const relaxed = { ...base, minCarbs: undefined };
      const retry = await fetchOnce(relaxed);
      if (Array.isArray(retry) && retry.length) results = retry;
    }
    const hadCalWindow =
      base.minCalories != null &&
      base.maxCalories != null &&
      Number.isFinite(Number(base.minCalories)) &&
      Number.isFinite(Number(base.maxCalories));
    if ((!Array.isArray(results) || results.length === 0) && hadCalWindow) {
      const loosing = {
        ...base,
        minCarbs: undefined,
        minCalories: Math.max(120, Math.round(Number(base.minCalories) * 0.72)),
        maxCalories: Math.min(2000, Math.round(Number(base.maxCalories) * 1.22)),
      };
      const wide = await fetchOnce(loosing);
      if (Array.isArray(wide) && wide.length) results = wide;
    }
    const tightProtein =
      base.minProtein != null &&
      String(base.minProtein).trim() !== '' &&
      String(base.minProtein) !== '5';
    if ((!Array.isArray(results) || results.length === 0) && tightProtein) {
      const lowP = {
        ...base,
        minCarbs: undefined,
        minProtein: '5',
        minCalories:
          base.minCalories != null ? Math.max(120, Math.round(Number(base.minCalories) * 0.65)) : base.minCalories,
        maxCalories:
          base.maxCalories != null ? Math.min(2000, Math.round(Number(base.maxCalories) * 1.28)) : base.maxCalories,
      };
      const pr = await fetchOnce(lowP);
      if (Array.isArray(pr) && pr.length) results = pr;
    }

    const r0 = Array.isArray(results) && results[0] && typeof results[0] === 'object' ? results[0] : null;
    if (process.env.SPOONACULAR_DEBUG === '1') {
...
async function rescueRecipeIdFromAltOrStatic(
  ...
  httpStats,
  nameCsHint,
  allowLiveSpoonacular,
  compressSpoonacular = false
) {
  ...
  if (compressSpoonacular || !allowLiveSpoonacular) return null;

  const callCtx = sc
    ? { ...sc, number: Math.max(shortlistN, 5), candidateCache: null, httpStats, allowLiveSpoonacular, compressSpoonacular }
    : {
...
function extractRecipeCaloriesApprox(recipe) {
  const nutrients = recipe?.nutrition?.nutrients ?? [];
  for (const n of nutrients) {
    if (String(n?.name || '').toLowerCase() === 'calories' && n.amount != null) {
...
function chooseBestMealRecipe(evaluations) {
  ...
  const sorted = [...evaluations].sort((a, b) => {
    ...
    const calA = extractRecipeCaloriesApprox(a.recipe);
    const calB = extractRecipeCaloriesApprox(b.recipe);
    if (calA != null && calB != null && calA !== calB) return calB - calA;
 ...
 */
async function fetchSpoonacularRecipeInformation(recipeId, timeoutMs, httpStats, allowLiveSpoonacular = false) {
  if (!spoonacularLiveOutboundEnabled(allowLiveSpoonacular)) return null;
 ...
 *   allowLiveSpoonacular?: boolean — true pouze z trainer úkolu `initial_plan` (registrace); jinak žádné placené HTTP v režimu plan-only
 * }} [opts]
 */
export async function searchMealMetadata(mealName, mealKey = null, opts = {}) {
  ...
  const shortlistN = opts?.shortlistSize ?? SPOONACULAR_SHORTLIST_SIZE;

  const httpStats = { count: 0 };
  const candidateCache = opts?.candidateCache;
  const allowLiveFlag = opts?.allowLiveSpoonacular === true;
  const compressFlag = opts?.compressSpoonacular === true;
  ...
  for (const candidate of candidates) {
    ...
    const callCtx = sc
      ? {
          ...
          candidateCache,
          httpStats,
          allowLiveSpoonacular: allowLiveFlag,
          compressSpoonacular: compressFlag,
        }
      : {
          number: shortlistN,
          mealType: 'lunch',
    ...
    if (bestSoFar >= CANDIDATE_LOOP_EXIT_MIN_SCORE) break;
  }
  ...
  if (!compressFlag && excludeActive && !best && candidates.length) {
  ...
  if (!best && allEvaluations.length) {
    ...
    // Pokud byl exclude aktivní a fallback by vrátil zakázané ID, ponecháme best=null a
    // pokračujeme rescue/empty cestou. Tím zabráníme tomu, aby refetch vracel ten samý duplikát.
    if (best && excludeActive) {
      const bestId = best.recipe?.id != null ? Number(best.recipe.id) : NaN;
      const isExcluded =
        Number.isFinite(bestId) &&
        (excludeIds instanceof Set
          ? excludeIds.has(bestId)
          : Array.isArray(excludeIds) && excludeIds.map((x) => Number(x)).includes(bestId));
      if (isExcluded) best = null;
    }
  }
  ...
  if (!best) {
    ...
    const rescuedIdNum = rescued?.id != null ? Number(rescued.id) : null;
    const excludeBlocksRescue =
      excludeActive &&
      rescuedIdNum != null &&
      (excludeIds instanceof Set
        ? excludeIds.has(rescuedIdNum)
        : Array.isArray(excludeIds) && excludeIds.map((x) => Number(x)).includes(rescuedIdNum));
    if (rescued?.id && !excludeBlocksRescue) {
      const image_url =
      ...
      let nut = { calories: null, protein_g: null, carbs_g: null, fat_g: null };
      if (!compressFlag) {
        const fullRec = await fetchSpoonacularRecipeInformation(Number(rescued.id), timeoutMs, httpStats, allowLiveFlag);
        if (fullRec) nut = extractNutrition(fullRec);
      }
      const synthetic = {
        ...EMPTY_MEAL(mealName),
        name: mealName,
  ...
  const needDetail =
    ...
    (recipeNeedsSpoonacularInformation(best.recipe) ||
      !scoreAccepted ||
      extractNutrition(best.recipe).calories == null);
  if (needDetail) {
    const full = await fetchSpoonacularRecipeInformation(recipeId, timeoutMs, httpStats, allowLiveFlag);
    if (full) outputRecipe = full;
  }
  if (!compressFlag && recipeId != null && Number.isFinite(recipeId) && !outputRecipe?.image) {
    const full = await fetchSpoonacularRecipeInformation(recipeId, timeoutMs, httpStats, allowLiveFlag);
    if (full) outputRecipe = full;
  }

  const nutrition = extractNutrition(outputRecipe);
  /** První hit z complexSearch — jen náhled obrázku; recipe_id musí odpovídat vybranému `_recipe` (odkaz / nákup). */
  ...
  const outputRecipeId = outputRecipe?.id != null ? Number(outputRecipe.id) : NaN;
  const recipeIdForMeta = Number.isFinite(outputRecipeId)
    ? outputRecipeId
    : recipeId != null && Number.isFinite(Number(recipeId))
      ? Number(recipeId)
      : Number.isFinite(leadRecipeId)
        ? leadRecipeId
        : null;
  ...
  // `name` = interní label pro shodu (ne UI); display_name_cs z AI. recipe_id = vybraný recept (shoda s `_recipe`).
  const result = {
...
export async function enrichMeal(mealName, mealKey = null, recipeId = null) {
  // POZASTAVENO: žádné živé Spoonacular mimo registraci (allowLiveSpoonacular v searchMealMetadata)
  const meta = await searchMealMetadata(mealName, mealKey, { allowLiveSpoonacular: false });
