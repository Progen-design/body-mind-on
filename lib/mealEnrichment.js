...
export async function searchMealMetadata(mealName, mealKey = null, opts = {}) {
  ...
  if (scoreAccepted && recipeId && recipeNeedsSpoonacularInformation(best.recipe)) {
  ...
  const isImageExact = scoreAccepted && hasImage;

  // `name` = interní label pro shodu (ne UI); display_name_cs v planOrchestratorResolve z přeloženého titulu receptu při ověření.
  const result = {
    name: outputRecipe.title || mealName,
    image_url: isImageExact ? (outputRecipe.image || null) : null,
    source: 'spoonacular',
