...
const MEAL_STATIC_SPOONACULAR_IDS = {
...
/**
 * Poslední záchrana: alternativní complexSearch (bez sdílené candidate cache) + statická ID mapa.
...
async function rescueRecipeIdFromAltOrStatic(
  ...
  const fid = MEAL_STATIC_SPOONACULAR_IDS[query] || MEAL_STATIC_SPOONACULAR_IDS[query.split(/\s+/)[0]];
  if (fid != null && Number.isFinite(Number(fid))) {
    return { id: Number(fid), leadRecipe: null };
  }
  return null;
}
