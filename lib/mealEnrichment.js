...
async function rescueRecipeIdFromAltOrStatic(
  ...
  const callCtx = sc
    ? { ...sc, number: Math.max(shortlistN, 5), candidateCache: null, httpStats, allowLiveSpoonacular }
    : {
...
export async function searchMealMetadata(mealName, mealKey = null, opts = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const callCtx = sc
      ? { ...sc, number: shortlistN, candidateCache, httpStats, allowLiveSpoonacular: allowLiveFlag }
      : {
          number: shortlistN,
          mealType: 'lunch',
          diet: 'standard',
  ...
  if (excludeActive && !best && candidates.length) {
    ...
    for (let page = 1; page <= 4; page++) {
      ...
      const callCtx = sc
        ? { ...sc, number: shortlistN, candidateCache, httpStats, offset: off, allowLiveSpoonacular: allowLiveFlag }
        : {
            number: shortlistN,
            mealType: 'lunch',
            diet: 'standard',
  ...
  if (recipeId != null && Number.isFinite(recipeId) && !outputRecipe?.image) {
    const full = await fetchSpoonacularRecipeInformation(recipeId, timeoutMs, httpStats, allowLiveFlag);
    if (full) outputRecipe = full;
  }
