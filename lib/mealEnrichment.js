...
/** Default shortlist size (overriden per opts / fastMode); odpovídá complexSearch `number`. Nižší = méně bodů za výsledek (0.01 + add-ons / result). */
const SPOONACULAR_SHORTLIST_SIZE = 2;
...
async function rescueRecipeIdFromAltOrStatic(
  ...
  const callCtx = sc
    ? { ...sc, number: Math.max(shortlistN, 3), candidateCache: null, httpStats }
    : {
        number: Math.max(shortlistN, 3),
        mealType: 'lunch',
        diet: 'standard',
        caloriesPerDay: 2000,
        mealsPerDay: 3,
