...
import { scoreMealMatch, normalizeMealQueryCs, buildMealSearchCandidates, removeDiacritics, translateMealQueryToEn } from './mealNormalization';
...
const GOOD_FOOD_HINTS = [
  ...
  'yogurt', 'nuts', 'honey', 'quinoa', 'oatmeal', 'oat', 'fish', 'seafood',
];
/** If meal name implies fish, Pexels image must not contain these (no fake match). */
const FISH_MEAL_HINTS = ['ryba', 'ryby', 'losos', 'treska', 'tunak', 'tuňák', 'fish', 'salmon', 'cod', 'tuna', 'seafood', 'mořské'];
const BURGER_FRIES_IMAGE_HINTS = ['burger', 'hamburger', 'fries', 'french fries', 'hranolky', 'bramburky'];

/** Query-type → forbidden image alt. Prevents yogurt/salad/breakfast from matching burger/pasta. */
const PEXELS_QUERY_MISMATCH = [
  { queryHints: ['jogurt', 'yogurt', 'tvaroh', 'cottage', 'ovesna', 'oatmeal', 'snidane', 'breakfast', 'müsli', 'muesli', 'vlocky', 'oat'], forbiddenAlt: ['burger', 'hamburger', 'pizza', 'fries', 'sandwich', 'hotdog', 'pasta', 'spaghetti', 'fusilli', 'noodle', 'nudle'] },
  { queryHints: ['houska', 'bun', 'bread', 'roll', 'celozrn', 'whole grain'], forbiddenAlt: ['pasta', 'spaghetti', 'fusilli', 'noodle', 'nudle'] },
  { queryHints: ['salat', 'salad'], forbiddenAlt: ['burger', 'hamburger', 'pizza', 'pasta'] },
  { queryHints: ['hovezi', 'beef', 'posirovane', 'poached', 'dusene', 'braised', 'steak'], forbiddenAlt: ['burger', 'hamburger'] },
  { queryHints: ['kruta', 'turkey', 'dusena', 'stewed'], forbiddenAlt: ['breaded', 'fried', 'cutlet', 'řízek'] },
];
...
export async function searchMealImageFallback(mealName, bestQuery = null) {
  ...
  // Prefer English for Pexels – better search relevance than Czech.
  // When Spoonacular returns nothing, use translated Czech name.
  const cleanMealName = normalizeMealQueryCs(mealName);
  const enFallback = translateMealQueryToEn(cleanMealName);
  const pexelsBase = (bestQuery && bestQuery.length >= 4 && bestQuery !== cleanMealName)
    ? bestQuery.slice(0, 50)
    : (enFallback || cleanMealName).slice(0, 45);

  const query = `${pexelsBase} food`;

  // Build scoring tokens from English query, Czech name, and translated terms.
  const enTokens = (bestQuery || enFallback || '')
    .toLowerCase()
    .replace(/[^a-z0-9 ]/g, ' ')
    .split(/\s+/)
    .filter((t) => t.length > 2);
  ...
  // Key ingredients = first 3 significant words (must match in alt for acceptance)
  const keyIngredients = allQueryTokens.slice(0, 3).filter((t) => t.length > 2);
  ...
  try {
    ...
    const mealNorm = removeDiacritics(cleanMealName).toLowerCase();
    const mealIsFish = FISH_MEAL_HINTS.some((h) => mealNorm.includes(h));
    const scorePhoto = (photo) => {
      const alt = String(photo?.alt || '').toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
      if (!alt) return -10;
      if (mealIsFish && BURGER_FRIES_IMAGE_HINTS.some((h) => alt.includes(h))) return -100;
      // Query–image mismatch: yogurt/salad/breakfast must not match burger
      for (const rule of PEXELS_QUERY_MISMATCH) {
        const queryHasHint = rule.queryHints.some((h) => mealNorm.includes(h) || (enFallback && enFallback.toLowerCase().includes(h)));
        const altHasForbidden = rule.forbiddenAlt.some((f) => alt.includes(f));
        if (queryHasHint && altHasForbidden) return -100;
      }
      let score = 0;
    ...
    // Require at least one key ingredient in alt (avoids generic "food" match)
    const best = ranked.find((r) => {
      const alt = String(r.photo?.alt || '').toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
      const hasKeyIngredient = keyIngredients.length === 0 || keyIngredients.some((k) => alt.includes(k));
      return hasKeyIngredient && r.score >= 4;
    }) || ranked[0];
    if (!best || best.score < 4) return PEXELS_EMPTY;
