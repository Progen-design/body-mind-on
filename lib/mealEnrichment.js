...
import { scoreMealMatch, normalizeMealQueryCs, buildMealSearchCandidates, removeDiacritics } from './mealNormalization.js';
import { buildComplexSearchQueryString } from './spoonacularComplexSearch.js';
import { shortenMealSearchQuery } from './mealQueryShorten.js';
import { lookupStaticSpoonacularRecipeId } from './mealStaticRecipeIds.js';
import { spoonacularLiveOutboundEnabled, assertSpoonacularLiveAllowed } from './spoonacularQuotaGate.js';
import { fetchSpoonacularWithRetry, throwIfSpoonacularResponseNotOk } from './spoonacularHttp.js';
import { translateAisle } from './spoonacularShopping.js';
...
async function callSpoonacular(query, timeoutMs = API_TIMEOUT_MS, ctx = null) {
  ...
  if (!spoonacularLiveOutboundEnabled(ctx?.allowLiveSpoonacular)) {
    assertSpoonacularLiveAllowed(ctx?.allowLiveSpoonacular);
    return [];
  }
  ...
  try {
    const res = await fetchSpoonacularWithRetry(
      () => fetchWithTimeout(url, { method: 'GET', headers: { Accept: 'application/json' } }, timeoutMs),
...
async function fetchSpoonacularRecipeInformation(recipeId, timeoutMs, httpStats, allowLiveSpoonacular = false) {
  if (!spoonacularLiveOutboundEnabled(allowLiveSpoonacular)) {
    assertSpoonacularLiveAllowed(allowLiveSpoonacular);
    return null;
  }
...
export { shortenMealSearchQuery, shortenSpoonacularQuery } from './mealQueryShorten.js';
