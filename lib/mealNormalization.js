 ...
 * Extends buildMealSearchHints with English translation candidates,
 * giving Spoonacular more chances to find a high-confidence match.
 *
 * Order (most → least specific):
 *   1. Original normalized Czech query
 *   2. Simplified Czech (before " s " / " se " connector)
 *   3. Full English translation
 *   4. Simplified English translation
 *   5. First 3 words (Czech)
 *   6. First word → Czech + English
 *
...
export function buildMealSearchCandidates(mealName) {
  ...
  const add = (s) => {
    ...
    if (trimmed && trimmed.length >= 4 && !seen.has(trimmed)) {
      seen.add(trimmed);
      candidates.push(trimmed);
    }
  };

  // 1. Full normalized Czech query
  add(base);
  ...
  // 4. Simplified English translation
  if (beforeConnector !== base) {
    add(translateMealQueryToEn(beforeConnector));
  }
  ...
  // 6. First word — Czech and English
  if (words.length > 1) {
    add(words[0]);
    add(translateMealQueryToEn(words[0]));
  }
