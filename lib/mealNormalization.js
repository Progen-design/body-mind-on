...
const CS_EN_RAW = [
  ...
  ['celozrna houska', 'whole grain bread'],
  ['houska', 'bread'],
  ['zelenina', 'vegetables'],
  ['brokolice', 'broccoli'],
  ['paprika', 'bell pepper'],
...
/** Recipe title tokens that, if present in recipe but not in query, suggest wrong match (e.g. chicken rice → chicken burger). */
const MISMATCH_DISH_HINTS = ['burger', 'pizza', 'sandwich', 'hotdog', 'taco', 'wrap', 'bagel', 'donut', 'cake', 'cookie', 'brownie', 'ice cream', 'waffle', 'fries', 'french fries', 'nugget'];

/** Query-type → forbidden recipe tokens. Fish query must never match burger/pizza etc. */
const QUERY_RECIPE_MISMATCH = [
  { queryHints: ['ryba', 'ryby', 'losos', 'treska', 'tunak', 'fish', 'salmon', 'cod', 'tuna', 'seafood'], forbiddenRecipe: ['burger', 'pizza', 'sandwich', 'steak', 'nugget', 'hotdog'] },
  { queryHints: ['chicken', 'kureci', 'kruti', 'drubez'], forbiddenRecipe: ['burger', 'pizza'] },
  { queryHints: ['polevka', 'soup'], forbiddenRecipe: ['steak', 'burger', 'pizza'] },
  { queryHints: ['salat', 'salad'], forbiddenRecipe: ['burger', 'pizza'] },
  { queryHints: ['snidane', 'breakfast'], forbiddenRecipe: ['burger', 'pizza', 'steak'] },
];
 ...
 * Stricter matching: avoids "Kuřecí prsa s rýží" matching a burger/pizza.
 */
export function scoreMealMatch(mealName, recipe) {
  ...
  // Penalty: recipe is a distinct dish type not suggested by query (no fake exact)
  const titleWords = new Set(titleNorm.split(/\s+/).filter((w) => w.length > 2));
  for (const hint of MISMATCH_DISH_HINTS) {
    if (titleWords.has(hint) && !inputNorm.includes(hint)) return 0;
  }

  // Category mismatch: fish query must never match burger; soup never steak; etc.
  for (const rule of QUERY_RECIPE_MISMATCH) {
    const queryHasHint = rule.queryHints.some((h) => inputNorm.includes(h));
    const recipeHasForbidden = rule.forbiddenRecipe.some((f) => titleNorm.includes(f));
    if (queryHasHint && recipeHasForbidden) return 0;
  }
