...
import { buildNutritionHtml, escapeHtml, recipePartsToHtml } from './recipeDetailHtml.js';
...
function macroNutrientsFromDisplayModel(model) {
  ...
  if (model?.calories != null) {
    nutrients.push({ name: 'Calories', amount: model.calories, unit: 'kcal', percentOfDailyNeeds: null });
  }
  if (model?.protein_g != null) {
    nutrients.push({ name: 'Protein', amount: model.protein_g, unit: 'g', percentOfDailyNeeds: null });
  }
  if (model?.carbs_g != null) {
    nutrients.push({ name: 'Carbohydrates', amount: model.carbs_g, unit: 'g', percentOfDailyNeeds: null });
  }
  if (model?.fat_g != null) {
    nutrients.push({ name: 'Fat', amount: model.fat_g, unit: 'g', percentOfDailyNeeds: null });
  }
  if (model?.fiber_g != null) {
    nutrients.push({ name: 'Fiber', amount: model.fiber_g, unit: 'g', percentOfDailyNeeds: null });
  }
  return nutrients;
}
...
export function buildMealRecipeModalHtml(model, options = {}) {
  ...
  const nutritionHtml = buildNutritionHtml(macroNutrientsFromDisplayModel(model));
