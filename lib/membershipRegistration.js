 ...
 * @param {number} [trialDni=7] Délka trialu START. Poukaz (lib/poukazy.js)
 *   sem pošle `vouchers.discount_days` (30) místo výchozích sedmi.
 * @returns {{ tier: string, status: string, trial_ends_at: string|null, started_at: string }}
 */
export function membershipFromRegistration(program, startedAt = new Date().toISOString(), trialDni = TRIAL_DNI) {
  const normalized = String(program || 'START').toUpperCase();
  const tier = normalized === 'ON_CLUB' || normalized === 'VIP' ? normalized : 'START';

  if (tier === 'START') {
    const dni = Number.isInteger(trialDni) && trialDni >= 1 && trialDni <= 365 ? trialDni : TRIAL_DNI;
    const konec = new Date(new Date(startedAt).getTime() + dni * 86_400_000).toISOString();
