...

{
  ...
  assert(roundCookableAmount(3.45, 'ks', 'vejce') === 3, `egg ${roundCookableAmount(3.45, 'ks', 'vejce')}`);
  ...
  assert(roundCookableAmount(1.4, 'ks', 'banán') === 1, `banana ${roundCookableAmount(1.4, 'ks', 'banán')}`);
...

{
  ...
  const halfEgg = validateDiscreteIngredientAmount({ name: 'vejce', unit: 'ks', amount: 3.5 });
  assert(halfEgg.ok === false, '3.5 eggs must fail (whole only)');
  const good = validateDiscreteIngredientAmount({ name: 'vejce', unit: 'ks', amount: 3 });
  assert(good.ok === true, '3 eggs ok');
  const can = validateDiscreteIngredientAmount({ name: 'tuňák', unit: 'konzerva', amount: 1.15 });
  assert(can.ok === false, '1.15 can must fail');
  const bread = validateDiscreteIngredientAmount({ name: 'chléb', unit: 'plátky', amount: 2.5 });
  assert(bread.ok === true, '2.5 bread ok');
...

{
  ...
  assert(!egg.includes('3.45') && !egg.includes('3,45') && !egg.includes('3.5') && !egg.includes('3,5'), `egg line ${egg}`);
  assert(/vejce\s+3\s|vejce\s+4\s/i.test(egg) || egg.includes('3') || egg.includes('4'), `egg cookable ${egg}`);
...

{
  ...
  assert(Number.isInteger(Number(bundle.ingredients[0].amount)), 'egg whole only');
...
{
  // Protein preferred over rice when both flexible
  const list = [
    { name: 'rýže', unit: 'g', amount: 80 },
    { name: 'kuřecí prsa', unit: 'g', amount: 120 },
  ];
  const r = catchUpFlexibleIngredients(list, 80);
  assert(r.list[1].amount > 120, `chicken should grow first, got ${r.list[1].amount}`);
  console.log('OK protein-first catch-up', r.list.map((x) => `${x.name}:${x.amount}`).join(', '));
}

{
  ...
  assert(scaled.portion_multiplier >= 0.85 && scaled.portion_multiplier <= 1.35, `mult ${scaled.portion_multiplier}`);
