 ...
 *
 * INTEGRAČNÍ test — sahá na produkční Supabase a na aktivní plán konkrétního
 * uživatele. Nepatří do `npm run test:unit` ani do CI; pouští se přes
 * `npm run test:integration` s vyplněnými klíči v .env.local.
 *
 * Bez klíčů se SKIPNE (node:test skip), ne aby spadl — dřív dělal process.exit(1),
 * což v CI shodilo celý job.
 *
 *   npm run test:integration
 *   node --test lib/nutrition/__tests__/shoppingListAggregate.test.mjs
 */
...
import { resolveCanonicalName, formatDisplayUnit } from '../../ingredientNormalize.js';
...
const credentialsMissing = !url || !key;
...
function findDayWithDuplicateZelenina(days) {
...
/** Čistá jednotková kontrola skloňování — nepotřebuje DB, běží vždy. */
test('formatDisplayUnit skloňuje jednotky', () => {
  assert.equal(formatDisplayUnit(1, 'plátek'), 'plátek', '1 plátek');
  assert.equal(formatDisplayUnit(3, 'plátek'), 'plátky', '3 plátky');
  assert.equal(formatDisplayUnit(31, 'plátek'), 'plátků', '31 plátků');
  assert.equal(formatDisplayUnit(21, 'konzerva'), 'konzerv', '21 konzerv');
  assert.equal(formatDisplayUnit(2.5, 'plátek'), 'plátku', '2,5 plátku');
});

test('nákupní seznam se agreguje proti reálnému aktivnímu plánu', {
  skip: credentialsMissing
    ? 'Chybí SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY — integrační test, běží jen lokálně'
    : false,
}, async () => {
  ...
  assert.ok(
    !planErr && planRow?.structured_plan_json?.days?.length,
    `Aktivní plán pro ${emailArg} nenalezen: ${planErr?.message || 'žádný řádek'}`,
  );
  ...
  const hit = findDayWithDuplicateZelenina(days);
  ...
  const weekMeals = allWeekMeals(days);
  ...
  const tunakLine = afterLines.find((l) => /tuňák \(v konzervě\)|tunak \(v konzerve\)/i.test(l));
  assert.match(tunakLine || '', /3 konzervy\b/, `Expected "tuňák 3 konzervy", got: ${tunakLine || 'missing'}`);

  const syrLine = afterLines.find((l) => /^sýr\b|^syr\b/i.test(l));
  assert.match(syrLine || '', /3 plátky\b/, `Expected "sýr 3 plátky", got: ${syrLine || 'missing'}`);

  const chlebLine = afterLines.find((l) => /celozrnn/i.test(l));
  assert.ok(chlebLine && / plátků/.test(chlebLine),
    `Expected "celozrnný chléb N plátků", got: ${chlebLine || 'missing'}`);
