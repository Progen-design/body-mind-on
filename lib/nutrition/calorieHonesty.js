...
/** Soft UX cap — prefer denser meals over 7–8 eating occasions. */
...
export function maxExtraMealsForTarget(dailyTarget, currentMealCount = 0) {
  ...
  const room = Math.max(0, MAX_MEALS_PER_DAY_CAP - current);
  // Prefer 1–2 dense extras, never blow past the soft cap
  let want = 1;
  if (t >= 3000) want = 2;
  else if (t >= 2400) want = 2;
