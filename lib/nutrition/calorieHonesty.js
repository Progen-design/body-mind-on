...
import { CANONICAL_DAY_CALORIE_TOLERANCE } from '../calorieTargetIntegrity.js';
...
/**
 * Hard UX cap — never more than 6 meals/day (7+ is a churn reason).
 * Structure by target (3 mains + snacks):
 *   <1800 → max 4 | 1800–3200 → max 5 | >3200 → max 6
 */
export const MAX_MEALS_PER_DAY_CAP = 6;
/** @deprecated Same as MAX_MEALS_PER_DAY_CAP — kept for older honesty metadata. */
export const MAX_MEALS_PER_DAY_CAP_HIGH = 6;
...
/**
 * Hard meal-count cap by daily calorie target.
 * <1800 → 4 | 1800–3200 → 5 | >3200 → 6 | never >6
 * @param {number} dailyTarget
 */
export function maxMealsForCalorieTarget(dailyTarget) {
...
/**
 * Max snacks (beyond 3 mains) for the target band.
 * @param {number} dailyTarget
 */
export function maxSnacksForCalorieTarget(dailyTarget) {
  return Math.max(0, maxMealsForCalorieTarget(dailyTarget) - 3);
}

/**
 * Extra meals allowed when filling a day deficit — only up to structure cap.
 * Prefer scaling mains first; snacks are last resort within the band.
 * @param {number} dailyTarget
...
export function maxExtraMealsForTarget(dailyTarget, currentMealCount = 0) {
  ...
  return Math.max(0, maxMealsForCalorieTarget(dailyTarget) - current);
}

function isMainMealType(meal) {
  const t = mealTypeToEn(meal?.type || meal?.meal_type);
  return t === 'breakfast' || t === 'lunch' || t === 'dinner';
}

function countSnackMeals(dayMeals) {
  return (dayMeals || []).filter((m) => mealTypeToEn(m?.type || m?.meal_type) === 'snack').length;
}
...
export function balanceDayMealsHonestly(dayMeals, dailyTarget) {
  ...
  // Upscale: grow MAIN meals first within 0.85–1.15, then snacks.
  if (sum < minSum) {
    const ranked = [...dayMeals]
      .filter((m) => Number(m?.kcal) > 0)
      .sort((a, b) => {
        const mainA = isMainMealType(a) ? 0 : 1;
        const mainB = isMainMealType(b) ? 0 : 1;
        return mainA - mainB || Number(b.kcal) - Number(a.kcal);
      });
    let remaining = minSum - sum;
    for (const m of ranked) {
      if (remaining < 8) break;
      const mult = Number(m.portion_multiplier) || 1;
      const before = Number(m.kcal) || 0;
      const base = before / mult;
      const maxKcal = Math.round(base * HONEST_MAX_SCALE);
      const room = maxKcal - before;
      if (room < 8) continue;
      const wantMult = Math.min(HONEST_MAX_SCALE, (before + Math.min(room, remaining)) / base);
      applyPortionScaleToStructuredMeal(m, wantMult, {
        allowUnverified: true,
        simpleStartMode: true,
      });
  ...
  // Downscale: shrink snacks first, then mains — target maxSum.
  if (sum <= maxSum) return dayMeals;

  const rankedDown = [...dayMeals]
    .filter((m) => Number(m?.kcal) > 0)
    .sort((a, b) => {
      const snackA = isMainMealType(a) ? 1 : 0;
      const snackB = isMainMealType(b) ? 1 : 0;
      return snackA - snackB || Number(b.kcal) - Number(a.kcal);
    });
  let excess = sum - maxSum;
  for (const m of rankedDown) {
    if (excess < 8) break;
    const mult = Number(m.portion_multiplier) || 1;
    const before = Number(m.kcal) || 0;
    const base = before / mult;
    const minKcal = Math.round(base * HONEST_MIN_SCALE);
    const room = before - minKcal;
    if (room < 8) continue;
    const wantMult = Math.max(HONEST_MIN_SCALE, (before - Math.min(room, excess)) / base);
    applyPortionScaleToStructuredMeal(m, wantMult, {
      allowUnverified: true,
      simpleStartMode: true,
    });
    excess -= before - (Number(m.kcal) || 0);
  }
  clampDayMealsToHonestScale(dayMeals);
  return dayMeals;
}
...
export function boostDayMealsWithFlexibleCatchUp(dayMeals, dailyTarget, opts = {}) {
  ...
  const preferMains = opts.preferMains === true;
  if (!Number.isFinite(target) || target <= 0 || !dayMeals?.length) return dayMeals;
  ...
  const mains = dayMeals.filter(isMainMealType);
  const primary = preferMains && mains.length ? mains : dayMeals;
  ...
  // If mains-only pass still short and we did not restrict to mains, stop.
  // When preferMains, fall through to snacks only if still short.
  if (preferMains && remaining >= 12) {
    const snacks = dayMeals
      .filter((m) => !isMainMealType(m) && Number(m?.kcal) > 0)
      .sort((a, b) => Number(b.kcal) - Number(a.kcal));
...
export function fillDayCaloriesByAddingLibraryMeals(dayMeals, dailyTarget, opts = {}) {
  ...
  const snackCap = maxSnacksForCalorieTarget(target);
  const exclusions = opts.exclusions || null;
  const catalogCandidates = opts.catalogFillCandidates || null;
  /** @type {((meal: object) => boolean)|null} viz komentář u použití níž */
  const dietFilter = typeof opts.dietFilter === 'function' ? opts.dietFilter : null;

  clampDayMealsToHonestScale(dayMeals);
  // 1) Scale MAIN meals up first (honest 0.85–1.15), then flex staples on mains.
  balanceDayMealsHonestly(dayMeals, target);
  boostDayMealsWithFlexibleCatchUp(dayMeals, target, { preferMains: true });
  // 2) Only then add a snack (unique titles) — never past structure cap / snack budget.
  // 3) If still short → underrun banner with real sum (prefer fewer meals over 7 snacks).
  ...
  while (target > 0 && sum < minGoal && added < maxExtra) {
    if (countSnackMeals(dayMeals) >= snackCap) break;
    if ((dayMeals || []).length >= maxMealsForCalorieTarget(target)) break;
    ...
    // DIETA PLATÍ I NA DOPOČET KALORIÍ.
    //
    // Vyloučené potraviny (`exclusions`) se hlídaly, lepek ne — takže dopočet
    // uměl do bezlepkového dne přisypat „Cottage s pečivem“ z knihovny.
    // Rozhodnutí přichází jako predikát od volajícího: tenhle modul nesmí
    // importovat dietaryPublishGate.js, protože ten importuje jeho.
    if (dietFilter && !dietFilter(meal)) {
      usedTitles.add(normalizeTitle(meal.display_name_cs || meal.name_cs));
      continue;
    }

    // HORNÍ POJISTKA (2026-09-17, produkční nález na 1800 kcal / den 5).
    //
    // Cyklus dřív jel jen na `sum < minGoal` bez horní meze. `pickMealToAdd`
    // s deficitem >= 250 kcal řadí podle NEJVĚTŠÍCH kalorií (`preferDense`),
    // takže při velkém deficitu vybral hustého kandidáta bez ohledu na to,
    // kolik místa v den ještě zbývalo — den pak přestřelil 1,15× strop.
    //
    // Přidané jídlo nesmí dostat den nad `round(target * HONEST_MAX_SCALE)`.
    // Když přestřelí, zkusí se zmenšit v rozsahu HONEST_MIN_SCALE..1,0 (NIKDY
    // nahoru — to by bylo vymyšlení kalorií navíc). Když se ani zmenšené
    // nevejde, kandidát se zahodí (`continue` -> `pickMealToAdd` zkusí
    // dalšího, protože titul/ID je teď v `usedTitles`/`usedCatalogIds`).
    // Když se nevejde nikdo, cyklus skončí a den zůstane poctivě pod cílem —
    // přesně to, co `under_target`/shortfall dál v kódu popisuje.
    const maxAllowed = Math.round(target * HONEST_MAX_SCALE);
    const baseKcal = Number(meal.kcal) || 0;
    if (baseKcal > 0 && sum + baseKcal > maxAllowed) {
      const maxScale = (maxAllowed - sum) / baseKcal;
      if (maxScale >= HONEST_MIN_SCALE) {
        applyPortionScaleToStructuredMeal(meal, Math.min(1, maxScale), {
          allowUnverified: true,
          simpleStartMode: true,
        });
      } else {
        usedTitles.add(normalizeTitle(meal.display_name_cs || meal.name_cs));
        if (meal.catalog_id != null) usedCatalogIds.add(meal.catalog_id);
        continue;
      }
    }
...
/**
 * Final gate: daily meal sum must stay within ±tolerance of canonical target.
 * Scales portions, drops honesty-added snacks, optionally swaps one meal from catalog.
 * @param {object[]} dayMeals
 * @param {number} dailyTarget
 * @param {{ tolerance?: number, catalogFillCandidates?: object[] }} [opts]
 */
export function enforceDayCalorieBand(dayMeals, dailyTarget, opts = {}) {
  const target = Number(dailyTarget);
  const tolerance = Number(opts.tolerance) || CANONICAL_DAY_CALORIE_TOLERANCE;
  if (!Number.isFinite(target) || target <= 0 || !Array.isArray(dayMeals)) {
    return {
      achieved_kcal: sumScaledDayKcal(dayMeals),
      within_band: true,
      adjusted: false,
    };
  }
  ...
  balanceDayMealsToCalorieTarget(dayMeals, target, tolerance);
  let sum = sumScaledDayKcal(dayMeals);

  while (sum > maxSum) {
    const addedIdx = dayMeals.findIndex((m) => m?.calorie_honesty_added === true);
    if (addedIdx >= 0) {
      dayMeals.splice(addedIdx, 1);
      adjusted = true;
      balanceDayMealsToCalorieTarget(dayMeals, target, tolerance);
      sum = sumScaledDayKcal(dayMeals);
      continue;
  ...
  if (sum > maxSum && Array.isArray(opts.catalogFillCandidates) && opts.catalogFillCandidates.length) {
    const largestIdx = dayMeals.reduce((best, m, i) => {
      const k = Number(m?.kcal) || 0;
      if (!best || k > best.kcal) return { idx: i, kcal: k };
      return best;
    }, null)?.idx;
    if (largestIdx != null && largestIdx >= 0) {
      const meal = dayMeals[largestIdx];
      const typeEn = mealTypeToEn(meal?.type || meal?.meal_type);
      const need = sum - maxSum;
      const smaller = opts.catalogFillCandidates
        .filter((r) => {
          const kcal = Number(r.kcal) || 0;
          const t = mealTypeToEn(r.meal_type || r.type);
          return kcal > 0 && kcal < (Number(meal?.kcal) || 0) - need * 0.5
            && (t === typeEn || t === 'snack' || typeEn === 'snack');
        })
        .sort((a, b) => Math.abs(Number(a.kcal) - (Number(meal?.kcal) - need)) - Math.abs(Number(b.kcal) - (Number(meal?.kcal) - need)));
      if (smaller[0]) {
        try {
          const replacement = buildCatalogFillMeal(
            smaller[0],
            typeEn === 'svacina' ? 'snack' : typeEn,
          );
          if (replacement) {
            dayMeals[largestIdx] = replacement;
            adjusted = true;
            balanceDayMealsToCalorieTarget(dayMeals, target, tolerance);
            sum = sumScaledDayKcal(dayMeals);
          }
  ...
  const withinBand = sum >= minSum && sum <= maxSum;
  const underTarget = sum < minSum;
  return {
    achieved_kcal: Math.round(sum),
    target_kcal: Math.round(target),
    within_band: withinBand,
    under_target: underTarget,
    shortfall_kcal: underTarget ? Math.max(0, Math.round(minSum - sum)) : 0,
    over_target_kcal: sum > maxSum ? Math.round(sum - maxSum) : 0,
    adjusted,
  };
}
