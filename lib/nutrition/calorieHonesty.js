...
export const FLEX_CATCHUP_MAX_SCALE = 1.28;
...
/**
 * Extra meals allowed when filling a day deficit — scales with daily target.
 * @param {number} dailyTarget
 */
export function maxExtraMealsForTarget(dailyTarget) {
 ...
 * @param {number} dailyTarget
 */
export function boostDayMealsWithFlexibleCatchUp(dayMeals, dailyTarget) {
  const target = Number(dailyTarget);
  if (!Number.isFinite(target) || target <= 0 || !dayMeals?.length) return dayMeals;
  ...
  for (const m of dayMeals) {
    ...
    const mult = Number(m.portion_multiplier) || 1;
    const base = Number(m.kcal) / mult;
    const mealTarget = Math.round(base * FLEX_CATCHUP_MAX_SCALE);
  ...
  for (const m of ranked) {
    ...
    const hardCap = Math.round(base * FLEX_CATCHUP_MAX_SCALE);
...
function pickMealToAdd(usedTitles, deficitKcal, catalogCandidates) {
  if (Array.isArray(catalogCandidates) && catalogCandidates.length) {
    const pool = catalogCandidates
      ...
      .sort((a, b) => {
        ...
        const da = Math.abs((Number(a.kcal) || 0) - deficitKcal);
        const db = Math.abs((Number(b.kcal) || 0) - deficitKcal);
        const densA = deficitKcal > 400 ? -(Number(a.kcal) || 0) : da;
        const densB = deficitKcal > 400 ? -(Number(b.kcal) || 0) : db;
        return snackBoost(typeA) - snackBoost(typeB) || densA - densB || da - db;
      });
  ...
  const pool = [...SIMPLE_START_RECIPES]
    ...
    .sort((a, b) => {
      ...
      if (deficitKcal > 400) {
        return snackBoost(a) - snackBoost(b) || b.calories - a.calories || da - db;
      }
...
export function fillDayCaloriesByAddingLibraryMeals(dayMeals, dailyTarget, opts = {}) {
  ...
  const maxExtra = opts.maxExtraMeals ?? maxExtraMealsForTarget(target);
...
export function planHasInflatedPortions(structuredPlan) {
  return maxPortionMultiplierInPlan(structuredPlan)
    > Math.max(INFLATION_INVALID_THRESHOLD, FLEX_CATCHUP_MAX_SCALE + 0.02);
}
