 ...
 */
import { resolveCanonicalName, normKey } from '../ingredientNormalize.js';
...
const BY_NORMALIZED = {
  ...
  citron: { kcal: 29, protein_g: 1.1, carbs_g: 9.3, fat_g: 0.3 },
  'citronova stava': { kcal: 22, protein_g: 0.4, carbs_g: 6.9, fat_g: 0.2 },
  ...
  jahody: { kcal: 32, protein_g: 0.7, carbs_g: 7.7, fat_g: 0.3 },
  'javorovy sirup': { kcal: 260, protein_g: 0, carbs_g: 67, fat_g: 0 },
  ...
  'paprika (cervena)': { kcal: 31, protein_g: 1, carbs_g: 6, fat_g: 0.3 },
  pepr: { kcal: 251, protein_g: 10, carbs_g: 64, fat_g: 3.3 },
  ...
  // PROMPT_KALORIE_OBSAH.md (2026-09-18) — chyběl řecký jogurt (bílý,
  // nízkotučný), na rozdíl od `bily jogurt` (běžný, ~3.5 g bílkovin/100 g)
  // má výrazně víc bílkovin a míň sacharidů. Bez vlastního klíče se
  // substring shodou trefoval do `bily jogurt`, tedy podhodnocoval bílkoviny
  // skoro na třetinu. Hodnoty: USDA FoodData Central, "Yogurt, Greek, plain,
  // nonfat" (100 g): 59 kcal, 10,2 g protein, 3,6 g carbs, 0,4 g fat.
  'recky jogurt': { kcal: 59, protein_g: 10, carbs_g: 3.6, fat_g: 0.4 },
  'ryba (napr. losos)': { kcal: 208, protein_g: 20, carbs_g: 0, fat_g: 13 },
  'ryba (napr. treska)': { kcal: 82, protein_g: 18, carbs_g: 0, fat_g: 0.7 },
  ryze: { kcal: 360, protein_g: 7, carbs_g: 79, fat_g: 0.7 },
  'ryze (bila)': { kcal: 360, protein_g: 7, carbs_g: 79, fat_g: 0.7 },
  'salat (napr. ledovy)': { kcal: 14, protein_g: 0.9, carbs_g: 3, fat_g: 0.1 },
  skorice: { kcal: 247, protein_g: 4, carbs_g: 81, fat_g: 1.2 },
  'sladke brambory': { kcal: 86, protein_g: 1.6, carbs_g: 20, fat_g: 0.1 },
  'smes salatu': { kcal: 15, protein_g: 1.4, carbs_g: 2.9, fat_g: 0.2 },
  'sojova omacka': { kcal: 53, protein_g: 8, carbs_g: 5, fat_g: 0 },
  spenat: { kcal: 23, protein_g: 2.9, carbs_g: 3.6, fat_g: 0.4 },
...
/** Prefer protein staples first, then carbs — better macros when catching up kcal. */
const CATCHUP_PRIORITY = [
  [/kureci|kruti|hovezi|veprov|maso|losos|ryba|tunak|sunka/, 1],
  [/tvaroh|cottage|protein/, 2],
  [/ryze|testovin|ovesn|quinoa|musli|brambor|cocka/, 3],
  [/jogurt|kefir|mleko/, 4],
  [/fazol/, 5],
  [/olej|maslo|arasid/, 6],
  [/orech|mandl|avokad/, 7],
...
export function lookupIngredientNutritionPer100g(name) {
  ...
  const n = resolved.matched ? resolved.key : normKey(name);
  if (!n) return null;
...
export function maxFlexCatchUpGrams(name, currentAmount) {
  ...
  if (/kureci|kruti|hovezi|veprov|maso|losos|ryba|tunak|sunka/.test(n)) {
    return Math.min(120, Math.max(30, Math.round(cur * 0.55)));
  }
  if (/tvaroh|cottage|protein/.test(n)) {
    return Math.min(140, Math.max(40, Math.round(cur * 0.55)));
  }
  if (/ryze|testovin|ovesn|quinoa|musli|brambor|cocka/.test(n)) {
    return Math.min(90, Math.max(25, Math.round(cur * 0.6)));
  }
  if (/jogurt|kefir|mleko/.test(n)) {
    return Math.min(150, Math.max(40, Math.round(cur * 0.5)));
  }
