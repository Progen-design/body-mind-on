...
import { prednostniPoolPodleTuku } from './cilTukuSlotu.js';
 ...
 *
 * NOUZOVÁ VĚTEV MUSÍ ZNÁT MAKRA STEJNĚ JAKO BĚŽNÝ VÝBĚR — docs/DALSI_KROK.md
 * 8.11 bod 1 (a 8.4/8.8): dřív se sem `cilovyPodilBilkovin`/`cilovyPodilTuku`
 * vůbec nepředávaly, defaultovaly na `null` v `catalogPickRank` a „TITLE/
 * FILTER MISS — emergency catalog pick" vybíral čistě podle kalorií. Tahle
 * větev je pro 8.11 klíčová: dokud makra neznala, ladění vah nebo přednostní
 * pool by tuhle cestu úplně minuly.
 *
 * PŘEDNOSTNÍ POOL (8.11 bod 2), i tady, ne jen v losování: bez tvrdého
 * stropu na tuk by `sorted[0]` mohl být recept, co jen „přeplatil" tučnost
 * kalorickou trefou — stejný problém, jaký bod 2 řeší pro
 * `pickFromTopKCatalogRow`. `minPocet` je tu 1 (jediný výběr, ne los).
 *
 * `opts.diag`, když je zadaný, dostane `vyberZPrednostnihoPoolu` — pro
 * diagnostiku v `[catalog-resolve] ... complete` (8.11 bod 5).
 *
 * @param {object[]} rows
 * @param {number} slotTarget
 * @param {{ mealType?: string, simpleStartMode?: boolean, slotMeal?: object,
 *           cilovyPodilBilkovin?: number|null, cilovyPodilTuku?: number|null,
 *           diag?: { vyberZPrednostnihoPoolu?: boolean } }} [opts]
 * @returns {object|null}
 */
export function pickClosestCatalogRow(rows, slotTarget, opts = {}) {
  ...
  const sorted = sortCatalogRowsForSimplePick(
    pool,
    slotTarget,
    mealType,
    opts.cilovyPodilBilkovin ?? null,
    opts.cilovyPodilTuku ?? null,
  );
  const { pool: kandidati, zPrednostniho } = prednostniPoolPodleTuku(sorted, opts.cilovyPodilTuku ?? null, 1);
  // `null` = žádný cíl na tuk pro tenhle pick (mimo rotaci) — odlišné od
  // `false` (cíl byl, ale přednostní pool měl málo kandidátů).
  if (opts.diag) opts.diag.vyberZPrednostnihoPoolu = opts.cilovyPodilTuku == null ? null : zPrednostniho;
  return kandidati[0] ?? pool[0] ?? null;
}
 ...
 *
 * PŘEDNOSTNÍ POOL PODLE TUKU (docs/DALSI_KROK.md 8.11, body 2+3) — penalta
 * v `catalogPickRank` je spojitá a dá se „přeplatit" kalorickou trefou;
 * losování samo o sobě to zhoršuje (páté místo v pořadí může být výrazně
 * tučnější než první). Řešení: `topK` zůstává, ale losuje se JEN
 * z kandidátů, kteří splňují `STROP_TUKU_VYBERU` (`prednostniPoolPodleTuku`,
 * lib/nutrition/cilTukuSlotu.js) — a teprve když je jich míň než `topK`,
 * doplní se zbytkem. Variabilita zůstává (změřeno: 64+ receptů do 35 % tuku
...
export function pickFromTopKCatalogRow(rows, slotTarget, seed, slotSalt, topK = 5, opts = {}) {
  ...
  // Cílový podíl bílkovin/tuku je nepovinný — bez něj se řadí jako dřív, jen
  // podle kalorií a jednoduchosti.
  const sorted = sortCatalogRowsForSimplePick(
    pool,
    slotTarget,
    mealType,
  ...
  const { pool: kandidatiProLos, zPrednostniho } = prednostniPoolPodleTuku(sorted, opts.cilovyPodilTuku ?? null, topK);
  // `null` = žádný cíl na tuk pro tenhle pick (mimo rotaci) — odlišné od
  // `false` (cíl byl, ale přednostní pool měl míň než topK kandidátů).
  if (opts.diag) opts.diag.vyberZPrednostnihoPoolu = opts.cilovyPodilTuku == null ? null : zPrednostniho;
  const k = Math.min(Math.max(1, topK), kandidatiProLos.length);
  const topPool = kandidatiProLos.slice(0, k);
