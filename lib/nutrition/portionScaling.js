 ...
 * @returns {object|null}
 */
export function pickFromTopKCatalogRow(rows, slotTarget, seed, slotSalt, topK = 5) {
  if (!Array.isArray(rows) || rows.length === 0) return null;
  const sorted = [...rows].sort(
    (a, b) => Math.abs(Number(a.kcal) - slotTarget) - Math.abs(Number(b.kcal) - slotTarget)
  );
  const k = Math.min(Math.max(1, topK), sorted.length);
  const pool = sorted.slice(0, k);
  const idx = seededPickIndex(seed, slotSalt, pool.length);
...
export function boostDayMealsToCalorieTarget(dayMeals, dailyTarget, minRatio = 0.85) {
  ...
  const verified = (dayMeals || []).filter((m) => m?.recipe_verified && Number(m.kcal) > 0);
