...
export function calculateNutritionTargets({
  ...
  // Sacharidy jsou zbytek energie po bílkovinách a tucích — kromě případu,
  // kdy jsou uložené. Dopočítat je i z uloženého cíle by znamenalo, že se
  // s každým posunem kalorií tiše mění poměr, který si člověk nastavil.
  const carbs = maUlozenaMakra && !shouldAdjust
    ? clamp(Math.round(ulozeneMakro.carbs), 40, 700)
    : clamp(Math.round((calories - protein * 4 - fat * 9) / 4), 40, 700);
