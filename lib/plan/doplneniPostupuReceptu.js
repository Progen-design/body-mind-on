 ...
 * 1000`, jako by šlo o úplné číslo. Stejný vzor se našel ještě na dvou
 * dalších místech (`scripts/report-beta-activation.mjs`,
 * `api/trainer/clients.js`), takže stránkování žije sdílené v
 * `lib/supabasePagination.js` (`nactiVsechnyRadky()`) — tohle je jen tenký
 * obal nad ním pro `recipes_catalog`.
 */
...
/**
 * PROMPT_NAKLADY_MINIMUM.md bod 1 (21. 9. 2026) — výchozí `gpt-4.1-mini`
 * (~6× levnější než gpt-4o), doplňování postupů je jednoduchá úloha
 * (přepsat postup podle surovin). Dřív gpt-4o ze stejného důvodu jako u
 * `recipeGenerator.js`: čeština je jediné, co od modelu kupujeme. Kvalitu
 * hlídá `posudPostup()` (brána kvality postupu) — mini, která ji nesplní,
 * se projeví jako zahozený výsledek, ne jako horší recept v katalogu.
 * Návrat bez deploye: env `DOPLNENI_MODEL=gpt-4o`.
 */
export const DOPLNENI_MODEL = process.env.DOPLNENI_MODEL?.trim() || 'gpt-4.1-mini';
...
// Stránkování je od 9. 9. 2026 (druhý nález) sdílené — viz
// lib/supabasePagination.js. `VELIKOST_STRANKY_SUPABASE` se odsud dál
// re-exportuje, ať existující import v `scripts/doplneni-postupu-receptu.mjs`
// nemusí měnit zdroj.
export { VELIKOST_STRANKY_SUPABASE } from '../supabasePagination.js';

/**
 * Načte VŠECHNY recepty `recipes_catalog` (volitelně omezené na jeden
 * zdroj) — tenký obal nad sdíleným `nactiVsechnyRadky()`
 * (lib/supabasePagination.js). Bez stránkování jeden `.select()` mlčky
 * ořízne odpověď na `VELIKOST_STRANKY_SUPABASE` řádků a zbytek katalogu do
 * běhu vůbec nedorazí (naměřeno 9. 9. 2026: 26 z 1114 receptů s nejvyšším
 * `id`).
 *
 * @param {{ client: any, zdrojFiltr?: string|null, velikostStranky?: number }} vstup
 * @returns {Promise<Array<Record<string, unknown>>>}
 */
export async function nactiVsechnyRecepty({ client, zdrojFiltr = null, velikostStranky }) {
  return nactiVsechnyRadky({
    client,
    tabulka: 'recipes_catalog',
    sloupce: 'id, name_cs, ingredients, instructions_cs, source',
    poradi: { sloupec: 'id', ascending: true },
    filtr: (dotaz) => (zdrojFiltr ? dotaz.eq('source', zdrojFiltr) : dotaz),
    ...(velikostStranky != null ? { velikostStranky } : {}),
  });
}
