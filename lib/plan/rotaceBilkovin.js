 ...
 */
import { rozparsujHint } from './proteinHint.js';
 ...
 *
 * `rybíz` (docs/DALSI_KROK.md 8.10): černý rybíz je ovoce, ne ryba, ale vzor
 * skupiny `ryby` (`/ryb/i`) ho chytil — vegetariánská objednávka pak dostala
 * „ryby a mořské plody" jako hlavní bílkovinu, protože černý rybíz byl jediná
 * položka v povoleném seznamu, kterou ten vzor zasáhl. Model udělal lososa,
 * `surovinyMimoSeznam` ho zahodil, položka spadla do `failed` — 46 receptů
 * ve frontě jen na týhle jedné shodě jménem. Jeden řádek tady opraví
 * klasifikaci všude naráz (rotace, adresář, `receptSplnujeBilkovinu`), protože
 * `skupinaSuroviny` konzultuje `NENI_ZDROJ_PORCE` jako první.
 *
 * Tohle NESTAČÍ samo o sobě — shoda jménem je obecně slabý zdroj pravdy
 * (dnes rybíz, zítra „sójová omáčka" jako zdroj bílkovin). Skutečná pojistka
 * je `VYLOUCENE_SKUPINY_PODLE_DIETY` níž, která skupinu vyloučí podle diety
 * bez ohledu na to, co chytí vzor.
 */
const NENI_ZDROJ_PORCE = [
  /vývar/i, /vyvar/i, /bujón/i, /bujon/i, /polévka/i, /polevka/i,
  /rybíz/i, /rybiz/i,
];
 ...
 *
 * NEMĚNIT bez rozmyslu: objednávka BEZ diety musí dostat přesně tuhle skupinu
 * ve stejném pořadí jako dnes (docs/DALSI_KROK.md 8.10, bod 3) — je to
 * zajištěné tím, že `bilkovinaProPolozku` tenhle seznam nerozšiřuje přímo,
 * ale jen přidává `DOPLNKOVE_BILKOVINY_PRI_VYLOUCENI` NAVÍC, a jen tehdy,
 * když dieta něco skutečně vyloučila.
 */
const CILOVE_BILKOVINY = Object.freeze(['hovezi', 'veprove', 'ryby', 'lusteniny', 'drubez']);

/**
 * Skupiny, kterými se `CILOVE_BILKOVINY` rozšiřuje, KDYŽ dieta z hlavního
 * seznamu něco vyloučila. docs/DALSI_KROK.md 8.10, bod 3: bez tohohle by
 * vegetariánovi (a veganovi) po vyloučení masa a ryb zbyla jediná kandidátní
 * skupina (`lusteniny`). Přidává se AŽ ZA `CILOVE_BILKOVINY`, ne před ně —
 * `lusteniny` má u vyloučené diety přednost, `vejce`/`mlecne` jsou doplněk.
 * U veganské položky obojí stejně vypadne podle `VYLOUCENE_SKUPINY_PODLE_DIETY`.
 */
const DOPLNKOVE_BILKOVINY_PRI_VYLOUCENI = Object.freeze(['vejce', 'mlecne']);

/** Rotace dává smysl u hlavních jídel; u snídaně a svačiny by nutila nesmysly. */
const SLOTY_S_ROTACI = new Set(['obed', 'vecere']);

/**
 * SKUPINY, KTERÉ DANÁ DIETA VYLUČUJE — BEZ OHLEDU NA TO, CO CHYTÍ VZOR SKUPINY.
 *
 * docs/DALSI_KROK.md 8.10, bod 2. Shoda jménem (`skupinaSuroviny`) je slabý
 * zdroj pravdy: dnes černý rybíz jako „ryby", zítra klidně „sójová omáčka"
 * jako zdroj bílkovin. Tahle mapa je nezávislá pojistka — skupina se
 * vegetariánovi/veganovi nesmí nabídnout, ani kdyby povolený seznam náhodou
 * obsahoval surovinu, kterou vzor skupiny chytí.
 *
 ...
 */
export const VYLOUCENE_SKUPINY_PODLE_DIETY = Object.freeze({
  vegan: Object.freeze(['drubez', 'ryby', 'hovezi', 'veprove', 'vejce', 'mlecne']),
  vegetarian: Object.freeze(['drubez', 'ryby', 'hovezi', 'veprove']),
});

/**
 * Sjednocení skupin, které vylučuje ZADANÁ kombinace dietních tagů.
 *
 * @param {string[]|null|undefined} dietTags
 * @returns {Set<string>}
 */
export function vyloucoveSkupinyProDietu(dietTags) {
  const vyloucene = new Set();
  for (const tag of dietTags || []) {
    const seznam = VYLOUCENE_SKUPINY_PODLE_DIETY[String(tag ?? '').trim().toLowerCase()];
    if (seznam) for (const s of seznam) vyloucene.add(s);
  }
  return vyloucene;
}
 ...
 *
 * Explicitní `protein_hint` z fronty vyhrává vždycky — je to OBJEDNÁVKA, ne
 * odvození, a `VYLOUCENE_SKUPINY_PODLE_DIETY` se na ni proto NEVZTAHUJE
 * (docs/DALSI_KROK.md 8.10, bod 4 v „Co nedělat"). Jinak se odvodí z
 * rozložení už existujících receptů toho slotu, a TADY se dieta uplatní.
 *
 ...
 *
 * @param {{protein_hint?: string|null, meal_type?: string, diet_tags?: string[]}} polozka
...
export function bilkovinaProPolozku(polozka, existujici, povoleneProPolozku) {
  ...
  // `protein_hint` nese od 23. 8. 2026 i minimální podíl bílkovin, takže je
  // to buď holý klíč skupiny (sedm starších řádků ve frontě), nebo JSON.
  // Parsuje jedině proteinHint.js; zdroj v JSONu chybět může.
  const zadany = rozparsujHint(polozka?.protein_hint).zdroj;
  if (zadany) return dostupne(zadany) ? zadany : null;

  if (!SLOTY_S_ROTACI.has(String(polozka?.meal_type ?? ''))) return null;

  // MAPA DIETY, APLIKOVANÁ PŘED `dostupne()` — docs/DALSI_KROK.md 8.10, bod 2.
  // Bez diety (`vyloucene` prázdná) je tenhle řádek no-op a `zakladniKandidati`
  // vyjde bit po bitu stejně jako `CILOVE_BILKOVINY.filter(dostupne)` dřív —
  // to je ten regresní požadavek z bodu 3.
  const vyloucene = vyloucoveSkupinyProDietu(polozka?.diet_tags);
  const zakladniKandidati = CILOVE_BILKOVINY.filter((k) => !vyloucene.has(k) && dostupne(k));
  const kandidati = vyloucene.size > 0
    ? zakladniKandidati.concat(
      DOPLNKOVE_BILKOVINY_PRI_VYLOUCENI.filter((k) => !vyloucene.has(k) && dostupne(k)),
    )
    : zakladniKandidati;

  return dalsiBilkovina(rozlozeniBilkovin(existujici), kandidati);
}
