...
function ensureExerciseDisplayNames(ex) {
  ...
  if (!existing) {
    ...
    return { out, patched: true };
  }
  if (!String(out.display_name_cs || '').trim()) out.display_name_cs = existing;
  if (!String(out.name_cs || '').trim()) out.name_cs = out.display_name_cs;
  if (!String(out.name || '').trim()) out.name = out.display_name_cs;
  return { out, patched: false };
}
