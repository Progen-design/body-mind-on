...
export async function replaceMealInStructuredPlan(structuredPlan, daySlotIndex, mealIndex, bodyMetrics = {}) {
  ...
  const honesty = fillDayCaloriesByAddingLibraryMeals(day.meals, dailyTarget);
  const daySum = honesty.achieved_kcal ?? sumScaledDayKcal(day.meals);
  const maxOk = Math.round(dailyTarget * 1.15);
  // Allow honest underrun; only reject severe overrun past honest band
  if (daySum > maxOk) {
    throw new Error('DAY_KCAL_OUT_OF_TOLERANCE');
  }

  day.daily_achieved_kcal = daySum;
  day.calorie_under_target = honesty.under_target === true;
  day.calorie_shortfall_kcal = honesty.shortfall_kcal;
