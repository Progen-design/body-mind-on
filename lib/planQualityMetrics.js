...
import { isTrustedExerciseMediaUrl } from './exerciseRegistryMedia.js';
...
export function computePlanQualityMetrics(planJson, bodyMetrics = {}, opts = {}) {
  ...
  for (const day of days) {
    ...
    for (const ex of exercises) {
      ...
      // Žádný canonical_key nemá od 14. 9. 2026 natvrdo daný fallback gif
      // (docs/DALSI_KROK.md 9.12) — cvik bez média je legitimní stav, jen se
      // počítá, negatuje se to (missing_gif_count není gate).
      const hasTrustedGif = Boolean(ex.gif_url && isTrustedExerciseMediaUrl(ex.gif_url));
      if (hasTrustedGif) {
        trustedGifCount += 1;
      } else if (!ex.gif_url && !ex.video_url) {
        missingGifCount += 1;
      }
