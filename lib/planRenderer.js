...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      const href =
        ridNum != null
          ? `${planRecipeAppBaseUrl()}/api/spoonacular-recipe?id=${ridNum}&format=html`
          : '';
      ...
      for (const ex of workout.exercises) {
        ...
        const gifUrl =
          ex.video_url && String(ex.video_url).trim()
            ? ` data-gif-url="${escapeHtml(String(ex.video_url).trim())}"`
            : '';
        html += `<li${exKey}${wgerId}${imgUrl}${gifUrl}>${escapeHtml(name)} – ${part}</li>`;
      }
      html += `</ul>`;
    } else {
