...
/** Text u jídla v HTML – nikdy ne „Jídlo“ jako finální label, pokud existuje AI / plánovač. */
function mealDisplayTextForHtml(m) {
  const bad = (s) => {
    const t = (s || '').trim();
    return !t || t === 'Jídlo' || t === 'Jídlo (neověřeno)';
  };
  const candidates = [m?.name_cs, m?.ai_name, m?.planner_suggestion_cs, m?.display_name_cs, m?.display_name];
  for (const c of candidates) {
    const t = typeof c === 'string' ? c.trim() : '';
    if (t && !bad(t)) return t;
  }
  const last = (m?.display_name_cs || m?.display_name || '').trim();
  if (last && !bad(last)) return last;
  return 'Zdravé jídlo';
}
...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      const text = mealDisplayTextForHtml(m);
