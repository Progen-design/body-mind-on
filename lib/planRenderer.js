...
import { aggregateShoppingIngredientLinesFromStructuredPlan, aggregateShoppingByAisleFromStructuredPlan } from './spoonacularShopping.js';
import { mealDisplayTitleForStructuredMeal } from './mealDisplayNameHelpers.js';
import { portionIngredientsFromStructuredMeal } from './mealPortionIngredients.js';
...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      const model = createMealDisplayModel(m, planRecipeAppBaseUrl());
      const normalizedMeal = model.normalizedMeal || m;
      const type = m.type ?? 'breakfast';
      const label = MEAL_TYPE_LABELS[type] ?? type;
      const text = model.title || mealDisplayTitleForStructuredMeal(normalizedMeal, sourcePlanHtml, dayName);
      const lookupId = catalogLookupIdFromMeal(normalizedMeal);
      ...
      const href = model.recipeUrl || getMealRecipeUrl(normalizedMeal, planRecipeAppBaseUrl());
      ...
      const portions = portionIngredientsFromStructuredMeal(normalizedMeal);
      if (portions.length) {
      ...
      const n = getMealNutritionDisplay(normalizedMeal);
      if (n.calories != null) dayCal += Number(n.calories) || 0;
      if (n.protein_g != null) dayProt += Number(n.protein_g) || 0;
      if (n.carbs_g != null) dayCarb += Number(n.carbs_g) || 0;
      if (n.fat_g != null) dayFat += Number(n.fat_g) || 0;
      if (n.fiber_g != null) dayFiber += Number(n.fiber_g) || 0;
      const parts = [];
      if (n.calories != null) parts.push(`${Math.round(n.calories)} kcal`);
      if (n.protein_g != null) parts.push(`B ${Math.round(n.protein_g)} g`);
      if (n.carbs_g != null) parts.push(`S ${Math.round(n.carbs_g)} g`);
      if (n.fat_g != null) parts.push(`T ${Math.round(n.fat_g)} g`);
      if (n.fiber_g != null) parts.push(`Vláknina ${Math.round(n.fiber_g)} g`);
      const ready = m?.recipe?.ready_in_minutes;
      if (ready != null) parts.push(`cca ${ready} min`);
      const health = m?.recipe?.health_score;
      if (health != null) parts.push(`zdraví ${Math.round(health)}/100`);
      if (parts.length) {
        html += `<p class="meal-nutrition-line"><small>${escapeHtml(parts.join(' · '))}</small></p>`;
      }
    }
    if (meals.length && (sumMealCalories(meals) != null || dayProt > 0)) {
      const sum = [
