...
function planRecipeAppBaseUrl() {
  if (typeof process === 'undefined') return 'https://app.bodyandmindon.cz';
  return String(process.env.NEXT_PUBLIC_APP_URL || process.env.APP_URL || 'https://app.bodyandmindon.cz').replace(/\/$/, '');
}
...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      if (ridNum != null) {
        const href = `${planRecipeAppBaseUrl()}/api/spoonacular-recipe?id=${ridNum}`;
        const aStyle =
          'display:inline-block;font-size:11px;font-weight:600;color:#7c3aed;text-decoration:none;padding:3px 10px;border:1px solid rgba(124,58,237,0.4);border-radius:5px;margin-top:2px;letter-spacing:0.3px;';
        html += `<p class="plan-meal-external-recipe" style="margin:0 0 10px;line-height:1.5;font-family:Arial,Helvetica,sans-serif;"><a href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" style="${aStyle}">📖 Recept</a></p>`;
      }
      const r = m.recipe;
