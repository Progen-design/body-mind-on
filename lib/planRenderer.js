...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      const text = m.recipe_verified === true ? (m.display_name_cs ?? m.display_name ?? m.recipe?.title ?? '') : 'Jídlo (neověřeno)';
    ...
    if (workout?.exercises?.length) {
      ...
      for (const ex of workout.exercises) {
        const name = ex.exercise_verified === true ? (ex.display_name_cs ?? ex.name ?? '') : 'Cvik (neověřeno)';
...
<li>Dostatečný pitný režim</li>
  ...
  const allMeals = days.flatMap((d) => d.meals ?? []).filter((m) => m.recipe_verified === true).map((m) => m.display_name_cs ?? m.display_name ?? m.recipe?.title).filter(Boolean);
