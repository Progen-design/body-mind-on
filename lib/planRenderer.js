...
/** Text za <b>Snídaně:</b> atd. – stejná priorita jako resolveMeals: trainer pole → HTML agenta → jiné negenerické → Zdravé jídlo. */
function mealDisplayTextForHtml(m, planHtml, dayName) {
  const fromTrainer = resolveTrainerMealDisplayLabel(m, planHtml || '', dayName);
  if (fromTrainer) return fromTrainer;
  const candidates = [m?.planner_suggestion_cs, m?.display_name_cs, m?.display_name];
  for (const c of candidates) {
    const t = typeof c === 'string' ? c.trim() : '';
    if (t && !isGenericUserMealLabel(t)) return t;
  }
  return 'Zdravé jídlo';
}
