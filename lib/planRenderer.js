...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      const displayName = m.recipe_verified ? (m.display_name ?? m.recipe?.title ?? '') : (m.display_name ?? 'Jídlo (neověřeno)');
      const text = displayName || '(jídlo)';
      html += `<p><b>${escapeHtml(label)}:</b> ${escapeHtml(text)}</p>`;
    }
...
<li>Dostatečný pitný režim</li>
  ...
  const allMeals = days.flatMap((d) => d.meals ?? []).map((m) => m.display_name ?? m.recipe?.title).filter(Boolean);
