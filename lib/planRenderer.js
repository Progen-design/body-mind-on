...
<p>Tréninků týdně: <strong>${workoutDaysCount}</strong>. Níže u každého dne najdeš konkrétní cviky.</p>`;
  ...
  for (const day of days) {
    ...
    for (const m of meals) {
      ...
      let imageUrl = (m.image_url && String(m.image_url).trim()) || null;
      if (!imageUrl && ridNum) {
        imageUrl = `https://img.spoonacular.com/recipes/${ridNum}-312x231.jpg`;
      }
      ...
      for (const ex of workout.exercises) {
        ...
        const wgerId =
          ex.wger_exercise_id != null && Number.isFinite(Number(ex.wger_exercise_id))
            ...
            : '';
        const gifUrl = ex.gif_url ?? ex.image_url ?? ex.video_url ?? null;
        const gifAttr = gifUrl ? ` data-gif-url="${escapeHtml(String(gifUrl))}"` : '';
        html += `<li${exKey}${wgerId}${gifAttr}>${escapeHtml(name)} – ${part}</li>`;
      }
      html += `</ul>`;
    } else {
