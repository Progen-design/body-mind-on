...
/**
 * Uplatněný poukaz uživatele (pro Stripe checkout — ať 30 dní zdarma
 * nepřepíše okamžitá platba).
 *
...
/** Stripe přijme `trial_end` nejdřív 48 h dopředu. */
const MIN_STRIPE_TRIAL_MS = 48 * 60 * 60 * 1000;

/**
 * `trial_end` (unix sekundy) pro Stripe subscription u uživatele s poukazem,
 * který platí během poukazového trialu: zbytek poukazu zůstane zdarma, první
 * platba je až po něm. Bez poukazu, bez trialu nebo méně než 48 h do konce
 * → undefined (Stripe pole vynecháme).
 *
 * @param {{ trial_ends_at?: string|null, stripe_subscription_id?: string|null } | null} membership
 * @param {object|null} poukaz
 * @param {number} [ted=Date.now()]
 * @returns {number|undefined}
 */
export function stripeTrialEndZPoukazu(membership, poukaz, ted = Date.now()) {
  if (!poukaz || !membership || membership.stripe_subscription_id) return undefined;
