...
/** Vrací minimální a maximální počet tréninkových dní pro zvolenou frekvenci. */
export function getFrequencyDayRange(v) {
  const normalized = normalizeFrequency(v);
  if (!normalized) return { min: 0, max: 7, normalized: null };
  if (normalized === '1-2x týdně') return { min: 1, max: 2, normalized };
  if (normalized === '2-3x týdně') return { min: 2, max: 3, normalized };
  if (normalized === '4-5x týdně') return { min: 4, max: 5, normalized };
  return { min: 0, max: 7, normalized };
}

/** Vrací číslo tréninků týdně (1 | 3 | 5) z freq_choice nebo textu. */
/**
 * Volba frekvence → počet tréninků týdně.
 *
 * Číslo se bere doslova, když přijde jako číslo nebo holý číselný řetězec —
 * „5x tydne“ dřív propadlo až na výchozí 3, protože se testovaly jen znaky
 * '1', '2' a '4'. Rozsahy zůstávají na horní hranici („2-3“ → 3, „4-5“ → 5),
 * to je původní chování.
 *
 * @param {unknown} v
 * @returns {number}
 */
export function getWeeklySessions(v) {
  ...
  // Rozsahy napřed — „4-5“ obsahuje i '5', takže samostatné číslo se smí
  // číst až tehdy, když o rozsah nejde.
  if (t.includes('0-1') || t.startsWith('1')) return 1;
  if (t.includes('2-3')) return 3;
  if (t.includes('4-5') || t.includes('4plus') || t.includes('4+')) return 5;

  const cislo = Number.parseInt(t, 10);
  if (Number.isFinite(cislo) && cislo >= 0 && cislo <= 14) return cislo;
