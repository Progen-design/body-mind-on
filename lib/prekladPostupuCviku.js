...
import { sparujOdpovedSDavkou, krokyProRadek } from './prekladPostupuParovani.js';
...
/**
 * @param {Array<{ id: string, instructions_en: string[] }>} rows
 * @returns {Promise<{ preklady: Map<string, string[]>, neznamaId: string[] }>}
 */
async function prelozDavkuOpenAI(rows) {
  ...
  const completion = await volejModel({
    ...
    promptSha256: PREKLAD_POSTUPU_PROMPT_SHA256,
    messages: [
  ...
  // ŽÁDNÉ Number(item.id) TADY. exercise_asset_registry.id je UUID —
  // Number(uuid) je vždy NaN, což dřív zahodilo úplně každou položku
  // z odpovědi modelu (viz komentář v lib/prekladPostupuParovani.js).
  /** @type {{ exercises?: Array<{ id?: unknown, steps_cs?: unknown }> }} */
  const parsed = JSON.parse(raw);
  const { preklady, neznamaId } = sparujOdpovedSDavkou(rows, parsed.exercises);

  if (neznamaId.length) {
    console.error(JSON.stringify({
      source: 'exercise-instructions-translate',
      event: 'unknown_id_in_response',
      prompt_sha256: PREKLAD_POSTUPU_PROMPT_SHA256,
      ids: neznamaId,
    }));
  }
...
export async function runExerciseInstructionTranslation(options = {}) {
  ...
  const { preklady, neznamaId } = await prelozDavkuOpenAI(pending);
  let translated = 0;
  /** @type {string[]} */
  const errors = neznamaId.map(
    (id) => `Odpoved modelu obsahovala neznamé id "${id}", ktere neni v davce - zahozeno`
  );
...
/**
 * Kolik cviků čeká na překlad postupu — bez volání OpenAI. Používá
 * api/cron/translate-recipes.js k rozhodnutí, jestli má fronta cviků vůbec
 * práci, ještě než se rozhodne, kterou frontu (recepty/cviky) v tomto běhu
 * spustit (lib/translateQueueOrchestrator.js).
 */
export async function zbyvaPrelozitPostupu() {
