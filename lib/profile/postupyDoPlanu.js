...
/**
 * Sloupce katalogu, které `postupZKatalogu` umí přečíst.
 *
 * Je to jediný zdroj pravdy pro `.select()` v `api/profile.js`. Kdyby se
 * seznam psal na dvou místech, dá se rozejít tiše: server by přestal vozit
 * `prep_minutes_estimated`, `prepMinut` by vyšlo pořád `null` a řádek
 * „Příprava: zhruba 12 min" by v UI nikdy nesvítil, přestože hodnotu má
 * 62 z 69 receptů v plánech. Žádný test jedné strany to nezachytí.
 */
export const SLOUPCE_KATALOGU_PRO_POSTUP = 'id, instructions_cs, prep_minutes_estimated';
