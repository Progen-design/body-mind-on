...
export function normalizujJednotku(unit) {
  ...
  if (/^pieces?$|^kus/.test(u)) return { typ: 'slovo', klic: 'piece' };
 ...
 * @returns {string}
 */
function tvarJednotky(klic, mnozstvi) {
  const j = JEDNOTKY[klic];
  if (!j) return '';
  if (mnozstvi < 1) return j.zlomek;
  if (mnozstvi === 1) return j.jedna;
  if (mnozstvi >= 2 && mnozstvi <= 4 && Number.isInteger(mnozstvi)) return j.dva;
  return j.vic;
}
