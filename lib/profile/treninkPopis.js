...
export function serieOpakovaniSlovy(serie, opakovani) {
  ...
  // PROMPT_UX_DOLADENI.md bod C — jednostranné cviky mají `opakovani` tvaru
  // „14-16 na stranu"/„10 na nohu" (workoutStartProgram.js `reps_note`
  // vždycky připojené na konec). Dřív se „na stranu" vlepilo doprostřed věty
  // — „3 série po 14–16 na stranu opakováních". Doplněk se vytáhne zvlášť
  // a připojí AŽ ZA slovo „opakováních": „3 série po 14–16 opakováních na
  // stranu".
  const shodaDoplnku = o.match(/\s+(na\s+.+)$/i);
  const doplnek = shodaDoplnku ? ` ${shodaDoplnku[1]}` : '';
  const zakladText = shodaDoplnku ? o.slice(0, shodaDoplnku.index).trim() : o;
  const opakovaniText = zakladText.replace(/(\d)\s*-\s*(\d)/, '$1–$2');
  // „30 s", „2 min" — výdrž na čas, ne opakování.
  const jeCas = /\d\s*(s|sek|min)\b/i.test(opakovaniText);
  return jeCas
    ? `${s} ${serieSlovo} po ${opakovaniText}${doplnek}`
    : `${s} ${serieSlovo} po ${opakovaniText} opakováních${doplnek}`;
}
