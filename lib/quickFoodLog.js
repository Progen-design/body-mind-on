...
/** OpenAI nebo síť nedostupná — není to chyba fotky ani uživatele. */
export const HLASKA_NEDOSTUPNE = 'Odhad teď není dostupný, zkus to prosím za chvíli.';

/**
 * Je chyba z `volejModel()` výpadek na straně OpenAI nebo sítě?
 *
 * 429 (rate limit i `insufficient_quota` — docházející kredit), 5xx, timeout
 * a spojení, které vůbec nevzniklo. Na takovou chybu patří 503 „zkus za
 * chvíli", ne 502 „nepodařilo se rozpoznat jídlo" — s fotkou je všechno
 * v pořádku, jen teď nemá kdo odhadovat. Ostatní 4xx od OpenAI (400 kvůli
 * obrázku apod.) sem nepatří.
 */
export function jeNedostupnostModelu(err) {
  const status = Number(err?.status);
  if (status === 429 || (status >= 500 && status <= 599)) return true;
  if (err?.name === 'APIConnectionError' || err?.name === 'APIConnectionTimeoutError') return true;
  if (['ECONNRESET', 'ETIMEDOUT', 'ENOTFOUND', 'ECONNREFUSED', 'EAI_AGAIN'].includes(err?.code)) return true;
  return /timed? ?out|fetch failed|network|socket hang up/i.test(String(err?.message || ''));
}
