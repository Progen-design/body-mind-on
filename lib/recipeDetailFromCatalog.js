...
export function buildSimpleStartFallbackRecipeHtml(options = {}) {
  ...
  const shoppingLines = (libraryRecipe?.ingredients || options.shoppingLines || tpl?.shopping_ingredient_lines || [])
    .map(sanitizeIngredientLineForDisplay)
    .filter(Boolean);
  const instructions_cs = libraryRecipe?.instructions || options.instructions
