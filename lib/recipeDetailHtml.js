...
export function recipePartsToHtml({ title, ingredients_cs, instructions_cs, image_url, nutritionHtml }) {
  ...
  if (image_url && String(image_url).trim()) {
    imageHtml = `<p style="margin:0 0 16px;"><img src="${escapeHtml(String(image_url).trim())}" alt="${escapeHtml(safeTitle)}" style="max-width:100%;max-height:280px;border-radius:12px;object-fit:cover;" /></p>`;
  }
