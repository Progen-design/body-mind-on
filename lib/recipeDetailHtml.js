...
export function buildMacroEnergyNutritionHtml(macrosInput) {
  ...
  if (!breakdown.hasMacros) {
    if (displayKcal) {
      return `<div class="recipe-nutrition-block"><h4 class="recipe-nutrition-title">Nutriční hodnoty na 1 porci</h4><p class="recipe-macro-kcal-line"><span class="recipe-macro-kcal-label">Kalorie</span> <strong>${escapeHtml(String(displayKcal))} kcal</strong></p><p class="recipe-macro-unavailable">Makra pro toto jídlo nejsou dostupná.</p></div>`;
    }
    return `<div class="recipe-nutrition-block"><h4 class="recipe-nutrition-title">Nutriční hodnoty na 1 porci</h4><p class="recipe-macro-unavailable">Makra pro toto jídlo nejsou dostupná.</p></div>`;
  }

  const delta = getMacroCalorieDelta(displayKcal ?? breakdown.totalMacroKcal, protein_g, carbs_g, fat_g);
  const statusNote = delta.status === 'WARNING'
    ? '<p class="recipe-macro-kcal-warning">Kalorie jsou zaokrouhlené podle porcí.</p>'
    : '';

  const stackedBar = `
    <div class="recipe-macro-energy-bar" role="img" aria-label="Poměr maker z kalorií">
      ${breakdown.proteinPercent > 0 ? `<span class="recipe-macro-energy-seg recipe-macro-energy-seg--protein" style="width:${breakdown.proteinPercent}%"></span>` : ''}
      ${breakdown.carbsPercent > 0 ? `<span class="recipe-macro-energy-seg recipe-macro-energy-seg--carbs" style="width:${breakdown.carbsPercent}%"></span>` : ''}
      ${breakdown.fatPercent > 0 ? `<span class="recipe-macro-energy-seg recipe-macro-energy-seg--fat" style="width:${breakdown.fatPercent}%"></span>` : ''}
    </div>`;

  const macroRows = [
    { key: 'protein', label: 'Bílkoviny', grams: protein_g, pct: breakdown.proteinPercent, barClass: 'recipe-macro-energy-seg--protein' },
    { key: 'carbs', label: 'Sacharidy', grams: carbs_g, pct: breakdown.carbsPercent, barClass: 'recipe-macro-energy-seg--carbs' },
    { key: 'fat', label: 'Tuky', grams: fat_g, pct: breakdown.fatPercent, barClass: 'recipe-macro-energy-seg--fat' },
  ].map((row) => {
    const value = `${Math.round(row.grams)} g`;
    return `<div class="recipe-nutrient-row recipe-macro-energy-row"><div class="recipe-nutrient-top"><span class="recipe-nutrient-label">${escapeHtml(row.label)}</span><span class="recipe-nutrient-value">${escapeHtml(value)} · ${row.pct} %</span></div><div class="recipe-nutrient-bar-wrap"><div class="recipe-nutrient-bar recipe-nutrient-bar-macro ${row.barClass}" style="width:${row.pct}%"></div></div></div>`;
  }).join('');
  ...
  return `<div class="recipe-nutrition-block"><h4 class="recipe-nutrition-title">Nutriční hodnoty na 1 porci</h4>${kcalLine}${stackedBar}<div class="recipe-nutrients recipe-nutrients--macro-energy">${macroRows}</div>${statusNote}</div>`;
}
