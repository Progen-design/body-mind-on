...
export function recipeFromCatalogApiUrl(recipeId, appBaseUrl, { format, meal } = {}) {
  ...
  const kcal = meal?.recipe?.calories ?? meal?.calories ?? meal?.kcal;
  const protein = meal?.recipe?.protein_g ?? meal?.protein_g;
  const carbs = meal?.recipe?.carbs_g ?? meal?.carbs_g;
  const fat = meal?.recipe?.fat_g ?? meal?.fat_g;
