...
import { kanonickePasmo, srovnejPasmo, pasmoPoptavky } from './recipeGenerationBands.js';
import { serializujHint, omezPodilProObjednavku } from './plan/proteinHint.js';
import { omezStropTukuProObjednavku } from './plan/fatHint.js';
import { MIN_RECEPTU_NA_SLOT } from './dietOptions.js';
import { MAX_POKUSU_POLOZKY } from './recipeGenerationNeed.js';

/**
 * Nejvíc kusů, které smí chtít jedna objednávka.
 *
 * Je to `MIN_RECEPTU_NA_SLOT`, protože právě tolik slot potřebuje, aby se
 * týden neopakoval — víc už jen odsouvá jinou díru. Zrcadlí ho i SQL plnič
 * `fill_recipe_queue_from_demand` (migrace 20260825120000); že se nerozešly,
 * hlídá test v lib/__tests__/frontaSlucovani.test.mjs.
 */
export const MAX_KUSU_NA_OBJEDNAVKU = MIN_RECEPTU_NA_SLOT;
...
/**
 * Jak dlouho po vyčerpání pokusů se ta samá specifikace znovu neobjednává.
 * Bez toho byla „zadní vrátka“: unikátní index kryje jen `pending`/`running`,
 * takže hned po pádu se stejná specifikace objednala znovu (nový řádek,
 * `pokusu = 0`) a platilo se za ni podruhé, potřetí, … — PROMPT_NAKLADY_MINIMUM.md
 * bod 2. Po dvou týdnech se katalog i slovník mohly změnit, tak se to smí
 * zkusit znovu.
 */
export const PAUZA_PO_VYCERPANI_DNI = 14;

/**
 * Selhala už přesně tahle specifikace nedávno (`failed` je od 21. 9. 2026
 * konečný stav — položka vyčerpala pokusy nebo model nevrátil nic použitelného)?
 * Best-effort: selhání dotazu objednávku NEBLOKUJE (radši zbytečná objednávka
 * než ztracená poptávka).
 */
async function nedavnoVycerpana(radek, client) {
  try {
    const od = new Date(Date.now() - PAUZA_PO_VYCERPANI_DNI * 86_400_000).toISOString();
    let q = client
      .from('recipe_generation_queue')
      .select('id')
      .eq('stav', 'failed')
      .eq('meal_type', radek.meal_type)
      .eq('diet_tags', radek.diet_tags)
      .eq('kcal_min', radek.kcal_min)
      .eq('kcal_max', radek.kcal_max)
      .gte('updated_at', od);
    q = radek.protein_hint == null ? q.is('protein_hint', null) : q.eq('protein_hint', radek.protein_hint);
 ...
 *           cilSlotu?: number|null, max_active_min?: number|null, pozadovano: number,
 *           priorita: number, zdroj: 'seed'|'demand' }} spec
 */
export async function objednejRecepty(spec, client = supabaseServer) {
  ...
  /* KANONICKÉ PÁSMO SLOTU MÁ PŘEDNOST — SE SEED OBJEDNÁVKOU BEZE ZMĚNY.
     Uživatelské pásmo (cil/2 až cil*2) tříštilo frontu: 100 otevřených
     položek pokrývalo 17 kombinací (slot × dieta × hint) a lišilo se jen
     pásmem, takže je unikát nespojil. Katalog je sdílený — kolik kalorií
     potřebuje konkrétní člověk, se řeší až škálováním porce při skládání
     jídelníčku. Viz KANONICKA_PASMA v lib/recipeGenerationBands.js.

     U DEMAND OBJEDNÁVKY KONKRÉTNÍHO SLOTU (`spec.cilSlotu`) SE KANONICKÉ
     PÁSMO SMÍ ROZŠÍŘIT — `pasmoPoptavky()`. Škálování porce mimo START je
     0,5–2,0× (lib/nutrition/portionScaling.js:146-155), takže recept
     použitelný pro tenhle konkrétní slot může ležet mimo to, co je pro chod
     TYPICKÉ. Kanonické pásmo 170–370 pro svačinu odmítalo recepty na
     380 kcal pro slot s cílem 369 (docs/DALSI_KROK.md 8.5) — `pasmoPoptavky()`
     rozšíří jen tu stranu, kterou cíl slotu doopravdy přesahuje, a rozšíření
     zaokrouhlí, ať se fronta znovu neroztříští jako před 25. 8. */
  const cilSlotu = Number(spec.cilSlotu);
  const maCilSlotu = spec.zdroj === 'demand' && Number.isFinite(cilSlotu) && cilSlotu > 0;
  const kanonicke = maCilSlotu
    ? pasmoPoptavky(spec.meal_type, cilSlotu)
    : kanonickePasmo(spec.meal_type);
  const pasmo = kanonicke
    ? { ...kanonicke, zmeneno: false, duvod: [] }
    : srovnejPasmo(spec);
  if (pasmo.zmeneno) {
  ...
  const radek = {
    ...
    /* STROP NA JEDNU OBJEDNÁVKU.
       Bez něj fronta narostla na 2 042 kusů ve 100 položkách, z toho jedna
       žádala 48. Víc než `MIN_RECEPTU_NA_SLOT` na jednu specifikaci nemá
       smysl: nad tím se slot přestává opakovat a další kusy jen odsouvají
       jinou díru. Když je potřeba víc, založí se objednávka znovu, až se
       tahle dodělá. */
    pozadovano: Math.min(Math.max(1, Number(spec.pozadovano) || 1), MAX_KUSU_NA_OBJEDNAVKU),
    ...
    //
    // `omezPodilProObjednavku()` je strop na to, co generátor umí vyrobit
    // (docs/DALSI_KROK.md 8.5) — jeden a tentýž pro SEED i DEMAND, protože
    // je to vlastnost modelu, ne toho, kdo objednávku založil.
    protein_hint: serializujHint({
      zdroj: spec.protein_zdroj ?? null,
      podil: omezPodilProObjednavku(spec.min_podil_bilkovin),
    }),
    // Tukový strop, docs/DALSI_KROK.md 8.8. Na rozdíl od bílkovin dostane
    // KAŽDÁ objednávka nějakou hodnotu, ne jen díra, na kterou přišel signál
    // — katalog je tučnější než cíl systematicky, ne jen v konkrétních
    // slotech, viz lib/plan/fatHint.js. `omezStropTukuProObjednavku()` vrátí
    // rozumný výchozí strop, i když `spec.max_podil_tuku` nikdo nezadal.
    fat_hint: omezStropTukuProObjednavku(spec.max_podil_tuku),
  };

  if (await nedavnoVycerpana(radek, client)) {
    return { created: false, duplicate: false, vycerpana: true, id: null };
  }
...
export async function objednejZNevyresenehoSlotu(kontext, client = supabaseServer) {
  ...
  return objednejRecepty({
    ...
    // Fallback pro neznámý meal_type (spadne do srovnejPasmo) — pro známý
    // chod se použije `cilSlotu` níž přes `pasmoPoptavky()`, viz objednejRecepty.
    kcal_min: maCil ? Math.round(cil / 2) : 200,
    kcal_max: maCil ? Math.round(cil * 2) : 900,
    cilSlotu: maCil ? cil : null,
    ...
    // stejně jako ten, co tam nebyl. `objednejRecepty()` podíl navíc ořízne
    // na `MAX_PODIL_OBJEDNAVKY` — co je nad ním, generátor stejně nevyrobí
    // (docs/DALSI_KROK.md 8.5).
...
export async function nactiFrontu(limit = 10, client = supabaseServer, opts = {}) {
  ...
  // Ruční spuštění konkrétní položky (`queue_id`) cap pokusů obchází — člověk
  // ví, proč ji chce zkusit znovu. Automatický běh bere jen položky, které
  // ještě nevyčerpaly `MAX_POKUSU_POLOZKY`: za neúspěšné pokusy se platí
  // stejně jako za úspěšné (PROMPT_NAKLADY_MINIMUM.md bod 2).
  if (opts.queueId != null) q = q.eq('id', opts.queueId);
  else q = q.lt('pokusu', MAX_POKUSU_POLOZKY);
