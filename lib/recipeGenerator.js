...
import { volejModel, estimateOpenAICostUSD } from './openai.js';
 ...
 * a čeština je jediné, co od modelu opravdu kupujeme. (Od 21. 9. 2026 je
 * výchozí gpt-4.1-mini, viz `RECIPE_GEN_MODEL` níž.)
 */
/*
 * PROMPT_NAKLADY_MINIMUM.md bod 1 (21. 9. 2026) — výchozí model je
 * `gpt-4.1-mini` ($0,40 / $1,60 za milion tokenů proti $2,50 / $10 u gpt-4o,
 * tedy ~6× levnější): generátor tvořil 99 % útraty ($19,04 z 30 dní).
 * Důvod pro 4o výš (mini v češtině vyrábí kostrbaté názvy) platil pro
 * gpt-4o-mini; u 4.1-mini se teprve měří. Měřítko je úspěšnost validace
 * podle modelu — `model` v řádku `recipe_generation_vysledek` (vráceno vs.
 * zapsáno). Když mini padá výrazně víc, je to nález k rozhodnutí, ne důvod
 * tiše vrátit 4o. Návrat bez deploye: env `RECIPE_GEN_MODEL=gpt-4o`.
 */
export const RECIPE_GEN_MODEL = process.env.RECIPE_GEN_MODEL?.trim() || 'gpt-4.1-mini';
...
/*
 * PROMPT_NAKLADY_AI.md (2026-09-18) — `RECIPE_GEN_RATES_USD_PER_MTOK` tu
 * dřív bylo zvlášť, se stejnými čísly jako `estimateOpenAICostUSD()`
 * v lib/openai.js — jedna cena udržovaná na dvou místech ručně. Sazby teď
 * bydlí jen v lib/openai.js (`OPENAI_MODEL_RATES_USD_PER_MTOK`).
 */
...
/**
 * NEJVIC NAZVU EXISTUJICICH RECEPTU, KTERE JDOU DO PROMPTU JAKO `uz_mame`.
 *
 * Zmereno na produkci 9. 9. 2026: slot `obed` ma 306 receptu a jejich nazvy
 * zaberou ~4 600 tokenu z ~9 500 tokenu vstupu jedne davky. Vstup je 73 %
 * ceny behu (gpt-4o, 2,50 USD/Mtok vstup proti 10 USD/Mtok vystup), takze
 * skoro polovina ceny generovani jde na seznam, o kterem komentar
 * u `MAX_KOMBINACI_V_PROMPTU` vys sam pise, ze modelu nepomaha - ctyri
 * konkretni shody, ktere mel videt, prehledl.
 *
 * Horsi nez cena je rust: seznam se posila CELY a roste s katalogem, takze
 * cena jednoho receptu roste s tim, kolik jich uz mame. Pri 3 000 receptech
 * ve slotu by `uz_mame` bylo ~15 000 tokenu na davku.
 *
 * Proti duplicitam nechrani tenhle seznam, ale `isDuplicateRecipe()`
 * v `recipeGeneratorRun.js` - ta porovnava proti CELEMU katalogu slotu
 * v kodu, ne v promptu, a strop se ji nedotkne. `uz_mame` zustava jako
 * napoveda, aby model nesahal po zjevne obsazenych nazvech; k tomu staci
 * kratky seznam tech nejrelevantnejsich. Poradi resi volajici
 * (`nazvyProPrompt()` v recipeGeneratorRun.js) - recepty se stejnou hlavni
 * bilkovinou jdou prvni, protoze prave u nich modelu hrozi duplicita.
 */
export const MAX_UZ_MAME = 60;
...
/**
 * SEZNAM JEDNOTEK BYL OTEVŘENÝ, SEZNAM SUROVIN UZAVŘENÝ — TA ASYMETRIE
 * ZPŮSOBILA „NEZNÁMOU SUROVINU", KTERÁ NEBYLA NEZNÁMÁ.
 *
 * docs/DALSI_KROK.md 8.9. Katalog počítá gramy jen pro `g` a `ml` — jiná
 * jednotka (`kus`, `kg`, `porce`, …) spadne na převodu v
 * `compute_nutrition_for_ingredients`, a dřív se ta chyba zapsala jako
 * neznámá SUROVINA (viz `units_unmatched` vs. `ingredients_unmatched`,
 * migrace 20260903210000). Změřeno: model v 100 % úspěšných receptů dnes už
 * `g`/`ml` sám používá — tohle je zábradlí pro těch pár procent, kdy ne.
 */
export const POVOLENE_JEDNOTKY = Object.freeze(['g', 'ml']);

/**
 * Recept s jednotkou mimo `POVOLENE_JEDNOTKY` se zahodí JEŠTĚ PŘED zápisem —
 * stejný vzor jako `surovinyMimoSeznam`, ale kontroluje JEDNOTKU, ne SUROVINU.
 * Bez tohohle by SQL převod selhal a `compute_nutrition_for_ingredients` by
 * to (od 8.9) správně nahlásil jako `units_unmatched` — tahle pojistka to
 * zachytí dřív a levněji, ještě před voláním DB.
 *
 * @param {{ ingredients: Array<{unit?: string}> }} recept
 * @param {string[]} [povoleneJednotky]
 * @returns {string[]} JEDNOTKY mimo seznam (ne suroviny), bez duplicit
 */
export function jednotkyMimoSeznam(recept, povoleneJednotky = POVOLENE_JEDNOTKY) {
  const povolene = new Set(povoleneJednotky);
  const mimo = [];
  for (const ing of recept?.ingredients || []) {
    const jednotka = String(ing?.unit ?? '');
    if (!povolene.has(jednotka) && !mimo.includes(jednotka)) mimo.push(jednotka);
  }
  return mimo;
}
 ...
 * @param {string[]} [spatneJednotky] `jednotkyMimoSeznam()` z předchozího pokusu — docs/DALSI_KROK.md 8.9
 * @param {string[]} [prekroceniStropuTuku] zprávy o `nad_stropem_tuku` z předchozího pokusu — docs/DALSI_KROK.md 8.13
 */
export function buildGeneratorInput(
  ...
  // POŘADÍ KLÍČŮ JE SOUČÁST NÁKLADŮ — PROMPT_NAKLADY_MINIMUM.md bod 4.
  //
  // OpenAI automaticky zlevní opakovaný ZAČÁTEK promptu (≥ 1 024 tokenů,
  // stejný prefix). Systémový prompt je statický a jde první; uživatelská
  // zpráva je JSON a její začátek se cachuje jen do prvního rozdílu. Dřív
  // stály nahoře `pocet_receptu`, `meal_type`, `kcal_*` (mění se každou
  // položku) a teprve za nimi `povolene_suroviny` — největší kus vstupu — takže
  // se necachoval vůbec. Teď jde napřed to, co se mění nejméně (slovník podle
  // diety, jednotky), pak co se mění podle slotu (`uz_mame`), a proměnné
  // zadání (počet, kalorie, bílkovina, opravy z minulého pokusu) na konec.
  // Pořadí klíčů NEMĚNIT bez měření `cached_tokens` v `ai_runs`.
  return {
    povolene_suroviny: povoleneSuroviny,
    // docs/DALSI_KROK.md 8.9 — seznam surovin je uzavřený, seznam jednotek
    // byl otevřený jen proto, že se to nikdy neřeklo výslovně. Posílá se
    // vždy, ne jen po chybě — je to zadání, ne oprava po faktu.
    povolene_jednotky: POVOLENE_JEDNOTKY,
    // Strop, ne cely katalog - viz MAX_UZ_MAME. Dedup proti duplicitam
    // bezi v kodu (isDuplicateRecipe) proti vsemu, tohle je jen napoveda.
    uz_mame: (uzMame || []).slice(0, MAX_UZ_MAME),
    pocet_receptu: kusu,
    ...
    // docs/DALSI_KROK.md 8.9 — JEDNOTKA z předchozího pokusu, ne surovina.
    // Nesmí jít do `tyhle_suroviny_neznam`: recept s `kg` u lososa nezná
    // problém losos, zná problém kg, a mísit ty dvě věci je přesně ta chyba,
    // kterou tenhle bod opravuje.
    ...(spatneJednotky.length ? { tyhle_jednotky_nepouzivej: spatneJednotky } : {}),
    ...
    /* TUKOVÝ STROP — docs/DALSI_KROK.md 8.8, obdoba bloku výš.
       ...
       (recept s NIŽŠÍM podílem je v pořádku).
       Od 8.13 (docs/DALSI_KROK.md) je nad tímhle zadáním ještě TVRDÁ
       validace v zapisRecept() (lib/recipeGeneratorRun.js) — ta ale
       nekontroluje přímo tohle číslo, nýbrž `validacniStropTuku()`
       (lib/plan/fatHint.js), který ho nikdy nepustí pod
       `MIN_TVRDY_STROP_TUKU` (0,45). Model tedy míří sem, na nižší číslo,
       ale i mírné přestřelení má šanci projít. */
    ...(Number.isFinite(Number(maxPodilTuku)) && Number(maxPodilTuku) > 0
      ...
      : {}),
    // docs/DALSI_KROK.md 8.13 — TVRDÝ strop z předchozího pokusu byl
    // překročen. Vlastní pole, stejný vzor jako `tyhle_jednotky_nepouzivej`
    // z 8.9: nese konkrétní číslo ("52 % kalorií z tuku, strop je 45 %"),
    // ne obecné "dej míň tuku", a NIKDY nejde do `tyhle_suroviny_neznam` —
    // není to název suroviny.
    ...(prekroceniStropuTuku.length ? { prekroceny_strop_tuku: prekroceniStropuTuku } : {}),
  };
}
 ...
 */
export function computeCostUsd(input_tokens = 0, output_tokens = 0, cached_tokens = 0) {
  return Number(estimateOpenAICostUSD(RECIPE_GEN_MODEL, input_tokens, output_tokens, cached_tokens).toFixed(6));
}
 ...
 */
export async function generateRecipeBatch(vstup) {
  const completion = await volejModel(
    {
      ...
      promptSha256: RECIPE_GEN_PROMPT_SHA256,
      messages: [
  ...
  return {
    ...
    usage,
    cost_usd: computeCostUsd(usage.input_tokens, usage.output_tokens, usage.cached_tokens),
  };
}
