 ...
 *
 * Zápis je dvoufázový a nikdy nenechá v tabulce recept bez nutrice:
 *   1. INSERT s active = false, pending_review = true, source = 'llm_generated'
 *   2. compute_recipe_nutrition
 *   3. complete = true  → dopsat kcal a makra
 *      complete = false → recept SMAZAT (jako by nikdy nevznikl)
 */
...
import { nactiFrontu } from './recipeGenerationQueue';
...
/**
 * @param {{ dryRun?: boolean, limit?: number, client?: any, openai?: OpenAI }} [opts]
 */
export async function runRecipeGenerator(opts = {}) {
  ...
  const fronta = await nactiFrontu(20, client);
  if (!fronta.length) {
    return { skipped: true, reason: 'prazdna_fronta', zapsano: 0 };
  }

  const povolene = await nactiPovoleneSuroviny(client);
  const povoleneSet = new Set(povolene.map(normalizeRecipeName));
  ...
  if (dryRun) {
    ...
    for (const p of fronta) {
      ...
      plan.push({
        ...
        vstup_pro_model: buildGeneratorInput(p, povolene, existujici.map((e) => e.name_cs), kusu),
      });
      zbyva -= kusu;
    }
    return {
  ...
  for (const p of fronta) {
    ...
    for (let pokus = 1; pokus <= 2 && zapsanoZPolozky < kusu; pokus += 1) {
      ...
      try {
        davka = await generateRecipeBatch(
          openai,
          buildGeneratorInput(p, povolene, existujici.map((e) => e.name_cs), kusu - zapsanoZPolozky, nedohledane),
        );
      } catch (err) {
      ...
      await client.from('ai_runs').insert({
        ...
        result: { queue_id: p.id, pokus, vraceno: davka.recepty.length, zapsano: zapsanoZPolozky },
      });
    }
    ...
    await client.from('recipe_generation_queue').update({
      ...
      stav: hotovo ? 'done' : (zapsanoZPolozky > 0 ? 'pending' : 'failed'),
      posledni_chyba: zapsanoZPolozky === 0 ? (nedohledane.join(', ') || 'nic nezapsáno') : null,
...
/**
 * Dvoufázový zápis. Recept bez úplné nutrice se z tabulky zase odstraní —
 * katalog nikdy neobsahuje řádek, ke kterému neumíme spočítat čísla.
 */
async function zapisRecept(r, polozka, client) {
  ...
  const { data: nutrice, error: chybaNutrice } = await client
    .rpc('compute_recipe_nutrition', { p_recipe_id: vlozeny.id });
  const n = Array.isArray(nutrice) ? nutrice[0] : nutrice;

  if (chybaNutrice || !n?.complete) {
    await client.from('recipes_catalog').delete().eq('id', vlozeny.id);
    return {
      ok: false,
      duvod: 'nutrice_neuplna',
      detail: n?.ingredients_unmatched || [chybaNutrice?.message || 'neznámé'],
    };
  }

  const { error: chybaUpdate } = await client.from('recipes_catalog').update({
    kcal: n.kcal,
  ...
  if (chybaUpdate) {
    await client.from('recipes_catalog').delete().eq('id', vlozeny.id);
    return { ok: false, duvod: 'update_nutrice_selhal', detail: [chybaUpdate.message] };
  }
