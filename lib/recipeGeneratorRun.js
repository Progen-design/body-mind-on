...
import { assertOpenAIDailyBudget } from './openai.js';
...
/**
 * Aktivní recepty chodu pro kontrolu „potřebuje slot ještě recept?“ —
 * jen sloupce, které kontrola čte (`lib/recipeGenerationNeed.js`).
 */
async function nactiAktivniProKontrolu(mealType, client) {
  const { data, error } = await client
    .from('recipes_catalog')
    .select('diet_tags, kcal, protein_g')
    .eq('meal_type', mealType)
    .eq('active', true);
  if (error) throw new Error(`recipes_catalog: ${error.message}`);
  return data || [];
...
/** Poptávka za posledních `OKNO_POPTAVKY_DNI` dní — z plánů skutečných uživatelů. */
async function nactiPoptavku(client) {
  const od = new Date(Date.now() - OKNO_POPTAVKY_DNI * 86_400_000).toISOString().slice(0, 10);
  const { data, error } = await client
    .from('catalog_slot_demand')
    .select('meal_type, diet_tags, nevyresenych, kandidatu_min')
    .gte('den', od);
  if (error) throw new Error(`catalog_slot_demand: ${error.message}`);
  return data || [];
}

/**
 * ROZPAD `zahozeno` PODLE DŮVODU — docs/DALSI_KROK.md 8.19.
 *
 * Na jeden zapsaný recept připadá osm až devět zahozených (12/93, 8/98,
 * 12/56 na produkci 4.–6. 9. 2026), ale `zahozeno` v `ai_runs.result`
 * bylo jedno slepené číslo — z dat nešlo poznat, jestli za tím stojí
 * dedup, kalorické pásmo, bílkoviny, nedohledané suroviny, nebo něco
 * jiného. Každý záznam v `zahozeno` už má `duvod` (nastavuje ho každé
 * `zahozeno.push()` v `provedBeh()` níž a `zapisRecept()`), takže rozpad
 * je jen sečtení toho, co tam už je — žádná nová validace ani nový důvod
 * zahození, jen viditelnost. Součet hodnot je vždy roven `zahozeno.length`,
 * protože jde o rozklad TÉHOŽ pole, ne o nezávisle vedené počítadlo,
 * které by se s ním mohlo rozejít.
 *
 * @param {Array<{duvod?: string}>} zahozeno
 * @returns {Record<string, number>}
 */
/**
 * Nazvy existujicich receptu pro `uz_mame` - nejrelevantnejsi prvni.
 *
 * `buildGeneratorInput` seznam orizne na `MAX_UZ_MAME`, takze zalezi na
 * poradi: recepty se stejnou hlavni bilkovinou jako objednavka jsou ty,
 * u kterych modelu opravdu hrozi, ze vymysli duplicitu. Zbytek jde za ne
 * a pri pretečeni vypadne. Bez hlavni bilkoviny se poradi nemeni.
 *
 * Na deduplikaci to nema vliv - `isDuplicateRecipe()` porovnava proti
 * celemu `existujici`, ne proti tomuhle vyberu.
 *
 * @param {Array<{name_cs?: string, ingredients?: Array<object>}>} existujici
 * @param {string|null} [hlavniBilkovina]
 * @returns {string[]}
 */
export function nazvyProPrompt(existujici, hlavniBilkovina = null) {
  const vse = Array.isArray(existujici) ? existujici : [];
  if (!hlavniBilkovina) return vse.map((e) => e?.name_cs).filter(Boolean);
  const shoda = [];
  const zbytek = [];
  for (const e of vse) {
    const nazev = e?.name_cs;
    if (!nazev) continue;
    if (receptSplnujeBilkovinu(e, hlavniBilkovina)) shoda.push(nazev);
    else zbytek.push(nazev);
  }
  return shoda.concat(zbytek);
}

export function rozpadZahozeniPodleDuvodu(zahozeno) {
  /** @type {Record<string, number>} */
  const rozpad = {};
  for (const z of Array.isArray(zahozeno) ? zahozeno : []) {
    const duvod = String(z?.duvod || 'neznamy_duvod');
    rozpad[duvod] = (rozpad[duvod] || 0) + 1;
  }
  return rozpad;
}
 ...
 *
 * PROMPT_NAKLADY_AI.md (2026-09-18) — `cost_usd` tady bylo `vysledek.cena_usd`,
 * tedy SOUČET přes celý běh, zatímco KAŽDÉ jednotlivé volání v běhu už svou
 * skutečnou cenu zapisuje samo (`volejModel()`, purpose `recipe_generation`,
 * viz níž). `openai_daily_usage` sčítá `cost_usd` přes celou `ai_runs` bez
 * ohledu na `purpose` (migrace 20260828100000), takže tenhle řádek počítal
 * útratu generátoru DRUHÝ krát navrch — přesně ta migrace, co se jinak
 * snažila dvojímu započítání zabránit, a její komentář to i předpokládal
 * ("Řádky recipe_generator_beh mají nulovou cenu"), jen to sem nikdy
 * nedotáhl. `cena_usd` zůstává v `result` pro čitelnost běhu, jen se
 * nesčítá s dolarovými sloupci.
 *
 * PROMPT_NAKLADY_AI_DODATEK.md (2026-09-18) — `0` bylo pořád číslo, které
 * by se dalo sečíst (a nesečetlo se jen díky filtru v `openai_daily_usage`,
 * který `audit-unit-economics.mjs` neměl). `null` říká "tenhle řádek cenu
 * nenese" explicitněji a `purpose` je navíc v `AGREGACNI_PURPOSY`
 * (lib/openai.js) — kdokoli cenu sčítá přes `sectiCenuBezAgregaci()`, tenhle
 * řádek automaticky vynechá, ať už nese 0, null, nebo cokoli jiného.
 */
async function zapisZaznamOBehu(client, vysledek, zacatek) {
  try {
    ...
    await client.from('ai_runs').insert({
      ...
      cost_usd: null,
      result: {
        cena_usd_za_beh: Number(vysledek?.cena_usd ?? 0),
        ...
        // docs/DALSI_KROK.md 8.19 — rozpad `zahozeno` podle `duvod`, ať jde
        // z jednoho běhu přečíst „z 93 zahozených bylo X duplicit, Y mimo
        // kalorické pásmo, Z pod bílkovinami, W nedohledaná surovina" místo
        // jednoho slepeného čísla. Součet hodnot vždy dá `zahozeno` výš.
        zahozeno_dle_duvodu: rozpadZahozeniPodleDuvodu(vysledek?.zahozeno),
...
/**
 * @param {{ dryRun?: boolean, limit?: number, queueId?: number|null, client?: any }} [opts]
 */
...
async function provedBeh(opts = {}) {
  ...
  // ROZPOČTOVÁ POJISTKA PLATÍ I NA GENERÁTOR.
  //
  // `assertOpenAIDailyBudget()` do teď hlídala jen `runAgent()` — tedy TEDa,
  // plány a ostatní agenty. Generátor receptů, který je ZDALEKA největší
  // položka (18,71 USD za 30 dní proti haléřům u TEDa), jel mimo ni. 9. 9.
  // 2026 na tom došel kredit: OpenAI vrátilo 429 a spadlo s tím i generování
  // plánů a TED, tedy věci, které uživatel opravdu vidí.
  //
  // Strop `RECIPE_GEN_MAX_PER_DAY` tohle neřeší: počítá KUSY receptů, ne
  // peníze, a běh, ve kterém model vrátí dávku a nic neprojde validací,
  // stojí stejně jako úspěšný, ale strop nesníží.
  //
  // Kontrola je tady, ne až u jednotlivého volání: půlka utraceného běhu
  // je horší než žádný. Suchý běh nic neplatí, ten se neptá.
  if (!dryRun) {
    // `zkontrolujRozpocet` je vstřikovatelný ze stejného důvodu jako `client`
    // a `openai` — test nesmí sahat na produkční tabulku útraty.
    // Generátor je `batch` — stropem $0,50 se zastaví PRVNÍ a TED s
    // uživatelskými akcemi zůstanou pod svým samostatným stropem.
    const zkontroluj = opts.zkontrolujRozpocet || (() => assertOpenAIDailyBudget('batch'));
    const rozpocet = await zkontroluj();
    if (!rozpocet.allowed) {
      return {
        skipped: true,
        reason: 'denni_rozpocet_vycerpan',
        utraceno_usd: rozpocet.spent,
        rozpocet_usd: rozpocet.budget,
        zapsano: 0,
      };
    }
  }
  ...
  let fronta = await nactiFrontu(20, client, { queueId: opts.queueId ?? null });
  if (!fronta.length) {
    return { skipped: true, reason: opts.queueId ? 'polozka_nenalezena' : 'prazdna_fronta', zapsano: 0 };
  }

  // JEN S REÁLNOU POPTÁVKOU — PROMPT_NAKLADY_MINIMUM.md bod 3.
  //
  // Cron to zapíná (`jenSPoptavkou`). Objednávka bez poptávky za poslední týden
  // (seed, doplňování „pro jistotu“) se automaticky nevaří — katalog má přes
  // tisíc receptů a předspouštěcí fáze nemá uživatele, pro které by se
  // doplňoval. Prázdná poptávka = běh končí BEZ volání modelu. Ruční běh
  // (admin endpoint, `queue_id`) bránu nemá: člověk ví, co chce.
  if (opts.jenSPoptavkou === true && opts.queueId == null) {
    const sloty = slotySPoptavkou(await nactiPoptavku(client));
    const sPoptavkou = fronta.filter((p) => maPoptavku(p, sloty));
    if (!sPoptavkou.length) {
      return { skipped: true, reason: 'zadna_poptavka', polozek_bez_poptavky: fronta.length, zapsano: 0 };
    }
    fronta = sPoptavkou;
  }
    ...
    for (const p of fronta) {
      ...
      plan.push({
        ...
        vstup_pro_model: buildGeneratorInput(
          p, proPolozku(p).nazvy, nazvyProPrompt(existujici), kusu,
  ...
  // --- Ostrý běh -----------------------------------------------------------
  // Vlastní klient tu dřív byl schválně, aby `lib/openai.js` úplně obešel —
  // přesně ten stav, co PROMPT_NAKLADY_AI.md zavírá. `generateRecipeBatch()`
  // teď volá `volejModel()` sama, žádný klient se sem nepředává.
  let zapsano = 0;
  let cenaCelkem = 0;
  const zahozeno = [];
  const chyby = [];
  ...
  for (const p of fronta) {
    if (zbyva <= 0) break;
    let kusu = Math.min(p.pozadovano - p.vyrobeno, RECIPE_GEN_BATCH, zbyva);
    if (kusu <= 0) continue;

    // POTŘEBUJE SLOT JEŠTĚ RECEPT? — PŘED voláním modelu, ne po něm.
    // Poptávka se od založení objednávky mohla naplnit (jiný běh, Spoonacular
    // import, ruční zásah). `nadbytecna` do 21. 9. 2026 vznikla jen jednou,
    // jednorázově migrací 20260904150000 — za běhu se nikde nekontrolovala,
    // takže se platilo za dávku pro slot, který už měl dost receptů.
    // Ruční běh s `queue_id` kontrolu obchází.
    if (opts.queueId == null) {
      const chybi = potrebaKusu(p, await nactiAktivniProKontrolu(p.meal_type, client));
      if (chybi <= 0) {
        await client.from('recipe_generation_queue').update({
          stav: 'nadbytecna',
          posledni_chyba: 'slot_uz_naplnen: katalog ma dost vyhovujicich receptu, model se nevolal',
          updated_at: new Date().toISOString(),
        }).eq('id', p.id);
        nadbytecnych += 1;
        continue;
      }
      kusu = Math.min(kusu, chybi);
    }
    ...
    // ZMERENE PODILY, NE JEN POCET.
    //
    // `zahozeno_pod_cilem_bilkovin` je nejvetsi duvod zahozeni (842 z 1 568
    // za 30 dni na produkci, mereno 9. 9. 2026), ale ciste cislo nerekne,
    // jestli recepty cil minuly o dve procenta nebo o patnact. Bez toho se
    // neda rozhodnout, jestli utahnout prompt, nebo povolit branu.
    const podCilemPct = [];
    ...
    for (let pokus = 1; pokus <= 2 && zapsanoZPolozky < kusu; pokus += 1) {
      ...
      try {
        davka = await generateRecipeBatch(
          buildGeneratorInput(
            p,
            povoleneProPolozku,
            nazvyProPrompt(existujici, hlavniBilkovina),
      ...
      for (const r of davka.recepty) {
        ...
        if (vysledek.ok) {
          ...
          if (vysledek.duvod === 'pod_cilem_bilkovin') {
            podCilemBilkovin += 1;
            if (Number.isFinite(Number(vysledek.zmereny_podil))) {
              podCilemPct.push(Math.round(Number(vysledek.zmereny_podil) * 100));
            }
          }
      ...
      // PROMPT_NAKLADY_AI.md (2026-09-18) — `generateRecipeBatch()` teď volá
      // `volejModel()`, která pro TOTÉŽ volání už zapsala vlastní řádek do
      // `ai_runs` (purpose `recipe_generation`, se skutečnou cenou a tokeny).
      // Tenhle řádek je bohatší diagnostika PO zpracování dávky (zahozeno
      // podle důvodu, hlavní bílkovina, …), kterou `volejModel()` v době
      // volání ještě neznala — jméno je proto jiné a dolarové sloupce jsou
      // `null` (PROMPT_NAKLADY_AI_DODATEK.md 2026-09-18 — `null`, ne `0`: viz
      // stejná poznámka u `zapisZaznamOBehu`), ať `openai_daily_usage`
      // nesečte stejné volání dvakrát. `purpose` je navíc v
      // `AGREGACNI_PURPOSY` (lib/openai.js) pro každý budoucí ruční součet.
      // Skutečná cena/tokeny zůstávají čitelné v `result` níž.
      await client.from('ai_runs').insert({
        purpose: 'recipe_generation_vysledek',
        model: RECIPE_GEN_MODEL,
        temperature: RECIPE_GEN_TEMPERATURE,
        prompt_sha256: RECIPE_GEN_PROMPT_SHA256,
        input_tokens: null,
        ...
        cost_usd: null,
        result: {
          // PROMPT_NAKLADY_MINIMUM.md bod 1 — model v řádku, ať jde po týdnu
          // spočítat úspěšnost validace podle modelu (vraceno vs. zapsáno).
          model: RECIPE_GEN_MODEL,
          skutecne_input_tokens: davka.usage.input_tokens,
          skutecne_cached_tokens: davka.usage.cached_tokens ?? 0,
          skutecne_output_tokens: davka.usage.output_tokens,
          skutecna_cena_usd: davka.cost_usd,
          queue_id: p.id,
          pokus,
          ...
          zahozeno_pod_cilem_bilkovin_pct: podCilemPct,
    ...
    // Pád infrastruktury (kredit, klíč, síť) pokus NEPOČÍTÁ — objednávka za to
    // nemůže. Bez toho by výpadek OpenAI spálil pokusy celé fronty.
    const pokusuPoBehu = infrastruktura ? p.pokusu : p.pokusu + 1;
    // Položka, která něco vyrobila, ale ještě není hotová, se vrací do fronty
    // jen do `MAX_POKUSU_POLOZKY`; pak je `failed` a automaticky se nebere.
    const vycerpana = !hotovo && !infrastruktura && zapsanoZPolozky > 0 && pokusuPoBehu >= MAX_POKUSU_POLOZKY;
    await client.from('recipe_generation_queue').update({
      ...
      stav: hotovo
        ? 'done'
        : (vycerpana ? 'failed' : (zapsanoZPolozky > 0 || infrastruktura ? 'pending' : 'failed')),
      posledni_chyba: zapsanoZPolozky === 0
        ? popisChybyBehu(chybaVolani, nedohledane)
        : (vycerpana ? `vycerpane_pokusy: vyrobeno ${p.vyrobeno + zapsanoZPolozky} z ${p.pozadovano}` : null),
      updated_at: new Date().toISOString(),
    }).eq('id', p.id);
  ...
  return { dry_run: false, zapsano, cena_usd: Number(cenaCelkem.toFixed(6)), zahozeno, chyby, nadbytecnych };
}
...
async function zapisRecept(r, polozka, client) {
  ...
  // BRÁNA KVALITY POSTUPU — lib/plan/kvalitaPostupu.js. Recept, kterému model
  // vrátil postup pod laťkou (míň než 4 kroky, vata typu "Připrav X" bez
  // obsahu, kcal/makra v textu, prázdný název), se nezapíše. `postup.ok` se
  // řídí VÝHRADNĚ blokujícími pravidly — `varovani` (chybějící surovina,
  // tepelná úprava bez teploty/času, surovina mimo ingredients) se jen
  // zaloguje, nikdy zápis neblokuje (druhé kolo opravy laťky, 9. 9. 2026 —
  // enumerace sloves/surovin jako blokující dávala stovky falešných zásahů).
  const postup = posudPostup({ kroky: r.instructions, suroviny, nazev: r.name_cs });
  if (!postup.ok) {
    return { ok: false, duvod: 'postup_pod_latkou', detail: postup.duvody };
  }
  if (postup.varovani.length) {
    console.log(JSON.stringify({
      source: 'recipe-generator',
      event: 'postup_varovani',
      name_cs: r.name_cs,
      varovani: postup.varovani,
    }));
  }
