    ...
    for (let pokus = 1; pokus <= 2 && zapsanoZPolozky < kusu; pokus += 1) {
      ...
      for (const r of davka.recepty) {
        ...
        if (vysledek.ok) {
          ...
          // Duvod "pod_cilem_bilkovin" do promptu nepatri — nejsou to nazvy
          // surovin, ktere model nezna, a `tyhle_suroviny_neznam` by z toho
          // udelalo nesmysl.
          if (vysledek.duvod !== 'pod_cilem_bilkovin' && vysledek.detail?.length) {
      ...
      await client.from('ai_runs').insert({
        ...
        result: {
          ...
          // Jen zadání do promptu, ne důvod zamítnutí — tuk se nezahazuje
          // (docs/DALSI_KROK.md 8.8). Slouží k porovnání se skutečným
          // podílem tuku u zapsaných receptů po nasazení.
