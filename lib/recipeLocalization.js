...
export async function batchTranslateRecipeTitlesToCzech(items) {
  ...
  if (!process.env.OPENAI_API_KEY) return items.map((i) => i.title || 'Jídlo');

  const toTranslate = items.map((i) => (i.title || '').trim().slice(0, 80)).filter(Boolean);
  if (!toTranslate.length) return items.map(() => 'Jídlo');
  ...
  try {
    ...
    if (!raw) return toTranslate;
    const parsed = JSON.parse(raw);
    const titles = Array.isArray(parsed?.titles) ? parsed.titles : toTranslate;
    let idx = 0;
...
export async function translateRecipeTitleToCzech(title, recipeId = null) {
  if (!title || typeof title !== 'string') return 'Jídlo (neověřeno)';
  ...
  try {
    ...
    const display_name_cs = raw && raw.length > 1 ? raw : title;
...
export async function getLocalizedRecipe(recipeId, recipe) {
  if (!recipeId || !recipe) {
    return {
      display_name_cs: recipe?.title || 'Recept',
      ingredients_cs: [],
      instructions_cs: [],
    };
  }
  ...
  let display_name_cs = recipe.title || 'Recept';
...
Žádný úvod, žádný markdown. Zachovej množství a jednotky.`,
      ...
      // Fallback: přeložit jen název
      display_name_cs = await translateRecipeTitleToCzech(recipe.title, recipeId);
    }
  }
