...
import { isGenericUserMealLabel } from './mealDisplayNameHelpers.js';
...
export async function batchTranslateRecipeTitlesToCzech(items) {
  ...
  try {
    ...
    return items.map((item) => {
      ...
      if (!r || r === t || isGenericUserMealLabel(r)) return '';
      return r;
    });
  } catch {
    return items.map(() => '');
...
export async function translateRecipeTitleToCzech(title, recipeId = null) {
  ...
  try {
    ...
    const display_name_cs =
      raw && raw.length > 1 && !isGenericUserMealLabel(raw) ? raw : '';
  ...
  if (Array.isArray(recipe.analyzedInstructions)) {
    for (const block of recipe.analyzedInstructions) {
      if (block?.steps?.length) {
        instructions.push(...block.steps.map((s) => s.step || '').filter(Boolean));
      }
    }
  }
  if (instructions.length === 0 && typeof recipe.instructions === 'string' && recipe.instructions.trim()) {
    instructions.push(
      ...recipe.instructions
        .replace(/<[^>]+>/g, '')
        .split(/\n+/)
...
Žádný úvod, žádný markdown. Zachovej množství a jednotky.`,
      ...
      if (raw) {
        const parsed = JSON.parse(raw);
        if (parsed.display_name_cs) {
          const d = String(parsed.display_name_cs).trim();
          if (d && !isGenericUserMealLabel(d)) display_name_cs = d;
        }
      ...
      const tt = (t && String(t).trim()) ? String(t).trim() : '';
      const rt = (recipe.title || '').trim();
      display_name_cs = tt || (!isGenericUserMealLabel(rt) ? rt : '') || 'Recept';
    }
  }
