 ...
 *
 * `cilovyPodilBilkovin` je nepovinný. Bez něj se rank chová přesně jako dřív,
 * takže volající, které bílkoviny neřeší (náhradník jídla, START knihovna),
 * nemusí nic měnit.
 *
 * @param {object} row
 ...
 */
export function catalogPickRank(row, slotTarget, mealType = 'lunch', cilovyPodilBilkovin = null) {
  ...
  return kcalDiff * 1.15 + bilkoviny - simplicity * 2.8;
}
 ...
 */
export function sortCatalogRowsForSimplePick(rows, slotTarget, mealType = 'lunch', cilovyPodilBilkovin = null) {
  const SIMPLE_FLOOR = -35;
  const scored = (rows || []).map((row) => ({
    row,
    rank: catalogPickRank(row, slotTarget, mealType, cilovyPodilBilkovin),
