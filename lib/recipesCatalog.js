...
async function resolveScaledCatalogSlot(slotMeal, ctx) {
  ...
  if (simpleStartMode && slotMeal?.planner_source === 'simple_meal_planner_agent' && hasSimpleStartRecipeTitle(slotMeal?.name_cs, mealType)) {
    return {
      row: null,
      meal: buildSimpleStartLibraryMeal(slotMeal.name_cs, mealType, {
        planner_source: slotMeal?.planner_source || null,
      }),
    };
  }
  ...
  if (!row) {
    if (simpleStartMode) {
      if (slotMeal?.planner_source === 'simple_meal_planner_agent' && hasSimpleStartRecipeTitle(slotMeal?.name_cs, mealType)) {
        return {
          row: null,
          meal: buildSimpleStartLibraryMeal(slotMeal.name_cs, mealType, {
            planner_source: slotMeal?.planner_source || null,
      ...
      return {
        row: null,
        meal: buildStartSafeFallbackMeal(slotMeal, slotTarget, slotSalt),
      };
    }
    return { row: null, meal: null };
  }

  const finalReason = simpleStartMode ? getFullContentStartBlockReason(row, mealType, slotMeal) : null;
  if (finalReason) {
    if (slotMeal?.planner_source === 'simple_meal_planner_agent' && hasSimpleStartRecipeTitle(slotMeal?.name_cs, mealType)) {
      return {
        row: null,
    ...
    logCatalogSimpleStart('ingredient complexity failed', {
      catalog_id: row.id,
      meal_type: mealType,
      agent_name: slotMeal?.name_cs ?? null,
      reason: finalReason,
      matchedTerm: finalReason,
    });
    logCatalogSimpleStart('fallback used because catalog detail too complex', {
      catalog_id: row.id,
      meal_type: mealType,
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (let mi = 0; mi < (day.meals ?? []).length; mi++) {
      ...
      if (row?.id) {
        ...
        verified++;
      } else if (meal.catalog_source === 'start_safe_fallback' || meal.catalog_source === 'simple_start_fallback') {
        startFallbackCount++;
        verified++;
      } else {
        unverified++;
    ...
    boostDayMealsToCalorieTarget(dayMeals, dailyTarget, 0.85);
