...
export async function resolveMealsFromCatalog(mealPlan, dietType, opts = {}) {
  ...
  /** Pool for honesty fill extras — snacks + denser mains from START catalog. */
  ...
  if (simpleStartMode) {
    try {
      ...
      console.warn('[catalog-simple-start] fill candidates fetch failed', err?.message || err);
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    let dayHonesty = null;
    if (simpleStartMode) {
      // Deficit → flex staples + add catalog meals; never invent kcal via multiplier alone
