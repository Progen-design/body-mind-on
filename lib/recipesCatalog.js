...
import { objednejZNevyresenehoSlotu, objednejZNizkeNabidky, zalogujPoptavkuSlotu } from './recipeGenerationQueue.js';
...
export async function pickSeededCatalogRecipe(params, slotTarget, pickSeed, slotSalt, pickOpts = {}) {
  ...
  const pickOptions = {
    ...
    // Cílový podíl bílkovin pro ZBYTEK dne. Nepovinný: bez něj se řadí jen
    // podle kalorií a jednoduchosti, jako před 23. 8. 2026.
...
export async function resolveMealsFromCatalog(mealPlan, dietType, opts = {}) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (let mi = 0; mi < (day.meals ?? []).length; mi++) {
      ...
      const { row, meal } = await resolveScaledCatalogSlot(m, {
        ...slotCtx,
        slotSalt,
        usedTodayIds,
        cilovyPodilBilkovin,
