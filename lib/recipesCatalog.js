...
async function resolveScaledCatalogSlot(slotMeal, ctx) {
  ...
  if (!row) {
    ...
    row = await pickClosestCatalogRecipe({
      ...
      limit: 48,
      simpleStartMode: false,
      // Still respect B1 source filter if set
      ...(Array.isArray(START_CATALOG_SOURCE_FILTER) && START_CATALOG_SOURCE_FILTER.length
        ? {}
        : {}),
    }, slotTarget);

    // If B1 filter blocked emergency (simpleStartMode false above bypasses start hard filter
    // but source filter only applies when simpleStartMode true). Re-fetch with source filter:
...
export async function resolveMealsFromCatalog(mealPlan, dietType, opts = {}) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    let dayHonesty = null;
    if (simpleStartMode) {
      // Deficit → add library meals; never invent kcal via portion_multiplier > 1.15
      dayHonesty = fillDayCaloriesByAddingLibraryMeals(dayMeals, dayTarget);
    } else {
    ...
    resolved.push({
      ...
      meals: dayMeals,
      _day_kcal: dayKcal,
      daily_achieved_kcal: dayHonesty?.achieved_kcal ?? dayKcal,
