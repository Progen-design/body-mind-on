...
import { objednejZNevyresenehoSlotu, objednejZNizkeNabidky } from './recipeGenerationQueue';
...
export async function fetchCatalogCandidates(params) {
  ...
  // Diverzita (nepoužít stejný recept dvakrát) je jediné, co smí fallback uvolnit.
  ...
  if (rows.length < limit) {
    ...
    // nabídka došla. Objednávka se založí jen jednou — duplicitu odmítne
    // unikátní index ve frontě, ne tenhle kód.
    objednejZNizkeNabidky({
      mealType,
      dietTags,
      kcalMin: Math.max(80, Math.floor(minKcal)),
      kcalMax: Math.ceil(maxKcal),
...
export async function resolveMealsFromCatalog(mealPlan, dietType, opts = {}) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (let mi = 0; mi < (day.meals ?? []).length; mi++) {
      ...
      if (!meal) {
        // Prázdné jídlo se do plánu nikdy nedostane. Radši plán nedoručíme.
