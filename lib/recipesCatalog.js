...
export async function pickSeededCatalogRecipe(params, slotTarget, pickSeed, slotSalt, pickOpts = {}) {
  ...
  return pickFromTopKCatalogRow(rows, slotTarget, pickSeed, slotSalt, topK, {
...
export function catalogRowToStructuredMeal(row, slotMeal = {}, scaled = null) {
  ...
  const display_name_cs = simplifyMealDisplayName(rawName, slotMeal.type || row.meal_type || 'lunch');
...
async function resolveScaledCatalogSlot(slotMeal, ctx) {
  ...
  const pickFn = (params, target, salt) =>
    pickSeededCatalogRecipe(params, target, catalogPickSeed, salt, { simpleStartMode });

  let row = await pickFn(pickParams, slotTarget, slotSalt);
  ...
  if (!row) {
    if (simpleStartMode) {
      console.log('[catalog-simple-start] fallback used', {
        mealType,
        slotTarget,
        slotSalt,
      });
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (let mi = 0; mi < (day.meals ?? []).length; mi++) {
      ...
      if (row?.id) {
        ...
        verified++;
      } else if (meal.catalog_source === 'start_safe_fallback') {
        startFallbackCount++;
        verified++;
      } else {
        unverified++;
