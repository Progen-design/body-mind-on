...
export async function resolveMealsFromCatalog(mealPlan, dietType, opts = {}) {
  ...
  // docs/DALSI_KROK.md 9.1/B2 — jen MĚŘENÍ, neopravuje se: kolik kandidátů
  // po dietním filtru zbylo pro jednotlivé typy jídel. Číslo je stropované
  // parametrem `limit` níž, takže neříká "kolik jich celkem existuje", ale
  // "kolik jich bylo k dispozici výběru" — přesně to, co potřebujeme rozlišit
  // málo receptů v katalogu od vah ve výběru.
  let kandidatuPoDietnimFiltru = { snack: 0, lunch: 0, dinner: 0 };
  try {
    ...
    kandidatuPoDietnimFiltru = {
      snack: snacks?.length ?? 0,
      lunch: lunches?.length ?? 0,
      dinner: dinners?.length ?? 0,
    };
  } catch (err) {
  ...
  // TREFA DO SACHARIDŮ — docs/DALSI_KROK.md 9.1/B2. Zrcadlo bloků výš, teď
  // pro třetí makro — bez něj nebylo z logu vidět, o kolik se den liší od
  // cíle ve VŠECH třech makrech, jen ve dvou z nich. Jen měření, žádný práh
  // tady nic neopravuje ani nezamítá.
  const cilSacharidu = Number(targets?.carbs_g);
  let trefaSacharidu = null;
  if (Number.isFinite(cilSacharidu) && cilSacharidu > 0 && resolved.length > 0) {
    const denniSacharidy = resolved.map((den) =>
      (den.meals || []).reduce((s, m) => {
        const c = Number(m?.carbs_g);
        return s + (Number.isFinite(c) && c > 0 ? c : 0);
      }, 0)
    );
    const prumer = denniSacharidy.reduce((a, b) => a + b, 0) / denniSacharidy.length;
    trefaSacharidu = {
      cil_g: Math.round(cilSacharidu),
      prumer_g: Math.round(prumer),
      min_g: Math.round(Math.min(...denniSacharidy)),
      max_g: Math.round(Math.max(...denniSacharidy)),
      procent_cile: Math.round((prumer / cilSacharidu) * 100),
    };
    console.log('[catalog-resolve] trefa do sacharidu', trefaSacharidu);
  }

  resolved._diag = {
    protein_trefa: trefaBilkovin,
    fat_trefa: trefaTuku,
    carbs_trefa: trefaSacharidu,
    kandidatu_po_dietnim_filtru: kandidatuPoDietnimFiltru,
  ...
  console.log('[catalog-resolve] resolveMealsFromCatalog complete', {
    ...
    // docs/DALSI_KROK.md 9.1/B2 — jen měření. U omezených diet (vegetarián,
    // bez lepku, bez laktózy) plán míjí makra i při správném cíli; nejde
    // dnes rozlišit, jestli je vinen malý katalog, nebo váhy ve výběru.
    // Tahle trojice a `kandidatu_po_dietnim_filtru` výš to má rozhodnout.
    protein_trefa: trefaBilkovin,
    carbs_trefa: trefaSacharidu,
    fat_trefa: trefaTuku,
    kandidatu_po_dietnim_filtru: kandidatuPoDietnimFiltru,
