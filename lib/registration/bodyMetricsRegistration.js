...
export function parseAndValidateRegistrationBody(b = {}) {
  ...
  if (birthDateRaw) {
    ...
    if (birthValidation.age < 13 || birthValidation.age > 90) {
      return { ok: false, status: 400, error: 'Věk musí být mezi 13 a 90 lety.' };
    }
 ...
 * @returns {Promise<{ userId: string|null, loginPassword: string|null, existingAccount: boolean, userChosePassword: boolean, authError: string|null }>}
 */
export async function createRegistrationAuthUser(payload, password) {
  ...
  if (authResult.error) {
    ...
    if (isAlready) {
      return { authError: 'existing_account', userId: null, loginPassword: null, existingAccount: true, userChosePassword: false };
    }
    return {
...
export async function applyRegistrationUserMetadata(payload, meta = {}) {
  ...
  try {
    ...
    const smartScaleMeta = parseSmartScalePreference(meta.smartScaleBody || {});
