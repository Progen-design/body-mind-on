...
import { dietTypeRejectionReason } from '../dietOptions.js';
import { validateRegistrationChoices } from '../validation/onboardingSchema.js';
import { DRUHY_SOUHLASU } from '../souhlasy.js';
import {
  PROGRAMS,
  validateHeightCm,
  validateWeightKg,
...
import { createAuthUserIfNew } from '../authHelpers.js';
import { trainingEnvironmentNotesSuffix } from '../trainingEnvironment.js';
import { normalizeTrainingExclusions } from '../trainingExclusions.js';
 ...
 * @returns {{ ok: true, payload: object, password: string, birthDateRaw: string, smartScaleBody: object, souhlasy: string[] } | { ok: false, status: number, error: string }}
 */
export function parseAndValidateRegistrationBody(b = {}) {
  const dietType = b.diet_type?.trim() || null;
  // `disabled` v <option> je jen kosmetika — POST s vypnutou dietou by prošel
  // a u paleo by znamenal nefiltrovaný jídelníček. Autorita je lib/dietOptions.js.
  const dietDuvod = dietTypeRejectionReason(dietType);
  if (dietDuvod) return { ok: false, status: 400, error: dietDuvod };
  // Neznámý goal/activity/den tréninku se má odmítnout, ne tiše nahradit
  // výchozí hodnotou — na SYROVÝCH hodnotách, před normalizeGoal/normalizeActivity
  // níž (docs/DALSI_KROK.md 9.1/C).
  const vyberValidace = validateRegistrationChoices({
    goal: b.goal,
    activity: b.activity,
    workoutDays: b.workout_days,
  });
  if (!vyberValidace.ok) {
    return { ok: false, status: 400, error: vyberValidace.error };
  }

  // SOUHLAS — GDPR čl. 7 odst. 1 (správce musí souhlas DOLOŽIT). Klient posílá
  // explicitní pole `souhlasy` (druhy z lib/souhlasy.js), ne holé `true` —
  // z požadavku tak jde poznat, S ČÍM přesně člověk souhlasil, ne jen že
  // něco odklikl. Bez tohohle pole (nebo s neúplným) se registrace odmítá TADY,
  // PŘED createRegistrationAuthUser() v api/body-metrics.js — jinak by zůstal
  // založený účet bez plánu i bez souhlasu. Kontrola je vázaná na DRUHY_SOUHLASU,
  // ne na natvrdo dva řetězce: přibude-li povinný druh souhlasu, vynutí se tu
  // automaticky. (Nepovinný druh — např. budoucí marketingový souhlas — by
  // potřeboval vlastní, měkčí pravidlo, ne rozšíření tohohle seznamu.)
  const souhlasyRaw = Array.isArray(b.souhlasy) ? b.souhlasy : [];
  const souhlasyPlatne = souhlasyRaw.filter((d) => DRUHY_SOUHLASU.includes(d));
  const souhlasChybi = DRUHY_SOUHLASU.some((d) => !souhlasyPlatne.includes(d));
  if (souhlasChybi) {
    return {
      ok: false,
      status: 400,
      error: 'Bez souhlasu s obchodními podmínkami a se zpracováním osobních (i zdravotních) údajů nemůžeme registraci dokončit.',
    };
  }
  ...
  // Popisky odpovídají polím v registraci: `dietary_restrictions` je
  // ZDRAVOTNÍ OMEZENÍ (alergie, intolerance, nemoci), `foods_to_avoid` je
  // CO NEJÍŠ. Do 8. 9. 2026 tu byly prohozené — věta „Co nejí: Alergie na
  // ořechy" popisovala alergii jako chuťovou preferenci.
  if (dietaryRestrictions) notesParts.push('Zdravotní omezení: ' + dietaryRestrictions);
  if (foodsToAvoid) notesParts.push('Co nejí: ' + foodsToAvoid);
  ...
  // z formuláře (src/components/registrace/StartRegistrace.tsx), spojená
  // tady do jednoho jsonb sloupce. normalizeTrainingExclusions() nikdy
  // nespadne, takže chybějící/neplatný vstup dá jen prázdná vyloučení.
  const trainingExclusions = normalizeTrainingExclusions({
    patterns: b.training_exclusion_patterns,
    muscles: b.training_exclusion_muscles,
    source: 'onboarding',
  ...
  const payload = {
    ...
    training_exclusions: trainingExclusions,
    devices,
  };
  ...
  // CÍL VÝŽIVY SE ULOŽÍ CELÝ, NE JEN KALORIE.
  //
  // Do 23. 8. 2026 se persistoval jen `calories_target` a makra se počítala
  // znovu při každém volání. Bílkoviny vychází z váhy, takže s každou její
  // změnou se posunuly bílkoviny a jako zbytek i sacharidy — každý týdenní
  // plán měl jiný cíl. Změřeno na produkci: dva po sobě jdoucí plány téhož
  // člověka, oba 2164 kcal, ale jednou B 158 g / S 232 g a podruhé
  // B 183 g / S 207 g.
  //
  // Cíl výživy je rozhodnutí o člověku a padne tady, při registraci. Mění se
  // jen vědomě — týdenním přepočtem váhy (weeklyWeightRecalc) nebo úpravou
  // v profilu, ne jako vedlejší efekt generování plánu.
  payload.calories_target = nutritionTargets.calories_target;
  payload.protein_target_g = nutritionTargets.protein_g;
  payload.carbs_target_g = nutritionTargets.carbs_g;
  payload.fat_target_g = nutritionTargets.fat_g;
  ...
  if (password && !passwordValidation.valid) {
    return { ok: false, status: 400, error: passwordValidation.error || PASSWORD_TOO_SHORT_MESSAGE_CS };
  }
  ...
  return {
    ...
    birthDateRaw,
    smartScaleBody: b,
    // Mimo `payload` schválně — `body_metrics` na to nemá sloupec, stejně
    // jako password/birthDateRaw výš. Zapisuje se přes zapisSouhlasy()
    // do souhlasy_uzivatelu, ne do téhle tabulky.
    souhlasy: souhlasyPlatne,
  };
}
...
export async function createRegistrationAuthUser(payload, password) {
  ...
  if (authResult.error) {
    // Chyba hesla (krátké, slabé) je chyba hesla — ne „účet už existuje".
    if (authResult.passwordError === true) {
      return {
        authError: 'password',
        passwordError: authResult.error,
        userId: null,
        loginPassword: null,
...
export async function persistBodyMetricsRow(payload) {
  ...
  if (dbErr && /training_exclusions|does not exist|column/i.test(dbErr.message || '')) {
    const fallbackPayload = { ...insertPayload };
    delete fallbackPayload.training_exclusions;
    ({ data: insertedRows, error: dbErr } = await supabaseServer
      .from('body_metrics')
      .insert([fallbackPayload])
      .select('id'));
...
export function buildRegistrationApiResponse(ctx) {
  ...
  if (accountCreated) {
    ...
    response._diagnostics = {
      ...
      required_modules: trainerResult?.required_modules ?? ['nutrition', 'training'],
