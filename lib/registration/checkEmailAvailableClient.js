 ...
 * @param {string} email
 * @param {{ signal?: AbortSignal }} [opts] `signal` zruší dotaz, když mezitím
 *   přišel novější (lib/registration/kontrolaEmailu.js).
 * @returns {Promise<{ available: boolean, rateLimited?: boolean, networkError?: boolean, aborted?: boolean }>}
 */
export async function fetchRegistrationEmailAvailable(email, opts = {}) {
  try {
    ...
    // Zrušený dotaz není výpadek — jen ho předběhl novější.
    if (err?.name === 'AbortError') return { available: false, aborted: true };
    return { available: false, networkError: true };
  }
}
