...
export async function runAgent(agentSlug, { userId = null, input = null, taskType = null, contractVersion = null } = {}) {
  ...
  const cached = useCache ? await readOpenAICache(cacheKey) : null;
  ...
  let response;
  try {
    response = await openai.responses.create(requestBody, { signal: controller.signal });
  } catch (err) {
  ...
  // Denní součet se NEZAPISUJE. Účtenkou je `ai_logs` níž (`estimated_cost_usd`)
  // a `openai_daily_usage` je od migrace 20260828100000 pohled nad ní
  // a nad `ai_runs`. Dřív se tu volalo `recordOpenAIUsage()`, které do součtu
  // nikdy nezahrnulo generátor receptů — a rozpočtová pojistka proto měřila
  // půlku útraty.
  if (useCache) {
