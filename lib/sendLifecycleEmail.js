 ...
 * @param {{ jmeno?: string|null }} [kontext] oslovení v e-mailech po trialu
 * @returns {Promise<{ok:boolean, message_id?:string, error_code?:string}>}
 */
export async function sendLifecycleEmail(toEmail, triggerKey, kontext = {}) {
  const started = Date.now();
  const to = String(toEmail || '').trim();

  if (!to.includes('@')) return { ok: false, error_code: 'invalid_recipient' };

  const content = getLifecycleEmailContent(triggerKey, kontext);
