...
import { workoutBlocksForBodyMetrics } from '../workoutTemplates.js';
import { planMealTypeToWeightKey, slotTargetKcal, mealSlotTypes } from '../nutrition/portionScaling.js';
 ...
 */
function buildFallbackMealSlot(dietKey, mealType, dayIndex, poradiVTypu = 0) {
  ...
  // Posun podle pořadí slotu V RÁMCI TÉHOŽ TYPU, ne jen podle dne. Při šesti
  // jídlech má den tři svačiny se stejným `type` i `dayIndex` — bez tohohle by
  // z tabulky vyšly třikrát stejné. Krok 1 zaručuje různé indexy vždy, když je
  // tabulka aspoň tak dlouhá jako počet opakování slotu (svačin je nejvýš 3,
  // tabulky mají 7 položek); násobek by při jiné délce mohl kolidovat.
  const idx = (dayIndex + poradiVTypu) % arr.length;
...
export function getDeterministicMealPlan(bodyMetrics) {
  ...
  const usedTypes = mealSlotTypes(mealsPerDay);
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const meals = usedTypes.map((type) => {
      const poradi = pouzitoZTypu[type] ?? 0;
      pouzitoZTypu[type] = poradi + 1;
      return buildFallbackMealSlot(dietKey, type, i, poradi);
    });
...
export function buildCatalogSkeletonPlan(bodyMetrics) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({ day_index: i, day_name: CZECH_DAYS[i], daily_target_kcal: daily, meals });
  }
  return { targets, meal_plan: { meals_per_day: mealsPerDay, days } };
}
