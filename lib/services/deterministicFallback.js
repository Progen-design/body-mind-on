...
/** Targets podle goal a weight. Export pro orchestrátor (bez MEAL_QUERIES). */
export function computeTargetsForPlan(bodyMetrics) {
  const goal = (bodyMetrics?.goal || 'udrzovani').toLowerCase();
  return computeTargets(goal, bodyMetrics?.weight_kg);
}
...
export function buildProfileTemplateMealPlan(bodyMetrics) {
  ...
  for (let i = 0; i < 7; i++) {
    const meals = usedTypes.map((type) => {
      const q = `${diet} ${type} balanced`.slice(0, 100);
      return {
        type,
        name_cs: '',
        search_query: q,
