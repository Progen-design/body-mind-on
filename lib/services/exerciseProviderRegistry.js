 ...
 */
import { resolveExercise as wgerResolve } from './wgerService.js';
import { resolveToCanonicalKey, getCanonicalExercise } from '../exerciseCanonicalMap.js';
...
const aiExerciseAlternativeCache = new Map();
const AI_ALT_CACHE_MAX = 400;

function aiAltCacheKey(term) {
  return String(term || '')
    .trim()
    .toLowerCase()
    .slice(0, 80);
...
function rememberAiAlternative(term, value) {
  const k = aiAltCacheKey(term);
  if (!k) return;
  if (aiExerciseAlternativeCache.size >= AI_ALT_CACHE_MAX) {
    const first = aiExerciseAlternativeCache.keys().next().value;
    aiExerciseAlternativeCache.delete(first);
  }
  aiExerciseAlternativeCache.set(k, value);
}

/**
 * Z již načteného řádku DB sestaví výsledek jako resolveFromRegistry.
 * @param {object} data
 * @param {string} [nameHintCs]
 * @returns {Promise<object|null>}
 */
async function materializeRegistryRow(data, nameHintCs) {
  if (!data) return null;

  const canonicalKey = data.canonical_key;
  ...
  const merged = mergeWithTrustedRegistryMedia(canonicalKey, {
    gif_url: data.gif_url,
    image_url: data.image_url || data.wger_exercise_image_url,
    wger_exercise_image_url: data.wger_exercise_image_url,
    source: data.source,
  });
  ...
  return {
    ...
    // Trusted GIF (lib/exerciseRegistryMedia.js) je natvrdo zapsaná URL ze
    // static.exercisedb.dev, ne výsledek živého volání ExerciseDB API — to
    // bylo odstraněno (docs/AUDIT_RAPIDAPI_REMOVAL.md). 'exercisedb' jako
    // název zdroje předstíral integraci, která v projektu neexistuje.
    source: merged.gif_url ? 'trusted_gif' : (data.source || 'registry'),
    wger_exercise_id: data.wger_exercise_id != null ? Number(data.wger_exercise_id) : null,
  };
}

// persistWgerMedia (zápis wger_exercise_image_url/wger_exercise_id zpátky do
// exercise_asset_registry) žije v lib/exerciseMediaBackfill.js — bez závislosti
// na lib/openai.js, ať se dá testovat i bez OPENAI_API_KEY v env.
export { persistWgerMedia };
 ...
 * @param {Map<string, object>|null} [registryRowsByKey] – přednačtené řádky z resolveWorkouts (jedno DB volání)
 * @returns {Promise<object|null>}
 */
async function resolveFromRegistry(canonicalKey, nameHintCs, registryRowsByKey = null) {
  if (!canonicalKey) return null;
  const cachedRow = registryRowsByKey?.get(canonicalKey) ?? null;
  if (cachedRow) {
    return materializeRegistryRow(cachedRow, nameHintCs);
  }
  try {
    ...
    if (!data) return null;
    return materializeRegistryRow(data, nameHintCs);
  } catch {
    return null;
  }
}
  ...
  return Promise.all(
    list.map(async (ex) => {
      ...
      const merged = mergeWithTrustedRegistryMedia(key, {
        gif_url: gifUrl,
        image_url: imgUrl,
        wger_exercise_id: row?.wger_exercise_id,
        source: row?.source,
      });
      gifUrl = merged.gif_url || null;
      imgUrl = merged.image_url || null;

      return {
        ...ex,
        canonical_key: key || null,
        display_name_cs: row?.display_name_cs || ex?.name || key || null,
        image_url: imgUrl || gifUrl || null,
        gif_url: gifUrl || null,
...
async function getAiExerciseAlternative(originalTerm) {
  ...
  const ck = aiAltCacheKey(originalTerm);
  if (ck && aiExerciseAlternativeCache.has(ck)) {
    const hit = aiExerciseAlternativeCache.get(ck);
    return hit === false ? null : hit;
  }
  try {
    const completion = await volejModel({
      purpose: 'cvik_alternativa_wger',
      model: 'gpt-4o-mini',
      temperature: 0.2,
      max_tokens: 50,
      messages: [
...
Odpověz POUZE názvem cviku, nic jiného. Např. "squat" nebo "push up" nebo "bent over row".`,
    ...
    const out = raw || null;
    if (ck) rememberAiAlternative(originalTerm, out === null ? false : out);
    return out;
  } catch {
    return null;
  }
 ...
 * @param {{ canonicalKey?: string|null, nameHintCs?: string|null, registryRowsByKey?: Map<string, object>|null }} [opts]
...
export async function resolveExercise(searchTerm, opts = {}) {
  const registryRowsByKey = opts.registryRowsByKey instanceof Map ? opts.registryRowsByKey : null;
  const empty = {
    name: null,
  ...
  const withDisplayCs = async (wgerResult, canonicalKeyFromTerm = null) => {
    ...
    // Doplněk k registry cache — dřív se totéž wger médium stahovalo znovu
    // při každém sestavení plánu, protože výsledek nikdy nezapisoval zpátky.
    await persistWgerMedia(supabaseServer, canonicalKey, wgerResult);
    return {
  ...
  if (canonicalFromAgent) {
    const fromReg = await resolveFromRegistry(canonicalFromAgent, nameHintCs || undefined, registryRowsByKey);
  ...
  if (canonicalKeyEarly) {
    const fromReg = await resolveFromRegistry(canonicalKeyEarly, nameHintCs || undefined, registryRowsByKey);
    if (fromReg) return fromReg;
  }
