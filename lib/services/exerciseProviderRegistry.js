...
async function resolveFromRegistry(canonicalKey, nameHintCs) {
  ...
  try {
    ...
    let imgUrl = data.image_url || null;
...
export async function resolveWorkoutExercises(exercises, supabase) {
  ...
  try {
    const { data } = await client
      .from('exercise_asset_registry')
      .select(
        'canonical_key, display_name_cs, gif_url, image_url, body_part, target, equipment, wger_exercise_id, wger_name_en'
      )
...
/**
 * Resolve cviku – nejdřív DB registry, pak wger (base_id z /exercise/search/).
