...
import { shouldRunAsyncPlanEnhancement } from '../openaiPlanConfig.js';
...
function buildEnhancementPrompt(bodyMetrics, planJson) {
  ...
  return `${BM_ON_CORE_AI_PRINCIPLES}

${BM_ON_PLAN_ENHANCEMENT_RULES}

${BM_ON_OUTPUT_SAFETY_RULES}

Jsi kouč. Uživatel už má hotový týdenní jídelníček z katalogu (recepty a kalorie jsou FINÁLNÍ).
...
Pravidla:
- planner_suggestion_cs: max 80 znaků, inspirace k jídlu, NE nový recept ani food-blog název.
...
- Enhancement nesmí přepsat jednoduchá jídla na složitá (burrito, kari, frittata, lasagne apod.).`;
}
...
async function fetchPlanEnhancementFromOpenAI(bodyMetrics, planJson) {
  ...
  try {
    const completion = await volejModel(
      {
        purpose: 'obohaceni_planu',
        model,
        messages: [
          {
            role: 'system',
            content: `${BM_ON_OUTPUT_SAFETY_RULES}\nVrať pouze validní JSON. Nesmíš měnit kalorie, catalog_id, recepty ani trénink. Enhancement nesmí přepsat jednoduchá jídla na složitá.`,
          },
...
export async function schedulePlanEnhancementAsync(params) {
  ...
  if (bodyMetrics?.user_id && !bodyMetrics?._coach_memory_summary) {
    try {
      const { buildPlanMemoryContext } = await import('./planMemoryContext.js');
