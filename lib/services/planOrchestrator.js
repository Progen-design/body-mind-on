...
function planPromptCriticalCaloriesBlock(targetKcal) {
  ...
  return `KRITICKÉ KALORICKÉ POŽADAVKY (po navázání na Spoonacular musí den držet součet kcal z receptů v tomto pásmu):
- Cíl: ${t} kcal/den (calories_per_day v targets).
- KAŽDÝ den: součet kalorií ze všech jídel (reálné porce dospělého, NE dětské / snack-only) v rozmezí ${lo}–${hi} kcal.
- Pokud by součet vypadal nízko: přidej type \"snack\" (200–500 kcal), nebo zvol kaloričtější main meals (beef stew, pasta chicken, grilled chicken rice, baked salmon).
...
- Vyhni se: jediný smoothie jako snídaně; salát bez proteinu a sacharidů jako večeře; jídla, která ve Spoonacular často vycházejí pod ~250 kcal jako jediný main meal.`;
}
...
Pravidla:
1. Přesně ${totalMeals} jídel za týden (${meals} × 7), každé jídlo unikátní spoonacular_query + type.
2. Respektuj cíl, diet_type, equipment a foods_to_avoid z PROFIL_JSON.
3. Nevracej prázdné meal_plan.days.
4. name_cs sladit se spoonacular_query (vejce ≠ tofu).
...
3. name_cs: česky jako konkrétní název pokrmu na talíři (např. „Pečený losos s bramborami“), ne obecné štítky typu „obilninová kaše“ ani anglické názvy.
...
- "meal_plan": { "meals_per_day": ${meals}, "days": [ přesně 7 objektů, den 0–6 ] } — POVINNÝ klíč \"days\" uvnitř meal_plan. Nepoužívej daily_plan / week_plan / schedule.
  ...
  Příklad SK (2100 kcal/den, 3 jídla): snídaně target_kcal 580, oběd 780, večeře 620. Příklad svačina (+1 jídlo): + target_kcal 320.
  České porce (inspirace — name_cs má odpovídat tomu, co opravdu hledá spoonacular_query): „Ovesná kaše s ořechy a jablky“ (~550 kcal), „Kuře na rýži se zeleninou“ (~750 kcal), „Losos s bramborovou kaší“ (~680 kcal).
 ...
 * @param {{ useOpenAI?: boolean, requestId?: string, validFrom?: string, valid_from?: string, validUntil?: string, valid_until?: string, mealsOnly?: boolean }} [opts]
...
export async function generateStructuredPlan(bodyMetrics, opts = {}) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const workout = workoutByDayIndex[dow] ?? workoutByDayIndex[i];
