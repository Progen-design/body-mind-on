...
function buildRepairStructuredPlanUserMessage(
  ...
  const slice = String(lastRaw || '').slice(0, 8000);
  return `Předchozí výstup byl neplatný (${failureReason}). Vrať výhradně opravený kompletní JSON objekt, bez markdown.

STRUKTURA (musí projít validací):
- "targets": calories_per_day, protein_g, carbs_g, fat_g
- "meal_plan": { "meals_per_day": ${meals}, "days": [ přesně 7 objektů ]; každý den přesně ${meals} jídel s type, name_cs, spoonacular_query }
- "workout_plan": dle pravidel — ${workoutPlanCardinalityRule}
- Cviky: pouze povolené canonical_key; spoonacular_query jen z předepsaných anglických dotazů pro daný typ jídla.

Neplatná předchozí odpověď (oprav na validní JSON se všemi povinnými poli):
${slice}`;
}
...
async function fetchStructuredPlanFromOpenAI(bodyMetrics) {
  ...
  const weeklySessionsLine =
    bodyMetrics?.weekly_sessions_user != null && String(bodyMetrics.weekly_sessions_user).trim() !== ''
      ? String(bodyMetrics.weekly_sessions_user).trim()
      : bodyMetrics?.weekly_sessions != null && String(bodyMetrics.weekly_sessions).trim() !== ''
        ? String(bodyMetrics.weekly_sessions).trim()
        : String(workouts);
  const workoutDaysLine =
    bodyMetrics?.workout_days != null && String(bodyMetrics.workout_days).trim() !== ''
      ...
      : 'flexibilní';
  const programLine =
    bodyMetrics?.program != null && String(bodyMetrics.program).trim() !== ''
      ? String(bodyMetrics.program).trim()
      : 'START';
  const bmWeeklySessionsForPrompt =
    bodyMetrics?.weekly_sessions_user != null && String(bodyMetrics.weekly_sessions_user).trim() !== ''
      ? Number(bodyMetrics.weekly_sessions_user) || workouts || 3
      : bodyMetrics?.weekly_sessions != null && String(bodyMetrics.weekly_sessions).trim() !== ''
        ? Number(bodyMetrics.weekly_sessions) || workouts || 3
        : Number(workouts) || 3;
  const dietTypeCs = (bodyMetrics?.diet_type && String(bodyMetrics.diet_type).trim()) || 'bez omezení';
  const foodsToAvoidCs = (bodyMetrics?.foods_to_avoid && String(bodyMetrics.foods_to_avoid).trim()) || 'žádné';
  const dietaryRestrictionsCs =
    (bodyMetrics?.dietary_restrictions && String(bodyMetrics.dietary_restrictions).trim()) || 'žádná';
  const allergiesCs = (bodyMetrics?.allergies && String(bodyMetrics.allergies).trim()) || 'žádné';
...
- Cíl: ${goal}
- Jídel denně: ${meals}
- TRÉNINK — Počet tréninkových dnů týdně: ${weeklySessionsLine}
- TRÉNINK — Preferované tréninkové dny (0=Neděle … 6=Sobota, nebo text z profilu): ${workoutDaysLine}
- TRÉNINK — Tréninkový program: ${programLine}
- WGER / TRÉNINK — Tréninkové dny PŘESNĚ: ${bmWeeklySessionsForPrompt} dní s cvičením podle registrace (body_metrics), ostatní dny = pouze rest (canonical_key: "rest").
- Kalendář day_index: Pondělí=1, Úterý=2, Středa=3, Čtvrtek=4, Pátek=5, Sobota=6, Neděle=0.
- 3 tréninky týdně (bez vybraných konkrétních dnů) ⇒ day_index 1 (pondělí), 3 (středa), 5 (pátek).
...
- Tréninkové dny (soulad s počtem výše): přesně ${bmWeeklySessionsForPrompt} dní v týdnu s cvičením; ostatní dny = pouze rest (canonical_key: "rest").
- Vybavení: ${equipment}
- JÍDELNÍČEK — Typ stravy: ${dietTypeCs}
- JÍDELNÍČEK — Potraviny k vyloučení: ${foodsToAvoidCs}
- JÍDELNÍČEK — Dietní omezení: ${dietaryRestrictionsCs}
- Alergie / další poznámky ke stravě: ${allergiesCs}
- Strava (technický profil pro skórování / Spoonacular): ${diet}
- Souhrn omezení (pro kontext): ${restrictions || 'žádná'}
