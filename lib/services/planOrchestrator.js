...
/** Resolve jídel – pouze Spoonacular. Bez ověřeného receptu se nepoužívá raw text. */
async function resolveMeals(mealPlan, diet) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      const recipeVerified = Boolean(recipe);
      dayMeals.push({
        type: m.type,
        display_name: recipeVerified ? recipe.title : 'Jídlo (neověřeno)',
...
/** Resolve cviků – pouze wger. Bez ověřeného cviku se nepoužívá raw text. */
  ...
  for (const w of workoutPlan?.days ?? []) {
    ...
    for (const ex of w.exercises ?? []) {
      ...
      try {
        ...
        resolvedEx = { name: null, image_url: null, video_url: null, source: 'none', wger_exercise_id: null };
      }
      // Ověřený = wger vrátil cvik (název z API, ne raw OpenAI). Obrázek/video může chybět – mnoho cviků ve wger nemá media.
      const exerciseVerified = resolvedEx?.source === 'wger' && (resolvedEx?.name ?? false);
      exercises.push({
        name: exerciseVerified ? (resolvedEx?.name || 'Cvik (neověřeno)') : 'Cvik (neověřeno)',
