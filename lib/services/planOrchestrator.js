...
import { searchMealMetadata, MEAL_CONFIDENCE_THRESHOLD, mapSpoonacularRecipe } from '../mealEnrichment';
...
import { extractIngredientLinesFromSpoonacularRecipe, aggregateShoppingIngredientLinesFromStructuredPlan } from '../spoonacularShopping';
...
/** Tvrdý strop Spoonacular volání na jeden plán. Default 50; env SPOONACULAR_MAX_REQUESTS_PER_PLAN (max 200). */
...
const MAX_SPOONACULAR_REQUESTS_PER_PLAN = readSpoonacularBudget();

function formatUnverifiedMealDisplayCs(meal, meta) {
  const q = (meal?.search_query || '').trim();
  if (q) {
    return q
      .split(/\s+/)
...
async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  /** Sdílená in-memory cache Spoonacular výsledků (stejný candidate + parametry) v rámci jednoho plánu. */
  const spoonacularCandidateCache = new Map();
  const mealSearchOptsBase = fastMode
    ? { maxCandidates: 1, timeoutMs: 3000, fastMode: true, shortlistSize: 3, candidateCache: spoonacularCandidateCache }
    : { maxCandidates: 3, shortlistSize: 5, candidateCache: spoonacularCandidateCache };
  ...
  // Deduplikace: stejný search_query + typ jídla = jeden request (parametry complexSearch závisí na typu)
  const dedupKey = (q, mt) => JSON.stringify({ q: q || '__empty__', mt: mt || 'lunch' });
  const queryToIndices = new Map();
  for (let i = 0; i < allMeals.length; i++) {
    const q = allMeals[i].searchQuery || '__empty__';
    const mt = allMeals[i].m?.type || 'lunch';
    const key = dedupKey(q, mt);
  ...
  const searchResults = await Promise.all(
    uniqueEntries.map(({ q, mt }) => {
      ...
      return withRetry(() => searchMealMetadata(q, null, mealSearchOpts), retries, fastMode ? 500 : SPOONACULAR_RETRY_DELAY_MS)
        .catch(() => null);
    })
  );
  ...
  uniqueEntries.forEach((entry, i) => {
    const key = dedupKey(entry.q, entry.mt);
  ...
  function isRecipeVerified(meta) {
    return meta?.recipe_id != null && (meta?.image_trust_level === 'exact' || (meta?.confidence_score ?? 0) >= MEAL_CONFIDENCE_THRESHOLD) && meta?._recipe;
  }
  ...
  uniqueEntries.forEach((_, i) => { diag.spoonacular_requests_total += (searchResults[i]?._spoonacularCalls ?? 0); });
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      const recipeVerified = meta?.recipe_id != null && (meta?.image_trust_level === 'exact' || (meta?.confidence_score ?? 0) >= MEAL_CONFIDENCE_THRESHOLD);
      const rawRecipe = meta?._recipe;
      let display_name_cs = formatUnverifiedMealDisplayCs(m, meta);
      let recipeObj = null;
