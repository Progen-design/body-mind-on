 ...
 */
import { volejModel } from '../openai.js';
import { aggregateShoppingIngredientLinesFromStructuredPlan } from '../spoonacularShopping.js';
import {
...
import { resolveSyncOpenAiForPipeline } from '../openaiPlanConfig.js';
import { parseStructuredPlan } from '../validation/parseStructuredPlan.js';
import { isSpoonacularRegistrationCompressEnabled } from '../spoonacularQuotaGate.js';
import { enrichAgentPlanV6, isV6Format } from './planOrchestrator_newFormat.js';
import { resolveMeals, resolveWorkouts, logOrchestrator } from './planOrchestratorResolve.js';
import { loadResolvedWorkoutsFromLatestPlan, enrichBodyMetricsWithPriorWorkoutAvoidance } from './priorPlanWorkouts.js';
import { writeAILog } from '../aiOps.js';
import { bodyMetricsToPlanInput } from '../bodyMetricsToPlanInput.js';
import { attachCalorieHonestyToPlan } from '../nutrition/calorieHonesty.js';
import { sumScaledDayKcal } from '../nutrition/portionScaling.js';
import {
  normalizePlanCalorieTargets,
  assertCalorieTargetConsistency,
} from '../calorieTargetIntegrity.js';
import {
  filterWorkoutPlanForTrainingEnvironment,
  parseTrainingEnvironment,
  parseTrainingEnvironmentDetail,
  parseAvailableEquipment,
  TRAINING_ENVIRONMENT_LABELS,
  formatAvailableEquipmentLabels,
} from '../trainingEnvironment.js';
import { supabaseServer } from '../supabaseServer.js';
import { addCalendarDaysIsoPrague, calendarDateIsoInPrague, weekdayIndexJsFromPragueIso } from '../czechCalendar.js';
import { buildPlanPromptProfileJson } from '../compactPlanPrompt.js';
import {
...
import { getOpenAiPlanModel } from '../openaiModels.js';
import { enforceWorkoutsPerWeekInPlan, scaleAndDiversifyWorkoutPlan } from '../workoutPlanScaler.js';
import { nactiKatalogovouZasobu, objednejChybejiciCviky } from '../exerciseCatalogPool.js';
import { usesStartStrengthProgram } from '../programTier.js';
import { deriveWorkoutDays } from '../validation/onboardingSchema.js';
import {
...
import { loadLatestProgression, savePrescriptions } from '../workoutProgressionStore.js';
import { applyExclusions } from '../trainingExclusions.js';
...
/**
 * START PROGRAM, první polovina: postaví A/B tréninky a načte progresi.
 *
 * Plán od GPT se pro START ZAHAZUJE. Není to plýtvání — pro tenhle program je
 * složení pevné a model, který si smí vybírat cviky, opakovatelnost rozbije.
 *
 * @param {object} structured
 * @param {object} bodyMetrics
 * @param {string} validFromIso
 * @param {string} requestId
 * @returns {Promise<{ lastByKey: Map<string, object>, weekIndex: number }>}
 */
async function prepareStartProgramWorkoutPlan(structured, bodyMetrics, validFromIso, requestId) {
  const workoutsPerWeek = Math.min(7, Math.max(0, Number(bodyMetrics?.workouts_per_week) || 3));
  const workoutDays = deriveWorkoutDays(workoutsPerWeek, bodyMetrics?.preferred_workout_days);

  // Kolikátý týden programu to je. `program_started_on` se bere z nejstarších
  // body_metrics uživatele; bez něj je to týden 0 a program začne od A.
  const weekIndex = startProgramWeekIndex(
    validFromIso,
    bodyMetrics?.program_started_on || bodyMetrics?.created_at || null
  );

  const lastByKey = bodyMetrics?.user_id
    ? await loadLatestProgression(bodyMetrics.user_id, { before: calendarDateIsoInPrague(validFromIso) })
    : new Map();

  const built = buildStartWorkoutDays({ bodyMetrics, workoutDays, weekIndex });

  // Vyloučení cviků/vzorů (lib/trainingExclusions.js) — substituční vrstva
  // NAD zmrazenými poli, volaná PŘED resolveWorkouts(), ať médium/postup
  // dotáhne normální resolve krok stejně jako u kteréhokoli jiného cviku ze
  // šablony (viz hlavička lib/trainingExclusions.js). Beze změny pro
  // uživatele bez `training_exclusions` — applyExclusions vrací tytéž
  // `built.days`.
  const { days: exclusionFilteredDays, warnings: exclusionWarnings } = applyExclusions(
    built.days,
    bodyMetrics?.training_exclusions,
    startProgramEnvironment(bodyMetrics)
  );
  structured.workout_plan = { workout_days: built.workout_days, days: exclusionFilteredDays };
  if (exclusionWarnings.length) {
    structured.plan_warnings = [...(structured.plan_warnings || []), ...exclusionWarnings];
  }

  log('info', 'START program: A/B full-body plán', {
    requestId,
    week_index: weekIndex,
    workouts_per_week: workoutsPerWeek,
    variants: built.days.map((d) => d.start_program_variant).join('-'),
    progression_known_exercises: lastByKey.size,
    training_exclusion_warnings: exclusionWarnings.length,
  });

  return { lastByKey, weekIndex };
}

/**
 * START PROGRAM, druhá polovina: dopočítá progresi na profiltrované dny
 * a uloží předpisy.
 *
 * @param {{ lastByKey: Map<string, object>, weekIndex: number }} state
 * @param {object} structured
 * @param {object} bodyMetrics
 * @param {string} validFromIso
 * @param {string} requestId
 */
async function finishStartProgramWorkoutPlan(state, structured, bodyMetrics, validFromIso, requestId) {
  const days = structured?.workout_plan?.days || [];
  const prescriptions = applyStartProgression(days, state.lastByKey);

  // day_index → datum. Musí to být tentýž výpočet, jakým se staví kalendář
  // plánu níž, jinak by předpis sedl na jiný den, než na který se uživateli
  // zobrazí trénink.
  /** @type {Record<number, string>} */
  const datesByDayIndex = {};
  for (let i = 0; i < 7; i += 1) {
    datesByDayIndex[weekdayIndexJsFromPragueIso(validFromIso, i)] = addCalendarDaysIsoPrague(validFromIso, i);
  }

  if (bodyMetrics?.user_id) {
    ...
    log(saved.error ? 'warn' : 'info', 'START program: předpisy progrese', {
      requestId,
      saved: saved.saved,
      error: saved.error,
      decisions: prescriptions.reduce((acc, p) => {
...
${typeof bodyMetrics?._plan_prompt_extra === 'string' && bodyMetrics._plan_prompt_extra.trim()
  ...
  for (let attempt = 1; attempt <= OPENAI_RETRY_COUNT; attempt++) {
    try {
      ...
      const openaiResponse = await volejModel({
        purpose: 'generovani_planu',
        model: planModel,
        messages: [
...
export async function generateStructuredPlan(bodyMetrics, opts = {}) {
    ...
    if (bodyMetrics?.user_id) {
      try {
        const { data: pinRows } = await supabaseServer
          ...
          .eq('user_id', bodyMetrics.user_id);
        pinnedMeals = pinRows || [];
      } catch (pinErr) {
        log('warn', 'meal pins load failed', { requestId, error: pinErr?.message });
      }
    }
  ...
  if (!resolvedMeals || !resolvedWorkouts) {
    ...
    /** @type {{ lastByKey: Map<string, object>, weekIndex: number }|null} */
    let startProgramState = null;
    // ── START: OPAKOVANÝ FULL-BODY PROGRAM A/B ────────────────────────────────
    //
    // START má dostat dva střídavé tréninky, které se opakují každý týden —
    // proto se pro něj přeskočí VŠECHNA čtyři místa, která pestrost vyrábějí:
    // `enforceWorkoutsPerWeekInPlan` (dopisuje dny z rotace),
    // `scaleAndDiversifyWorkoutPlan` (`diversified_days`), uvnitř ní
    // `deduplicateExercisesAcrossWeek` (`usedAcrossWeek` cvik zopakovaný v týdnu
    // aktivně vymění) a `rotatedTemplatesForBodyMetrics`. Nemažou se, ON_CLUB
    // a VIP na nich jedou dál.
    //
    // Změřeno na plánu c0c89c40: START dával 15 cviků, 15 různých.
    if (usesStartStrengthProgram(bodyMetrics)) {
      startProgramState = await prepareStartProgramWorkoutPlan(
        structured, bodyMetrics, validFromIso, requestId
      );
    } else {
      ...
      metricsForWorkouts._catalog_exercise_pool = await nactiKatalogovouZasobu(bodyMetrics);
      const scalerStats = scaleAndDiversifyWorkoutPlan(structured.workout_plan, metricsForWorkouts);
      await objednejChybejiciCviky(scalerStats, bodyMetrics, requestId);
    }
    // Filtr prostředí platí pro oba programy — cvik, na který uživatel nemá
    // náčiní, nesmí projít ani ve START. Progrese se proto dopočítává až za ním
    // (viz buildStartProgramWorkoutPlan).
    filterWorkoutPlanForTrainingEnvironment(structured.workout_plan, bodyMetrics);
    if (startProgramState) {
      await finishStartProgramWorkoutPlan(startProgramState, structured, bodyMetrics, validFromIso, requestId);
    }
  ...
  /** day_index → { workout_name, start_program_variant } ze skeletonu START programu. */
  const startProgramDayMeta = new Map(
    (structured?.workout_plan?.days || [])
      .filter((d) => d?.start_program_variant)
      .map((d) => [Number(d.day_index), {
        workout_name: d.workout_name,
        start_program_variant: d.start_program_variant,
      }])
  );
  ...
  const trainingEnv = parseTrainingEnvironment(bodyMetrics);
  const trainingEquipment = parseAvailableEquipment(bodyMetrics);
  const trainingEnvDetail = parseTrainingEnvironmentDetail(bodyMetrics);
  let trainingEnvLabel = TRAINING_ENVIRONMENT_LABELS[trainingEnv] || TRAINING_ENVIRONMENT_LABELS.gym;
  if (trainingEnv === 'home_equipment' && trainingEquipment.length) {
    trainingEnvLabel += ` · ${formatAvailableEquipmentLabels(trainingEquipment).join(', ')}`;
  } else if (trainingEnv === 'other' && trainingEnvDetail) {
    trainingEnvLabel += ` · ${trainingEnvDetail}`;
  }
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({
      ...
      day_index: dow,
      day_name: dayName,
      daily_target_kcal: mealDay?.daily_target_kcal || structured?.targets?.calories_per_day || null,
      daily_achieved_kcal: mealDay?.daily_achieved_kcal
        ?? (mealDay?.meals ? sumScaledDayKcal(mealDay.meals) : null),
      calorie_under_target: mealDay?.calorie_under_target === true,
      calorie_shortfall_kcal: mealDay?.calorie_shortfall_kcal ?? 0,
      meals: sortMealsChronologically(mealDay?.meals ?? []),
      workout: workout
        ? (workoutsResolveSource === 'prior_plan'
            // mealsOnly slibuje trénink BEZE ZMĚNY (banner, api/profile-preferences.js)
            // — `workout` je tu celý objekt z minulého plánu
            // (loadResolvedWorkoutsFromLatestPlan), ne jen jeho `exercises`. Skládat ho
            // znovu z kusů by slib porušilo podruhé: `startProgramDayMeta` se tu bere
            // z NOVĚ vygenerovaného `structured.workout_plan`, které se v mealsOnly
            // větvi vůbec nestaví (viz větev výš), takže by bylo prázdné a `workout_name`
            // / `start_program_variant` by zase zmizely (docs/DALSI_KROK.md 8.2,
            // změřeno na plánu 64bf0ee1…).
            ? { ...workout, day_index: dow }
            : {
                day_index: dow,
                duration_minutes: bodyMetrics?.workout_duration_min ?? 60,
                exercises: workout.exercises,
                // Označení „Trénink A“ / „Trénink B“ nese `structured.workout_plan`,
                // ale `resolveWorkouts` vrací jen day_index a exercises — bez tohohle
                // řádku se název cestou ztratil a uživatel neviděl, že se dva
                // tréninky střídají (změřeno na plánu ee814006: workout_name null).
                ...(startProgramDayMeta.get(dow) || {}),
              })
        : null,
    });
  }
  ...
  // POZOR na vyznam: tenhle citac merí, jestli ma cvik MEDIUM, ne jestli se
  // dohledal v cvicebni databazi. Registry (exercise_asset_registry) plni
  // gif_url, ne image_url — bez nej hlasil kazdy plan "exercises_resolved: 0"
  // a "exercises_fallback: 15", i kdyz se vsech 15 cviku dohledalo spravne
  // vcetne canonical_key a wger_exercise_id. Stalo to hodiny hledani chyby,
  // ktera neexistovala.
  const exercisesResolved = resolvedWorkouts.flatMap((w) => w.exercises).filter((e) => e.gif_url || e.image_url || e.video_url).length;
  const exercisesFallback = resolvedWorkouts.flatMap((w) => w.exercises).filter((e) => !e.gif_url && !e.image_url && !e.video_url).length;
  ...
  const planOut = {
    ...
    training_environment_label: trainingEnvLabel,
    ...
    // Vyloučení cviků/vzorů (lib/trainingExclusions.js) — prázdné pole pro
    // naprostou většinu uživatelů (žádná training_exclusions), jinak texty
    // typu "Kvůli tvým omezením je dnešek kratší." pro UI.
    plan_warnings: structured?.plan_warnings || [],
    days,
    ...
    _diagnostics: {
      ...
      // Perzistováno od 8.4 — do té doby se `trefaBilkovin`/`trefaTuku` počítaly
      // v recipesCatalog.js a zahazovaly. Dopad tukové penalty (i bílkovinné)
      // se bez tohohle dal změřit jen ručním dotazem do structured_plan_json.
      protein_trefa: resolvedMeals?._diag?.protein_trefa ?? null,
      fat_trefa: resolvedMeals?._diag?.fat_trefa ?? null,
  ...
  const daySummaries = days.map((d) => ({
    achieved_kcal: Number(d.daily_achieved_kcal) || sumScaledDayKcal(d.meals || []),
    target_kcal: Number(d.daily_target_kcal) || Number(planOut.targets?.calories_per_day) || null,
    under_target: d.calorie_under_target === true,
    shortfall_kcal: Number(d.calorie_shortfall_kcal) || 0,
    meals_added: (d.meals || []).filter((m) => m?.calorie_honesty_added === true).length,
  }));
  attachCalorieHonestyToPlan(planOut, daySummaries);

  normalizePlanCalorieTargets(planOut, bodyMetrics);
  const targetCheck = assertCalorieTargetConsistency(planOut, {
    ...bodyMetrics,
    calories_target: planOut.targets?.calories_per_day,
  });
  if (!targetCheck.ok) {
    log('warn', 'calorie_target_inconsistency_corrected', {
      requestId,
      ...targetCheck,
    });
    planOut.targets.calories_per_day = targetCheck.expected;
    normalizePlanCalorieTargets(planOut, { ...bodyMetrics, calories_target: targetCheck.expected });
  }
  if (targetCheck.expected > 0) {
    planOut._diagnostics = planOut._diagnostics || {};
    planOut._diagnostics.calorie_target_kcal = targetCheck.expected;
    planOut._diagnostics.calorie_target_mismatch = targetCheck.ok ? 0 : 1;
  }
