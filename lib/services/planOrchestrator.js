...
async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  const mealSearchOpts = fastMode ? { maxCandidates: 1, timeoutMs: 3000 } : { maxCandidates: 2 };
  ...
  const diag = { spoonacular_requests_total: 0, meals_resolved_primary: 0, meals_resolved_fallback: 0, meals_unverified: 0, average_confidence_score: 0 };
  ...
  const MAX_FALLBACK_ATTEMPTS = fastMode ? 1 : 2;
  await Promise.all(
