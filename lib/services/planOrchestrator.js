...
import { translateRecipeTitleToCzech } from '../recipeLocalization';
...
/** Resolve jídel – trust-aware Spoonacular pipeline (shortlist, scoring, confidence). Nikdy raw recipe.title do UI. */
async function resolveMeals(mealPlan, diet) {
  const resolved = [];
  for (const day of mealPlan?.days ?? []) {
    const dayMeals = [];
    for (const m of day.meals ?? []) {
      const searchQuery = m.search_query || '';
      let meta = null;
      try {
        meta = await withRetry(() => searchMealMetadata(searchQuery, null), 2, SPOONACULAR_RETRY_DELAY_MS);
      } catch {
        // ignore
      }
      ...
      if (recipeVerified && rawRecipe) {
        display_name_cs = await translateRecipeTitleToCzech(rawRecipe.title || '', meta.recipe_id);
        recipeObj = {
          id: rawRecipe.id,
...
/** Resolve cviků – pouze wger. Canonical display_name_cs je jediný user-facing název, nikdy raw wger name. */
  ...
  for (const w of workoutPlan?.days ?? []) {
    ...
    for (const ex of w.exercises ?? []) {
      ...
      try {
        ...
        resolvedEx = { name: null, display_name_cs: null, canonical_key: null, image_url: null, video_url: null, source: 'none', wger_exercise_id: null };
      }
      const exerciseVerified = resolvedEx?.source === 'wger' && (resolvedEx?.name ?? false);
      const display_name_cs = exerciseVerified ? (resolvedEx?.display_name_cs ?? resolvedEx?.name ?? 'Cvik (neověřeno)') : 'Cvik (neověřeno)';
      exercises.push({
