...
function formatUnverifiedMealDisplayCs(meal, meta) {
  const cs = (meal?.name_cs || meal?.ai_name || '').trim();
 ...
 * U ověřeného receptu: display_name_cs = přeložený titul Spoonacular (soulad s receptem); planner_suggestion_cs = původní name_cs z plánovače.
 */
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  /** U ověřeného receptu je zdroj pravdy pro UI název přeložený titul Spoonacular (soulad s receptem / surovinami). */
  const translateJobs = [];
  for (let i = 0; i < allMeals.length; i++) {
    const meta = metas[i];
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      const nameCsFromAi = (m.name_cs || m.ai_name || '').trim();
      let display_name_cs = nameCsFromAi || formatUnverifiedMealDisplayCs(m, meta);
      let recipeObj = null;
      ...
      dayMeals.push({
        ...
        planner_suggestion_cs: nameCsFromAi || null,
