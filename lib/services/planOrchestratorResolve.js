 ...
 */
import { buildSpoonacularContext, calorieRangeForMealType } from '../spoonacularComplexSearch.js';
import { resolveExercise } from './exerciseProviderRegistry.js';
...
import { resolveMealsFromCatalog } from '../recipesCatalog.js';
...
export async function resolveWorkouts(workoutPlan, opts = {}) {
  ...
  if (canonicalKeys.length) {
    try {
      const { data } = await supabaseServer
        ...
        .select(
          'canonical_key, display_name_cs, gif_url, image_url, wger_exercise_image_url, body_part, target, equipment, exercisedb_name, trust_level, wger_exercise_id, wger_name_en, level, mechanic, instructions_cs, easier_key, harder_key'
        )
  ...
  // není mezi cviky dnešního plánu, takže první dávka o ní neví. Pořád
  // jeden batch dotaz na canonical_key, ne dotaz za každý cvik zvlášť.
  // Chybějící/prázdné jméno se v obtiznostAPostupProCvik prostě přeskočí —
  // tlačítko na variantu bez popisku by nemělo co ukázat.
  const variantKeysNeeded = [...new Set(
    [...registryRowsByKey.values()].flatMap((row) => [row?.easier_key, row?.harder_key])
  )].filter((k) => k && !registryRowsByKey.has(k));
  if (variantKeysNeeded.length) {
    try {
      const { data } = await supabaseServer
        .from('exercise_asset_registry')
        .select('canonical_key, display_name_cs')
        .in('canonical_key', variantKeysNeeded);
      for (const row of data || []) {
        const k = row?.canonical_key;
        if (k && !registryRowsByKey.has(k)) registryRowsByKey.set(k, row);
      }
    } catch {
      // non-fatal — varianta se pak jen nepropíše, plán se sestaví dál
    }
  }
  ...
  for (const w of workoutPlan?.days ?? []) {
    ...
    for (const ex of w.exercises ?? []) {
      ...
      let sets = Number(ex.sets ?? 3);
      if (!Number.isFinite(sets) || sets < 1) sets = 3;
      const keyLower = String(canonicalKey || '').trim().toLowerCase();
      if (!['warmup', 'cooldown', 'rest', 'stretch'].includes(keyLower) && sets > MAX_PUBLISHABLE_WORKOUT_SETS) {
        sets = MAX_PUBLISHABLE_WORKOUT_SETS;
      }
      const registryRow = canonicalKey ? registryRowsByKey.get(canonicalKey) : null;
      exercises.push({
        ...
        video_url: media.video_url ?? resolvedEx?.video_url ?? null,
        source: media.gif_url ? 'exercisedb' : (resolvedEx?.source ?? 'none'),
        wger_exercise_id: resolvedEx?.wger_exercise_id ?? null,
        // Obtížnost + postup + lehčí/těžší varianta — cíl "každý cvik má
        // obtížnost a konkrétní český postup". Chybějící pole se vůbec
        // nevrací (viz obtiznostAPostupProCvik), takže se sem nedopisuje
        // placeholder.
        ...obtiznostAPostupProCvik(registryRow, registryRowsByKey),
      });
    }
