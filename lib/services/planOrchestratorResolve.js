...
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  const baseShortlist = mealSearchOptsBase.shortlistSize ?? 5;
      ...
      if (meta && typeof meta === 'object') {
        ...
        if (Number.isFinite(rid) && usedRecipeIds.has(rid)) {
          ...
          try {
            ...
            if (refetched && typeof refetched === 'object') meta = refetched;
          } catch {
            /* ponechat base */
          }
        }
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      dayMeals.push({
        ...
        recipe: recipeObj,
        image_url: recipeObj?.image || spoonacularImageUrl || null,
