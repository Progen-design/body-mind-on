...
export function buildSpoonacularContextForMealSlot(bodyMetrics, targets, m) {
  ...
  const out = { ...base };
  /**
   * Profilové pásmo pro typ jídla — nikdy ho nesmí přepisovat celé „GPT target_kcal ±15 %“,
   * pokud je target_kcal příliš nízký (jinak complexSearch vrací jen ~200 kcal porce a dny padají pod 72 %).
   */
...
function readSpoonacularBudget() {
  ...
  // Úsporný default: ~1 complexSearch na slot týdne (28 jídel), bez desítek refetchů
  return isSpoonacularRegistrationCompressEnabled() ? 32 : 90;
}
...
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  const liveSpoonacular = opts.allowLiveSpoonacular === true;
  const compressSpoonacular =
    opts.compressSpoonacular === true ||
    (liveSpoonacular && isSpoonacularRegistrationCompressEnabled());
  const bodyMetrics = opts.bodyMetrics ?? null;
  const targets = opts.targets ?? {};
  const spoonacularCandidateCache = new Map();
  const mealSearchOptsBase =
    fastMode || compressSpoonacular
      ? {
          maxCandidates: 1,
          timeoutMs: compressSpoonacular ? 4000 : 3000,
          fastMode: true,
          shortlistSize: compressSpoonacular ? 2 : 3,
          candidateCache: spoonacularCandidateCache,
          skipDailyDedup: true,
          allowLiveSpoonacular: liveSpoonacular,
          compressSpoonacular,
        }
      : {
          maxCandidates: 3,
  ...
  const retries = fastMode || compressSpoonacular ? 1 : 2;
  ...
  const MAX_REBALANCE_ITERATIONS = compressSpoonacular ? 0 : metas.length * 2;
  ...
  const MAX_FALLBACK_ATTEMPTS = compressSpoonacular ? 0 : fastMode ? 1 : 3;
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      const kcalFromRecipe =
        recipeObj?.calories != null && Number.isFinite(Number(recipeObj.calories))
          ? Math.round(Number(recipeObj.calories))
          : null;
      const kcalFromMeta =
        meta?.calories != null && Number.isFinite(Number(meta.calories)) ? Math.round(Number(meta.calories)) : null;
      const mealOut = {
        type: m.type,
        ...
        /** Kalorie z Spoonacular i při „illustrative“ shodě (meta.calories z výsledku vyhledávání). */
        kcal: kcalFromRecipe ?? kcalFromMeta ?? null,
        recipe_id:
