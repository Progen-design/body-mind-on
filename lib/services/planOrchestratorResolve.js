...
import { extractMealNameFromPlanHtmlForDay } from '../mealDisplayNameHelpers';
...
/** Generické placeholdery z lokalizace – nesmí přepsat český název z AI. */
function isGenericMealDisplayLabel(s) {
  const t = (s || '').trim();
...
function formatUnverifiedMealDisplayCs(meal, meta) {
  ...
  const cs = csRaw && !isGenericMealDisplayLabel(csRaw) ? csRaw : '';
...
export function mealSpoonacularQuery(m) {
  ...
  const fromCs = (m?.name_cs || '').trim().toLowerCase();
 ...
 * Zobrazený název: name_cs / ai_name z JSON (ne generické „Jídlo“), volitelně text z opts.sourcePlanHtml,
 * pak přeložený titul Spoonacular nebo dotaz / „Zdravé jídlo“. planner_suggestion_cs = negenerický návrh z plánovače.
 */
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      let displayPrimary = plannerRaw && !isGenericMealDisplayLabel(plannerRaw) ? plannerRaw : '';
      if (!displayPrimary && sourcePlanHtml && day.day_name) {
        const fromHtml = extractMealNameFromPlanHtmlForDay(
          sourcePlanHtml,
          day.day_name,
          m?.type || 'breakfast'
        );
        const ht = (fromHtml || '').trim();
        if (ht && !isGenericMealDisplayLabel(ht)) displayPrimary = ht.slice(0, 120);
      }
      ...
      if (recipeVerified && rawRecipe) {
        ...
        const translatedOk = !isGenericMealDisplayLabel(cs) && cs !== rawTitle;
        if (displayPrimary) {
          ...
          display_name_cs =
            (fromFmt && !isGenericMealDisplayLabel(fromFmt) ? fromFmt : '') || rawTitle || 'Zdravé jídlo';
        }
      ...
      dayMeals.push({
        ...
        planner_suggestion_cs: plannerRaw && !isGenericMealDisplayLabel(plannerRaw) ? plannerRaw : null,
