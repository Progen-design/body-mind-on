...
function ensureImageUrl(meal, meta, sourceMeal) {
  ...
  const fid =
    STATIC_RECIPE_IDS[q] ||
    (twoWords && twoWords !== q ? STATIC_RECIPE_IDS[twoWords] : undefined) ||
    STATIC_RECIPE_IDS[firstWord];
  if (fid != null && Number.isFinite(Number(fid))) {
    const id = Number(fid);
    meal.image_url = `https://img.spoonacular.com/recipes/${id}-312x231.jpg`;
    if (meal.recipe_id == null) meal.recipe_id = id;
  }
}
 ...
 * Zobrazený název vždy z AI (name_cs); Spoonacular recept = obrázek + výživa + ingredience (title zůstává anglicky v recipe).
 */
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  for (const day of mealPlan?.days ?? []) {
    ...
    for (const m of day.meals ?? []) {
      ...
      /** UI vždy z názvu z promptu (AI); Spoonacular jen média a výživa. */
      ...
      const mealOut = {
        ...
        image_url:
          recipeObj?.image ||
          spoonacularImageUrl ||
          (meta?.recipe_id != null && Number.isFinite(Number(meta.recipe_id))
            ? `https://img.spoonacular.com/recipes/${Number(meta.recipe_id)}-312x231.jpg`
            : null),
