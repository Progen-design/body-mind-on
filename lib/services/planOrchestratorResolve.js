...
export async function resolveMeals(mealPlan, diet, opts = {}) {
  ...
  async function resolvePrimaryMealMeta(entry) {
    ...
    const spoonacularContext = buildSpoonacularContext(bodyMetrics, targets, mealType);
  ...
  for (let i = 0; i < metas.length; i++) {
    ...
    if (!query) continue;
    let meta = metas[i];
    const rid = mealMetaRecipeId(meta);
    if (rid != null && usedRecipeIds.has(rid)) {
      ...
      try {
        ...
        if (refetched && typeof refetched === 'object') {
          ...
          if (refetchedRid != null && !usedRecipeIds.has(refetchedRid)) {
            meta = refetched;
            console.log(
              '[SPOON]',
              m?.type,
    ...
    metas[i] = meta && typeof meta === 'object' ? { ...meta } : meta;
