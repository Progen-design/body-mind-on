 ...
 * @param {{ requestId?: string, validFrom?: string, valid_from?: string, validUntil?: string, valid_until?: string, mealsOnly?: boolean, allowLiveSpoonacular?: boolean }} [opts]
 */
export async function enrichAgentPlanV6(structured, bodyMetrics, opts = {}) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({
      ...
      daily_target_kcal: mealDay?.daily_target_kcal ?? null,
      daily_achieved_kcal: mealDay?.daily_achieved_kcal ?? null,
  ...
  const planOut = {
    ...
    workouts_per_week: (structured.workout_plan?.workout_days ?? workoutDays).length,
    workout_days: structured.workout_plan?.workout_days ?? workoutDays,
    days,
    _format: 'v6',
    ...
    _diagnostics: {
      ...
      dedupe_v2: resolvedMeals?._diag?.dedupe_v2 ?? null,
    },
  };
