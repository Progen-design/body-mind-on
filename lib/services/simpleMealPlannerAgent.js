...
function pickTemplateForSlot(pool, dayIndex, mi, exclusions, mealType) {
  const len = pool.length || 1;
  for (let offset = 0; offset < len; offset += 1) {
    const tplIdx = (dayIndex + mi + offset) % len;
    const mealTpl = pool[tplIdx];
    if (isTemplateAllowedForExclusions(mealTpl, exclusions)) return mealTpl;
  }
  const fallbackName = cheeseFreeAlternativeName(mealType);
  const hit = pool.find((item) => item.name_cs === fallbackName);
  return hit || pool.find((item) => isTemplateAllowedForExclusions(item, exclusions)) || pool[0];
}
...
export function buildSimpleStartMealSkeleton({ bodyMetrics, targets, days = 7, mealsPerDay: mealsPerDayIn }) {
  ...
  for (let dayIndex = 0; dayIndex < days; dayIndex++) {
    const meals = usedTypes.map((type, mi) => {
      ...
      const mealTpl = pickTemplateForSlot(pool, dayIndex, mi, exclusions, type);
