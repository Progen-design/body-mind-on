 ...
 */
import { planMealTypeToWeightKey, slotTargetKcal, jitteredDailyCalorieTarget, mealSlotTypes } from '../nutrition/portionScaling.js';
import {
...
export const START_MEAL_TEMPLATES = {
  standard: {
    breakfast: [
      ...
      // Pozice v poli NENÍ libovolná: `pickTemplateForSlot` rotuje podle
      // indexu (dayIndex*5 + mi*3 + offset) % délka_poolu — přidání položek
      // mění délku, a tedy i to, co padne na existující dny. Ověřeno přes
      // verify-start-meal-variability, že s touhle pozicí zůstává týden
      // v ±15% pásmu pro všech 7 dní (řešeno tady, ne v PR 3 — kalorické
      // vědomí schedu leru je samostatná věc).
    ...
    lunch: [
      ...
      // `MIN_DISTINCT_BY_TYPE` i tak vracel pořád totéž. Pozice v poli
      // ověřená přes verify-start-meal-variability (viz komentář u
      // breakfastu výš) — jinde v poli mění rotaci tak, že jeden den
      // v týdnu spadne pod ±15% pásmo.
    ...
    dinner: [
      ...
      // vysokém denním cíli je až ~768 kcal. Pozice na začátku pole ověřená
      // přes verify-start-meal-variability (viz komentář u breakfastu výš).
