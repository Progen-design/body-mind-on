 ...
 */
import { getWgerExerciseImages, getWgerExerciseById } from '../wgerClient.js';
 ...
 * Pořadí: nejdřív exercise-translation (stejné chování jako když /exercise/search/ vracelo 404 –
 * lepší relevance pro běžné názvy), pak exerciseinfo?name_search= jako doplněk.
 * @param {string} searchTerm - např. "squat", "push up", "dřep"
 * @param {{ language?: number }} [opts] - language: 2=EN, 9=CS (rezervováno pro budoucí rozšíření)
 * @returns {Promise<{ id: number, name: string, language: number } | null>}
 */
export async function searchExercise(searchTerm, opts = {}) {
  ...
  return searchExerciseViaExerciseInfo(query);
}
...
export async function resolveExercise(searchTerm, opts = {}) {
  ...
  if (!image_url || !gif_url) {
    ...
    gif_url = extra.gif || null;
    if (!image_url) image_url = extra.image || gif_url || null;
  }
...
export async function resolveExerciseById(exerciseBaseId) {
  ...
  gif_url = extra.gif || null;
  if (!image_url) image_url = extra.image || null;
  if (!gif_url && !image_url && !video_url) {
    video_url = extra.gif || video_url;
  }
  const hasMedia = Boolean(image_url || gif_url || video_url);
