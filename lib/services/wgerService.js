...
/**
 * Vyhledá cvik přes /exercise/search/ — vrací base_id (PK pro exerciseimage, exerciseinfo).
 * data.id v návrzích je jiné ID; vždy používat base_id.
 * @param {string} searchTerm
 * @returns {Promise<{ id: number, name: string, language: number } | null>}
 */
async function searchExerciseViaSearchApi(searchTerm) {
  ...
  try {
    ...
    return {
      id: Number(baseId),
      name: (first.data?.name || first.value || searchTerm).trim(),
 ...
 * Primárně /exercise/search/ + base_id; pak exercise-translation.
...
export async function searchExercise(searchTerm, opts = {}) {
  ...
  const primary = await searchExerciseViaSearchApi(query);
...
export async function resolveExerciseById(exerciseBaseId) {
  ...
  const [imgResult, videoResult] = await Promise.all([getExerciseImage(id), getExerciseVideo(id)]);
