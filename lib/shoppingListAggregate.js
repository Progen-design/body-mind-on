...
function aggregateParsedEntries(entries, { useNormalization }) {
  ...
  if (useNormalization) sloucKusyDoGramu(groups);
...
/**
 * „banán 135 g" a „banán 3 ks" jsou tatáž surovina ve dvou jednotkách — na
 * tištěném nákupním seznamu vypadají jako dvojí nákup. Když u kanonické
 * suroviny existuje řádek v gramech I v kusech a známe váhu kusu
 * (`PIECE_WEIGHT_G`, zdroj `unit_conversions`), kusy se přepočtou na gramy
 * a sloučí. Samotné kusy zůstávají kusy („3 ks" se nemění na „360 g"),
 * a bez známé váhy se nic neslučuje — dva poctivé řádky místo špatného součtu.
 * @param {Map<string, object>} groups
 */
function sloucKusyDoGramu(groups) {
  for (const [key, entry] of [...groups.entries()]) {
    if (!entry.aggregatable || entry.unit !== 'ks') continue;
    const vahaKusu = PIECE_WEIGHT_G[entry.canonicalKey];
    if (!vahaKusu) continue;
    const gramy = groups.get(`${entry.canonicalKey}|g`);
    if (!gramy || !gramy.aggregatable) continue;
    gramy.amount += entry.amount * vahaKusu;
    groups.delete(key);
  }
}
