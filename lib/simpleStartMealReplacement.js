...
import { resolveSimpleStartTitle, findSimpleStartRecipeByTitle } from './simpleStartRecipeLibrary.js';
import {
...
export function pickSimpleStartMealAlternative({
  ...
  function buildCandidates(excludedNorm) {
    ...
    for (const tpl of pool) {
      ...
      // `findSimpleStartRecipeByTitle` (ne vlastní `r.meal_type === type`
      // hledání) — PROMPT_PRO_CODE.md bod E. Zkusí nejdřív přesnou shodu
      // typu, a když ji nenajde, spadne na shodu jen podle názvu napříč
      // VŠEMI typy. Tenhle picker měl dřív vlastní, přísnější kontrolu, což
      // ho rozešlo se zbytkem kódu — `buildStartSafeFallbackMeal()` v
      // lib/startSimpleMealFilter.js tuhle funkci (a tedy tenhle fallback)
      // používá už dávno. Naměřeno: 7 šablon má recept otagovaný jiným
      // meal_type (např. `Jogurt s ovocem` je recept pro snack, ale
      // šablona ho nabízí i pro breakfast) — bez fallbacku byly tyhle
      // recepty pro picker neviditelné, ačkoli existují. Riziko křížení
      // názvů ověřeno: v celé knihovně je jediná kolize (`Cottage s
      // pečivem` 2×, obě varianty jsou snack, 330 a 340 kcal) — fallback
      // podle názvu proto nemá jak sáhnout vedle na jiný typ.
      const lib = findSimpleStartRecipeByTitle(title, type);
      // ROZSYNCHRONIZACE START_MEAL_TEMPLATES / SIMPLE_START_RECIPES (14 ze
      // 60 šablon nemá knihovní recept vůbec, naměřeno — na rozdíl od těch
      // 7 výš, kde recept existuje, jen jinak otagovaný). Bez tohohle picker
      // klidně vybere šablonu bez receptu — ta se dál staví přes
  ...
  // PRÁH TOLERANCE — PROMPT_PRO_CODE.md bod D/E. Čistě procentní ±20 % je
  // u nízkých cílů (svačina, večeře na nižší denní kalorie) v absolutních
  // kcal příliš úzké pásmo na šířku knihovny — samotných 20 % ze 308 kcal
  // je jen ±62 kcal. `max(20 %, 120 kcal)` dává podlahu v absolutních
  // kcal, ne jen v procentech.
  //
  // ZMĚŘENÝ DOSAH, AŤ SE Z TOHOTO NESTANE "PRÁH JE VYŘEŠENÝ": pomáhá jen
  // do cíle ~2200 kcal/den. Napříč reálnými cíli (kandidátů v pásmu,
  // s opraveným lookupem výš):
  //   1800/5 kcal, snack   @252 kcal:  2 -> 4
  //   2200/5 kcal, breakfast @440:     3 -> 4
  //   2200/5 kcal, snack     @308:     3 -> 6
  //   2200/5 kcal, dinner    @528:     3 -> 5
  //   2800/5 kcal, lunch     @784:     0 -> 0   (žádný práh nepomůže)
  //   3300/5 kcal, breakfast @660:     0 -> 0
  //   3300/5 kcal, lunch     @924:     0 -> 0
  // Od ~2800 kcal/den nemá knihovna ŽÁDNÝ kandidát v žádné rozumné
  // toleranci — to je strop kalorického rozsahu šablon (samostatná vada,
  // viz verify-start-calorie-consistency, 60 z 64 FAILů je na cíli
  // 3300 kcal/den), tenhle práh ho neřeší a řešit nemá.
  const TOLERANCE_FLOOR_KCAL = 120;
  const inTolerance = (c) => distFromTarget(c) <= Math.max(targetKcal * 0.2, TOLERANCE_FLOOR_KCAL);
