...
/** Profil pod aktuální getPublicAppUrl(). */
export function getProfileUrl() {
  return `${getPublicAppUrl()}/profil`;
}

/** Přihlášení s návratem do profilu (výchozí pro plan e-maily a CTA). */
export function getLoginRedirectToProfileUrl() {
  return `${getPublicAppUrl()}/login?redirect=${encodeURIComponent('/profil')}`;
}

/** Primární CTA v plan e-mailu — nepřihlášený uživatel jde na login, po přihlášení na profil. */
export function getPlanEmailCtaUrl() {
  return getLoginRedirectToProfileUrl();
}

/** Přihlášení pod aktuální getPublicAppUrl() (s redirectem do profilu). */
