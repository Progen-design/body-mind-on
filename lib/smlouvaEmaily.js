 ...
 */
export function emailPotvrzeniSmlouvy({ tarif, cenaKc, dalsiPlatba, lhutaDo }) {
  const subject = `Předplatné ${tarif} je aktivní`;
  const { html, text } = sestav({
    title: subject,
    radky: [
      ['Tarif', tarif],
      ['Cena', `${cenaKc} Kč měsíčně`],
      ['Další platba', dalsiPlatba || 'podle období ve Stripe'],
    ],
    odstavce: [
      'Díky, první platba proběhla a předplatné běží. Tenhle e-mail je potvrzení uzavřené smlouvy — schovej si ho.',
      ...
      `Jak odstoupit do ${LHUTA_ODSTOUPENI_DNI} dnů: do ${lhutaDo} můžeš od smlouvy odstoupit bez udání důvodu — v Profil → Účet a předplatné → Odstoupit od smlouvy, nebo e-mailem na info@bodyandmindon.cz. Služba začala hned, proto vrátíme zaplacenou částku po odečtení poměrné části za dny, kdy už běžela.`,
