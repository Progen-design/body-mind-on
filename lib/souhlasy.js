...
import { DRUHY_SOUHLASU, DRUH_SOUHLASU_KOMUNITA, UCINNOST_PRAVNICH_TEXTU } from './souhlasyKonstanty.js';
...
export { DRUHY_SOUHLASU, DRUH_SOUHLASU_KOMUNITA, UCINNOST_PRAVNICH_TEXTU };

/**
 * @param {string} userId
 * @param {{ druhy?: string[], zdroj?: string, client?: object, povoleneDruhy?: string[] }} [opts]
...
export async function zapisSouhlasy(userId, opts = {}) {
  ...
  //
  // `povoleneDruhy` je tu kvůli souhlasu s pravidly komunity: ten do
  // `DRUHY_SOUHLASU` nepatří (registrace vyžaduje ten seznam celý), ale
  // zapisovat se má stejným logem, ne druhou cestou do téže tabulky.
  const {
    druhy = [],
    zdroj = 'registrace',
    client = supabaseServer,
    povoleneDruhy = DRUHY_SOUHLASU,
  } = opts;

  if (!userId) return { ok: false, error: 'chybi user_id' };

  const radky = druhy
    .filter((d) => povoleneDruhy.includes(d))
    .map((druh) => ({
      user_id: userId,
      druh,
