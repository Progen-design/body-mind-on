...

{
  ...
  assert(getMainIngredientLimit('snidane') === 10, 'snidane limit 10');
  ...
  assert(gate.pass, `snidane 5 mains passes limit 10: ${gate.reason}`);
  breakfast.push(
    ing('yogurt'), ing('berries'), ing('walnuts'),
    ing('chia seeds'), ing('almond butter'), ing('protein powder'),
  );
  ...
  assert(!failGate.pass && failGate.reason === 'too_complex', `snidane 11 mains rejected: ${failGate.reason}`);
  console.log('OK snidane allows 10, rejects 11');
}

{
  ...
  assert(getMainIngredientLimit('obed') === 10, 'obed limit 10');
  ...
  lunch.push(
    ing('avocado'), ing('corn'), ing('zucchini'),
    ing('broccoli'), ing('mushrooms'),
  );
  const failGate = evaluateImportGate(
  ...
  assert(!failGate.pass && failGate.reason === 'too_complex', `11 mains rejected: ${failGate.reason}`);
  console.log('OK obed allows 10, rejects 11');
}
