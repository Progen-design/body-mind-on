...
test('recept bez zdrojových kroků se modelu neposílá', () => {
  // Kontrola `instructions_cs` ho stejně odmítne — poslat ho znamená
  // pětkrát zaplatit za překlad postupu, který ve zdroji není. Recept 47
  // („Finger Foods: Frittata Muffins“, instructions = []) tak vyčerpal
  // všech pět pokusů, než tahle podmínka vznikla.
  assert.ok(
    KOD.includes('extractInstructionStepsEn(row.instructions).length === 0'),
    'prázdný zdrojový postup musí frontu opustit bez volání modelu'
  );
  const vyrazeni = KOD.indexOf('extractInstructionStepsEn(row.instructions).length === 0');
  const volani = KOD.indexOf('translateBatchWithOpenAI(pending)');
  assert.ok(vyrazeni < volani, 'vyřazení musí proběhnout před placeným voláním');
});

test('cron velikost dávky nepřebíjí — má ji na jednom místě', () => {
  // Snížení výchozí dávky v překladači nemělo účinek, protože cron si
  // předával `{ batch: 20 }` natvrdo. Běh po nasazení hlásil `recipes: 20`.
  const cron = readFileSync(new URL('../../../api/cron/translate-recipes.js', import.meta.url), 'utf8');
  assert.ok(
    !/runCatalogRecipeTranslation\(\s*\{[^}]*batch/.test(cron),
    'cron nesmí velikost dávky přepisovat — patří k překladači'
  );
});
