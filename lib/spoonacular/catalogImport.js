 ...
 * Includes English aliases for Spoonacular ingredient names.
 */
...
/** @type {string[]} */
const SEASONINGS_NORMALIZED = SEASONINGS.map((s) => normalizeIngredientName(s))
  .filter(Boolean)
 ...
 * @returns {boolean}
 */
export function recipePassesSimplicityFilter(recipe) {
  if (countMainIngredients(recipe) > MAX_MAIN_INGREDIENTS) return false;
  const steps = extractInstructionStepsEn(recipe.analyzedInstructions);
  if (steps.length === 0 || steps.length > MAX_INSTRUCTION_STEPS) return false;
  return true;
}
 ...
 * @typedef {{ imported: number, updated: number, matched: number, rejected: number, totalResults: number|null, quotaLeft: number|null, requestsUsed: number, filters: CatalogImportFilters, offset?: number, nextOffset?: number, stoppedReason?: string, errors?: string[] }} ImportResult
 */
...
export async function runSpoonacularCatalogImport(options = {}) {
  ...
  let offset;
  if (cursorManaged) {
    offset = await getSpoonacularImportCursor(mealType);
  } else {
  ...
  async function persistCursorAfterSuccess(pageOffset, resultCount) {
    ...
    await saveSpoonacularImportCursor(mealType, nextOffset);
  }

  for (let page = 0; page < pages; page += 1) {
    if (requestsUsed >= maxRequests) {
      return {
        imported, updated, matched, rejected, totalResults, quotaLeft, requestsUsed, filters,
        offset: startOffset, nextOffset, stoppedReason: 'max_requests_cap',
      };
    }
    ...
    if (quotaLeft != null && quotaLeft < QUOTA_STOP_THRESHOLD) {
      ...
      return {
        imported, updated, matched, rejected, totalResults, quotaLeft, requestsUsed, filters,
        offset: startOffset, nextOffset, stoppedReason: 'quota_low', errors,
      };
    }
    ...
    if (results.length === 0) {
      if (cursorManaged) {
        nextOffset = 0;
        await saveSpoonacularImportCursor(mealType, 0);
      }
      return {
        imported, updated, matched, rejected, totalResults, quotaLeft, requestsUsed, filters,
        offset: startOffset, nextOffset, stoppedReason: 'no_results', errors,
      };
    }
    ...
    for (const recipe of results) {
      if (!recipePassesSimplicityFilter(/** @type {Record<string, unknown>} */ (recipe))) {
        rejected += 1;
        continue;
      }
      try {
    ...
    if (requestsUsed >= maxRequests) {
      return {
        imported, updated, matched, rejected, totalResults, quotaLeft, requestsUsed, filters,
        offset: startOffset, nextOffset, stoppedReason: 'max_requests_cap', errors,
      };
    }
    if (quotaLeft != null && quotaLeft < QUOTA_STOP_THRESHOLD) {
      return {
  ...
  return {
    imported, updated, matched, rejected, totalResults, quotaLeft, requestsUsed, filters,
    offset: startOffset, nextOffset, errors: errors.length ? errors : undefined,
  };
}
