...
export function evaluateRecipeSimplicity(recipe, catalogMealType) {
  ...
  if (steps.length === 0) {
    const readyOnly = Number(recipe.readyInMinutes);
    if (Number.isFinite(readyOnly) && readyOnly > rules.maxReadyTime) {
...
export async function runSpoonacularQueryImport({ queryRow, runId, dryRun = false }) {
  ...
  const filters = buildImportFiltersForMealType(catalogMealType, queryRow.params);
  ...
  try {
    ...
    pointsUsed = readQuotaRequestCost(res) ?? estimateSpoonacularRequestCost(number);
    quotaLeft = readQuotaLeft(res);
    quotaUsed = readQuotaUsed(res);
    ...
    skippedFilter = gateBatch.skipped.length;
...
export async function runDailySpoonacularCatalogImport(options = {}) {
  ...
  const queriesPerType = options.queriesPerMealType ?? DEFAULT_QUERIES_PER_MEAL_TYPE;
  ...
  logImportStructured({
    ...
    run_id: runId,
    dry_run: dryRun,
    queries_per_meal_type: queriesPerType,
    points_cap: pointsCap,
  });

  for (const mealType of CRON_IMPORT_MEAL_TYPES) {
    if (pointsUsed >= pointsCap) break;

    /** @type {Array<{ query_signature: string, inserted: number, api_results: number, skipped_duplicate: number, skipped_filter: number, error?: string }>} */
    const queryRuns = [];
    let typeImported = 0;
    let typeDuplicate = 0;
    let typeFilter = 0;
    ...
    const queries = await selectImportQueries(mealType, queriesPerType);
    ...
    for (const queryRow of queries) {
      ...
      typeImported += result.inserted;
      typeDuplicate += result.skipped_duplicate;
      typeFilter += result.skipped_filter;
      typeFetched += result.api_results;
    ...
    totalImported += typeImported;
    totalSkippedDuplicate += typeDuplicate;
    totalSkippedFilter += typeFilter;
    totalFetched += typeFetched;

    byType[mealType] = {
      imported: typeImported,
      skipped_duplicate: typeDuplicate,
      skipped_filter: typeFilter,
      fetched: typeFetched,
      queries: queryRuns,
    };
  }
