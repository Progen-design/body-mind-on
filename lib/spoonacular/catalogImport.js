 ...
 * @typedef {{ imported: number, updated: number, matched: number, totalResults: number|null, quotaLeft: number|null, requestsUsed: number, filters: CatalogImportFilters, offset?: number, nextOffset?: number, stoppedReason?: string, errors?: string[] }} ImportResult
 */
...
export async function runSpoonacularCatalogImport(options = {}) {
  ...
  for (let page = 0; page < pages; page += 1) {
    if (requestsUsed >= maxRequests) {
      return {
        imported, updated, matched, totalResults, quotaLeft, requestsUsed, filters,
