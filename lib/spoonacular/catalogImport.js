...
import {
  ...
  // Musí být i tady, ne jen v `export … from` níž — re-export lokální vazbu nevytváří,
  // takže mapSpoonacularRecipeToCatalogRow na něj padal na ReferenceError.
  nutrientAmount,
  setActivePantrySet,
...
import {
  ...
  mergeReasonCounts,
} from './importFilterPipeline.js';
import {
  advanceImportQueryAfterRun,
  applyQueryParamsToSearch,
  DEFAULT_RESULTS_PER_QUERY,
  MAX_QUERIES_PER_RUN,
  reopenExhaustedImportQueries,
  selectImportQueriesGlobal,
} from './importQueryRotation.js';
import { createImportRunLogger, logImportStructured } from './importRunLog.js';
import { loadPantryNormalizedSet } from './pantryIngredients.js';
import { cacheRawRecipes } from './rawCache.js';
...
export { estimateSpoonacularRequestCost } from './importBudget.js';
...
export { DEFAULT_CATALOG_IMPORT_FILTERS, buildImportFiltersForMealType } from './catalogImportGate.js';
...
/** Sloty, které katalog zná. Spoonacular taxonomie mezi ně nemapuje 'vecere' — proto si ho dotaz říká sám. */
const CATALOG_MEAL_TYPES = new Set(['snidane', 'obed', 'svacina', 'vecere']);

/**
 * @param {unknown} value
 * @returns {string|null}
 */
export function normalizeCatalogMealType(value) {
  const raw = String(value ?? '').trim().toLowerCase();
  return CATALOG_MEAL_TYPES.has(raw) ? raw : null;
}
...
export function parseCatalogImportFilters(raw) {
  ...
  const minCalories = parseOptionalNumber(raw.minCalories, { min: 0, max: 5000, label: 'minCalories' });
  if (minCalories != null) filters.minCalories = minCalories;
  ...
  // `maxCarbs` (gramy) — jediná cesta, jak z importu dostat nízkosacharidové
  // recepty. Spoonacular nemá dietu `low_carb`; náš tag `low_carb` se počítá
  // z maker až při zápisu (podíl energie ze sacharidů, viz
  // LOW_CARB_MAX_CARB_ENERGY_SHARE). Dokud tenhle parametr chyběl, nešlo na
  // ten slot mířit vůbec — a katalog to ukázal: 17. 8. 2026 měl low_carb
  // 9 obědů, 8 snídaní a 8 svačin proti 43 večeřím.
  const maxCarbs = parseOptionalNumber(raw.maxCarbs, { min: 0, max: 400, label: 'maxCarbs' });
  if (maxCarbs != null) filters.maxCarbs = maxCarbs;
...
/**
 * @typedef {{ minProtein?: number, maxSugar?: number, minCalories?: number, maxCalories?: number, maxReadyTime?: number, maxCarbs?: number, sort?: string, sortDirection?: string }} CatalogImportFilters
...
function applyCatalogImportFilters(params, filters) {
  ...
  // Bez minCalories vrací dotaz hlavně lehké recepty, které nepadnou do žádného
  // kalorického pásma slotu — viz kcalBandForMealSlot v lib/recipesCatalog.js.
  if (f.minCalories != null) params.set('minCalories', String(f.minCalories));
  ...
  if (f.maxCarbs != null) params.set('maxCarbs', String(f.maxCarbs));
}
...
export function mapSpoonacularRecipeToCatalogRow(recipe, catalogMealType) {
  ...
  // Nepovinné — chybějící čas přípravy recept neblokuje, jen se nedá filtrovat na rychlost.
  const readyRaw = Number(recipe.readyInMinutes);
  const ready_in_minutes = Number.isFinite(readyRaw) && readyRaw > 0 ? Math.round(readyRaw) : null;
 ...
 *
 * @param {{
 *   queryRow: { id: number, meal_type: string, catalog_meal_type?: string|null, params: Record<string, unknown>, query_signature: string, next_offset: number },
 *   runId: string,
 *   dryRun?: boolean,
 * }} opts
...
export async function runSpoonacularQueryImport({ queryRow, runId, dryRun = false }) {
  ...
  // Slot říká dotaz; Spoonacular typ je jen to, co posíláme na API. Odvození přes
  // resolveImportMealTypes zůstává fallbackem pro řádky bez catalog_meal_type — na
  // 'vecere' namapovat neumí, protože Spoonacular večeři jako typ nemá.
  const catalogMealType = normalizeCatalogMealType(queryRow.catalog_meal_type)
    ?? resolveImportMealTypes(String(queryRow.params?.type || queryRow.meal_type)).catalogMealType;
  ...
  try {
    ...
    /** @type {Record<string, unknown>[]} */
    const upsertRows = [];
    /** První pád z vady v kódu — propíše se do spoonacular_import_runs.error. */
    let mapCrash = null;
    for (const recipe of filtered.kept) {
      try {
        ...
        // ReferenceError/TypeError je vada v kódu, ne vlastnost receptu. Nesmí se
        // schovat pod `map_error`, jinak vypadá jako legitimní filtr — přesně tak
        // 43 běhů unikl chybějící import nutrientAmount a vypadalo to, že recepty
        // odmítá produktové pravidlo.
        const isCodeBug = detail instanceof ReferenceError || detail instanceof TypeError;
        const reason = isCodeBug ? 'map_crash' : 'map_error';
        skippedFilterReasons[reason] = (skippedFilterReasons[reason] || 0) + 1;

        const payload = {
          source: 'import-spoonacular',
          event: reason,
          run_id: runId,
          query_signature: queryRow.query_signature,
          source_id: String(/** @type {{ id?: unknown }} */ (recipe)?.id ?? ''),
          error: detail.message,
          ...(isCodeBug
            ? { stack: String(detail.stack || '').split('\n').slice(0, 4).join(' | ') }
            : {}),
        };
        if (isCodeBug) {
          console.error(JSON.stringify(payload));
          if (!mapCrash) mapCrash = `${detail.name}: ${detail.message}`;
        } else {
    ...
    await runLogger.finish({
      ...
      quota_left: quotaLeft,
      quota_used: quotaUsed,
      // Vada v mapperu musí být vidět i v tabulce, ne jen v logu Vercelu, ktery
      // ma retenci jen par hodin.
      ...(mapCrash ? { error: `map_crash: ${mapCrash}` } : {}),
    });
...
export async function runDailySpoonacularCatalogImport(options = {}) {
  ...
  // Rotace se před výběrem sama doplní: dotazy vyčerpané déle než měsíc se
  // vrací do hry (docs/DALSI_KROK.md 9.3 krok 1). V dry runu se DB nemění.
  if (!dryRun) {
    const znovuOtevrene = await reopenExhaustedImportQueries();
    if (znovuOtevrene.length > 0) {
      logImportStructured({
        event: 'queries_reopened',
        run_id: runId,
        count: znovuOtevrene.length,
        query_ids: znovuOtevrene.map((q) => q.id),
      });
    }
  }
  ...
  if (queries.length === 0) {
    // BĚH SE ZAPISUJE I NAPRÁZDNO — docs/DALSI_KROK.md 9.3 krok 3.
    //
    // Bez řádku v `spoonacular_import_runs` nejde z dat rozlišit „cron
    // neběžel" od „cron běžel a neměl co dělat" — přesně tahle slepota
    // schovala 18 dní stojící import. Stejný princip jako zápis o běhu
    // generátoru v lib/recipeGeneratorRun.js (od 26. 8. i při nule receptů).
    //
    // `error` musí zůstat NULL: větev `import_beh_chyba` v
    // system_health_alerts_zaklad bere neprázdný error jako critical selhání,
    // a prázdný pool není selhání běhu. Důvod nese query_signature a o pool
    // samotný se stará větev `import_rotace_vycerpana` (warning).
    stoppedReason = 'pool_prazdny';
    if (!dryRun) {
      const skipLogger = createImportRunLogger(runId, 'none', 'preskoceno:pool_prazdny', 0);
      await skipLogger.finish({});
    }
    logImportStructured({ event: 'run_skipped', run_id: runId, reason: 'pool_prazdny', dry_run: dryRun });
  }
