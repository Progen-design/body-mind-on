...
export async function runSpoonacularCatalogImport(options = {}) {
  ...
  for (let page = 0; page < pages; page += 1) {
    ...
    for (const recipe of results) {
      const typedRecipe = /** @type {Record<string, unknown>} */ (recipe);
      const gate = evaluateImportGate(typedRecipe, catalogMealType);
      if (!gate.pass) {
        recordImportGateSkip(gate.reason || 'too_complex');
        console.log('[import-spoonacular] skipped', { reason: gate.reason, title: typedRecipe.title });
        continue;
      }

      const evaluation = evaluateRecipeSimplicity(typedRecipe, catalogMealType);
      if (!evaluation.pass) {
        recordRejection(evaluation.reason || 'rejected');
        continue;
      }

      candidateRecipes.push(typedRecipe);
    }
    ...
    try {
      protectedSourceIds = await fetchComputedNutritionSourceIds(
        candidateRecipes.map((r) => String(r.id ?? '')),
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
    ...
    for (const recipe of candidateRecipes) {
      const sourceId = String(recipe.id ?? '').trim();
      if (sourceId && protectedSourceIds.has(sourceId)) {
        skippedProtected += 1;
        rejected += 1;
        rejectedReason.protected = (rejectedReason.protected || 0) + 1;
        console.log('[import-spoonacular] skipped', { reason: 'protected', source_id: sourceId });
        continue;
      }
      try {
...
export async function runDailySpoonacularCatalogImport() {
  const pointsCap = maxPointsPerRun();
  let totalImported = 0;
  let totalUpdated = 0;
  let totalFetched = 0;
  let totalRejected = 0;
  let totalSkippedComplex = 0;
  let totalSkippedNotRecipe = 0;
  let totalSkippedMissingNutrition = 0;
  ...
  for (const type of CRON_IMPORT_MEAL_TYPES) {
    const remainingPoints = pointsCap - pointsUsed;
    ...
    totalSkippedComplex += result.skipped_complex;
    totalSkippedNotRecipe += result.skipped_not_recipe;
    totalSkippedMissingNutrition += result.skipped_missing_nutrition;
    totalSkippedProtected += result.skipped_protected;
    ...
    if (result.stoppedReason === 'quota_low' || result.stoppedReason === 'max_requests_cap') {
      const stopped = {
        ...
        skipped_complex: totalSkippedComplex,
        skipped_not_recipe: totalSkippedNotRecipe,
        skipped_missing_nutrition: totalSkippedMissingNutrition,
        skipped_protected: totalSkippedProtected,
        pointsUsed,
        requestsUsed,
        quotaLeft,
      ...
      console.log('[cron/import-spoonacular] summary', {
        fetched: stopped.fetched,
        imported: stopped.imported + stopped.updated,
        skipped_complex: stopped.skipped_complex,
        skipped_not_recipe: stopped.skipped_not_recipe,
        skipped_missing_nutrition: stopped.skipped_missing_nutrition,
        skipped_protected: stopped.skipped_protected,
      });
      return stopped;
    }
  }
  ...
  console.log('[cron/import-spoonacular] summary', {
    ...
    imported: done.imported + done.updated,
    skipped_complex: done.skipped_complex,
    skipped_not_recipe: done.skipped_not_recipe,
    skipped_missing_nutrition: done.skipped_missing_nutrition,
    skipped_protected: done.skipped_protected,
  });
  return done;
}
