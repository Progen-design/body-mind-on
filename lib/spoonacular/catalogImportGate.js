 ...
 * @returns {{ pass: boolean, reason: 'weak_instructions'|null, duvody: string[] }}
 */
export function evaluateInstructionsQualityGate(vstup) {
  const { ok, duvody } = posudPostup(vstup);
  return { pass: ok, reason: ok ? null : 'weak_instructions', duvody };
}
