...
async function translateBatchWithOpenAI(rows) {
  ...
  const completion = await volejModel({
    purpose: 'preklad_receptu_katalog',
    model: TRANSLATE_MODEL,
    ...
    promptSha256: TRANSLATE_PROMPT_SHA256,
    messages: [
      {
        role: 'system',
...
export async function runCatalogRecipeTranslation(options = {}) {
  ...
  for (let i = 0; i < pending.length; i += 1) {
    ...
    // BRÁNA KVALITY POSTUPU — docs/DALSI_KROK.md, laťka z lib/plan/kvalitaPostupu.js.
    // Tohle je první okamžik, kdy recept má český postup vůbec, takže je to
    // i první možné místo, kde se dá zarazit před aktivací. Překlad smí jen
    // převádět, ne dopisovat (viz prompts/catalog-translate.md) — recept,
    // který laťku nesplní, se NEAKTIVUJE a čeká na scripts/doplneni-postupu-receptu.mjs,
    // které mu (na rozdíl od překladu) smí postup přepsat.
    const kvalita = evaluateInstructionsQualityGate({
      kroky: instructionsCs,
      suroviny: updatedIngredients,
      nazev: nameCs,
    });
    if (!kvalita.pass) {
      errors.push(`Recipe ${row.id}: postup pod laťkou kvality — ${kvalita.duvody.join('; ')}`);
      continue;
    }
    // Varování NEBLOKUJÍ a NESPOUŠTĚJÍ přepis — jen se zalogují k ruční
    // kontrole (druhé kolo opravy laťky, 9. 9. 2026).
    if (kvalita.varovani.length) {
      console.log(JSON.stringify({
        source: 'catalog-translate',
        event: 'postup_varovani',
        recipe_id: row.id,
        varovani: kvalita.varovani,
      }));
    }
...
/**
 * Kolik receptů čeká na překlad — bez volání OpenAI. Používá
 * api/cron/translate-recipes.js k rozhodnutí, jestli má fronta receptů
 * vůbec práci, ještě než se rozhodne, kterou frontu (recepty/cviky) v tomto
 * běhu spustit (lib/translateQueueOrchestrator.js).
 */
export async function countRemainingUntranslated() {
