...
// PROMPT_NAKLADY_AI.md (2026-09-18) — vlastní ceník tu byl zvlášť, se stejnými
// čísly jako `OPENAI_MODEL_RATES_USD_PER_MTOK['gpt-4o-mini']` v lib/openai.js.
// Sazby teď bydlí jen tam.

const PROMPT_PATH = join(process.cwd(), 'prompts', 'prep-time-estimate.md');

/**
 * Konce řádků se normalizují na LF PŘED otiskem i před odesláním modelu.
 *
 * Bez toho je otisk vlastnost checkoutu, ne promptu: repo běží s core.autocrlf=true,
 * takže se soubor po commitu na Windows přepíše na CRLF a stejný text dostane jinou
 * SHA. Kalibrace a produkční běh by pak vypadaly jako dva různé prompty a --rescore
 * by kalibrační běhy přestal najít. Otisk má identifikovat ZADÁNÍ, ne to, na jakém
 * systému se soubor vytáhl.
 */
export const PREP_TIME_PROMPT = readFileSync(PROMPT_PATH, 'utf8').replace(/\r\n/g, '\n');
 ...
 *
 * Aktivní a pasivní čas jsou dvě POVINNÁ pole. Dřív se vracelo jedno `minutes`
 * a prompt jen prosil, ať se do něj čekání nepočítá — model to nedodržoval:
 * ze 63 receptů nad limitem jich 40 % odůvodnilo překročení namáčením nebo
 * chlazením. Rozdělením se z toho stává strukturální vlastnost odpovědi:
 * nezapočítat pasivní čas do aktivního nejde obejít, protože pro něj existuje
 * vlastní kolonka, kterou model musí vyplnit.
 *
 * passive_minutes má minimum 0 (žádné čekání), active_minutes minimum 1 —
 * jídlo, jehož příprava trvá nula minut, neexistuje.
 */
const RESPONSE_SCHEMA = {
  ...
  json_schema: {
    ...
    schema: {
      ...
      required: ['active_minutes', 'passive_minutes', 'confidence', 'reasoning'],
      properties: {
...
export function computeCostUsd({ input_tokens = 0, output_tokens = 0 }) {
  return Number(estimateOpenAICostUSD(PREP_TIME_MODEL, input_tokens, output_tokens).toFixed(6));
}

/**
 * Jedno volání modelu pro jeden recept.
 *
 * `record: false` je pro `scripts/estimate-prep-time.mjs`: ten hned po
 * naparsování odpovědi dopisuje VLASTNÍ, bohatší řádek do `ai_runs`
 * (`recipe_id`, přesný `purpose` podle režimu kalibrace/produkce, a
 * `result.active_minutes` atd., na kterém stojí `--rescore`). Bez
 * `record: false` by tu vznikly dva řádky za totéž volání.
 *
 * @param {ReturnType<typeof buildEstimateInput>} input
 * @param {{ purpose?: string, record?: boolean }} [opts]
 * @returns {Promise<{ activeMinutes: number, passiveMinutes: number, confidence: number, reasoning: string, usage: { input_tokens: number, output_tokens: number }, cost_usd: number }>}
 */
export async function estimatePrepTime(input, { purpose = 'odhad_doby_pripravy', record = true } = {}) {
  const completion = await volejModel(
    {
      ...
      promptSha256: PREP_TIME_PROMPT_SHA256,
      messages: [
  ...
  return {
    activeMinutes: Number(parsed.active_minutes),
    passiveMinutes: Number(parsed.passive_minutes),
