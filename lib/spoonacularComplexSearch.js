 ...
 * @param {number} ctx.number - počet výsledků (shortlist; výchozí 2, max 25)
...
export function buildComplexSearchQueryString(ctx, apiKey) {
  ...
  params.set('number', String(Math.max(1, Math.min(Number(number) || 2, 25))));
  ...
  /** fillIngredients=false — Spoonacular účtuje +0.025 b/recipe; suroviny stejně doplňuje information při potřebě (mealEnrichment). */
