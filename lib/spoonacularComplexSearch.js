...
export function calorieRangeForMealType(mealType, caloriesPerDay, mealsPerDay) {
  ...
  /** U 2400+ kcal/den musí být i snack „jídlo“, ne 150kcal mini – jinak dny padají pod 72 % součtu. */
  const heavy = day >= 2400;
  const pct = {
    breakfast: { lo: heavy ? 0.24 : 0.22, hi: heavy ? 0.37 : 0.36 },
    lunch: { lo: heavy ? 0.28 : 0.26, hi: heavy ? 0.43 : 0.42 },
    dinner: { lo: heavy ? 0.26 : 0.24, hi: heavy ? 0.41 : 0.4 },
    snack: { lo: heavy ? 0.13 : 0.07, hi: heavy ? 0.2 : 0.18 },
  };
