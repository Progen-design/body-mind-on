 ...
 * Agregace napříč jídly/dny: stejná surovina + jednotka + (syrové/suché/vařené).
 */
import {
 ...
 * Agreguje stejné suroviny napříč jídly dne.
 */
export function aggregateShoppingIngredientLinesForDayIndex(planJson, dayIndex) {
  ...
  return aggregateShoppingIngredientLines(collectShoppingLinesFromMeals(d.meals));
}
...
export function aggregateShoppingIngredientLinesFromStructuredPlan(planJson) {
  ...
  const allLines = [];
  for (const d of days) {
    allLines.push(...collectShoppingLinesFromMeals(d?.meals));
  }
