...
const AGENT_NAME_FOOD_TERMS = [
  ...
  { key: 'ovoce', aliases: ['ovoce', 'ovoci', 'banán', 'jabl', 'malin', 'borův'] },
  { key: 'pečiv', aliases: ['pečiv', 'toast', 'chléb', 'chleba'] },
...
export function getAgentIngredientAlignmentReason(row, slotMeal) {
  ...
  const ingNorm = normalizeMatchText(ingredientLinesFromRow(row).join(' '));
  const matched = required.filter((term) => ingredientTextMatchesTerm(ingNorm, term));
  const minRequired = Math.max(2, Math.ceil(required.length * 0.6));
  if (matched.length >= minRequired) return null;
  return 'agent_ingredient_mismatch';
}
