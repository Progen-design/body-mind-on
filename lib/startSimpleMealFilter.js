 ...
 * @returns {string|null}
 */
export function getCriticalNameIngredientAlignmentReason(name, ingNorm) {
  const nameNorm = normalizeMatchText(name);
  if (!nameNorm || !ingNorm) return null;

  if (includesAnyToken(nameNorm, ALIGNMENT_TOKEN_GROUPS.pastryName) && !includesAnyToken(ingNorm, ALIGNMENT_TOKEN_GROUPS.pastryIngredient)) {
...
export function getAgentIngredientAlignmentReason(row, slotMeal) {
  ...
  const critical = getCriticalNameIngredientAlignmentReason(slotMeal.name_cs, ingNorm);
