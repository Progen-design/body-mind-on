...
export function getAgentIntentMismatchReason(row, slotMeal) {
  ...
  const text = normalizeMatchText(combinedText(row));
  const forbidden = [
    ...(slotMeal.forbidden_catalog_terms || []),
    ...(slotMeal.name_cs ? [] : []),
  ];
  for (const term of forbidden) {
    const t = normalizeMatchText(term);
    if (t && text.includes(t)) return `forbidden_term:${term}`;
  }
...
export function getSimpleStartBlockReason(recipe, slotMeal) {
  ...
  const hard = getHardStartBlockReason(recipe, mealType);
  if (hard) return hard;
  if (slotMeal?.allowed_catalog_match_terms?.length) {
    return getAgentIntentMismatchReason(recipe, slotMeal);
  }
  return null;
}
