...
import { buildSimpleStartLibraryMeal, hasSimpleStartRecipeTitle } from './simpleStartRecipeLibrary.js';
 ...
 * @returns {string[]}
 */
export function buildSimpleFallbackInstructions(displayName, shoppingLines = []) {
  const name = String(displayName || '').toLowerCase();
  if (/rýž|ryz/i.test(name) && /vejce|vajec/i.test(name)) {
    return ['Uvař rýži podle návodu na obalu.', 'Na pánvi připrav vejce a krátce přidej zeleninu.', 'Smíchej rýži s vejci a zeleninou, zakápni olejem a podávej.'];
  }
  if (/tvaroh|jogurt|cottage|kefír/i.test(name)) {
    return ['Všechny suroviny dej do misky nebo talíře.', 'Promíchej a sněz.'];
  }
  if (/sendvič|pečiv/i.test(name)) {
    return ['Připrav pečivo a náplň.', 'Slož sendvič a podávej se zeleninou.'];
  }
  if (/vejce|omeleta/i.test(name)) {
    return ['Připrav vejce podle zvyku (vařená, míchaná nebo omeleta).', 'Podávej s pečivem nebo zeleninou.'];
  }
  if (/rýž|těstovin|brambor|čočk|fazole/i.test(name)) {
    return ['Uvař přílohu podle návodu na obalu.', 'Připrav maso nebo vejce a zeleninu.', 'Všechno dej na talíř a dochutíš solí a pepřem.'];
  }
  if (/kuře|krůt|tuňák/i.test(name)) {
    return ['Maso osol a opepři.', 'Uvař nebo opeč maso s přílohou.', 'Podávej se zeleninou.'];
  }
  if (shoppingLines.length <= 3) {
    return ['Připrav suroviny podle seznamu.', 'Slož jídlo a ihned sněz.'];
  }
  return ['Připrav suroviny podle seznamu.', 'Uvař nebo poskládej jídlo — bez složitých omáček.', 'Podávej hned, sol a pepř dle chuti.'];
}
...
export function buildStartSafeFallbackMeal(slotMeal, slotTarget, seed = 0) {
  ...
  if (
    ...
    && hasSimpleStartRecipeTitle(slotMeal?.name_cs, slotMealType)
  ) {
    return buildSimpleStartLibraryMeal(slotMeal?.name_cs, slotMealType, {
  ...
  const simple_instructions_cs = buildSimpleFallbackInstructions(display_name_cs, shopping_ingredient_lines);
