...
const IMPERIAL_UNIT_RE = /\b\d+(\.\d+)?\s*(libra|libry|lb|lbs|cups?|oz|ounces?|tbsp|tablespoons?|tsp|teaspoons?)\b/i;
const ABSURD_SALT_RE = /\b\d+\s*porc[eí]\s+sol|\bserving(s)?\s+of\s+salt\b/i;
...
export function getFullContentStartBlockReason(row, mealType = 'lunch', slotMeal = null) {
  ...
  if (slotMeal?.allowed_catalog_match_terms?.length) {
    return getAgentIntentMismatchReason(row, slotMeal);
  }
