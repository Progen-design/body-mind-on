 ...
 */
import { ingredientLinesForValidation, ingredientLinesFromRow, scoreRecipeSimplicity, sanitizeIngredientLineForDisplay } from './recipeSimplicityScore.js';
import { applyPortionScaleToStructuredMeal, clampedPortionMultiplier } from './nutrition/portionScaling.js';
import {
...
import { buildDietaryPublishRules, checkCandidateAgainstDiet, maRestriktivniDietu } from './dietaryRules.js';
...
const START_SAFE_FALLBACK_BY_TYPE = {
  ...
  snack: [
    ...
    },
    {
      ...
      carbs_g: 28,
      fat_g: 10,
      shopping_ingredient_lines: ['celozrné pečivo 2 plátky', 'šunka 60 g', 'zelenina'],
    },
  ],
  lunch: [
    {
...
export function getStrictBreakfastReason(row, mealType) {
  ...
  // Metrické řádky, ne zdrojový `original` — jinak se blokuje na jednotce,
  // kterou uživatel nikdy neuvidí. Viz `ingredientLinesForValidation`.
  const ingLines = ingredientLinesForValidation(row);
...
export function getFullContentStartBlockReason(row, mealType = 'lunch', slotMeal = null) {
  ...
  // Vlastní generované recepty tímhle filtrem neprocházejí.
  //
  // PROČ: tenhle filtr je obrana proti importu ze Spoonacularu — blokuje food
  // blog názvy, imperiální jednotky, dvacetipoložkové seznamy surovin, dezerty
  // vydávané za snídani. Recept, který si vygenerujeme sami, tyhle vady mít
  // nemůže: prompt ho staví jen ze surovin ve slovníku, s limitem na počet
  // surovin, na aktivní čas podle slotu, česky a na jednu porci. Pouštět ho
  // přes heuristiku psanou na cizí data znamená zahazovat vlastní práci —
  // změřeno na produkci, filtr propouštěl 19 ze 128 snídaní.
  //
  // Tvrdé brány TOHLE NEOBCHÁZÍ a platí dál: nutrice se počítá ze surovin,
  // Atwater, počet hlavních surovin, dietní příznaky a kalorické pásmo slotu.
  // Ty jsou v triggeru enforce_recipe_catalog_rules a v zápisu generátoru.
  if (String(row.source || '') === 'llm_generated') return null;
 ...
 * @param {object[]} [excluded] jídla, která se nesmí vrátit (nahrazované + ostatní jídla dne).
 *   Prázdné pole = dnešní chování beze změny — generátor plánu (buildSimpleStartMealSkeleton
 *   → resolveSimpleStartLocalSlot) tenhle parametr neposílá vůbec, jen záměna
 *   jídla (lib/planMealReplace.js), která ví, co nahrazuje.
 * @returns {{ row: null, meal: object }}
 */
export function resolveSimpleStartLocalSlot(slotMeal, slotTarget, slotSalt = 0, bodyMetrics = null, excluded = []) {
  const restrictiveDiet = maRestriktivniDietu(bodyMetrics);
  const mealType = slotMeal?.type || 'lunch';
  const exclusions = parseDietaryExclusions(bodyMetrics || slotMeal?._body_metrics || {});
  let resolvedTitle = resolveSimpleStartTitle(slotMeal?.name_cs);

  if (mealContainsExcludedFood({ name_cs: resolvedTitle, ...slotMeal?.fallback_meal_template }, exclusions)) {
    resolvedTitle = cheeseFreeAlternativeName(mealType);
  }
  ...
  // `excluded.some(...)`: knihovní jídlo pro `resolvedTitle` může vyjít stejné
  // jako to, co se nahrazuje (typicky když picker výš žádnou lepší shodu
  // nenašel a vrátil zpátky totéž jméno) — bez týhle kontroly by se vrátilo
  // rovnou tady, ještě před fallbackem, který o vyloučení už ví.
  if (libraryMeal && !excluded.some((ex) => isSameSimpleStartMeal(libraryMeal, ex)) && !mealContainsExcludedFood(libraryMeal, exclusions)) {
    const meal = JSON.parse(JSON.stringify(libraryMeal));
    const baseKcal = Number(meal.kcal) || 0;
    const targetMult = clampedPortionMultiplier(baseKcal, slotTarget, { simpleStartMode: true, restrictiveDiet });
    applyPortionScaleToStructuredMeal(meal, targetMult, {
      allowUnverified: true,
      simpleStartMode: true,
    });
    meal.name_cs = meal.display_name_cs;
    return {
      row: null,
      meal,
    };
  ...
  return {
    ...
    meal: buildStartSafeFallbackMeal(
      { ...slotMeal, name_cs: resolvedTitle || slotMeal?.name_cs, _body_metrics: bodyMetrics },
      slotTarget,
      slotSalt,
      excluded
    ),
...
export function buildStartSafeFallbackMeal(slotMeal, slotTarget, seed = 0, excluded = []) {
  const slotMealType = slotMeal?.type || 'lunch';
  const bodyMetrics = slotMeal?._body_metrics || {};
  const exclusions = parseDietaryExclusions(bodyMetrics);

  // DIETA PLATÍ I V NOUZOVÉ CESTĚ.
  //
  // Do 10. 8. 2026 tahle funkce hlídala jen vyloučené potraviny (mléko, sýr,
  // volný text), o lepku nevěděla nic a snapshot vybírala jen podle meal_type
  // a vzdálenosti od cíle. Výsledek nebyl „nedokonalá náhrada“, ale ZHORŠENÍ:
  // změřeno, že „Tvaroh s ovocem“ (bezlepkový) nahradila snapshotem „Cottage
  // s pečivem“ a „Tvaroh s vločkami“ snapshotem „Vejce s pečivem“. Do plánu
  // tak lepek přidala právě ta cesta, která ho měla odklidit.
  const rules = buildDietaryPublishRules(bodyMetrics);
  const dietOk = (candidate) => checkCandidateAgainstDiet(candidate, rules).ok;
  // STEJNÁ DÍRA, JINÝ DRUH VADY (produkční nález, PR #233): tahle funkce
  // vybírala nezávisle na tom, co se nahrazuje, takže uměla vrátit přesně
  // nahrazované jídlo — u standard/breakfast se snapshotem #308 „Vejce
  // s pečivem a zeleninou“ 100 % jistě, protože to je zároveň nejbližší
  // kalorický snapshot. `excluded` (nahrazované jídlo + ostatní jídla dne,
  // z lib/planMealReplace.js) se kontroluje na každém místě níž, kde tahle
  // funkce kandidáta staví — ne jen jednou na konci, protože kandidát může
  // vzniknout čtyřmi různými cestami.
  const jeVyloucene = (candidate) => excluded.some((ex) => isSameSimpleStartMeal(candidate, ex));

  let resolvedTitle = resolveSimpleStartTitle(slotMeal?.name_cs);
  if (mealContainsExcludedFood({ name_cs: resolvedTitle, ...slotMeal?.fallback_meal_template }, exclusions)) {
    resolvedTitle = cheeseFreeAlternativeName(slotMealType);
  }
  if (hasSimpleStartRecipeTitle(resolvedTitle, slotMealType)) {
    ...
    if (libraryMeal && !jeVyloucene(libraryMeal) && dietOk(libraryMeal)) {
      const meal = JSON.parse(JSON.stringify(libraryMeal));
      const targetMult = clampedPortionMultiplier(Number(meal.kcal) || 0, slotTarget, { simpleStartMode: true });
      applyPortionScaleToStructuredMeal(meal, targetMult, {
        allowUnverified: true,
        simpleStartMode: true,
      });
    ...
    if (libraryMeal) {
      console.warn('[start-fallback] knihovni jidlo neproslo dietou nebo je vyloucene', {
        title: resolvedTitle,
        diet_type: rules.dietType,
        vylouceno: jeVyloucene(libraryMeal),
        reason: jeVyloucene(libraryMeal) ? 'excluded' : checkCandidateAgainstDiet(libraryMeal, rules).code,
      });
    }
  }
  const mealType = slotMeal?.type || 'lunch';
  // Prefer real catalog snapshot macros over invented START_SAFE_FALLBACK numbers.
  // Snapshot nese jen název a makra, žádné suroviny — dieta se proto posuzuje
  // na jídle vyrobeném z knihovny podle toho názvu, ne na snapshot řádku.
  const snapPool = SIMPLE_START_CATALOG_SNAPSHOT.filter((s) => {
    if (String(s.meal_type) !== String(mealType)) return false;
    if (jeVyloucene(s)) return false;
    const probe = buildSimpleStartLibraryMeal(String(s.title || '').trim(), mealType, {})
      || { name_cs: s.title, kcal: s.kcal, carbs_g: s.carbs_g };
    return dietOk(probe);
  });
  const targetKcal = Number(slotTarget) || Number(slotMeal?.fallback_meal_template?.kcal) || 400;
  // `|| SIMPLE_START_CATALOG_SNAPSHOT[0]` tu stávalo jako poslední záchrana a
  // byla to díra: obcházela `snapPool`, takže při prázdném poolu vrátila první
  // řádek snapshotu bez ohledu na dietu. Když pro dietu není čistý snapshot,
  // pokračuje se na šablony níž, které dietu kontrolují.
  const snap = [...snapPool].sort(
    (a, b) => Math.abs(a.kcal - targetKcal) - Math.abs(b.kcal - targetKcal)
  )[0] || null;

  if (snap) {
    console.error('[start-fallback] using catalog snapshot meal (not invented macros)', {
      agent_name: slotMeal?.name_cs ?? null,
      snapshot_id: snap.id,
      snapshot_title: snap.title,
      meal_type: mealType,
    });
    const catalogTitle = String(snap.title || '').trim();
    const meal = buildSimpleStartLibraryMeal(catalogTitle, mealType, {
      planner_source: slotMeal?.planner_source || 'simple_meal_planner_agent',
    }) || {
      type: mealType,
      name_cs: catalogTitle,
      display_name_cs: catalogTitle,
      display_name: catalogTitle,
      recipe_verified: true,
      shopping_ingredient_lines: [],
      recipe: {},
    };
    // Names must match the catalog snapshot row, never the agent slot title.
    meal.name_cs = catalogTitle;
    meal.display_name_cs = catalogTitle;
    meal.display_name = catalogTitle;
    const slotHint = String(slotMeal?.name_cs || '').trim();
    meal.planner_suggestion_cs =
      slotHint && slotHint !== catalogTitle ? slotHint : null;
    meal.kcal = snap.kcal;
    meal.protein_g = snap.protein_g;
    meal.carbs_g = snap.carbs_g;
    meal.fat_g = snap.fat_g;
    meal.catalog_id = snap.id;
    meal.catalog_source = 'simple_start';
    meal.recipe_id = snap.id;
    meal.recipe_verified = true;
    if (meal.recipe) {
      meal.recipe.id = snap.id;
      meal.recipe.title = catalogTitle;
      meal.recipe.title_cs = catalogTitle;
      meal.recipe.calories = snap.kcal;
      meal.recipe.protein_g = snap.protein_g;
      meal.recipe.carbs_g = snap.carbs_g;
      meal.recipe.fat_g = snap.fat_g;
  ...
  if (agentTpl?.name_cs && agentTpl?.kcal
    && !jeVyloucene(agentTpl)
    && !mealContainsExcludedFood(agentTpl, exclusions)
    && dietOk(agentTpl)) {
    tpl = agentTpl;
  } else {
    const templates = START_SAFE_FALLBACK_BY_TYPE[mealType] || START_SAFE_FALLBACK_BY_TYPE.lunch;
    // `pool = allowed.length ? allowed : templates` byl druhý tichý bypass —
    // při prázdném výběru vrátil celý seznam včetně toho, co dietu porušuje.
    // Dieta se neuvolňuje: když nezbude nic, vrací se null a volající to musí
    // řešit (žádný plán je lepší než plán s lepkem pro celiaka). Stejné
    // pravidlo teď platí i pro `excluded` — prázdný `allowed` kvůli
    // vyloučením vrací null, ne nefiltrovaný seznam.
    const allowed = templates.filter(
      (item) => !jeVyloucene(item) && !mealContainsExcludedFood(item, exclusions) && dietOk(item)
    );
    if (!allowed.length) {
      console.error('[start-fallback] pro dietu/vyloucene nezbyla zadna nouzova sablona', {
        meal_type: mealType,
        diet_type: rules.dietType,
        agent_name: slotMeal?.name_cs ?? null,
      });
    ...
    const idx = Math.abs(Number(seed) || 0) % allowed.length;
    tpl = allowed[idx];
  }
  ...
  const meal = {
    ...
    recipe: {
      ...
      portion_multiplier: 1,
      ingredients: [...shopping_ingredient_lines],
    },
    image_url: null,
    image_trust_level: 'none',
    shopping_ingredient_lines: [...shopping_ingredient_lines],
