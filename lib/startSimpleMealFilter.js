...
export function buildStartSafeFallbackMeal(slotMeal, slotTarget, seed = 0) {
  ...
  const exclusions = parseDietaryExclusions(slotMeal?._body_metrics || {});
  let resolvedTitle = resolveSimpleStartTitle(slotMeal?.name_cs);
  if (mealContainsExcludedFood({ name_cs: resolvedTitle, ...slotMeal?.fallback_meal_template }, exclusions)) {
  ...
  const snapPool = SIMPLE_START_CATALOG_SNAPSHOT.filter(
    (s) => String(s.meal_type) === String(mealType)
  );
  ...
  )[0] || SIMPLE_START_CATALOG_SNAPSHOT[0];
  ...
  let tpl;
  if (agentTpl?.name_cs && agentTpl?.kcal && !mealContainsExcludedFood(agentTpl, exclusions)) {
    tpl = agentTpl;
  } else {
    const templates = START_SAFE_FALLBACK_BY_TYPE[mealType] || START_SAFE_FALLBACK_BY_TYPE.lunch;
    const allowed = templates.filter((item) => !mealContainsExcludedFood(item, exclusions));
