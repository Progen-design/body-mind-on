...
/**
 * Přeskočená událost. `errorMessage` NENÍ chyba serveru — je to důvod, proč se
 * událost nezpracovala, a musí zůstat v DB.
 *
 * Do 13. 8. 2026 se zahazoval: dva `skipped_unknown_price` z 12. 8. měly
 * `error_message: null`, takže z databáze nešlo zjistit, KTERÉ price ID
 * neznáme. Bez toho se nedá ani poznat, jestli šlo o překlep v env, nebo
 * o zákazníka, který zaplatil za produkt, co neumíme obsloužit.
 *
 * @param {string} eventId
 * @param {string} result
 * @param {string|null} [errorMessage]
 */
export async function skipStripeEvent(eventId, result, errorMessage = null) {
  if (!errorMessage) {
  ...
  const { error } = await supabaseServer
    ...
    .eq('stripe_event_id', eventId);
  if (error && !isMissingTableError(error)) {
    console.error('[stripeEventStore] skip mark failed:', error.message);
  }
}
