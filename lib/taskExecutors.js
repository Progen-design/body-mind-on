...
import { buildCoachFallbackMessage, isOpenAIQuotaOrRateLimitError } from './coachFallbackMessage.js';
...
export async function executeCoachTask(task) {
  ...
  try {
    ...
    console.warn('[executeCoachTask] OpenAI quota/rate limit — using fallback message', {
      task_id: task.id,
      user_id: task.user_id,
    ...
    fallbackMeta = { reason: 'openai_quota_or_rate_limit' };
  }
