...
/**
 * Last-resort: katalogový plán (recipes_catalog + portion scaling) když AI selhalo při registraci.
 * Holá HTML šablona jen když katalogový pipeline také selže.
  ...
  try {
    ...
    if (catalogPipeline?.ok && catalogPipeline.planHtml) {
      const persisted = await persistPlanFromUnified(catalogPipeline, { ...bm, user_id: userId }, {
        generatedBy: 'reg_catalog_rescue',
        deactivateOld: true,
      });
      if (persisted?.plan_id) {
        console.info('[persistPublishableFallbackPlanForUser] catalog rescue plan persisted', {
          plan_id: persisted.plan_id,
          user_id: userId,
          daily_calories: catalogPipeline.targets?.calories_per_day ?? null,
        });
        return {
          plan_id: persisted.plan_id,
