...
/**
 * Týdenní rozsah pro initial_plan: první den = kalendářní den registrace (created_at metrik) v Praze,
 * aby první den plánu odpovídal dni registrace, ne „náhodnému“ UTC dni při pozdním běhu úlohy.
 */
function initialPlanWeekRangeFromRegistration(bm) {
  const reg = bm?.created_at ? new Date(bm.created_at) : new Date();
  const from = calendarDateIsoInPrague(reg);
  return { from, until: addCalendarDaysIsoPrague(from, 6) };
}
  ...
  if (task.task_type === 'initial_plan' && bm?.email && finalGenerated?.html) {
    ...
    if (!emailValid.ok) {
      ...
      const sendResult = await sendPlanEmail(bm.email, planHtml, {
        ...
        bodyMetrics: bm,
        // První e-mail po registraci = celý týden jídel i trénink (nezávisle na výchozím PLAN_OUTPUT_MODE).
        planOutputMode: 'nutrition_training',
      });
