...
/**
 * Týdenní rozsah pro initial_plan / nový insert: od data registrace (body_metrics.created_at),
 * ne z předchozího řádku plánu (ten míval chybné valid_from v budoucnosti).
 * Registrace v minulosti → začátek od dnes (aktuální sedmidenní okno).
 */
function initialPlanWeekRangeFromRegistration(bm) {
  const todayIso = new Date().toISOString().split('T')[0];
  const registrationIso = bm?.created_at
    ? new Date(bm.created_at).toISOString().split('T')[0]
    : todayIso;
  const regCapped = registrationIso > todayIso ? todayIso : registrationIso;
  const from = regCapped < todayIso ? todayIso : regCapped;
