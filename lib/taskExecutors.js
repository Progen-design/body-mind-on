...
/** Sedm po sobě jdoucích kalendářních dní od dnes: valid_from = dnes, valid_until = dnes + 6. */
function sevenDayRangeFromTodayIso() {
  const todayIso = new Date().toISOString().split('T')[0];
  return { from: todayIso, until: addCalendarDaysIso(todayIso, 6) };
}

/**
 * Týdenní rozsah pro initial_plan: první den = den generování (registrace), dalších 6 dní = 7 dní celkem.
 */
function initialPlanWeekRangeFromRegistration(_bm) {
  return sevenDayRangeFromTodayIso();
}
