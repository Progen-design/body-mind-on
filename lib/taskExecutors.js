...
import { validatePublishedPlanHtml } from './validatePlanHtml.js';
import { runUnifiedPlanPipeline, persistPlanFromUnified } from './unifiedPlanPipeline.js';
import { prepocitejCilPriObnove } from './weeklyWeightRecalc.js';
import { recordProductEvent } from './recordProductEvent.js';
import { schedulePlanEnhancementAsync } from './services/planEnhancementAsync.js';
import {
  buildCoachFallbackMessage,
  getOpenAICoachFallbackReason,
  isOpenAICoachFallbackError,
} from './coachFallbackMessage.js';
import { getHabitById } from './habits.js';
import { getNextPlanRangeFromCurrentPlan, getNextWeekRange, buildDeterministicFallbackPlanHtml } from './generatePlan.js';
import { getTaskSchemaHintAsync, getTaskSpecAsync } from './aiTaskRegistry.js';
import { sendPlanEmail } from './mail.js';
import { getAgentConfig } from './getAgentConfig.js';
import { writeAILog } from './aiOps.js';
import { buildDietaryPublishRules, hasAnyDietaryRestriction } from './dietaryPublishGate.js';
import { writeSharedMemoryFact } from './aiSharedMemory.js';
import { addCalendarDaysIsoPrague, calendarDateIsoInPrague, trialStartIsoPrague } from './czechCalendar.js';
import { jePlatnyDnes, todayIso } from './weeklyPlanProducer.js';
import { taskPayloadAllowsSpoonacularRegistration } from './spoonacularQuotaGate.js';
...
// docs/DALSI_KROK.md 9.1/D — `habits` odsud vypadlo. Registrační initial_plan
// běží vždy s `useOpenAI: false` (createInitialAITasks.js task payload
// nenastavuje `useOpenAI`), tedy výhradně přes deterministický/katalogový
// pipeline (`simpleStartMode`) — ten nemá žádnou cestu, jak vyplnit
// `planJson.habits`/`mindset_week`/`mindset`/`mindset_tip`, ani jak dostat
// tahle slova do HTML. `planHasHabitsModule()` proto nemohla uspět u ŽÁDNÉ
// z deseti čerstvých registrací 7. 9. 2026 — ne proto, že by modul chyběl,
// ale protože je pro tuhle cestu nedosažitelný ze své podstaty. Skutečné
// návyky se sbírají jinudy a fungují: `selected_habits` z registračního
// formuláře se ukládají do `user_habits` v `api/body-metrics.js`, nezávisle
// na `ai_tasks`/`initial_plan`.
const INITIAL_PLAN_REQUIRED_MODULES = ['nutrition', 'training'];

function getInitialPlanModuleMeta(task) {
  const required =
    Array.isArray(task?.payload?.required_modules) && task.payload.required_modules.length
      ? task.payload.required_modules
      : INITIAL_PLAN_REQUIRED_MODULES;
  const planScope = task?.payload?.plan_scope || 'initial_7_day_trial';
  return { required_modules: required, plan_scope: planScope };
}
...
function dayHasWorkoutPart(day) {
  const w = day?.workout;
  if (w == null) return true;
  const exs = w?.exercises ?? [];
  if (exs.length === 0) return false;
  return exs.some((ex) => String(ex?.canonical_key || '').toLowerCase() !== 'rest');
}

function planHasTrainingModule(days) {
  if (!Array.isArray(days) || days.length === 0) return false;
  if (!days.every((d) => dayHasWorkoutPart(d))) return false;
  return days.some((d) => {
...
function planHasHabitsModule(planJson, planHtml) {
  const candidates = [planJson?.habits, planJson?.mindset_week, planJson?.mindset, planJson?.mindset_tip];
  for (const c of candidates) {
    if (Array.isArray(c) && c.length > 0) return true;
    if (typeof c === 'string' && c.trim().length > 8) return true;
    if (c && typeof c === 'object' && !Array.isArray(c) && Object.keys(c).length > 0) return true;
  }
  const html = String(planHtml || '');
  return /návyk|navyk|habit|mindset|Návyky/i.test(html);
}

function validateInitialPlanModules(planJson, planHtml, requiredModules) {
  const days = Array.isArray(planJson?.days) ? planJson.days : [];
  const missing = [];
  const completed = [];

  if (requiredModules.includes('nutrition')) {
    if (days.length === 7 && days.every(dayHasMeals)) completed.push('nutrition');
    else missing.push('nutrition');
  }

  if (requiredModules.includes('training')) {
    if (planHasTrainingModule(days)) completed.push('training');
    else missing.push('training');
  }

  if (requiredModules.includes('habits')) {
    if (planHasHabitsModule(planJson, planHtml)) completed.push('habits');
    else missing.push('habits');
  }

  return { ok: missing.length === 0, missing_modules: missing, completed_modules: completed };
}
...
/**
 * 7denní rozsah pro initial_plan: od dne registrace (Europe/Prague), ne příští pondělí.
 */
function initialPlanWeekRangeFromRegistration(bm) {
  const from = trialStartIsoPrague(bm?.created_at ? new Date(bm.created_at) : new Date());
  return { from, until: addCalendarDaysIsoPrague(from, 6) };
}
...
function buildCoachTitle(taskType) {
  const titles = {
    ...
    apple_health_daily_review: 'Denní vyhodnocení Apple Watch',
  };
...
async function persistTrainerPlan({ userId, bm, generated, taskType, latestPlan, selectedHtmlSource = null, planRange: planRangeFromPipeline = null }) {
  ...
  // OSTRÁ CHYBA (8. 9. 2026, viz jePlatnyDnes v weeklyPlanProducer.js):
  // dřív se tu deaktivoval VŠECHNO, co bylo aktivní, bez ohledu na to, jestli
  // nový plán (range.from/until) vůbec dnešek pokrývá. `weekly_plan_update`
  // vzniká `WEEKLY_PRODUCER_LEAD_DAYS` (1) den před koncem platnosti aktuálního
  // plánu, takže se typicky generuje o den dřív, než nový plán začne — a starý
  // plán, který dnešek ještě pokrýval, tak zůstal bez náhrady až do půlnoci.
  // Deaktivuje se proto jen to, co dnešek OPRAVDU přestalo pokrývat.
  const dnesProAktivaci = todayIso();
  await supabaseServer
    .from('ai_generated_plans')
    .update({ is_active: false })
    .eq('user_id', userId)
    .eq('is_active', true)
    .or(`valid_until.lt.${dnesProAktivaci},valid_from.gt.${dnesProAktivaci}`);
  ...
  const insertPayload = {
    ...
    // Aktivní jen když okno platnosti opravdu pokrývá dnešek — plán na
    // budoucí týden se vloží jako neaktivní a aktivuje ho noční
    // sync_plan_activation() (supabase/migrations/20260823200000) v den,
    // kdy začne.
    is_active: jePlatnyDnes(range.from, range.until, dnesProAktivaci),
  };
  ...
  console.info('[persistTrainerPlan] plan_insert_success', { plan_id: data?.id ?? null, is_active: insertPayload.is_active, user_id: userId });
  return {
...
export async function executeTrainerTask(task) {
  let bm = await loadLatestBodyMetrics(task.user_id);
  const latestPlan = await loadLatestPlan(task.user_id);
  const forceRegenerate = task.payload?.force_regenerate === true;

  // Kalorický cíl se přepočítá z odvozené váhy (7denní medián vážení) JEN tady,
  // při týdenní obnově — ne po každém vážení. `initial_plan` se nepřepočítává
  // schválně: v den registrace žádná řada vážení neexistuje a registrační váha
  // je to jediné, co o člověku víme.
  //
  // Když měření chybí (za 14 dní nic), funkce cíl nechá být a vrátí `bm`
  // beze změny. Registrační snímek v `body_metrics.weight_kg` se nepřepisuje
  // nikdy — mění se jen `calories_target`.
  if (task.task_type === 'weekly_plan_update') {
    const prepocet = await prepocitejCilPriObnove(supabaseServer, bm, { taskId: task.id });
    bm = prepocet.bodyMetrics;
  }
  ...
  let planRange;
  if (task.task_type === 'weekly_plan_update') {
    // `target_from` posílá producent (lib/weeklyPlanProducer.js) a je to TÁŽ hodnota,
    // na které stojí idempotence úlohy. Musí mít přednost před odvozením z
    // latestPlan.valid_until: to vrací rozsah začínající posledním dnem starého
    // plánu, takže u doběhu po delší pauze by úloha vyrobila plán na týden, který
    // už dávno uplynul. Klíč v ai_tasks a rozsah v ai_generated_plans by se navíc
    // rozešly a druhý běh producenta by týž plán založil znovu pod jiným klíčem.
    const targetFrom = typeof task.payload?.target_from === 'string' && task.payload.target_from
      ? task.payload.target_from
      : null;

    if (targetFrom) {
      planRange = { from: targetFrom, until: addCalendarDaysIsoPrague(targetFrom, 6) };
    } else if (forceRegenerate) {
  ...
  const moduleMeta = task.task_type === 'initial_plan' ? getInitialPlanModuleMeta(task) : null;
  // weekly + initial: always resolve meals with honesty fill (catalog path)
  const simpleStartMode =
    task.task_type === 'initial_plan' && moduleMeta?.plan_scope === 'initial_7_day_trial';

  const pipelineResult = await runUnifiedPlanPipeline({
    ...existingOpts,
    useOpenAI: task.payload?.useOpenAI === true,
    allowLiveSpoonacular: taskPayloadAllowsSpoonacularRegistration(task),
    onboardingSoftGate: task.task_type === 'initial_plan',
    ...(moduleMeta ? { plan_scope: moduleMeta.plan_scope } : {}),
    simpleStartMode,
  });
  ...
  if (!pipelineResult?.ok) {
    ...
    if (task.task_type === 'initial_plan' && task.user_id) {
      await recordProductEvent({
        user_id: task.user_id,
        event_name: 'plan_generation_failed',
        properties: { program: 'START', error_code: 'openai_error', success: false },
        source: 'taskExecutors',
      });
    }
  ...
  if (task.task_type === 'initial_plan') {
    const moduleMeta = getInitialPlanModuleMeta(task);
    const moduleCheck = validateInitialPlanModules(
      rawParsed,
      chosenHtml,
      moduleMeta.required_modules
    );
    if (!moduleCheck.ok) {
      console.warn('[executeTrainerTask] initial_plan module gaps (non-blocking for START onboarding)', {
        missing_modules: moduleCheck.missing_modules,
        completed_modules: moduleCheck.completed_modules,
        plan_scope: moduleMeta.plan_scope,
      });
    }
  }
  ...
  const generated = {
    ...
    fallback_used: pipelineResult.generation_source === 'fallback' || pipelineResult.generation_source === 'catalog',
    truth_check: null,
  ...
  // KDO DOSTANE PLÁN E-MAILEM.
  //
  // Do 14. 8. 2026 tahle podmínka zněla `task.task_type === 'initial_plan'`,
  // takže e-mail chodil JEN u prvního plánu po registraci. `weekly_plan_update`
  // plán vygeneroval, uložil, označil za aktivní — a nikomu ho neposlal.
  // Uživatel se o novém týdnu dozvěděl jen tak, že si sám otevřel aplikaci.
  //
  // Obě cesty jdou přes `sendPlanEmail`, tedy přes dietní bránu na hranici
  // publikace. Posílat weekly jinudy by znamenalo druhou cestu k člověku,
  // kterou brána nehlídá — a právě to je to, čemu se ta brána snaží zabránit.
  const posilatEmail = task.task_type === 'initial_plan' || task.task_type === 'weekly_plan_update';
  const jeNavazujiciTyden = task.task_type === 'weekly_plan_update';
  const emailZdroj = jeNavazujiciTyden ? 'trainer_weekly_plan_update' : 'trainer_initial_plan';

  if (posilatEmail && bm?.email && finalGenerated?.html) {
    ...
    if (planAlreadyEmailed) {
      ...
      if (!emailValid.ok) {
        ...
        if (!claimedSend) {
          ...
          // SELHÁNÍ E-MAILU NESMÍ SHODIT ÚLOHU. Plán je v tu chvíli vygenerovaný,
          // uložený a aktivní; kdyby výjimka z odesílání probublala ven, úloha
          // by skončila jako `failed` a scheduler by ji přehrál — tedy vyrobil
          // plán znovu. `sendPlanEmail` sice chyby vrací jako `{ ok: false }`,
          // ale dietní brána i příprava šablony uvnitř házet můžou.
          let sendResult = null;
          try {
            sendResult = await sendPlanEmail(bm.email, planHtml, {
              ...
              planId,
              // Navazující týden má vlastní předmět, ať v schránce nesplyne
              // s uvítacím e-mailem po registraci.
              followUpWeek: jeNavazujiciTyden,
            });
          } catch (mailErr) {
            sendResult = { ok: false, message: mailErr?.message || String(mailErr) };
            console.error('[executeTrainerTask] sendPlanEmail vyhodil výjimku', {
              plan_id: planId,
              source: emailZdroj,
          ...
          if (!emailSent && planId) {
            // Uvolnit claim, aby šlo odeslání zkusit znovu — bez toho by
            // `email_sent = true` navždy tvrdilo, že e-mail odešel.
    ...
    // ZÁMĚRNĚ POŘÁD JEN `initial_plan`. Tenhle blok byl uvnitř podmínky na
    // typ úlohy, takže rozšíření e-mailu na weekly by ho rozjelo i tam.
    // Dolepšování plánu je placené volání navíc a nikdo o něj u týdenní obnovy
    // nežádal — zapnout se dá, ale má to být vlastní rozhodnutí, ne vedlejší
    // efekt opravy e-mailu.
    if (task.task_type === 'initial_plan' && sideEffect?.plan_id && finalGenerated?.planJson) {
      schedulePlanEnhancementAsync({
        planId: sideEffect.plan_id,
        userId: task.user_id,
        bodyMetrics: { ...bm, user_id: task.user_id },
        planJson: finalGenerated.planJson,
      });
    }
  }
  ...
  if (task.task_type === 'initial_plan' && task.user_id) {
    await recordProductEvent({
      ...
      properties: { program: 'START', plan_type: 'initial_7_day', success: true },
      source: 'taskExecutors',
    });
  }
  ...
  const initialModuleMeta = task.task_type === 'initial_plan' ? getInitialPlanModuleMeta(task) : null;
  const initialModuleCheck =
    task.task_type === 'initial_plan'
      ? validateInitialPlanModules(
          finalGenerated?.planJson,
          finalGenerated?.html,
          initialModuleMeta.required_modules
        )
      : null;
  return {
    ok: true,
    result: {
      ...
      ...(initialModuleMeta
        ? {
            required_modules: initialModuleMeta.required_modules,
            completed_modules: initialModuleCheck?.completed_modules ?? [],
            plan_scope: initialModuleMeta.plan_scope,
          }
        : {}),
      trainer_ai_attempted: true,
...
async function writeCoachSharedFacts(task) {
  ...
  if (taskType === 'apple_health_daily_review') {
    await writeSharedMemoryFact({
      userId,
      memoryType: 'shared_apple_health_review',
      content: 'Daily Apple Health review generated from aggregated recovery metrics.',
      sourceAgentSlug: 'coach',
    });
  }
...
async function loadUserHabitsForCoach(userId) {
  if (!userId) return [];
  const { data } = await supabaseServer
    .from('user_habits')
    .select('habit_id, sort_order')
    .eq('user_id', userId)
    .order('sort_order', { ascending: true })
    .limit(8);
  return (data ?? []).map((row) => {
    const meta = getHabitById(row.habit_id);
    return meta
      ? { id: row.habit_id, label: meta.label, description: meta.description }
      : { id: row.habit_id, label: row.habit_id, description: null };
  });
}

async function loadLatestPlanRowForCoach(userId) {
  if (!userId) return null;
  const { data } = await supabaseServer
    .from('ai_generated_plans')
    .select('id, valid_from, valid_until, plan_type, created_at')
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
...
async function persistCoachMessageRecord(task, { title, content, parsed, fallbackMeta = null }) {
  ...
  await writeAILog({
    ...
    agent_slug: 'coach',
    action: task.task_type,
    status: fallbackMeta ? 'fallback' : 'completed',
    result: { message_id: messageId, ...(fallbackMeta || {}) },
  });
  ...
  return {
    ...
    result: {
      ...
      summary: `${title} (${task.task_type})${fallbackMeta ? ' [fallback]' : ''}`,
      structured: parsed,
      ...(fallbackMeta
        ? {
            side_effect: 'coach_message_fallback_created',
            fallback: true,
            reason: fallbackMeta.reason || 'openai_quota_or_rate_limit',
          }
        : {}),
    },
  };
...
export async function executeCoachTask(task) {
  ...
  try {
    ...
    if (!isOpenAICoachFallbackError(err)) {
      throw err;
    }
    const fallbackReason = getOpenAICoachFallbackReason(err) || 'openai_unavailable';
    console.warn('[executeCoachTask] OpenAI unavailable — using fallback message', {
      task_id: task.id,
      user_id: task.user_id,
      reason: fallbackReason,
      error: String(err?.message || err || '').slice(0, 160).replace(/\bsk-(?:proj-)?[A-Za-z0-9_-]{8,}\b/g, 'sk-[REDACTED]'),
    });
    const [bm, userHabits, latestPlan] = await Promise.all([
      loadLatestBodyMetrics(task.user_id).catch(() => null),
      loadUserHabitsForCoach(task.user_id),
      loadLatestPlanRowForCoach(task.user_id),
    ]);
    parsed = buildCoachFallbackMessage({
      bodyMetrics: bm,
      userHabits,
      latestPlan,
      taskType: task.task_type,
    });
    fallbackMeta = { reason: fallbackReason };
  }

  const content = parsed?.message || parsed?.coaching_plan?.weekly_focus || '';
  if (!content || !String(content).trim()) {
    throw new Error('Coach message content empty after generation and fallback');
  }

  const { messageId, result } = await persistCoachMessageRecord(task, {
    title,
  ...
  return { ok: true, result: { ...result, message_id: messageId } };
}
...
export async function persistFallbackPlanForUser(userId) {
  try {
    ...
    // Žádná deaktivace tady. Tenhle řádek se vždycky vkládá s is_active:
    // false (je to interní artefakt, uživatel ho nikdy neuvidí) — deaktivovat
    // kvůli němu uživatelův skutečný aktivní plán by ho nechalo bez
    // jakéhokoli aktivního plánu úplně zbytečně (stejná chyba jako
    // v persistTrainerPlan, viz jePlatnyDnes v weeklyPlanProducer.js).
    const { data, error } = await supabaseServer
      .from('ai_generated_plans')
      .upsert(
        {
...
/**
 * Last-resort při selhání initial_plan tasku: stejný katalogový pipeline jako primární cesta.
 * Holá HTML šablona jen když katalog selže úplně.
 */
export async function persistPublishableFallbackPlanForUser(userId) {
  try {
    const bm = await loadLatestBodyMetrics(userId);
    if (!bm) return { plan_id: null, error: 'no_body_metrics' };

    const regDateIso = bm?.created_at ? calendarDateIsoInPrague(new Date(bm.created_at)) : calendarDateIsoInPrague(new Date());
    const range = { from: regDateIso, until: addCalendarDaysIsoPrague(regDateIso, 6) };

    const catalogPipeline = await runUnifiedPlanPipeline({
      bm: { ...bm, user_id: userId },
      useOpenAI: false,
      validFrom: range.from,
      validUntil: range.until,
      onboardingSoftGate: true,
      plan_scope: 'initial_7_day_trial',
      simpleStartMode: true,
    });

    if (catalogPipeline?.ok && catalogPipeline.planHtml) {
      const persisted = await persistPlanFromUnified(catalogPipeline, { ...bm, user_id: userId }, {
        generatedBy: 'ai-task:initial_plan',
        deactivateOld: true,
      });
      if (persisted?.plan_id) {
        console.info('[persistPublishableFallbackPlanForUser] catalog plan persisted', {
          plan_id: persisted.plan_id,
          user_id: userId,
          generation_source: catalogPipeline.generation_source ?? 'catalog',
        });
        return {
          plan_id: persisted.plan_id,
          valid_from: persisted.valid_from ?? range.from,
          valid_until: persisted.valid_until ?? range.until,
          bm,
          planHtml: catalogPipeline.planHtml,
        };
      }
      console.warn('[persistPublishableFallbackPlanForUser] catalog pipeline ok but persist failed', {
        user_id: userId,
        error: persisted?.error,
      });
    } else {
      console.warn('[persistPublishableFallbackPlanForUser] catalog pipeline failed, using HTML template', {
        user_id: userId,
        error: catalogPipeline?.error,
        validation: catalogPipeline?.validation?.errors?.[0] ?? null,
      });
    }

    // ===== DETERMINISTICKÁ ŠABLONA SE PRO DIETY NEPOUŽÍVÁ =====
    //
    // buildDeterministicFallbackPlanHtml() je natvrdo napsaná sedmidenní
    // rotace bez ingrediencí a normalizeDietTypeForGpt() (lib/generatePlan.js)
    // z ní zná jen vegetarian / vegan / standard. gluten_free, lactose_free
    // i low_carb spadnou na `standard`, což je ovesná kaše, müsli, celozrnný
    // toast, žitný chléb a těstoviny. Přesně tohle 10. 8. 2026 odešlo
    // bezlepkovému uživateli.
    //
    // Dopsat do šablony zbylé diety by znamenalo druhou, ručně udržovanou
    // databázi jídel, kterou nic neporovnává s katalogem — a `foods_to_avoid`
    // ani alergie z volného textu by stejně neuměla. Nouzová šablona je
    // z definice neověřená, takže pro kohokoli s omezením je špatná odpověď.
    //
    // Volající to má vzít jako „plán zatím není“ a nechat uživatele na retry
    // CTA. Žádný plán je nepříjemnost; plán s alergenem je zdravotní riziko.
    const fallbackRules = buildDietaryPublishRules(bm);
    if (hasAnyDietaryRestriction(fallbackRules)) {
      console.error('[persistPublishableFallbackPlanForUser] dieta vylucuje nouzovou sablonu — nepublikuji', {
        user_id: userId,
        diet_type: bm?.diet_type ?? null,
        gluten_free: fallbackRules.glutenFree,
        lactose_free: fallbackRules.lactoseFree,
        vegetarian: fallbackRules.vegetarian,
        vegan: fallbackRules.vegan,
      });
      await writeAILog({
        user_id: userId,
        agent_slug: 'system',
        action: 'plan_publish_blocked_diet',
        status: 'blocked',
        result: {
          stage: 'last_resort_template',
          diet_type: bm?.diet_type ?? null,
          reason: 'deterministic_template_is_diet_unaware',
        },
      }).catch(() => {});
      // Rozsvítí `generovani_selhalo` v system_health_alerts (čte product_events
      // za posledních 24 h). Ticho by znamenalo, že o tom nikdo neví.
      await recordProductEvent({
        user_id: userId,
        event_name: 'plan_generation_failed',
        properties: { reason: 'dietary_publish_gate', diet_type: bm?.diet_type ?? null },
    ...
    // Stejná oprava jako v persistTrainerPlan (viz jePlatnyDnes v
    // weeklyPlanProducer.js): deaktivovat jen plány, které dnešek OPRAVDU
    // přestaly pokrývat, a nový aktivovat jen když jeho okno dnešek pokrývá.
    const dnesProAktivaci = todayIso();
    await supabaseServer
      .from('ai_generated_plans')
      .update({ is_active: false })
      ...
      .upsert(
        {
          ...
          generation_prompt: 'Last-resort HTML template when AI and catalog rescue failed',
          user_context: bm,
          valid_from: range.from,
          valid_until: range.until,
          is_active: jePlatnyDnes(range.from, range.until, dnesProAktivaci),
