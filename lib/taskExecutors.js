...
export async function executeTrainerTask(task) {
  ...
  if (task.task_type === 'initial_plan') {
    ...
    if (!moduleCheck.ok) {
      const reason = 'initial_plan_missing_required_modules';
      console.error('[executeTrainerTask] initial_plan missing required modules', {
      ...
      const failPayload = {
        ...
        missing_modules: moduleCheck.missing_modules,
        completed_modules: moduleCheck.completed_modules,
        required_modules: moduleMeta.required_modules,
        plan_scope: moduleMeta.plan_scope,
        ...
        published_to_user: false,
        email_sent: false,
        root_failure_stage: 'initial_plan_missing_required_modules',
      };
      if (task?.id) {
        await supabaseServer.from('ai_tasks').update({
          status: 'failed',
          result: failPayload,
          last_error: `${reason}: ${moduleCheck.missing_modules.join(', ')}`,
          processed_at: new Date().toISOString(),
        }).eq('id', task.id);
      }
      throw new Error(`${reason}: ${moduleCheck.missing_modules.join(', ')}`);
    }
  }
