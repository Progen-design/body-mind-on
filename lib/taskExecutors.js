...
async function persistTrainerPlan({ userId, bm, generated, taskType, latestPlan }) {
  ...
  if (error) {
    console.error('[persistTrainerPlan] insert failed', { user_id: userId, db_error: error.message, code: error.code });
    throw new Error(`Failed to insert plan: ${error.message}`);
  }
  console.info('[persistTrainerPlan] plan inserted', { plan_id: data?.id ?? null, is_active: true, user_id: userId });
  return {
