...
/** Plán smí být publikován: AI výstup, AI retry, nebo deterministický fallback když AI selhal (timeout/invalid). */
const AI_PUBLISHABLE_SOURCES = ['ai', 'ai_retry', 'ai_retry_truth', 'deterministic_fallback'];
...
export async function executeTrainerTask(task) {
  ...
  // Initial plan: začíná dnem registrace (body_metrics.created_at), ne pondělím
  const registrationDateIso = bm?.created_at
    ? new Date(bm.created_at).toISOString().split('T')[0]
  ...
  const planRange =
    task.task_type === 'weekly_plan_update'
      ...
      : {
          from: latestPlan?.valid_from || registrationDateIso,
          until:
            latestPlan?.valid_until || addDays(registrationDateIso, 6),
        };
  const targetStartDate = new Date((planRange.from || new Date().toISOString().split('T')[0]) + 'T12:00:00');
  ...
  console.info('[executeTrainerTask] html_length', genValid.length, 'selected', selectedHtmlSource, 'generation_source', generated?.generation_source ?? null, 'fallback_used', generated?.fallback_used ?? false);
