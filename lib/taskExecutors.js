...
/**
 * Persist deterministic fallback plan for user so they always have a plan in profile and can receive email.
 * Call when initial_plan task did not complete (failed/timeout). Returns null on error.
...
export async function persistFallbackPlanForUser(userId) {
  try {
    ...
    const planHtml = buildDeterministicFallbackPlanHtml(bm);
    const planValid = validatePublishedPlanHtml(planHtml);
    if (!planValid.ok) {
      console.warn('[persistFallbackPlanForUser] Fallback HTML invalid', { reason: planValid.reason, user_id: userId });
      return null;
    }
    ...
    const { data, error } = await supabaseServer.from('ai_generated_plans').insert({
      ...
      macros: { protein_g: macros.protein_g, fat_g: macros.fat_g, carbs_g: macros.carbs_g },
      workout_plan: {}, exercises_data: {}, meal_plan: {},
      generated_by: 'deterministic_fallback_after_failure',
      generation_prompt: 'Fallback plan when AI did not complete',
      user_context: bm,
      valid_from: range.from,
      valid_until: range.until,
      is_active: true,
    ...
    if (error || !data?.id) {
      console.error('[persistFallbackPlanForUser] insert failed', { user_id: userId, error: error?.message });
      return null;
    }
    ...
    console.error('[persistFallbackPlanForUser]', err?.message);
    return null;
  }
}
