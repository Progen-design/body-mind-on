...
const PLAN_SECTION_PATTERNS = [/jídelníček|jidelníček|jidelnicek/i, /trénink|trenink/i, /snídaně|snidane/i, /oběd|obed/i, /večeře|vecere/i, /regenerace/i, /suplementace/i, /mindset/i];

/** Validace HTML plánu pro publish. Vrací { ok, length, matchedSections, reason }. */
function validatePublishedPlanHtml(html) {
  if (!html || typeof html !== 'string') {
    return { ok: false, length: 0, matchedSections: 0, reason: 'html_missing_or_not_string' };
  }
  const len = html.trim().length;
  const matchedCount = PLAN_SECTION_PATTERNS.filter((p) => p.test(html)).length;
  if (len < 1000) {
    return { ok: false, length: len, matchedSections: matchedCount, reason: `html_too_short_${len}` };
  }
  if (matchedCount < 3) {
    return { ok: false, length: len, matchedSections: matchedCount, reason: `insufficient_sections_${matchedCount}` };
  }
  return { ok: true, length: len, matchedSections: matchedCount, reason: null };
}
...
async function persistTrainerPlan({ userId, bm, generated, taskType, latestPlan }) {
  ...
  if (!planValid.ok) throw new Error(`Trainer plan html invalid (${planValid.reason}) – not persisting`);
  ...
  console.info('[persistTrainerPlan] inserting trainer plan', { user_id: userId, html_length: htmlLength, plan_type: planType, task_type: taskType });
...
export async function executeTrainerTask(task) {
  ...
  console.info('[executeTrainerTask] generated_html_length', { length: genValid.length, matchedSections: genValid.matchedSections ?? 0 });
  console.info('[executeTrainerTask] validated_html_length', { length: valValid.length, matchedSections: valValid.matchedSections ?? 0 });
  console.info('[executeTrainerTask] selected_html_source', selectedHtmlSource, { validation_warning: validation.validationWarning ?? null });

  if (!finalValid.ok) {
    console.error('[executeTrainerTask] Generated trainer plan is incomplete', { reason: finalValid.reason, length: finalValid.length });
    throw new Error(`Generated trainer plan is incomplete (${finalValid.reason})`);
  }
  ...
  if (task.task_type === 'initial_plan' && bm?.email && finalGenerated?.html) {
    ...
    const hasMinLength = planHtml && planHtml.length >= 200;
    const hasPlanSections = /<h3|Jídelníček|jidelníček|Trénink|trenink/i.test(planHtml || '');
    if (!hasMinLength || !hasPlanSections) {
      console.warn('[executeTrainerTask] Plan validation failed – email not sent (length or sections). Plan saved to DB.');
