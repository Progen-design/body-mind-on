...
async function runPlanValidators(planHtml, bm, userId) {
  ...
  if (nutConfig.enabled && planHtml) {
    try {
      ...
      if (parsed.corrected_html && typeof parsed.corrected_html === 'string') {
        ...
        const mayReplace = correctedValid.ok && (
          !originalValid.ok ||
          (originalValid.ok && correctedValid.length > originalValid.length)
        );
  ...
  if (validatorReplacementApplied) {
    if (!originalValidation.ok && correctedValidation.ok) validatorReplacementReason = 'corrected_valid_origin_invalid';
    else if (originalValidation.ok && correctedValidation.ok && correctedValidation.length > originalValidation.length) validatorReplacementReason = 'both_valid_corrected_longer';
    else validatorReplacementReason = 'corrected_applied';
  } else {
    if (!anyCorrectedHtml) validatorReplacementReason = 'no_corrected_html';
    else if (originalValidation.ok && correctedValidation.ok) validatorReplacementReason = 'no_replacement_origin_preferred';
    else if (!correctedValidation.ok && originalValidation.ok) validatorReplacementReason = 'no_replacement_corrected_invalid_kept_origin';
