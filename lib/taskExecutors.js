...
import { runPlanPipeline } from './aiOrchestrator';
...
import { runPlanValidators } from './planValidators';

const IS_PRODUCTION = process.env.NODE_ENV === 'production';

/** Plán smí být publikován uživateli pouze když pochází z AI asistenta. Fallback NENÍ publikovatelný. */
const AI_PUBLISHABLE_SOURCES = ['ai', 'ai_retry', 'ai_retry_truth'];
...
async function persistTrainerPlan({ userId, bm, generated, taskType, latestPlan, selectedHtmlSource = null }) {
...
export async function executeTrainerTask(task) {
  ...
  const generated = await runPlanPipeline({
    ...bm,
  ...
  console.info('[executeTrainerTask] selected_html_source', selectedHtmlSource, 'generation_source', generated?.generation_source ?? null, 'fallback_used', generated?.fallback_used ?? false, 'published_plan_validation_reason', finalValid.reason ?? 'ok');

  // PRAVIDLO: Finální plán smí být publikován pouze z AI asistenta. Fallback NENÍ publikovatelný.
  if (!isPublishableFromAI(generated?.generation_source)) {
    const reason = generated?.generation_source === 'deterministic_fallback'
      ? 'Plán vznikl z deterministického fallbacku – ne publikovatelný (AI-first pravidlo)'
      : `generation_source=${generated?.generation_source ?? 'unknown'} není publikovatelný`;
    console.error('[executeTrainerTask] Plan not publishable – AI-first rule', { generation_source: generated?.generation_source, fallback_used: generated?.fallback_used });
    const failPayload = {
  ...
  const resultPayload = {
    ...
    validatorReplacementApplied: validation.validatorReplacementApplied ?? false,
    validatorReplacementReason: validation.validatorReplacementReason ?? null,
    generation_source: generated?.generation_source ?? null,
    fallback_used: generated?.fallback_used ?? false,
...
export async function persistFallbackPlanForUser(userId) {
  try {
    ...
    let planHtml = buildDeterministicFallbackPlanHtml(bm);
...
export async function persistPublishableFallbackPlanForUser(userId) {
  try {
    ...
    const { data, error } = await supabaseServer.from('ai_generated_plans').insert({
      ...
      macros: { protein_g: macros.protein_g, fat_g: macros.fat_g, carbs_g: macros.carbs_g },
      workout_plan: {}, exercises_data: {}, meal_plan: {},
      generated_by: 'registration_deterministic_plan',
      generation_prompt: 'Last-resort deterministic plan when AI failed during registration',
      user_context: bm,
      valid_from: range.from,
      valid_until: range.until,
      is_active: true,
