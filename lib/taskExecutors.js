...
const PLAN_SECTION_NAMES = ['Jídelníček', 'Trénink', 'Snídaně', 'Oběd', 'Večeře', 'Regenerace', 'Suplementace', 'Mindset'];
...
/** Validace HTML plánu pro publish. Vrací { ok, length, matchedSections: string[], reason }. */
function validatePublishedPlanHtml(html) {
  if (!html || typeof html !== 'string') {
    return { ok: false, length: 0, matchedSections: [], reason: 'html_missing_or_not_string' };
  }
  const len = html.trim().length;
  const matchedSections = PLAN_SECTION_PATTERNS
    .map((p, i) => (p.test(html) ? PLAN_SECTION_NAMES[i] : null))
    .filter(Boolean);
  if (len < 1000) {
    return { ok: false, length: len, matchedSections, reason: `html_too_short_${len}` };
  }
  if (matchedSections.length < 3) {
    return { ok: false, length: len, matchedSections, reason: `insufficient_sections_${matchedSections.length}` };
  }
  return { ok: true, length: len, matchedSections, reason: null };
}
