...
async function runPlanValidators(planHtml, bm, userId) {
  ...
  if (validatorReplacementApplied) {
    ...
    if (originalValidation.ok && correctedValidation.ok) validatorReplacementReason = 'no_replacement_both_valid';
...
async function persistTrainerPlan({ userId, bm, generated, taskType, latestPlan }) {
  ...
  console.info('[persistTrainerPlan] inserting trainer plan', { user_id: userId, html_length: htmlLength, matched_sections: planValid.matchedSections ?? [], missing_core_sections: planValid.missingCoreSections ?? [], plan_type: planType, task_type: taskType, published_plan_validation_reason: planValid.reason ?? 'ok' });
