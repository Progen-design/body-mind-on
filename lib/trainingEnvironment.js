...
export const TRAINING_ENVIRONMENT_OPTIONS = Object.freeze([
  ...
  { value: 'other', label: TRAINING_ENVIRONMENT_LABELS.other },
]);

export const TRAINING_ENVIRONMENT_VALUES = Object.freeze(
  TRAINING_ENVIRONMENT_OPTIONS.map((o) => o.value)
);
...
const EQUIPMENT_REQUIRES = Object.freeze({
  ...
  // Bench press bez lavice nejde — na rozdíl od ostatních položek tu nestačí "aspoň jedno".
  bench_press: { all: ['bench', 'dumbbells'] },
 ...
 * @returns {'gym'|'home_bodyweight'|'home_equipment'|'other'}
 */
export function parseTrainingEnvironment(bodyMetrics) {
  ...
  if (direct === 'gym' || direct === 'home_bodyweight' || direct === 'home_equipment' || direct === 'other') {
    return direct;
  }
  ...
  if (match) {
    ...
    if (label.includes('ostatn')) return 'other';
...
/**
 * Free-text detail for "Ostatní" (stored in notes).
 * @param {object|null|undefined} bodyMetrics
 * @returns {string|null}
 */
export function parseTrainingEnvironmentDetail(bodyMetrics) {
  const direct = String(bodyMetrics?.training_environment_detail || '').trim();
  if (direct) return direct.slice(0, 280);

  const notes = String(bodyMetrics?.notes || '');
  const match = notes.match(/Detail prostředí:\s*([^.;]+)/i);
  if (!match) return null;
  const detail = match[1].trim();
  return detail ? detail.slice(0, 280) : null;
}

/**
 * Map stored env to workout/AI-safe env. "other" → bodyweight (never unknown/empty).
 * @param {string|null|undefined} env
 * @returns {'gym'|'home_bodyweight'|'home_equipment'}
 */
export function resolveWorkoutTrainingEnvironment(env) {
  const normalized = String(env || '').trim().toLowerCase();
  if (normalized === 'gym') return 'gym';
  if (normalized === 'home_equipment') return 'home_equipment';
  // home_bodyweight + other + anything unknown → bodyweight path
 ...
 */
export function trainingEnvironmentNotesSuffix(env, equipment = [], detail = null) {
  ...
  if (env === 'other') {
    const cleaned = String(detail || '').trim().replace(/[.;]+/g, ',').slice(0, 280);
    if (cleaned) parts.push(`Detail prostředí: ${cleaned}`);
  }
  return parts.join('. ');
}
...
/**
 * @param {string|null|undefined} notes
 * @param {'gym'|'home_bodyweight'|'home_equipment'|'other'|string|null} env
 * @param {string[]} equipment
 * @param {string|null|undefined} detail
 * @returns {string|null}
 */
export function mergeTrainingEnvironmentIntoNotes(notes, env, equipment = [], detail = null) {
  ...
  const suffix = trainingEnvironmentNotesSuffix(normalizedEnv, equipment, detail);
  return base ? `${base}. ${suffix}` : suffix;
}

function equipmentHas(required, available) {
  if (!required) return true;
  if (Array.isArray(required.all)) {
    return required.all.every((item) => available.includes(item));
  }
...
export function trainingEnvironmentDisplayFromMetrics(bodyMetrics) {
  ...
  const detail = parseTrainingEnvironmentDetail(bodyMetrics);
  if (env === 'home_equipment' && equipment.length) {
    return `Typ: ${label} · Pomůcky: ${formatAvailableEquipmentLabels(equipment).join(', ')}`;
  }
  if (env === 'other' && detail) {
    return `Typ: ${label} · ${detail}`;
  }
  return `Typ: ${label}`;
}

/**
 * Jednoručkové varianty barbellových cviků — vlastní canonical_key (viz
 * poznámka u CANONICAL_EXERCISES v exerciseCanonicalMap.js), jinak by
 * „Nářadí“ dál hlásilo velkou činku i s jednoručkami v ruce.
 */
const HOME_DUMBBELL_VARIANTS = Object.freeze({
  bench_press: { canonical_key: 'dumbbell_bench_press', search_term: 'dumbbell bench press', name_cs: 'Tlaky s jednoručkami na lavici', sets: 3, reps: '10' },
  bent_over_row: { canonical_key: 'dumbbell_row', search_term: 'dumbbell row', name_cs: 'Přítahy s jednoručkou', sets: 3, reps: '10' },
  romanian_deadlift: { canonical_key: 'dumbbell_romanian_deadlift', search_term: 'dumbbell romanian deadlift', name_cs: 'Rumunský mrtvý tah s jednoručkami', sets: 3, reps: '10' },
});
...
function resolveHomeEquipmentReplacement(key, availableEquipment = []) {
  ...
  if (key === 'lat_pulldown') {
    if (hasDumbbells) {
      return HOME_DUMBBELL_VARIANTS.bent_over_row;
    }
    return BODYWEIGHT_REPLACEMENTS.lat_pulldown;
  }
  if (key === 'chest_press') {
    if (hasBench && hasDumbbells) {
      return HOME_DUMBBELL_VARIANTS.bench_press;
    }
    if (hasDumbbells) {
      // overhead_press zůstává (produkční registry mu už dnes hlásí equipment
      // 'dumbbell', ne 'barbell' ze statické mapy — viz poznámka níž u
      // vlastní větve 'overhead_press'), takže tu není co konvertovat.
  ...
  if (key === 'bench_press') {
    ...
    return BODYWEIGHT_REPLACEMENTS.bench_press;
  }
  if (key === 'bent_over_row') {
    if (hasDumbbells) {
      return HOME_DUMBBELL_VARIANTS.bent_over_row;
    }
    return BODYWEIGHT_REPLACEMENTS.bent_over_row;
  }
  if (key === 'romanian_deadlift') {
    if (hasDumbbells) {
      return HOME_DUMBBELL_VARIANTS.romanian_deadlift;
    }
    return BODYWEIGHT_REPLACEMENTS.romanian_deadlift;
  }
...
export function adaptExerciseForTrainingEnvironment(exercise, env, availableEquipment = []) {
  ...
  if (env === 'home_equipment') {
    // Tyhle klíče mají v CANONICAL_EXERCISES equipment: 'barbell', a barbell
    // není mezi volbami v registraci vůbec — takže tu se vždy vybírá varianta
    // podle skutečného vybavení, ne jen když EQUIPMENT_REQUIRES (OR-seznam)
    // usoudí, že "aspoň jedna" položka stačí.
    if (GYM_MACHINE_ONLY.has(key) || key === 'goblet_squat' || HOME_DUMBBELL_VARIANTS[key]) {
...
export function filterWorkoutPlanForTrainingEnvironment(workoutPlan, bodyMetrics = {}) {
  const storedEnv = parseTrainingEnvironment(bodyMetrics);
  const env = resolveWorkoutTrainingEnvironment(storedEnv);
  const equipment = parseAvailableEquipment(bodyMetrics);
  ...
  if (adapted > 0) {
    console.info('[trainingEnvironment] adapted exercises', { env, storedEnv, equipment, adapted });
  }
