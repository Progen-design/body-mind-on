...
import { sanitizeRecipeMealMismatchesInPlan, normalizeWorkoutExerciseDurationsInPlan, normalizePublishableWorkoutExercisesInPlan } from './planDataIntegrity.js';
import { logPlanQualityEvent } from './planQuality.js';
import { applyWorkoutDurationMinutesToPlan } from './workoutPlanScaler.js';
import { generateStructuredPlan } from './services/planOrchestrator.js';
import { bodyMetricsToPlanInput } from './bodyMetricsToPlanInput.js';
import { renderPlanHtmlFromStructured } from './planRenderer.js';
import { validateStructuredPlan } from './validation/structuredPlanValidators.js';
import { sendPlanEmail } from './mail.js';
import { stripPlanMediaAttrsFromHtml } from './emailTemplates.js';
import { addCalendarDaysIsoPrague, calendarDateIsoInPrague } from './czechCalendar.js';
...
import { resolveSyncOpenAiForPipeline, isOpenAiPlanEnabled } from './openaiPlanConfig.js';
import { schedulePlanEnhancementAsync } from './services/planEnhancementAsync.js';
import { getMembership } from './membershipHelpers.js';
import { resolveProgramTier } from './programTier.js';
import {
...
async function syncBodyMetricsCalorieTarget(bm, opts = {}) {
  if (!bm?.id) return bm;
  const patch = buildCalorieTargetBodyMetricsPatch(bm, opts);
  const { error } = await supabaseServer
    .from('body_metrics')
    .update(patch)
    .eq('id', bm.id);
  if (error) {
    // console.error, ne warn: tohle znamená, že se kalorický cíl NEULOŽIL.
    // Jako `warn` to v logu leželo potřetí, aniž by to někdo řešil.
    console.error('[unifiedPlanPipeline] calories_target sync failed', {
      error: error.message,
      body_metrics_id: bm.id,
      patch_keys: Object.keys(patch),
    });
    return { ...bm, ...patch };
  }
  // docs/DALSI_KROK.md 8.1 — jedno z pěti míst, kde se `calories_target`
  // opravdu mění. Bez `forceRecalculate` (drtivá většina volání) vrací
  // `buildCalorieTargetBodyMetricsPatch` beze změny stejné číslo, takže tady
  // emit typicky nic neudělá — `emitCalorieTargetChangedEvent` to pozná sama.
  if (bm.user_id) {
    await emitCalorieTargetChangedEvent(bm.user_id, {
      oldCaloriesTarget: bm.calories_target,
      patch,
...
export async function runUnifiedPlanPipeline(input) {
  let bm = input?.bm ?? input?.body_metrics ?? input;
  if (!bm || typeof bm !== 'object') {
    return { ok: false, error: 'Chybí body_metrics' };
  }

  if (input?.recalculateCalorieTarget === true) {
    bm = await syncBodyMetricsCalorieTarget(bm, { forceRecalculate: true });
  } else if (bm?.id) {
    bm = await syncBodyMetricsCalorieTarget(bm);
  }
  ...
  if (bm?.user_id) {
    const { buildPlanMemoryContext } = await import('./services/planMemoryContext.js');
  ...
  // PROGRAM MUSÍ DORAZIT DO PLÁNOVAČE, jinak si ho odvodí sám a rozejde se
  // s profilem. `memberships.tier` je autoritativní (mění se platbou),
  // `body_metrics.program` je jen to, co uživatel vyplnil v registraci —
  // api/profile.js má tutéž prioritu, obojí přes lib/programTier.js.
  // Bez tohohle řádku by START program (A/B trénink s progresí) nikdy
  // nenaskočil pro uživatele, který má tier jen v memberships.
  if (bm?.user_id) {
    try {
      const membershipForProgram = await getMembership(bm.user_id);
      orchestratorBody._program_tier = resolveProgramTier(bm, membershipForProgram);
    } catch (e) {
      console.warn('[unifiedPlanPipeline] nacteni membership pro program selhalo', e?.message);
    }
  }
  ...
  try {
    ...
    normalizePlanCalorieTargets(planResult, bm);
    const calorieTargetCheck = assertCalorieTargetConsistency(planResult, bm);
    if (!calorieTargetCheck.ok) {
      safeLog('calorie_target_mismatch', {
        userId: bm?.user_id ?? null,
        ...calorieTargetCheck,
      });
      planResult.targets = planResult.targets || {};
      planResult.targets.calories_per_day = calorieTargetCheck.expected;
      normalizePlanCalorieTargets(planResult, { ...bm, calories_target: calorieTargetCheck.expected });
    }

    const dietaryGate = enforceDietaryPublishGate(planResult, bm);
    if (!dietaryGate.ok) {
      // Detail, ne jen počet. „Dietary publish gate failed: 5“ nikomu neřekne,
      // které jídlo a jakým výrazem — a bez toho se příčina hledá v logu ručně.
      safeLog('dietary_gate_failed', {
        userId: bm?.user_id ?? null,
        diet_type: bm?.diet_type ?? null,
        violations: dietaryGate.violations,
        violation_details: dietaryGate.violationDetails ?? [],
      });
      return {
        ok: false,
        error: 'Dietary publish gate failed',
        _diagnostics: {
          ...(planResult._diagnostics || {}),
          diet_type: bm?.diet_type ?? null,
          dietary_gate_violations: dietaryGate.violations,
          dietary_gate_violation_details: dietaryGate.violationDetails ?? [],
        },
      };
    }
    if (dietaryGate.replaced > 0 || dietaryGate.fallbackUsed) {
      planResult = dietaryGate.planJson;
      postProcessStructuredPlan(planResult);
      safeLog('dietary_publish_gate', {
        userId: bm?.user_id ?? null,
        replaced: dietaryGate.replaced,
        fallback_used: dietaryGate.fallbackUsed === true,
      });
    }
...
export async function persistPlanFromUnified(pipelineResult, bm, opts = {}) {
  ...
  const validFrom = pipelineResult.valid_from ?? calendarDateIsoInPrague(new Date());
  const validUntil = pipelineResult.valid_until ?? addCalendarDaysIsoPrague(validFrom, 6);
  const insertPayload = {
    user_id: userId,
    email,
    plan_type: planType,
    ...
    // dřív tu bylo natvrdo `true`, i pro plán vygenerovaný o den dřív, než
    // začne platit. `is_active` teď odpovídá tomu, jestli okno platnosti
    // opravdu pokrývá dnešek — plán na budoucí týden se vloží jako neaktivní
    // a aktivuje ho noční sync_plan_activation() v den, kdy začne.
    is_active: jePlatnyDnes(validFrom, validUntil),
  };
  ...
  // Deaktivuje jen plány, které dnešek OPRAVDU přestaly pokrývat — ne
  // všechny, co byly aktivní. Starý plán, kterému dnešek ještě běží, musí
  // zůstat aktivní, i když se právě vložil jeho nástupce na příští týden.
  if (opts.deactivateOld !== false && userId) {
    ...
    await supabaseServer
      ...
      .eq('user_id', userId)
      .eq('is_active', true)
      .or(`valid_until.lt.${dnes},valid_from.gt.${dnes}`);
  }
...
export async function generatePlanForEmailViaUnified(email, options = {}) {
  ...
  if (bm?.id) {
    await syncBodyMetricsCalorieTarget(bm);
  }
