...
export async function persistPlanFromUnified(pipelineResult, bm, opts = {}) {
  ...
  const insertPayload = {
    ...
    email,
    plan_type: planType,
    plan_html: pipelineResult.planHtml,
    ...
    user_context: bm,
    valid_from: pipelineResult.valid_from ?? new Date().toISOString().slice(0, 10),
    valid_until: pipelineResult.valid_until ?? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    is_active: true,
  };
  ...
  const { data, error } = await supabaseServer.from('ai_generated_plans').insert(insertPayload).select('id').maybeSingle();
  if (error) {
    if (/structured_plan_json|does not exist/i.test(error.message)) {
      delete insertPayload.structured_plan_json;
      const retry = await supabaseServer.from('ai_generated_plans').insert(insertPayload).select('id').maybeSingle();
 ...
 * @param {object} [options] - { bmOverride?, validFrom?, validUntil?, skipEmail?, planChangeContext?, loginUrl?, ... }
...
export async function generatePlanForEmailViaUnified(email, options = {}) {
  ...
  const persistResult = await persistPlanFromUnified(pipelineResult, bm, {
    generatedBy: 'unified-pipeline-email',
    deactivateOld: true,
  });
  ...
  if (!options.skipEmail && email) {
    const sendOpts = {
      ...
      loginUrl: options.loginUrl ?? 'https://app.bodyandmindon.cz/login',
  ...
  return { ok: true, message: options.skipEmail ? 'Plán přegenerován.' : 'Plán vygenerován a odeslán.' };
}
