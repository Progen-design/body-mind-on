 ...
 * ─────────────────────────────────────────────────────────────────────────────
 */
import { resolveToCanonicalKey } from './exerciseCanonicalMap.js';
import { normalizeMealQueryCs } from './mealNormalization.js';

const MEAL_HEADINGS = ['Snídaně', 'Oběd', 'Večeře'];
const SKIP_LI_PREFIXES = ['odpočinek', 'lehká procházka', 'trénink celkem', 'závěr', 'zaver'];

/** Blok „Trénink tento den“ + <ul> (shodný opening tag u <b>/<strong> díky \1). */
const TRAINING_DAY_UL_BLOCK_RE =
  /<p[^>]*>\s*<(b|strong)(?:\s[^>]*)?>\s*Trénink tento den[^<]*<\/\1>\s*<\/p>\s*<ul[^>]*>([\s\S]*?)<\/ul>/gi;
...
function parseExercisesFromHtml(html) {
  ...
  const ulMatch = html.match(TRAINING_DAY_UL_BLOCK_RE);
  if (ulMatch) {
...
function getTrainingBlocksPerDay(html) {
  if (!html || typeof html !== 'string') return [];
  const blocks = html.match(TRAINING_DAY_UL_BLOCK_RE) || [];
  const out = [];
...
/** Extrahuje text sekce mezi <h3>sectionName</h3> a dalším <h3>. */
function extractSection(html, sectionName) {
  if (!html || typeof html !== 'string' || !sectionName) return '';
  const escaped = sectionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(`<h3[^>]*>\\s*${escaped}\\s*</h3>\\s*([\\s\\S]*?)(?=<h3|$)`, 'i');
...
function extractSupplementSection(html) {
  return extractSection(html, 'Suplementace');
}
...
export function validatePlanTruth(html, enrichmentResult = null) {
  ...
  // Weak quality: slabá konkrétnost sekcí (soft gate rozšíření)
  const weak_quality_flags = [];
  if (meals.length < 18) weak_quality_flags.push('weak_meal_detail');
  const shortMealNames = meals.filter((m) => (normalizedKey(m.name) || '').length < 6).length;
  if (shortMealNames > 4) weak_quality_flags.push('weak_meal_detail');
  // Počet „hlavních“ cviků na den (bez rozcvičky, závěru, odpočinku) – min. 3 na tréninkový den
  const MAIN_KEYS = new Set(['warmup', 'cooldown', 'rest']);
  const mainCountsPerDay = blocksPerDay
    .filter((keys) => keys.length > 0)
    .map((keys) => keys.filter((k) => !MAIN_KEYS.has(k)).length);
  const minMainItems = mainCountsPerDay.length > 0 ? Math.min(...mainCountsPerDay) : 10;
  if (mainCountsPerDay.length > 0 && minMainItems < 3) weak_quality_flags.push('weak_training_detail');
  const regenText = extractSection(html || '', 'Regenerace');
  if (regenText.length > 0 && regenText.length < 40) weak_quality_flags.push('weak_regeneration_detail');
  const mindsetText = extractSection(html || '', 'Mindset');
  if (mindsetText.length > 0 && mindsetText.length < 30) weak_quality_flags.push('weak_mindset_detail');
  const shoppingText = extractSection(html || '', 'Nákupní seznam');
  if (shoppingText.length > 0 && shoppingText.length < 20) weak_quality_flags.push('weak_shopping_list');

  /** Soft gate: plán je vhodný k publikaci bez retry, pokud není repetitivní, šablonovitý ani slabý. */
  const soft_gate_passed =
    repetitive_meals.length === 0 &&
    repetitive_training_days.length === 0 &&
    unjustified_supplements.length === 0 &&
    weak_quality_flags.length === 0;
  const soft_gate_reason = soft_gate_passed
    ...
    : [
        ...
        weak_quality_flags.length ? `weak_quality: ${weak_quality_flags.join(', ')}` : null,
      ]
        .filter(Boolean)
        .join('; ') || 'unknown';
