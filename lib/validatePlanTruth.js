...
export function validatePlanTruth(html, enrichmentResult = null) {
  ...
  const workoutBlocksItemCount = blocksPerDay.filter((keys) => keys.length > 0).map((keys) => keys.length);
  const minTrainingItems = workoutBlocksItemCount.length > 0 ? Math.min(...workoutBlocksItemCount) : 10;
  if (workoutBlocksItemCount.length > 0 && minTrainingItems < 3) weak_quality_flags.push('weak_training_detail');
