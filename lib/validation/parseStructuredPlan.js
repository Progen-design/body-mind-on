...
export function parseStructuredPlan(raw, _bodyMetrics) {
  ...
  const rootDaysCount = Array.isArray(n?.days) ? n.days.length : 0;
  const mealPlanDaysCount = Array.isArray(n?.meal_plan?.days) ? n.meal_plan.days.length : 0;
  if (mealPlanDaysCount > 0 && (!Array.isArray(n.days) || rootDaysCount < mealPlanDaysCount)) {
    const { days: _partialRootDays, ...rest } = n;
    n = rest;
  }
