...
export function parseStructuredPlan(raw, _bodyMetrics) {
  const topKeys =
    raw && typeof raw === 'object' && !Array.isArray(raw) ? Object.keys(raw).slice(0, 40) : [];
  console.log('[parseStructuredPlan] start', {
    rawType: raw == null ? String(raw) : typeof raw,
    topKeys,
    daysLength: Array.isArray(raw?.days) ? raw.days.length : null,
    mealPlanDaysLength: Array.isArray(raw?.meal_plan?.days) ? raw.meal_plan.days.length : null,
    hasTargetsObject: !!(raw && typeof raw === 'object' && raw.targets && typeof raw.targets === 'object'),
  });
  ...
  if (!targets || typeof targets !== 'object') {
    console.log('[parseStructuredPlan] reject', { reason: 'missing_or_invalid_targets', hasTargets: !!raw.targets });
    return { valid: false, plan: null };
  }
  if (!mealPlan?.days || !Array.isArray(mealPlan.days) || mealPlan.days.length < 7) {
    console.log('[parseStructuredPlan] reject', {
      reason: 'meal_plan_days_need_7',
