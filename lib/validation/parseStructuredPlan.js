...
function normalizeMeal(m) {
  ...
  const spoonacular_query = (
    pickMealStringField(m, ['spoonacular_query', 'spoonacularQuery', 'meal_search_query', 'query_en']) ||
    (typeof m.spoonacular_query === 'string' ? m.spoonacular_query.trim() : '') ||
    (typeof m.search_query === 'string' ? m.search_query.trim() : '')
  ).slice(0, 100);
  const search_query =
...
export function parseStructuredPlan(raw, _bodyMetrics) {
  ...
  if (workoutPlan?.days && Array.isArray(workoutPlan.days) && workoutPlan.days.length > 0) {
    ...
    const derivedIndices = mappedWd.filter((x) => x.exercises.length > 0).map((x) => x.day_index);
    let wdIndices = Array.isArray(workoutPlan.workout_days) ? workoutPlan.workout_days : [];
    wdIndices = wdIndices.map((x) => Number(x)).filter((n) => Number.isFinite(n) && n >= 0 && n <= 6);
    if (!wdIndices.length) wdIndices = derivedIndices;
    plan.workout_plan = {
      workout_days: wdIndices,
      days: mappedWd,
    };
  } else {
