...
import { MEAL_TRAINING_COHERENCE, normalizeEquipmentKeys, profileLacksEquipmentCategory } from './mealTrainingCoherence.js';
import {
  passesMacroKcalGate,
...
export async function validateStructuredPlan(planJson, bm, opts = {}) {
  ...
  const bmCalories = asNum(bm?.calories_target);
  if (calories != null && bmCalories != null && Math.round(calories) !== Math.round(bmCalories)) {
    errors.push(
      `Kalorický cíl plánu (${Math.round(calories)} kcal) nesedí s body_metrics.calories_target (${Math.round(bmCalories)} kcal)`,
    );
  }
  ...
  const MIN_DAY_CALORIE_RATIO = 1 - 0.10;
  const MAX_DAY_CALORIE_RATIO = 1 + 0.10;
  ...
  // Permanent arithmetic gate: P×4 + C×4 + F×9 ≈ kcal (±10 %)
  for (const day of days) {
    const dayLabel = day.day_name ?? `den ${day.day_index ?? '?'}`;
    for (const m of day.meals ?? []) {
      const mealLabel = (m.display_name_cs || m.name_cs || m.display_name || 'jídlo').trim();
      const kcal = asNum(m?.kcal ?? m?.recipe?.calories);
      const protein = asNum(m?.protein_g ?? m?.recipe?.protein_g);
      const carbs = asNum(m?.carbs_g ?? m?.recipe?.carbs_g);
      const fat = asNum(m?.fat_g ?? m?.recipe?.fat_g);
      if (kcal == null || protein == null || carbs == null || fat == null) continue;
      if (!passesMacroKcalGate(kcal, protein, carbs, fat)) {
        const fromMacros = calculateCaloriesFromMacros(protein, carbs, fat);
        warnings.push(
          `${dayLabel}/${mealLabel}: makro gate ±10 % — uvedeno ${Math.round(kcal)} kcal vs ${fromMacros} z P/C/F`
        );
      }
    }
  }
