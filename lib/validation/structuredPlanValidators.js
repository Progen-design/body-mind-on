  ...
  if (calories != null && calories >= 800) {
    for (let i = 0; i < days.length; i++) {
      ...
      if (ratio < MIN_DAY_CALORIE_RATIO) {
        errors.push(
          `${label}: součet kalorií z jídel je ${Math.round(sum)} kcal (~${Math.round(ratio * 100)} % cíle dne ${Math.round(dayTarget)} kcal), minimum ${Math.round(MIN_DAY_CALORIE_RATIO * 100)} %`
        );
