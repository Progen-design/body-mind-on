...
export async function validateStructuredPlan(planJson, bm, opts = {}) {
  ...
  const MIN_DAY_CALORIE_RATIO = 0.72;
  const EXTREME_LOW_DAY_CALORIE_RATIO = 0.5;
  if (calories != null && calories >= 800) {
    for (let i = 0; i < days.length; i++) {
      ...
      const ratio = sum / calories;
      if (ratio < EXTREME_LOW_DAY_CALORIE_RATIO) {
        errors.push(
          `${label}: součet kalorií z jídel je ${Math.round(sum)} kcal (~${Math.round(ratio * 100)} % cíle ${Math.round(calories)} kcal), minimum ${Math.round(EXTREME_LOW_DAY_CALORIE_RATIO * 100)} %`
        );
      } else if (ratio < MIN_DAY_CALORIE_RATIO) {
        warnings.push(
          `${label}: součet kalorií z jídel je ${Math.round(sum)} kcal (~${Math.round(ratio * 100)} % cíle ${Math.round(calories)} kcal), minimum ${Math.round(MIN_DAY_CALORIE_RATIO * 100)} %`
        );
      } else if (ratio > 1.2) {
        warnings.push(
          `${label}: součet kalorií ${Math.round(sum)} kcal přesahuje ~120 % denního cíle (${Math.round(calories)} kcal)`
        );
      }
    }
  }
