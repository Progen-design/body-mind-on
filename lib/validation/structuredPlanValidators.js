...
export async function validateStructuredPlan(planJson, bm) {
  ...
  if (workoutDayCount > 7) {
    ...
    const tolerance = Number.isFinite(tol) && tol >= 0 ? tol : 1;
    ...
    } else if (Math.abs(workoutDayCount - expectedWorkoutsPerWeek) > tolerance) {
      errors.push(
        `Profil: ${expectedWorkoutsPerWeek} tréninků týdně (tolerance ±${tolerance}), plán má ${workoutDayCount} tréninkových dnů s cviky`
      );
    }
  }
