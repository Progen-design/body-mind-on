...
function fmtTargetMacro(value) {
  if (value == null || !Number.isFinite(Number(value))) {
    return `<span style="color:#475569;font-weight:400">—</span>`;
  }
  return `${Math.round(Number(value))}<span style="font-size:12px;color:#64748B;font-weight:400"> g</span>`;
}

function fmtMacroInline(value, color) {
  if (value == null) return `<span style="color:#475569;font-weight:600">—</span>`;
  ...
  return rows
    .map((row) => {
      ...
      const commentHtml = row.comment
        ? `<td align="right" valign="middle" style="font-family:${FONT_BODY};font-size:12px;color:#64748B;font-weight:400;line-height:1.55;padding-left:12px;max-width:220px;">${escapeHtml(row.comment)}</td>`
        : '';
      return `<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#1E293B" style="background-color:#1E293B;border:1px solid rgba(14,165,233,0.2);border-left:3px solid ${row.color};border-radius:10px;border-collapse:separate !important;margin-top:8px;"><tr><td bgcolor="#1E293B" style="background-color:#1E293B;padding:16px 20px;border-radius:10px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td valign="middle"><div style="font-family:${FONT_BODY};font-size:11px;color:${row.color};letter-spacing:1px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">${escapeHtml(row.label)}</div><div style="font-family:${FONT_BODY};font-size:20px;color:#E2E8F0;font-weight:700;letter-spacing:-0.3px;line-height:1;">${valueHtml}</div></td>${commentHtml}</tr></table></td></tr></table>`;
    })
    .join('');
}
...
function renderMealCardFull(meal, day, planJson, appBaseUrl) {
  ...
  const macroLine = `${fmtMacroInline(macros.protein_g, '#0EA5E9')} bílkovin · ${fmtMacroInline(macros.carbs_g, '#22D3EE')} sacharidů · ${fmtMacroInline(macros.fat_g, '#A78BFA')} tuků · ${fmtMacroInline(macros.fiber_g, '#10B981')} vlákniny`;

  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#1E293B" style="background-color:#1E293B;border:1px solid ${accentRgba};border-left:3px solid ${accent};border-radius:12px;border-collapse:separate !important;margin-bottom:8px;"><tr><td bgcolor="#1E293B" style="background-color:#1E293B;padding:16px 20px;border-radius:12px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:10px;"><tr><td valign="middle" style="font-family:${FONT_BODY};font-size:11px;color:${accent};letter-spacing:1px;font-weight:600;text-transform:uppercase;">${timeMeta.icon}&nbsp;${escapeHtml(timeMeta.time_word)} · ${escapeHtml(timeMeta.time)}</td><td align="right" valign="middle">${recipeButton}</td></tr></table><div class="meal-name-mobile" style="font-family:${FONT_BODY};font-size:18px;color:#E2E8F0;font-weight:700;letter-spacing:-0.3px;line-height:1.2;margin-bottom:10px;">${escapeHtml(title || label)}</div><div style="font-family:${FONT_BODY};font-size:12px;color:#94A3B8;font-weight:500;line-height:1.55;">${macroLine}</div></td></tr></table>`;
}
...
function renderDailyTotalPill(day) {
  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#1E293B" style="background-color:#1E293B;border:1px solid rgba(14,165,233,0.3);border-radius:999px;border-collapse:separate !important;margin-top:12px;"><tr><td bgcolor="#1E293B" style="background-color:#1E293B;padding:14px 22px;border-radius:999px;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td valign="middle" style="font-family:${FONT_BODY};font-size:13px;color:#64748B;font-weight:500;">Celkem za den</td><td align="right" valign="middle" style="font-family:${FONT_BODY};font-size:18px;color:#E2E8F0;font-weight:700;letter-spacing:-0.3px;">${escapeHtml(kcalDisplay)} <span style="font-size:12px;color:#64748B;font-weight:500;">kcal</span></td></tr></table></td></tr></table>`;
}
...
function renderWorkoutFull(day, coachVoice) {
  ...
  if (!exercises.length) {
    ...
    return `<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#1E293B" style="background-color:#1E293B;border:1px solid rgba(14,165,233,0.25);border-left:3px solid #0EA5E9;border-radius:12px;border-collapse:separate !important;margin-top:12px;"><tr><td bgcolor="#1E293B" style="background-color:#1E293B;padding:20px;border-radius:12px;"><div style="font-family:${FONT_BODY};font-size:11px;color:#0EA5E9;letter-spacing:1px;font-weight:600;text-transform:uppercase;margin-bottom:8px;">▲ Pohyb</div><div style="font-family:${FONT_BODY};font-size:18px;color:#E2E8F0;font-weight:700;line-height:1.2;margin-bottom:8px;">${escapeHtml(restCopy.intro)}</div><div style="font-family:${FONT_BODY};font-size:13px;color:#64748B;font-weight:400;line-height:1.55;">${escapeHtml(restCopy.description)}</div></td></tr></table>`;
  }

  const intensity = inferWorkoutIntensity(workout, day);
  const copy = getWorkoutCopy(intensity, coachVoice);
  const borderColor = intensity === 'hard' ? '#EF4444' : intensity === 'easy' ? '#10B981' : '#A78BFA';
  ...
  const inlineExerciseList = exercises
    .map((ex) => {
      ...
      return `${escapeHtml(name)} <strong style="color:#E2E8F0;font-weight:600;">${escapeHtml(repsUnit)}</strong>`;
    })
    .join(' · ');

  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#1E293B" style="background-color:#1E293B;border:1px solid ${borderRgba};border-left:3px solid ${borderColor};border-radius:12px;border-collapse:separate !important;margin-top:12px;"><tr><td bgcolor="#1E293B" style="background-color:#1E293B;padding:20px;border-radius:12px;"><div style="font-family:${FONT_BODY};font-size:11px;color:${borderColor};letter-spacing:1px;font-weight:600;text-transform:uppercase;margin-bottom:8px;">▲ Pohyb</div><div style="font-family:${FONT_BODY};font-size:18px;color:#E2E8F0;font-weight:700;line-height:1.2;margin-bottom:8px;">${escapeHtml(copy.intro)}</div><div style="font-family:${FONT_BODY};font-size:13px;color:#64748B;font-weight:400;line-height:1.55;margin-bottom:12px;">${escapeHtml(copy.description)}</div><div style="font-family:${FONT_BODY};font-size:13px;color:#94A3B8;font-weight:400;line-height:1.7;">${inlineExerciseList}</div></td></tr></table>`;
}
...
function renderDayFull(day, index, totalDays, planJson, appBaseUrl, coachVoice, validFrom) {
  ...
  return `<tr><td class="px-mobile" bgcolor="#0A1018" style="background-color:#0A1018;padding:0 20px 12px 20px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#121826" style="background-color:#121826;border:1px solid rgba(14,165,233,0.2);border-radius:16px;border-collapse:separate !important;"><tr><td bgcolor="#0EA5E9" style="background-color:#0EA5E9;background-image:linear-gradient(135deg,#0EA5E9 0%,#A78BFA 100%);border-radius:16px 16px 0 0;padding:20px 24px;"><div style="font-family:${FONT_BODY};font-size:11px;color:#FFFFFF;letter-spacing:1px;font-weight:600;text-transform:uppercase;margin-bottom:6px;">${escapeHtml(ordinalLabel)}</div><div class="day-name-mobile" style="font-family:${FONT_BODY};font-size:24px;color:#FFFFFF;font-weight:700;letter-spacing:-0.5px;line-height:1.1;">${escapeHtml(dayName)}</div><div style="font-family:${FONT_BODY};font-size:13px;color:#FFFFFF;font-weight:500;margin-top:4px;">${escapeHtml(dateDisplay)}</div></td></tr><tr><td class="px-card-mobile" bgcolor="#121826" style="background-color:#121826;padding:20px;border-radius:0 0 16px 16px;">${mealsHtml}${dailyTotalHtml}${workoutHtml}</td></tr></table></td></tr>`;
}
...
function compactWorkoutLine(day, coachVoice) {
  ...
  const tail = moreCount > 0 ? ` <span style="color:#64748B;font-weight:500;">+ ${moreCount} dalších</span>` : '';
...
function renderDayCompact(day, index, planJson, appBaseUrl, coachVoice, validFrom) {
  ...
  const mealRow = (timeMeta, label, title) => {
    if (!title) return '';
    return `<tr><td valign="top" width="16" style="padding-top:2px;font-family:${FONT_BODY};font-size:12px;color:${timeMeta.color};font-weight:700;">${timeMeta.icon}</td><td style="padding:0 0 6px 6px;font-family:${FONT_BODY};font-size:13px;color:#94A3B8;line-height:1.5;"><strong style="color:#E2E8F0;font-weight:600;">${escapeHtml(label)}:</strong> ${escapeHtml(title)}</td></tr>`;
  };
  ...
  return `<tr><td class="px-mobile" bgcolor="#0A1018" style="background-color:#0A1018;padding:0 20px 8px 20px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#121826" style="background-color:#121826;border:1px solid rgba(14,165,233,0.18);border-radius:12px;border-collapse:separate !important;"><tr><td bgcolor="#121826" style="background-color:#121826;padding:16px 20px;border-radius:12px;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:10px;"><tr><td valign="top"><div style="font-family:${FONT_BODY};font-size:11px;color:#0EA5E9;letter-spacing:1px;font-weight:600;text-transform:uppercase;margin-bottom:2px;">${escapeHtml(ordinalLabel)}</div><div class="day-compact-mobile" style="font-family:${FONT_BODY};font-size:18px;color:#E2E8F0;font-weight:700;letter-spacing:-0.3px;">${escapeHtml(dayName)} <span style="font-size:13px;color:#64748B;font-weight:500;letter-spacing:0;"> · ${escapeHtml(dateShort)}</span></div></td><td align="right" valign="middle"><a href="${escapeHtml(dayUrl)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;background-color:rgba(14,165,233,0.15);border:1px solid rgba(14,165,233,0.3);border-radius:999px;padding:6px 12px;font-family:${FONT_BODY};font-size:11px;color:#0EA5E9;font-weight:600;letter-spacing:0.5px;text-decoration:none;text-transform:uppercase;">Otevřít →</a></td></tr></table><table width="100%" cellpadding="0" cellspacing="0" border="0">${mealRowsHtml}</table><div style="font-family:${FONT_BODY};font-size:12px;color:#64748B;line-height:1.6;padding-top:10px;margin-top:8px;border-top:1px solid rgba(14,165,233,0.12);"><strong style="color:${workoutInfo.color};font-weight:600;">▲ Pohyb:</strong> ${workoutInfo.text}</div></td></tr></table></td></tr>`;
}
...
export function buildWeeklyPlanEmailV7Document(options = {}) {
  ...
  const ctaUrl = String(options.ctaUrl || getDefaultLoginUrl()).replace(/\/$/, '');
  ...
  const daysHtml = days
    .map((day, idx) => renderDayFull(day, idx, days.length, planJson, appBaseUrl, coachVoice, validFrom))
    .join('');
