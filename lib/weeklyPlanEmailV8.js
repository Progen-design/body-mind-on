...
import { isRecipeConsistentWithMealDisplay, formatExerciseSetsRepsDisplay } from './planDataIntegrity.js';
...
function mealRecipeUrl(meal, _appBaseUrl) {
...
function mealMacros(meal) {
  const r = meal?.recipe && typeof meal.recipe === 'object' ? meal.recipe : null;
  if (!r && meal?.calories == null && meal?.protein_g == null) {
    return { protein_g: null, carbs_g: null, fat_g: null, fiber_g: null, calories: null };
  }
  return {
    protein_g: toMacroNumber(r?.protein_g ?? meal?.protein_g),
    carbs_g: toMacroNumber(r?.carbs_g ?? meal?.carbs_g),
    fat_g: toMacroNumber(r?.fat_g ?? meal?.fat_g),
    fiber_g: toMacroNumber(r?.fiber_g ?? meal?.fiber_g),
    calories: toMacroNumber(r?.calories ?? meal?.calories),
  };
}
...
function renderDayCompact(day, index, planJson, appBaseUrl, coachVoice, validFrom) {
  ...
  return (
    `<tr><td class="px-mobile" bgcolor="${COL.PAGE_BG}" style="background-color:${COL.PAGE_BG};padding:0 20px 8px 20px;">` +
...
export function buildWeeklyPlanEmailV8Document(options = {}) {
  ...
  // Všechny dny plně rozbalené (stejná karta jako Den 01)
  const daysHtml = days
