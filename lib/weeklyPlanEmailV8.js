...
const EMAIL_CONTAINER_MAX_PX = 840;
...
function renderFooterCtaBlock({ ctaUrl, webViewUrl, appBaseUrl }) {
  const profilUrl = `${String(appBaseUrl || '').replace(/\/$/, '')}/profil`;
  const planUrl = webViewUrl || ctaUrl;
  const primary = renderBulletproofButton(ctaUrl, 'Otevřít plán v aplikaci →');
  const btnApp = renderBulletproofButton(profilUrl, 'Otevřít aplikaci', { gradient: false, bg: '#172033', fullWidth: false });
...
function renderWorkoutFull(day, coachVoice, appBaseUrl) {
  ...
  const trainingCta = renderBulletproofButton(
    `${String(appBaseUrl || '').replace(/\/$/, '')}/profil`,
...
export function buildWeeklyPlanEmailV8Document(options = {}) {
  ...
  const dayTodayHtml = days.length
    ? renderDayFull(days[0], 0, days.length, planJson, appBaseUrl, coachVoice, validFrom)
    : '';
  const daysRestHtml = days.length > 1
    ? days.slice(1).map((day, idx) => renderDayCompact(day, idx + 1, planJson, appBaseUrl, coachVoice, validFrom)).join('')
    : '';

  const profileExtraHtml = (() => {
    const parts = [];
    if (bm?.activity) parts.push(`Aktivita: ${escapeHtml(String(bm.activity))}`);
    if (bm?.weekly_sessions_user != null) parts.push(`Tréninky týdně: ${escapeHtml(String(bm.weekly_sessions_user))}`);
    if (!parts.length) return '';
    return `<div style="font-family:${FONT_BODY};font-size:13px;color:${COL.TEXT_MUTED};line-height:1.6;margin-top:12px;">${parts.join(' · ')}</div>`;
  })();

  const heroCtaHtml = renderBulletproofButton(ctaUrl, 'Otevřít plán v aplikaci →');
  ...
  const filledHtml = applyVars(html, {
    ...
    hero_headline: escapeHtml('Tvůj plán je připraven'),
    hero_subline: escapeHtml('Prémiový týdenní plán — jídla, makra, trénink i návyky na jednom místě.'),
  });
