...
import { EMAIL_CONTAINER_MAX_PX } from './designTokens.js';
...
function renderFooterCtaBlock({ ctaUrl, webViewUrl, appBaseUrl }) {
  ...
  const primary = renderBulletproofButton(ctaUrl, 'Otevřít plán');
...
export function buildWeeklyPlanEmailV8Document(options = {}) {
  ...
  const heroCtaHtml = renderBulletproofButton(ctaUrl, 'Otevřít plán');
  ...
  const filledHtml = applyVars(html, {
    ...
    hero_subline: escapeHtml('Jídla, makra a trénink — stejně jako v profilu.'),
  });
