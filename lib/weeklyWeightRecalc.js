...
export async function prepocitejCilPriObnove(db, bodyMetrics, opts = {}) {
  ...
  // MAKRA SE PŘEPISUJÍ SPOLU S CÍLEM.
  //
  // Týdenní přepočet je vědomá změna cíle — jediné místo, kde se makra mají
  // hnout sama od sebe. Kdyby se přepsaly jen kalorie, zůstal by v profilu
  // starý poměr bílkovin k novému kalorickému cíli a součet by nesouhlasil.
  // `cilProVahu` volá výpočet s `forceRecalculate`, takže `verdikt.cile`
  // nese čerstvě odvozená makra, ne ta uložená.
  const patch = {
    calories_target: verdikt.novyCil,
    protein_target_g: verdikt.cile?.protein_g ?? null,
    carbs_target_g: verdikt.cile?.carbs_g ?? null,
    fat_target_g: verdikt.cile?.fat_g ?? null,
  };
  if (bodyMetrics?.id) {
  ...
  // docs/DALSI_KROK.md 8.1 — páté místo, kde se `calories_target` opravdu
  // mění, a JEDINÉ z pěti, které nejde přes `buildCalorieTargetBodyMetricsPatch()`
  // (`vyhodnotPrepocet` má vlastní výpočet). `verdikt.zmenit` už tu ověřilo
  // `novyCil !== staryCil` (ř. 82) — `emitCalorieTargetChangedEvent` to
  // přepočítá znovu, ale nezávisle na tomhle rozhodnutí, ne aby mu důvěřovala.
  await emitCalorieTargetChangedEvent(userId, {
    oldCaloriesTarget: verdikt.staryCil,
    patch,
