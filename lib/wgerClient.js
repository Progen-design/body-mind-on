 ...
 */
import { WGER_API_V2_BASE, WGER_PUBLIC_ORIGIN } from './wgerApiConstants.js';

const WGER_BASE = WGER_API_V2_BASE;
/** // FIX (ab2652e+): 24h in-process cache pro exerciseinfo (méně opakovaných GET při batch resolve). */
const exerciseInfoCache = new Map();
const exerciseInfoCacheUntil = new Map();
const EXERCISEINFO_TTL_MS = 24 * 60 * 60 * 1000;
...
export async function getWgerExerciseById(wgerExerciseId) {
  ...
  const now = Date.now();
  const until = exerciseInfoCacheUntil.get(id) || 0;
  if (now < until && exerciseInfoCache.has(id)) {
    return exerciseInfoCache.get(id);
  }
  ...
  try {
    ...
    const data = await res.json();
    const mapped = mapWgerExercise(data);
    exerciseInfoCache.set(id, mapped);
    exerciseInfoCacheUntil.set(id, now + EXERCISEINFO_TTL_MS);
    return mapped;
  } catch {
    return null;
  }
...
/**
 * Vyhledání podle anglického názvu – deleguje na searchExercise() ve wgerService (translation → exerciseinfo).
 * @param {string} nameEn
 * @returns {Promise<object|null>}
 */
export async function searchWgerExercise(nameEn) {
  ...
  try {
    const { searchExercise } = await import('./services/wgerService.js');
    const hit = await searchExercise(nameEn.trim());
    if (!hit?.id) return null;
    return getWgerExerciseById(hit.id);
  } catch {
    return null;
  }
}
