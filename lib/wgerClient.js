...
export function normalizeWgerMediaUrl(u) {
  ...
  return `https://wger.de${t.startsWith('/') ? '' : '/'}${t}`;
}
...
/**
 * Vyhledání podle anglického názvu; používá base_id z návrhů (PK pro exerciseinfo).
 * @param {string} nameEn
 * @returns {Promise<object|null>}
 */
export async function searchWgerExercise(nameEn) {
  ...
  try {
    const url = `${WGER_BASE}/exercise/search/?term=${encodeURIComponent(nameEn.trim().slice(0, 80))}&language=english`;
    const res = await fetchWithTimeout(url, {
      method: 'GET',
      headers: { Accept: 'application/json' },
    });
...
export function mapWgerExercise(data) {
  ...
  const enTranslation = Array.isArray(data.translations)
    ? data.translations.find((t) => t.language === 2)
    : null;
  const name = enTranslation?.name || data.name || '';
