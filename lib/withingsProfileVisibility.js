...
// Explicitní přípona, aby modul šel načíst i pod `node --test` (test:unit běží
// bez webpacku, který jinak bezpříponové importy v lib/ dořeší).
import { wantsDevice, getProfileDevices } from './registrationDevices.js';
...
export function shouldShowWithingsSection(profile) {
  ...
  if (wantsDevice(getProfileDevices(profile), 'scale')) return true;
  return false;
}

/** Zobrazit Withings OAuth / sync UI (ne jen obecný tělesný vývoj). */
export function shouldShowWithingsConnectUi(profile) {
  if (profile?.has_withings_connection === true) return true;
  if (getSmartScaleProvider(profile) === 'withings') return true;
  if (wantsDevice(getProfileDevices(profile), 'scale')) return true;
  return false;
