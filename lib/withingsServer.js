...
import {
  ...
  normalizeWithingsMeasuresFromRows,
  stripRawPayload,
...
async function fetchBodySnapshotsFromEav(userId, options = {}) {
  ...
  for (const row of rows || []) {
    const key = `${row.withings_measure_group_id || 'unknown'}:${row.measured_at}`;
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(row);
  }

  return [...grouped.values()]
    .map((groupRows) => snapshotRowToPublic(normalizeWithingsMeasuresFromRows(groupRows, userContext)))
