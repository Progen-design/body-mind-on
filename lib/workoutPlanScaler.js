...
/**
 * Nabídka pro náhrady: šablony z kódu PLUS katalog z databáze.
 *
 * Katalog přiveze orchestrátor v `_catalog_exercise_pool` (viz
 * lib/exerciseCatalogPool.js) — už profiltrovaný podle toho, jaké náčiní
 * uživatel má. Tady se jen připojí za šablony, takže šablonový cvik má při
 * shodě klíče přednost a zachová si svoje série a opakování.
 */
function sestavNabidku(bodyMetrics = {}) {
  const templates = rotatedTemplatesForBodyMetrics(bodyMetrics);
  const pool = buildExercisePoolFromTemplates(templates);
  for (const cvik of bodyMetrics?._catalog_exercise_pool || []) {
    const k = String(cvik?.canonical_key || '').trim().toLowerCase();
...
function deduplicateExercisesAcrossWeek(trainingDays, bodyMetrics = {}, stats = {}) {
  const pool = sestavNabidku(bodyMetrics);
  ...
  trainingDays.forEach((day, sessionIndex) => {
    ...
    for (const ex of exercises) {
      ...
      if (usedInDay.has(k) || usedAcrossWeek.has(k) || avoidKeys.has(k)) {
        ...
        // Cvik se v týdnu opakuje a nabídka nemá čím ho nahradit. Tohle je
        // ten okamžik, kvůli kterému existuje exercise_import_queue — bez
        // zaznamenání by se díra projevila jen tím, že uživatel dostane
        // stejný trénink dvakrát, a nikdo by se to nedozvěděl.
        stats.dosla_nabidka = (stats.dosla_nabidka || 0) + 1;
      }
      out.push(ex);
      usedInDay.add(k);
    ...
    while (out.filter(isMainExercise).length < MIN_MAIN_EXERCISES) {
      ...
      if (!replacement) {
        // Den nemá ani minimální počet hlavních cviků a doplnit ho není čím.
        stats.dosla_nabidka = (stats.dosla_nabidka || 0) + 1;
        break;
      }
      out.push(replacement);
...
export function scaleAndDiversifyWorkoutPlan(workoutPlan, bodyMetrics = {}) {
  const stats = {
    scaled_days: 0, diversified_days: 0, deduped_exercises: 0, target_min: 60,
    // Kolikrát skladači došla nabídka. Nenulová hodnota znamená, že katalog
    // na tenhle profil nestačí — orchestrátor z toho udělá objednávku.
    dosla_nabidka: 0,
  };
  const days = workoutPlan?.days;
  if (!Array.isArray(days) || days.length === 0) return stats;
  ...
  stats.deduped_exercises = deduplicateExercisesAcrossWeek(trainingDays, bodyMetrics, stats);
