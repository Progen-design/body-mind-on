...
/** Rotující bloky pro posilovnu — stroje a činky, max 1–2 core doplňky. */
...
/** Rotace bloků podle uživatele/týdne — stejný profil nedostane vždy stejné pořadí. */
export function rotatedTemplatesForBodyMetrics(bodyMetrics = {}) {
  const templates = sessionTemplatesForBodyMetrics(bodyMetrics);
  const off = rotationOffset(bodyMetrics) % templates.length;
  return [...templates.slice(off), ...templates.slice(0, off)];
}
...
function buildExercisePoolFromTemplates(templates) {
  const pool = new Map();
