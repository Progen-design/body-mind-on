/**
 * START PROGRAM — střídavé full-body tréninky A/B/C/D, které se přes týdny
 * rotují (viz `startVariantForSession()`, docs/DALSI_KROK.md 8.15).
 *
 ...
 *
 * LOGIKA. StrongLifts / Starting Strength: pár dní stejný trénink, technika se
 * opakováním usadí a posun je měřitelný na téže čince — proto se varianta drží
 * několik tréninků v kuse, ne že by se každý den měnila. Rotace přes A-D teď
 * (8.15) běží i při 1–3 trénincích týdně, jen pomaleji, než při 4+: viz
 * `startVariantForSession()`. Pět cviků na trénink schválně, ne šest: zbytek
 * času patří rozcvičení a pauzám, ne dalšímu cviku.
 *
...
/**
 * ── VARIANTY C A D: TLAKOVÝ A TAHOVÝ DEN ────────────────────────────────────
 *
 * Původně jen pro 4+ tréninků týdně (rozhodnuto 10. 8. 2026, `absolute % 2`
 * u 5× týdně znamenalo, že tři dny ze sedmi jsou totéž — změřeno na čtyřech
 * čerstvých registracích: žena / nabírání svalů dostala A, B, B, A, B, muž /
 * redukce B, A, A, ačkoli katalog má 205 použitelných cviků).
 *
 * OD 8.15 (5. 9. 2026, docs/DALSI_KROK.md) SE ROTUJE PŘES VŠECHNY ČTYŘI
 * VŽDY, i při 1–3 trénincích týdně — `startVariantForSession()` níž. Práh,
 * který C a D při běžných 3× týdně úplně vynechával, dal na produkci jen
 * devět různých cviků na plán a „Prkno“ 42× napříč 21 plány, přestože C a D
 * byly odjakživa hotové pro všechna tři prostředí a čekaly nepoužité. Při
 * 3× týdně teď vyjde A-B-C, další týden D-A-B, další C-D-A — všechny čtyři
 * varianty do tří týdnů. Při 5 trénincích vyjde beze změny A-B-C-D-A.
 *
...
/**
 * C A D PRO DOMOV BEZ VYBAVENÍ — docs/DALSI_KROK.md 8.16.
 *
 * Do 6. 9. 2026 sdílely C a D skoro všechno s A/B (`squat`, `pushup`,
 * `superman`, `glute_bridge`, `dead_bug`, `plank`/`plank_side` — 20 pozic,
 * jen 9 unikátních). 8.15 rozšířila rotaci na čtyři varianty i při 3×
 * týdně, ale u tohohle prostředí to nic nezměnilo, protože nebylo CO
 * rotovat. `gym` a `home_equipment` po 8.15 mají 14, resp. 13 unikátních
 * cviků za čtyři týdny; `home_bodyweight` zůstalo na 9.
 *
 * A a B se NEMĚNÍ (odladěné, lidé na nich mají progresi) — mají mezi sebou
 * `pushup` a `superman` už 2× ze 2, takže C/D je nesmí použít vůbec, jinak
 * by cvik vyšel 3× ze 4 variant. `squat`, `glute_bridge`, `plank`, `lunges`,
 * `russian_twist` a `plank_side` mají v A/B 1×, takže smí být nejvýš v JEDNÉ
 * z variant C/D.
 *
 * ŠEST NOVÝCH KLÍČŮ (`exercise_asset_registry`, `equipment_class =
 * 'body_weight'`, ověřený vizuál, žádné vybavení navíc): `bent_knee_hip_raise`,
 * `crunch_hands_overhead`, `mountain_climber`, `glute_kickback`, `calf_raise`,
 * `single_leg_butt_kick`. `pull_up`, `chin_up`, `bench_dips` a
 * `incline_push_up*` mají v registru taky `body_weight`, ale potřebují
 * hrazdu nebo lavici — do tohohle prostředí nepatří (`EQUIPMENT_REQUIRES`,
 * lib/trainingEnvironment.js, by je stejně vyměnil).
 *
 * VLASTNÍ VÁHA DOMA NEMÁ TLAK ANI TAH JINÝ NEŽ `pushup` (A/B ho tam mají
 * dvakrát právě proto — viz komentář u HOME_BW_A/B výš). `pushup` je ale na
 * stropu 2/4 už jen z A+B. `mountain_climber` se cvičí ve vzporu (zátěž na
 * ramena a paže + dynamika nohou a core), takže v C/D zastává roli „horní
 * tělo", aniž by sáhl na `pushup` — je to jediný takový klíč z těch šesti.
 *
 * Výsledek: 20 pozic, 15 unikátních cviků, žádný nad 2/4 — viz test
 * "home_bodyweight má po 8.16 aspoň 13 unikátních cviků a žádný nad 2 ze 4".
 */
const HOME_BW_C = Object.freeze([
  ex('single_leg_butt_kick', 'single leg butt kick', 'Kopy jednonož', 3, { reps_min: 10, reps_max: 12, reps_note: 'na nohu' }),
  ex('mountain_climber', 'mountain climber', 'Mountain climber', 3, { duration_sec: 30 }),
  ex('glute_kickback', 'glute kickback', 'Zapažování', 3, { reps_min: 12, reps_max: 15, reps_note: 'na nohu' }),
  ex('bent_knee_hip_raise', 'bent knee hip raise', 'Zvedání pánve s pokrčenými koleny', 3, { reps_min: 12, reps_max: 15 }),
  ex('plank', 'plank', 'Prkno', 3, { duration_sec: 40 }),
]);

const HOME_BW_D = Object.freeze([
  ex('calf_raise', 'calf raise', 'Zvedání na špičky', 3, { reps_min: 15, reps_max: 20 }),
  ex('mountain_climber', 'mountain climber', 'Mountain climber', 3, { duration_sec: 30 }),
  ex('glute_bridge', 'hip bridge', 'Zvedání pánve', 3, { reps_min: 15, reps_max: 20 }),
  ex('crunch_hands_overhead', 'overhead crunch', 'Zkracovačky nad hlavou', 3, { reps_min: 12, reps_max: 15 }),
  ex('dead_bug', 'dead bug', 'Dead bug', 3, { reps_min: 10, reps_max: 12, reps_note: 'na stranu' }),
]);
 ...
 *
 * VŽDY SE ROTUJE PŘES VŠECHNY ČTYŘI, BEZ OHLEDU NA POČET TRÉNINKŮ TÝDNĚ
 * (docs/DALSI_KROK.md 8.15 — zadání tam omylem píše variantám C/D adresu
 * `workoutTemplates.js`, ve skutečnosti jsou to `GYM_C`/`GYM_D` atd. výš
 * v tomhle souboru, `START_PROGRAM_VARIANTS`). Do 5. 9. 2026 platilo, že do
 * čtyř tréninků týdně (`PRAH_ROZSIRENE_ROTACE`) se rotovaly jen A/B — C a D
 * byly kompletně napsané pro všechna tři prostředí, ale nikdy se nepoužily.
 * Většina uživatelů trénuje 3× týdně, takže dostávala jen dvě šablony po
 * sedmi cvicích s překryvem — změřeno na produkci 5. 9. 2026: devět různých
 * cviků na plán, „Prkno“ 42× napříč 21 plány.
 *
 * Při 3× týdně to teď dá A-B-C, další týden D-A-B, další C-D-A — všechny
 * čtyři varianty do tří týdnů, místo dvou napořád.
 *
...
export function startVariantForSession(weekIndex, sessionIndex, sessionsPerWeek) {
  ...
  return PORADI_VARIANT[absolute % PORADI_VARIANT.length];
}
