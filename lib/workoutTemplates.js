...
/**
 * Domácí šablony podle vybavení (scaler i fallback sdílejí stejnou logiku výběru).
 * @param {object} bodyMetrics
 * @param {'session'|'fallback'} kind
 * @returns {WorkoutTemplateBlock[]}
 */
function homeTemplatesForBodyMetrics(bodyMetrics = {}, kind = 'session') {
  ...
  return kind === 'session' ? HOME_BODYWEIGHT_TEMPLATES : HOME_BODYWEIGHT_FALLBACK_BLOCKS;
}

/**
 * Šablony pro scaler / replace-workout (gym vs domácí vlastní váha / vybavení).
 * @param {object} [bodyMetrics]
 * @returns {WorkoutTemplateBlock[]}
 */
export function sessionTemplatesForBodyMetrics(bodyMetrics = {}) {
  const env = parseTrainingEnvironment(bodyMetrics);
  if (env === 'gym') return GYM_TEMPLATES;
  if (env === 'home_equipment') return homeTemplatesForBodyMetrics(bodyMetrics, 'session');
  return HOME_BODYWEIGHT_TEMPLATES;
}
...
export function workoutBlocksForBodyMetrics(bodyMetrics = {}) {
  ...
  if (env === 'home_equipment') return homeTemplatesForBodyMetrics(bodyMetrics, 'fallback');
