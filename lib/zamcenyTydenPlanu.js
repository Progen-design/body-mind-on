...
export async function najdiKandidatyNaUkazku(opts = {}) {
  ...
  /**
   * Nejzazší platnost skutečného (nezamčeného) plánu + kdo má ukázku,
   * která ještě neprošla (`valid_until >= dnes`) — ne podle data začátku,
   * to by se s posunutým `od` měnilo den co den a ukázka by vznikala znovu.
   */
  const konecPlanu = new Map();
  const maNepropadlouUkazku = new Set();
  for (const p of plany || []) {
    if (p.locked) {
      const konecUkazky = p.valid_until ? String(p.valid_until).split('T')[0] : null;
      if (konecUkazky && konecUkazky >= dnes) maNepropadlouUkazku.add(p.user_id);
      continue;
    }
  ...
  for (const m of trialy) {
    ...
    if (maNepropadlouUkazku.has(m.user_id)) continue;

    // Ukázka navazuje na poslední den plánu, ale nikdy nezačíná v minulosti.
