 ...
 *     fáze 2 = START; bez prorace, žádná vratka. Zrušit jde release schedule.
 *
...
/**
 * Naplánuje downgrade na začátek dalšího období. Bez prorace, bez vratky.
 * @returns {Promise<{ od: string|null, scheduleId: string }>}
 */
export async function naplanujDowngrade(stripe, sub, cenaStart) {
  ...
  return { od: konec ? new Date(konec * 1000).toISOString() : null, scheduleId: schedule.id };
}
