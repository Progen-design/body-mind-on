...
export default function Profil() {
  ...
  const preferencesFieldsSnapshot = (formState) => JSON.stringify({
    ...
    training_environment_detail: formState.training_environment_detail || '',
  ...
  const scrollToProfileAnchor = (sectionId, elementId, { openShoppingList = false } = {}) => {
    openProfileSection(sectionId);
    if (openShoppingList && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('bmo:open-shopping-list'));
    }
    let attempts = 0;
    const tryScroll = () => {
      ...
      attempts += 1;
      if (attempts < 15) setTimeout(tryScroll, 80);
    };
  ...
  const openPreferencesWorkspace = () => {
    ...
    setPreferencesForm({
      ...
      training_environment_detail: parseTrainingEnvironmentDetail(lm) || '',
  ...
  async function handleSavePreferences(e) {
    ...
    try {
      ...
      if (
        preferencesForm.training_environment === 'other'
        && !String(preferencesForm.training_environment_detail || '').trim()
      ) {
        setPreferencesError('Napiš, kde a s čím budeš cvičit.');
        return;
      }
      ...
      const res = await fetch('/api/profile-preferences', {
        ...
        body: JSON.stringify({
          ...
          training_environment_detail: preferencesForm.training_environment === 'other'
            ? String(preferencesForm.training_environment_detail || '').trim()
            : '',
    ...
    <>
      ...
      <style jsx>{`
        ...
        @media (max-width: 768px) {
          ...
          .profile-hero-main { flex-wrap: wrap; justify-content: center; gap: 12px; }
          .profile-hero-avatar-wrap { margin-left: 0; }
          .profile-hero-avatar-btn { width: 64px; height: 64px; }
          ...
          .membership-status-block { align-items: center; order: 2; }
          .profile-quick-nav-account {
            display: flex; flex-wrap: wrap; gap: 8px; order: 3;
          }
          .profile-quick-nav-account .profile-quick-nav-btn {
            flex: 1 1 calc(50% - 4px); min-width: 0; min-height: 44px; touch-action: manipulation;
          }
          .plan-goal-in-card { padding-top: 14px; margin-top: 14px; }
          .plan-goal-row { flex-direction: column; gap: 12px; padding: 12px 14px; }
          .plan-goal-stats { gap: 14px; padding: 12px 16px; min-width: 0; flex-wrap: wrap; justify-content: center; }
