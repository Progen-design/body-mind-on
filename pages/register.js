...
export default function RegisterAll() {
  // Identifikace (můžeš předvyplnit z URL/localStorage)
  ...
  const [weeklySessionsUser, setWeeklySessionsUser] = useState(''); // 1-7 (vol.)
  const [notes, setNotes] = useState('');

  const [loading, setLoading] = useState(false);
  const [msg, setMsg] = useState(null);

  // Předvyplnění z URL/localStorage
  useEffect(() => {
    try {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        email: email || null,
        name: name || null,
        gender: gender || null, // server si případně znormalizuje
  ...
  return (
    <>
      ...
      <main className="container" style={{maxWidth: 900, margin: '40px auto', padding: '0 16px'}}>
        <h1 style={{marginBottom: 12}}>Registrace</h1>
        ...
        <form onSubmit={onSubmit} className="grid">
          ...
          <div className="row single">
            <div>
              ...
              <textarea rows={4} placeholder="Zdravotní omezení, preference jídel, vybavení…"
