/**
 * Aplikuje migraci ai_supporting_documents (tabulka + seed) na produkci.
 * Usage: node scripts/run-ai-supporting-documents-migration.mjs [SUPABASE_PAT]
...
if (!pat) {
  console.error('Usage: node scripts/run-ai-supporting-documents-migration.mjs [SUPABASE_PAT]');
...
async function run() {
  ...
  try {
    ...
    console.log('Done. Trainer will receive supporting_documents from ai_supporting_documents.');
  } catch (err) {
    console.error('FAILED:', err.message);
    console.error('\nAlternative: copy SQL from supabase/migrations/20260323_ai_supporting_documents_apply.sql');
