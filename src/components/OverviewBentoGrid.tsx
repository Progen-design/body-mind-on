  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 auto-rows-auto">
      ...
      */}
      ...
      {/* 
        ========================================================================
        KARTA 5: Denní návyky & Kontrola zlozvyků (col-span-1 md:col-span-2 lg:col-span-2)
        Interaktivní přehled návyků se streaky a čistými dny
        ========================================================================
      */}
      <motion.div
        initial={{ opacity: 0, y: 12 }}
        ...
        className="col-span-1 md:col-span-2 lg:col-span-2 relative overflow-hidden rounded-3xl p-5 sm:p-6 bg-[#0c1017]/95 backdrop-blur-xl border border-cyan-500/30 shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex flex-col justify-between"
      >
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-emerald-950/60 border border-emerald-500/40 flex items-center justify-center text-[#39ff14]">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-bold text-white tracking-tight">
                  Denní návyky &amp; Kontrola zlozvyků
                </h3>
                <span className="text-[10px] text-slate-400">20. 8. Dnes • {completedHabitsCount} ze {habits.length} splněno</span>
              </div>
            </div>
            <button
              onClick={onCompleteAllHabits}
              className="px-3 py-1.5 rounded-xl text-xs font-bold text-[#39ff14] bg-emerald-950/60 border border-emerald-500/40 hover:bg-emerald-900/60 transition-all active:scale-95"
            >
              Splnit vše pro dnes
            </button>
          </div>
            ...
            {habits.slice(0, 2).map(habit => (
              <div
                ...
                className={`p-3.5 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${
                  habit.completed
                    ? 'bg-slate-900/90 border-[#39ff14]/40 shadow-[0_0_12px_rgba(57,255,20,0.15)]'
                    : 'bg-slate-900/60 border-slate-800 hover:border-slate-700'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
                    habit.completed ? 'bg-[#39ff14] text-slate-950 font-bold' : 'bg-slate-800 text-slate-500'
                  }`}>
                    <Check className="w-4 h-4 stroke-[3]" />
                  </div>
                  <div>
                    <div className="text-xs font-bold text-white">{habit.title}</div>
                    <div className="text-[11px] text-slate-400">{habit.subtitle}</div>
                  </div>
                </div>
                <span className="text-[11px] font-bold text-amber-400 bg-amber-950/50 px-2 py-0.5 rounded-full border border-amber-500/30">
                  🔥 {habit.streakDays} d
                </span>
              </div>
            ))}
          ...
          {badHabits.length > 0 && (
            <div className="p-3 rounded-2xl bg-slate-900/70 border border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-slate-200 block">{badHabits[0].title}</span>
                  <span className="text-[10px] text-slate-400">{badHabits[0].description}</span>
                </div>
              </div>
              <div className="text-right">
                <span className="text-xs font-extrabold text-[#39ff14] block">
                  {badHabits[0].cleanDaysStreak} dní čistý
                </span>
                <span className="text-[10px] text-slate-500">Bez relapsu</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="pt-3">
          <button
            onClick={() => onSelectTab('naviky')}
            className="w-full py-2 rounded-xl text-xs font-bold text-slate-300 bg-slate-900/80 hover:bg-slate-800 border border-slate-800 flex items-center justify-center gap-1.5 transition-all"
          >
