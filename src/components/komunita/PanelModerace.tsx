...
export const PanelModerace: React.FC<Props> = ({ onZmena }) => {
  ...
  return (
    <div className="p-4 rounded-3xl bg-povrch border border-cyan-500/30 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center text-akcent-cyan">
            <Shield className="w-4 h-4" />
          </div>
          <div className="text-sm font-bold text-white">Moderace</div>
        </div>
        {nacitam && <Loader2 className="w-4 h-4 text-slate-500 animate-spin" />}
      </div>
