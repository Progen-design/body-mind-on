...
const PROFIL = cti('src/components/ProfileSection.tsx');
...
test('všechna tři místa používají sdílenou komponentu, ne vlastní text', () => {
  for (const [jmeno, zdroj] of [['ProfileSection', PROFIL], ['BodyCompositionSection', TELO], ['App', APP]] as const) {
...
test('nabídka se ukazuje jen tam, kde zařízení chybí', () => {
  assert.match(PROFIL, /\{\(!slozeni \|\| !zdraviPosledni\) && \(/);
