...
SELECT * FROM (
  SELECT 'critical'::text AS severity, 'uzivatel_bez_planu'::text AS kod,
    ...
    string_agg(u.email::text, ', '::text) AS detail, count(*) AS pocet
  FROM auth.users u JOIN memberships m ON m.user_id = u.id
  WHERE (m.status = ANY (ARRAY['active'::text, 'trial'::text]))
    AND NOT (EXISTS (SELECT 1 FROM ai_generated_plans p WHERE p.user_id = u.id AND p.is_active))
  HAVING count(*) > 0
  UNION ALL
  SELECT 'critical'::text, 'generovani_selhalo'::text,
    'Generovani planu selhalo za poslednich 24 h'::text, count(*)::text || 'x'::text, count(*)
  FROM product_events WHERE event_name = 'plan_generation_failed'::text
    AND created_at > (now() - interval '24 hours') HAVING count(*) > 0
  UNION ALL
  ...
  FROM (SELECT r.email FROM registrations r LEFT JOIN auth.users u ON lower(u.email::text) = lower(r.email)
    WHERE u.id IS NULL GROUP BY r.email HAVING count(*) >= 2) s HAVING count(*) > 0
  UNION ALL
  SELECT 'critical'::text, 'uvizl_na_paywallu'::text,
    'Clenstvi ve stavu pending_payment - mel dostat trial'::text,
    string_agg(u.email::text, ', '::text), count(*)
  FROM memberships m JOIN auth.users u ON u.id = m.user_id
  WHERE m.status = 'pending_payment'::text HAVING count(*) > 0
  UNION ALL
  SELECT 'warning'::text, 'registrations_viselec'::text, 'Registrace ulozena, ucet nevznikl'::text,
    string_agg(DISTINCT r.email, ', '::text), count(DISTINCT r.email)
  FROM registrations r LEFT JOIN auth.users u ON lower(u.email::text) = lower(r.email)
  WHERE u.id IS NULL HAVING count(*) > 0
  UNION ALL
  SELECT 'warning'::text, 'necela_kusova_surovina'::text,
    'V aktivnim planu je necely pocet kusovych surovin'::text, 'napr. 3,45 vejce'::text, count(*)
  ...
  WHERE p.is_active AND (i.value ->> 'unit') = ANY (ARRAY['ks', 'plátky', 'plátek', 'konzerva', 'stroužek'])
  ...
  SELECT 'warning'::text, 'apple_health_nesynchronizuje'::text,
    'Apple Health nesynchronizoval vic nez 48 h'::text, string_agg(u.email::text, ', '::text), count(*)
  FROM apple_health_connections c JOIN auth.users u ON u.id = c.user_id
  WHERE c.status = 'active'::text AND (c.last_sync_at IS NULL OR c.last_sync_at < (now() - interval '48 hours'))
  HAVING count(*) > 0
  UNION ALL
  SELECT 'warning'::text, 'recept_mimo_gate'::text,
    'Recept ma kcal mimo toleranci vuci makrum'::text, string_agg(recipes_catalog.name_cs, ', '::text), count(*)
  FROM recipes_catalog WHERE recipes_catalog.active
    AND (abs(recipes_catalog.kcal::numeric - (recipes_catalog.protein_g * 4 + recipes_catalog.carbs_g * 4 + recipes_catalog.fat_g * 9))
      / NULLIF(recipes_catalog.kcal, 0)::numeric) > 0.20 HAVING count(*) > 0
  UNION ALL
  SELECT 'warning'::text, 'calorie_target_mismatch'::text,
    'Aktivni plan ma jiny kaloricky cil nez body_metrics'::text, string_agg(p.email::text, ', '::text), count(*)
  FROM ai_generated_plans p JOIN LATERAL (
    SELECT calories_target FROM body_metrics bm WHERE bm.user_id = p.user_id ORDER BY bm.created_at DESC LIMIT 1
  ) latest ON true WHERE p.is_active = true AND p.daily_calories IS NOT NULL AND latest.calories_target IS NOT NULL
    AND p.daily_calories <> latest.calories_target HAVING count(*) > 0
  UNION ALL
  ...
  FROM ingredient_normalization_misses m JOIN ai_generated_plans p ON p.id = m.plan_id AND p.is_active
  WHERE m.seen_at > (now() - interval '7 days') HAVING count(DISTINCT m.raw_name) > 0
  UNION ALL
  SELECT 'critical'::text, 'import_zadny_novy_recept'::text,
    'Zadny novy recept v katalogu 2+ dny'::text,
    to_char(max(rc.created_at), 'YYYY-MM-DD HH24:MI') AS detail, 1
  FROM recipes_catalog rc HAVING max(rc.created_at) < (now() - interval '2 days')
  UNION ALL
  ...
  FROM recipes_catalog WHERE nutrition_source = 'computed_from_ingredients' HAVING count(*) < 217
) alerts;
